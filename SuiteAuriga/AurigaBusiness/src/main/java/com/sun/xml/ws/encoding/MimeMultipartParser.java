...
import org.jvnet.mimepull.EngMimeMessage;
...
public final class MimeMultipartParser {
    ...
    public MimeMultipartParser(InputStream in, String contentType, StreamingAttachmentFeature feature, boolean isOrderRequested) {
    	ContentType ct = new ContentType(contentType);
        String boundary = ct.getParameter("boundary");
        if (boundary == null || boundary.equals("")) {
            throw new WebServiceException("MIME boundary parameter not found" + contentType);
        ...
        if(isOrderRequested) {
        	message = (feature != null)
                    ? new EngMimeMessage(in, boundary, feature.getConfig())
                    : new EngMimeMessage(in, boundary);
        } else {
        	message = (feature != null)
                    ? new MIMEMessage(in, boundary, feature.getConfig())
                    : new MIMEMessage(in, boundary);
    ...
    private ByteArrayOutputStream cloneInputStream(InputStream is) throws IOException {
    ...
    private String searchService(InputStream in) {
    	String service = "";
		...
		try {
			while( (bytesRead = in.read(contents)) != -1){
				...
				if(strFileContents.contains("xmlns:add=")) {
					//TODO: regexp 
				}
			}
		} catch (IOException e) {
