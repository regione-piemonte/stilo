...
abstract class MimeCodec implements Codec {
    ...
    public void decode(InputStream in, String contentType, Packet packet) throws IOException {
    	// Retrieve the name of the service that has been invocating
    	OrderAttachServicesEnum serviceName = null;
    	if(packet.component instanceof ServletAdapter) {
    		// Vado a recuperare il servizio chiamato analizzando il packet.component solo se  e'  di tipo ServletAdapter (il component cambia a seconda del giro di elaborazione request/response)
    		ServletAdapter ad = (ServletAdapter) packet.component;
    		serviceName = OrderAttachServicesEnum.fromValue(ad.getName());
    	}
    	
    	// se e' stato trovato un enum di un servizio dove applicare l'ordinamento degli attach (quindi serviceName risulta valorizzato), 
    	// allora si attivera' la procedura  di cambio header per la gestione dell'ordinamento. 
        MimeMultipartParser parser = new MimeMultipartParser(in, contentType, features.get(StreamingAttachmentFeature.class), 
        		(serviceName != null ? true : false));
