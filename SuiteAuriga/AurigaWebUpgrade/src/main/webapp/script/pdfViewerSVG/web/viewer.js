...
var PDFViewerApplication = {
  ...
  bindEvents: function bindEvents() {
    ...
    eventBus.on('saveSelection', webViewerSaveSelection);
    eventBus.on('eraser', webViewerEraser);
  ...
  unbindEvents: function unbindEvents() {
    ...
    eventBus.off('saveSelection', webViewerSaveSelection);
    eventBus.off('eraser', webViewerEraser);
...
function webViewerInitialized() {
  ...
  appConfig.eraserEnable = params.enableRitagli !== undefined && params.enableRitagli;
...
function webViewerEraser(event) {
  if (event.source.items.eraserButton.classList.contains("on")) {
    Array.prototype.slice.call(document.getElementsByClassName("canvasWrapper")).forEach(function (element) {
...
function webViewerSaveSelection() {
  var queryString = document.location.search.substring(1);
  var params = (0, _ui_utils.parseQueryString)(queryString);
  var saveSelectionName = params.saveselectionname;
  var selection = "";
  var selectionList = document.querySelectorAll('.thumbnail.selected');
  ...
  xhttp.open("GET", "springdispatcher/preview/saveSelection?selection=" + selection + "&parName=Selection" + saveSelectionName, true);
  xhttp.send();
}
  ...
  if (cmd === 0) {
    ...
    switch (evt.keyCode) {
      ...
      case 46:
        ...
        if (selectedEl) {
          if (selectedEl[0].parentElement) {
            selectedEl[0].parentElement.remove();
          } else {
            selectedEl[0].parentNode.remove();
          }
        }
        break;
    }
...
var THUMBNAIL_CURRENT_CLASS = 'current';
  ...
  _createClass(PDFThumbnailViewer, [{
    ...
    value: function scrollThumbnailIntoView(pageNumber) {
      ...
      if (pageNumber !== this._currentPageNumber) {
        ...
        prevThumbnailView.div.classList.remove(THUMBNAIL_CURRENT_CLASS);
        thumbnailView.div.classList.add(THUMBNAIL_CURRENT_CLASS);
      }
      ...
      if (numVisibleThumbs > 0) {
        ...
        if (pageNumber >= last) {
          thumbnailView.div.scrollIntoView(false);
        } else if (pageNumber <= first) {
          thumbnailView.div.scrollIntoView(true);
        } else {
  ...
  function PDFThumbnailView(_ref) {
    ...
    div.ondblclick = function () {
      ...
      if (div.classList.contains('selected')) {
        ...
        var numberOfSelectedPage = document.querySelectorAll('.thumbnail.selected').length;
  ...
  _createClass(PDFPageView, [{
    ...
    value: function _bindMouse() {
      ...
      div.addEventListener('mousedown', function (evt) {
        if (PDFViewerApplication.toolbar.items.eraserButton.classList.contains("on") && evt.target.localName !== "rect") {
          ...
          var drawing = new SVG(evt.currentTarget.getElementsByClassName("canvasWrapper")[0].children[0]).size('100%', '100%');
          var option = {
            stroke: "grey",
            fill: "white",
            'stroke-width': 2,
          ...
          document.getElementById(rect.node.id).addEventListener('click', function (ev) {
            var _this = this;
    ...
    value: function draw() {
      ...
      var finishPaintTask = function finishPaintTask(error) {
        ...
        _this2.error = error;
        _this2.stats = pdfPage.stats;
        if (_this2.onAfterDraw) {
          _this2.onAfterDraw();
        }
    ...
    value: function paintOnSvg(wrapper) {
      ...
      var ensureNotCancelled = function ensureNotCancelled() {
        if (cancelled) {
          throw new _pdfjsLib.RenderingCancelledException('Rendering cancelled, page ' + _this3.id, 'svg');
        }
      };
      var pdfPage = this.pdfPage;
...
var Toolbar = function () {
  ...
  _createClass(Toolbar, [{
    ...
    value: function _bindListeners() {
      ...
      items.eraserButton.addEventListener('click', function () {
        eventBus.dispatch('eraser', { source: self });
      });
      ...
      items.saveSelection.addEventListener('click', function () {
        eventBus.dispatch('saveSelection');
      });
