...
(function (root, factory) {
...
})(typeof window !== "undefined" ? window : void 0, function (window, document) {
  ...
  SVG.utils = {
    ...
    filterSVGElements: function filterSVGElements(nodes) {
      return this.filter(nodes, function (el) {
        return _instanceof(el, window.SVGElement);
      });
    }
  };
  SVG.defaults = {
  ...
  SVG.extend(SVG.PathArray, {
    ...
    parse: function parse(array) {
      ...
      if (_instanceof(array, SVG.PathArray)) return array.valueOf(); // prepare for parsing
  ...
  SVG.Number = SVG.invent({
    ...
    create: function create(value, unit) {
      ...
      if (typeof value === 'number') {
        ...
        if (_instanceof(value, SVG.Number)) {
  ...
  SVG.Element = SVG.invent({
    ...
    extend: {
      ...
      parent: function parent(type) {
        ...
        while (parent && _instanceof(parent.node, window.SVGElement)) {
          if (typeof type === 'string' ? parent.matches(type) : _instanceof(parent, type)) return parent;
      ...
      ,
      doc: function doc() {
        return _instanceof(this, SVG.Doc) ? this : this.parent(SVG.Doc);
      ...
      svg: function svg(_svg) {
        ...
        if (_svg && _instanceof(this, SVG.Parent)) {
          // dump raw svg
  ...
  SVG.FX = SVG.invent({
    ...
    extend: {
      ...
      target: function target(_target) {
        if (_target && _instanceof(_target, SVG.Element)) {
      ...
      queue: function queue(fn) {
        if (typeof fn == 'function' || _instanceof(fn, SVG.Situation)) this.situations.push(fn);
      ...
      dequeue: function dequeue() {
        ...
        if (this.situation) {
          if (_instanceof(this.situation, SVG.Situation)) {
            this.start();
          } else {
      ...
      initAnimations: function initAnimations() {
        ...
        for (i in s.animations) {
          ...
          for (j = source.length; j--;) {
            ...
            // of a SVG.Number
            if (_instanceof(s.animations[i][j], SVG.Number)) source[j] = new SVG.Number(source[j]);
  ...
  SVG.extend(SVG.FX, {
    ...
    x: function x(_x2, relative) {
      if (_instanceof(this.target(), SVG.G)) {
        this.transform({
          x: _x2
        }, relative);
        return this;
    ...
    ,
    size: function size(width, height) {
      if (_instanceof(this.target(), SVG.Text)) {
    ...
    viewbox: function viewbox(x, y, width, height) {
      if (_instanceof(this.target(), SVG.Container)) {
    ...
    update: function update(o) {
      if (_instanceof(this.target(), SVG.Stop)) {
        if (typeof o == 'number' || _instanceof(o, SVG.Number)) {
          return this.update({
            offset: arguments[0],
            color: arguments[1],
            opacity: arguments[2]
  ...
  SVG.Box = SVG.invent({
    create: function create(x, y, width, height) {
      if (_typeof(x) == 'object' && !_instanceof(x, SVG.Element)) {
  ...
  SVG.BBox = SVG.invent({
    ...
    create: function create(element) {
      ...
      if (_instanceof(element, SVG.Element)) {
        ...
        try {
          ...
          if (_instanceof(element, SVG.Shape)) {
  ...
  SVG.Matrix = SVG.invent({
    ...
    create: function create(source) {
      ...
      source = _instanceof(source, SVG.Element) ? source.matrixify() : typeof source === 'string' ? arrayToMatrix(source.split(SVG.regex.delimiter).map(parseFloat)) : arguments.length == 6 ? arrayToMatrix([].slice.call(arguments)) : Array.isArray(source) ? arrayToMatrix(source) : _typeof(source) === 'object' ? source : base; // merge source
    ...
    construct: {
      ...
      screenCTM: function screenCTM() {
        ...
        if (_instanceof(this, SVG.Nested)) {
    ...
    attr: function attr(a, v, n) {
      ...
      if (a == null) {
        ...
        if (a == 'fill' || a == 'stroke') {
          ...
          if (_instanceof(v, SVG.Image)) v = this.doc().defs().pattern(0, 0, function () {
            this.add(v);
          });
    ...
    extend: {
      ...
      each: function each(block, deep) {
        ...
        for (i = 0, il = children.length; i < il; i++) {
          if (_instanceof(children[i], SVG.Element)) block.apply(children[i], [i, children]);
          if (deep && _instanceof(children[i], SVG.Container)) children[i].each(block, deep);
        }
  ...
  SVG.extend(SVG.Parent, {
    ungroup: function ungroup(parent, depth) {
      if (depth === 0 || _instanceof(this, SVG.Defs) || this.node == SVG.parser.draw) return this;
      parent = parent || (_instanceof(this, SVG.Doc) ? this : this.parent(SVG.Parent));
      depth = depth || Infinity;
      this.each(function () {
        if (_instanceof(this, SVG.Defs)) return this;
        if (_instanceof(this, SVG.Parent)) return this.ungroup(parent, depth - 1);
  ...
  SVG.ViewBox = SVG.invent({
    create: function create(source) {
      ...
      if (_instanceof(source, SVG.Element)) {
        we = source;
        he = source;
        ...
        while (width.unit == '%') {
          wm *= width.value;
          width = new SVG.Number(_instanceof(we, SVG.Doc) ? we.parent().offsetWidth : we.parent().width());
          we = we.parent();
        }

        while (height.unit == '%') {
          hm *= height.value;
          height = new SVG.Number(_instanceof(he, SVG.Doc) ? he.parent().offsetHeight : he.parent().height());
          he = he.parent();
        } // ensure defaults
  ...
  SVG.on = function (node, events, listener, binding, options) {
    ...
    var n = _instanceof(node, SVG.Element) ? node.node : node; // ensure instance object for nodes which are not adopted
  ...
  SVG.off = function (node, events, listener, options) {
    var n = _instanceof(node, SVG.Element) ? node.node : node;
  ...
  SVG.extend(SVG.Element, {
    ...
    fire: function fire(event, data) {
      // Dispatch event
      if (_instanceof(event, window.Event)) {
  ...
  SVG.extend(SVG.Element, {
    ...
    forward: function forward() {
      ...
      if (_instanceof(p, SVG.Doc)) p.node.appendChild(p.defs().node);
      return this;
  ...
  SVG.extend(SVG.Element, {
    ...
    maskWith: function maskWith(element) {
      ...
      this.masker = _instanceof(element, SVG.Mask) ? element : this.parent().mask().add(element); // store reverence on self in mask
  ...
  SVG.extend(SVG.Element, {
    ...
    clipWith: function clipWith(element) {
      ...
      this.clipper = _instanceof(element, SVG.ClipPath) ? element : this.parent().clip().add(element); // store reverence on self in mask
  ...
  SVG.extend(SVG.Line, SVG.Polyline, SVG.Polygon, SVG.Path, {
    ...
    marker: function marker(_marker, width, height, block) {
      ...
      _marker = _instanceof(arguments[1], SVG.Marker) ? arguments[1] : this.doc().marker(width, height, block);
  ...
  SVG.extend(SVG.Element, SVG.FX, {
    ...
    ,
    dx: function dx(x) {
      return this.x(new SVG.Number(x).plus(_instanceof(this, SVG.FX) ? 0 : this.x()), true);
    } // Relative move over y axis
    ,
    dy: function dy(y) {
      return this.y(new SVG.Number(y).plus(_instanceof(this, SVG.FX) ? 0 : this.y()), true);
  ...
  SVG.Set = SVG.invent({
    ...
    create: function create(members) {
      if (_instanceof(members, SVG.Set)) {
  ...
  function parseMatrix(matrix) {
    if (!_instanceof(matrix, SVG.Matrix)) matrix = new SVG.Matrix(matrix);
    return matrix;
  ...
  function assignNewId(node) {
    ...
    for (var i = node.childNodes.length - 1; i >= 0; i--) {
      if (_instanceof(node.childNodes[i], window.SVGElement)) assignNewId(node.childNodes[i]);
    }
