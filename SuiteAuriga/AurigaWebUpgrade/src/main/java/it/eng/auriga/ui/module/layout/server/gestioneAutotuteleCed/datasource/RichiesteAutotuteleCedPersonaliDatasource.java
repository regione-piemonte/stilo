...
package it.eng.auriga.ui.module.layout.server.gestioneAutotuteleCed.datasource;
...
import it.eng.auriga.ui.module.layout.server.gestioneAutotuteleCed.bean.RichiesteAutotuteleCedPersonaliBean;
import it.eng.auriga.ui.module.layout.server.gestioneAutotuteleCed.bean.RichiesteAutotuteleCedPersonaliXmlBean;
import it.eng.auriga.ui.module.layout.server.gestioneAutotuteleCed.bean.RichiesteAutotuteleCedPersonaliXmlBeanDeserializationHelper;
...
@Datasource(id = "RichiesteAutotuteleCedPersonaliDatasource")
public class RichiesteAutotuteleCedPersonaliDatasource extends AurigaAbstractFetchDatasource<RichiesteAutotuteleCedPersonaliBean> {

	private static Logger mLogger = Logger.getLogger(RichiesteAutotuteleCedPersonaliDatasource.class);

	@Override
	public PaginatorBean<RichiesteAutotuteleCedPersonaliBean> fetch(AdvancedCriteria criteria, Integer startRow, Integer endRow, List<OrderByBean> orderby) throws Exception {
		...
		List<RichiesteAutotuteleCedPersonaliXmlBean> lListResult = XmlListaUtility.recuperaLista(xmlOut, RichiesteAutotuteleCedPersonaliXmlBean.class);
		
		getSession().setAttribute(FETCH_SESSION_KEY, lListResult);
		
		List<RichiesteAutotuteleCedPersonaliBean> lList = new ArrayList<RichiesteAutotuteleCedPersonaliBean>(lListResult.size());		
		for (RichiesteAutotuteleCedPersonaliXmlBean lRichiesteAutotuteleCedXmlBean : lListResult){
			RichiesteAutotuteleCedPersonaliBean lRichiesteAutotuteleCedBean = new RichiesteAutotuteleCedPersonaliBean();
		...
		PaginatorBean<RichiesteAutotuteleCedPersonaliBean> lPaginatorBean = new PaginatorBean<RichiesteAutotuteleCedPersonaliBean>();
	...
	private DmpkWfTrovalistalavoroBean createInputBean(boolean forNroRecordTot,AdvancedCriteria criteria, AurigaLoginBean loginBean) throws Exception {
		...
		RichiesteAutotuteleCedPersonaliBean filter = new RichiesteAutotuteleCedPersonaliBean(); 
		...
		if((bachSize != null && bachSize.intValue() > 0) || (overflowLimit != null && overflowLimit.intValue() > 0)) {
			...
			String[] colAndFlgDescOrderBy = getColAndFlgDescOrderBy(RichiesteAutotuteleCedPersonaliXmlBean.class, setNumColonneOrdinabili);
	...
	@Override
	public Map<String, ErrorBean> validate(RichiesteAutotuteleCedPersonaliBean bean) throws Exception {
		return null;
	}

	@Override
	public String getNomeEntita() {
		return "richiesteAutotuteleCedPersonali";
	}
	...
	@Override
	public RichiesteAutotuteleCedPersonaliBean get(RichiesteAutotuteleCedPersonaliBean bean) throws Exception {
		return bean;
	}

	@Override
	public RichiesteAutotuteleCedPersonaliBean add(RichiesteAutotuteleCedPersonaliBean bean) throws Exception {
		return bean;
	}

	@Override
	public RichiesteAutotuteleCedPersonaliBean remove(RichiesteAutotuteleCedPersonaliBean bean) throws Exception {
		return bean;
	}

	@Override
	public RichiesteAutotuteleCedPersonaliBean update(RichiesteAutotuteleCedPersonaliBean bean, RichiesteAutotuteleCedPersonaliBean oldvalue)
			throws Exception {
		return bean;
	}
	...
	protected ExportBean asyncExport(ExportBean bean) throws Exception {
		...
		saveParameters(loginBean, bean, jobId, loginBean.getIdUserLavoro(), RichiesteAutotuteleCedPersonaliXmlBean.class.getName());

		saveRemapInformations(loginBean, jobId, createRemapConditionsMap(),  RichiesteAutotuteleCedPersonaliXmlBeanDeserializationHelper.class);
