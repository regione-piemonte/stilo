...
public abstract class ListaDatiLiquidazioneAVBItem extends GridItem {

	protected ListGridField id;	
	protected ListGridField uriExcel;	
	protected ListGridField attoImpegno;
	protected ListGridField oggettoSpesaOggetto;	
	protected ListGridField beneficiarioSede;
	protected ListGridField beneficiarioPagamentoIva;
	protected ListGridField beneficiarioPagamentoCodFiscale;
	...
	public ListaDatiLiquidazioneAVBItem(String name) {

//		super(name);
		super(name, "listaDatiLiquidazioneAVB", true);
		...
		// Visibili
		attoImpegno         			  = new ListGridField("attoImpegno", "Determinazione di impegno n.");  				 attoImpegno.setCanSort(true);         				 attoImpegno.setCanEdit(false);          			   attoImpegno.setCanHide(false);       
		oggettoSpesaOggetto               = new ListGridField("oggettoSpesaOggetto", "Oggetto");                             oggettoSpesaOggetto.setCanSort(true);               oggettoSpesaOggetto.setCanEdit(false);                oggettoSpesaOggetto.setCanHide(false);
		beneficiarioSede 				  = new ListGridField("beneficiarioSede", "Beneficiario");          				 beneficiarioSede.setCanSort(true); 				 beneficiarioSede.setCanEdit(false);  				   beneficiarioSede.setCanHide(false);
		beneficiarioPagamentoIva          = new ListGridField("beneficiarioPagamentoIva", "P. IVA");                         beneficiarioPagamentoIva.setCanSort(true);          beneficiarioPagamentoIva.setCanEdit(false);           beneficiarioPagamentoIva.setCanHide(false);
		beneficiarioPagamentoCodFiscale   = new ListGridField("beneficiarioPagamentoCodFiscale", "Codice fiscale");          beneficiarioPagamentoCodFiscale.setCanSort(true);   beneficiarioPagamentoCodFiscale.setCanEdit(false);    beneficiarioPagamentoCodFiscale.setCanHide(false);
		documentoDiDebito                 = new ListGridField("documentoDiDebito", "Documento di debito");                   documentoDiDebito.setCanSort(true);                 documentoDiDebito.setCanEdit(false);                  documentoDiDebito.setCanHide(false);
		beneficiarioPagamentoImporto      = new ListGridField("beneficiarioPagamentoImporto", "Importo");                    beneficiarioPagamentoImporto.setCanSort(true);      beneficiarioPagamentoImporto.setCanEdit(false);       beneficiarioPagamentoImporto.setCanHide(false);
		...
		setGridFields(// Nascoste
			      ...
			      oggettoSpesaOggetto,
			      beneficiarioSede,
			      beneficiarioPagamentoCodFiscale,
			      beneficiarioPagamentoIva,
			      documentoDiDebito,
			      beneficiarioPagamentoImporto,
				deleteButtonField
			);	
	}
	...
	protected VLayout buildLayout() {
		this.toolStrip.removeMember(this.multiselectButton);
		this.toolStrip.addButton(this.multiselectButton, 0);
	...
	public ListGrid buildGrid() {
		...
		grid.addSort(new SortSpecifier("oggettoSpesaNumDetImpegno", SortDirection.ASCENDING));
		grid.addSort(new SortSpecifier("oggettoSpesaOggetto", SortDirection.ASCENDING));
	...
	//************************************************************
	//* Bottoni per la selezione multipla
	//* *********************************************************
	@Override
	protected GridMultiToolStripButton[] getGridMultiselectButtons() {
		...
		if (downloadZipMultiButton == null) {
			downloadZipMultiButton = new GridMultiToolStripButton("buttons/download_zip.png", grid, "Esporta tutte le liquidazioni", false) {
					...
					public void doSomething() {
						onClickDownloadZipMultiButton();
					}
				};
		}
		gridMultiselectButtons.add(downloadZipMultiButton);
	...
	public List<ToolStripButton> buildCustomEditButtons() {
		...
		downloadTemplateExcelButton.setPrompt("Download template excel dati di liquidazione");
	...
	public void onClickDownloadTemplateExcelButton() {
		String uriFileExcel = AurigaLayout.getParametroDB("URI_EXCEL_DATI_LIQUIDAZIONE_AVB");
//		String uriFileExcel = "C:\\Users\\micrega\\Downloads\\Modello per più fatture 1.2021_FV.xlsx";
		Record lRecord = new Record();
		lRecord.setAttribute("displayFilename", "Modello_per_piu_fatture.xls");
	...
	private void onClickDownloadButton(ListGridRecord record) {
		...
		final GWTRestDataSource lGwtRestDataSource = new GWTRestDataSource("LiquidazioniAVBDatasource");
		lGwtRestDataSource.executecustom("calcolaInfoFile", recordInfoFile, new DSCallback() {
			...
			public void execute(DSResponse response, Object rawData, DSRequest request) {
				if(response.getStatus() == DSResponse.STATUS_SUCCESS) {					
					...
					String nomeFile = "Dati_Liquidazioni_AVB"+ fileExt;
	...
	private void onClickDeleteButton(ListGridRecord record) {
		grid.deselectAllRecords();
		if (!record.getAttributeAsBoolean("isUploaded")) {
			uriFilesExcelToDelete.add(record.getAttribute("uriExcel"));
			afterOnClickDeleteButton(uriFilesExcelToDelete);
		}
		removeData(record);
	}
	
	public abstract void afterOnClickDeleteButton(List<String> uriXls);
	
	private void onClickImportXlsButton(RecordList listaLiquidazioni, final int numeroFileCaricati) {
//		afterOnClickDeleteButton(uriFilesExcelToDelete);
		Layout.showWaitPopup("Caricamento liquidazioni in corso...");
		final GWTRestDataSource lContabilitaADSPDataSource = new GWTRestDataSource("LiquidazioniAVBDatasource");
		Record rec = new Record ();
		rec.setAttribute("listaDati", listaLiquidazioni);
		lContabilitaADSPDataSource.performCustomOperation("importaDatiLiquidazioniFromExcel", rec, new DSCallback() {							
			@Override
			public void execute(DSResponse response, Object rawData, DSRequest request) {
				Layout.hideWaitPopup();
				if(response.getStatus() == DSResponse.STATUS_SUCCESS) {
					Record record = response.getData()[0];
					RecordList attributeAsRecordList = record.getAttributeAsRecordList("listaLiquidazioni");
					for (int i=0; i < attributeAsRecordList.getLength(); i++) {
						Record liquidazione = attributeAsRecordList.get(i);
						if (liquidazione.getAttributeAsBoolean("isInError") != null && liquidazione.getAttributeAsBoolean("isInError")) {
							Record rec = new Record ();
							rec.setAttribute("descrizione", liquidazione.getAttribute("errorMessage") != null ? liquidazione.getAttribute("errorMessage") : null);
							rec.setAttribute("idError", liquidazione.getAttribute("nomeFile"));
							listaRecordCaricatiInError.add(rec);
						} else {
							addData(attributeAsRecordList.get(i));
					...
					if (listaRecordCaricatiInError.getLength() > 0) {
						ErroreMassivoPopup errorePopup = new ErroreMassivoPopup(getName(), "Nome File", listaRecordCaricatiInError, numeroFileCaricati, LARG_POPUP_ERR_MASS,
	...
	private void onClickPreviewButton(ListGridRecord record) {
		...
		lGwtRestDataSource.executecustom("calcolaInfoFile", recordInfoFile, new DSCallback() {
			...
			public void execute(DSResponse response, Object rawData, DSRequest request) {
				if(response.getStatus() == DSResponse.STATUS_SUCCESS) {					
					String uri    = recordInfoFile.getAttribute("uriFile");
					String displayFilename =  "Dati_Liquidazioni_AVB.pdf";
					Record record = response.getData()[0];
					InfoFileRecord infoFile = new InfoFileRecord(record);
	...
	private void onClickDownloadZipMultiButton(){
		...
		final GWTRestDataSource lGwtRestDataSource = new GWTRestDataSource("LiquidazioniAVBDatasource");
		lGwtRestDataSource.executecustom("createZipExportLiquidazioni", record, new DSCallback() {
	...
	private void addAllDataAndRefresh(final RecordList listaRecord, boolean skipRefreshNroAllegato, int numeroFileCaricati) {
		...
		RecordList listaLiquidazioni = new RecordList();
		for(int i = 0; i < listaRecord.getLength(); i++) {
			Record record = listaRecord.get(i);
			Record liquidazione = new Record();
			...
			if (infoFile == null || infoFile.getMimetype() == null 	|| (!infoFile.getMimetype().equals("application/excel") 
				...
				liquidazione.setAttribute("mimeType",infoFile.getMimetype());
				liquidazione.setAttribute("uriExcel",record.getAttribute("uriFileImportExcel"));
				liquidazione.setAttribute("nomeFile",record.getAttribute("nomeFile"));
				listaLiquidazioni.add(liquidazione);
			}
		}		
		onClickImportXlsButton(listaLiquidazioni, numeroFileCaricati);
	}
