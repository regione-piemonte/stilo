...
public class ProponentiWindow extends ModalWindow {
	
	protected ProponentiWindow window;
	protected ProponentiDetail detail;	
	protected ListaProponentiItem gridItem;
	protected ToolStrip detailToolStrip;
		
	public ProponentiWindow(ListaProponentiItem gridItem, String nomeEntita, final Record record, boolean canEdit) {
		...
		if(record != null) {
			if(canEdit) {
				setTitle(I18NUtil.getMessages().editDetail_titlePrefix() + " struttura proponente");				
			} else {
				setTitle(I18NUtil.getMessages().viewDetail_titlePrefix() + " struttura proponente");	
			}			
		} else {
			setTitle("Nuova struttura proponente");
		}
		
		detail = new ProponentiDetail(gridItem, nomeEntita, record, canEdit);		
		detail.setHeight100();
		detail.setWidth100();
			...
			saveButton.addClickHandler(new ClickHandler() {
				...
				public void onClick(ClickEvent event) {
					...
					if(detail.validate()) {						
						...
						RecordList listaProponenti = lRecord.getAttributeAsRecordList("listaProponenti");
						saveData(listaProponenti.get(0));						
