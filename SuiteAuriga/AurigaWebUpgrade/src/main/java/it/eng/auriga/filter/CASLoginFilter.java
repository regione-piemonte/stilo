...
public class CASLoginFilter implements Filter {
	...
	private static Logger mLogger = Logger.getLogger(CASLoginFilter.class);
...
//		if(!(usernameSSO==null) && domain.equals("localhost")) {
//			mLogger.debug("SONO IN LOCALE");
//		} else if (!(usernameSSO==null) && domain.equals("cruscotto.bari.avmtb.rsr.rupar.puglia.it")) {
//			usernameSSO=usernameSSO.concat("_A662");
//		} else if (!(usernameSSO==null) && domain.equals("cruscotto.ruvodipuglia.avmtb.rsr.rupar.puglia.it")) {
//			usernameSSO=usernameSSO.concat("_H645");
//		}
//		usernameSSO="test.protocollo";
		...
		if (req.getRequestURI().contains("rest")) {
			if (usernameSSO == null) {
				...
				out.print(IOUtils.toString(CASLoginFilter.class.getResourceAsStream("login.jsp")));
				return;
			}
