...
package it.eng.auriga.ui.module.layout.client.regoleProtocollazioneAutomaticaCaselle;
...
import it.eng.auriga.ui.module.layout.client.protocollazione.DestinatarioProtItem;
...
public class UoDestinatariaCanvas extends ReplicableCanvas {
	...
	private CheckboxItem flgAssDestinatarioItem;
	private CheckboxItem flgPCDestinatarioItem;
	
	public UoDestinatariaCanvas(UoDestinatariaItem item) {
		super(item);
	}
...
//		mDynamicForm.setCellBorder(1);
								...
								if (data.getLength() > 0) {
									for (int i = 0; i < data.getLength(); i++) {
										...
										if (value.equals(codice) && (flgSelXFinalita == null || "1".equals(flgSelXFinalita))) {
											...
											mDynamicForm.clearErrors(true);
											trovato = true;
											((UoDestinatariaItem) getItem()).manageOnChanged();
											break;
										}
									}
								}
		...
		codRapidoItem.setValidators(new CustomValidator() {
			...
			protected boolean condition(Object value) {
					if (codRapidoItem.getValue() != null && !"".equals(codRapidoItem.getValueAsString().trim()) 
							&& organigrammaItem.getValue() == null) {
						return false;
					}
				return true;
			}
		...
		organigrammaDS.addParam("finalita", ((UoDestinatariaItem) getItem()).getFinalitaLoadComboOrganigramma());
...
//				mDynamicForm.setValue("organigramma", record.getAttributeAsString("id"));
			...
			protected void clearSelect() {
				...
				if(((UoDestinatariaItem)getItem()).isDimOrganigrammaNonStd()) {
...
//				}				
		...
		// check assegna al destinatario
		flgAssDestinatarioItem = new CheckboxItem("flgAssDestinatario", I18NUtil.getMessages().regole_protocollazione_automatica_caselle_pec_peo_flgAssegnaPerCompetenzaItem_title());
		flgAssDestinatarioItem.setRequired(false);
		flgAssDestinatarioItem.setColSpan(1);
		flgAssDestinatarioItem.setWidth(25);
		flgAssDestinatarioItem.addChangedHandler(new ChangedHandler() {
			
			@Override
			public void onChanged(ChangedEvent event) {
				boolean checked = event.getValue() != null && (Boolean) event.getValue();
					if (checked) {										
						mDynamicForm.setValue("flgCcDestinatario", false);
					}
					if (!checked) {										
						mDynamicForm.setValue("flgCcDestinatario", true);
					}
				mDynamicForm.redraw();
			}
		});
		flgAssDestinatarioItem.setShowIfCondition(new FormItemIfFunction() {
			
			@Override
			public boolean execute(FormItem item, Object value, DynamicForm form) {
				return ((UoDestinatariaItem) getItem()).showFlgAssegnaAlDestinatarioItem();
			}
		});
		
		// check assegna per conoscenza
		flgPCDestinatarioItem = new CheckboxItem("flgCcDestinatario", I18NUtil.getMessages().regole_protocollazione_automatica_caselle_pec_peo_flgAssegnaPerConoscenzaItem_title());
		flgPCDestinatarioItem.setRequired(false);
		flgPCDestinatarioItem.setColSpan(1);
		flgPCDestinatarioItem.setWidth(70);
		flgPCDestinatarioItem.addChangedHandler(new ChangedHandler() {

			@Override
			public void onChanged(ChangedEvent event) {
				boolean checked = event.getValue() != null && (Boolean) event.getValue();
					if (checked) {					
						mDynamicForm.setValue("flgAssDestinatario", false);
					}
					if (!checked) {					
						mDynamicForm.setValue("flgAssDestinatario", true);
					}
				mDynamicForm.redraw();
			}
		});
		flgPCDestinatarioItem.setShowIfCondition(new FormItemIfFunction() {
		...
		mDynamicForm.setFields(
			...
			organigrammaItem,
			flgAssDestinatarioItem,
			flgPCDestinatarioItem
		);
	...
	public class UoRegistrazioneLookupOrganigramma extends LookupOrganigrammaPopup {

		public UoRegistrazioneLookupOrganigramma(Record record) {
			super(record, true, new Integer(2));
		}
