...
public class DestinatarioProtUscitaCanvas extends IndirizzoCanvas {
	...
	public boolean showSelectOrganigramma(String tipoDestinatario) {
		if(AurigaLayout.getParametroDBAsBoolean("DEST_INT_CON_SELECT") && !((DestinatarioProtItem) getItem()).isProtPregresso()) {
			// essendo il destinatario di una prot. in uscita quando il tipo non è valorizzato non deve comparire la select organigramma (soggetto interno) ma denominazione perchè di default il soggetto deve essere esterno (persona giuridica)
			return tipoDestinatario != null && tipoDestinatario.equals("UP_UOI"); 
		}
		return false;
	}
	...
	public boolean isIndirizzoObbligatorio() {
		if(mezzoTrasmissioneDestinatarioItem != null) {
			return mezzoTrasmissioneDestinatarioItem.isMezzoTrasmissioneValorizzato() && !mezzoTrasmissioneDestinatarioItem.isEmailPECPEO() && mezzoTrasmissioneDestinatarioItem.showMezzoTrasmissioneItem() && !mezzoTrasmissioneDestinatarioItem.isDestinatarioGruppo() && mezzoTrasmissioneDestinatarioItem.isObbligDettagliMezzoTrasmissione();
		}
		return super.isIndirizzoObbligatorio();
	}
...
//					lRecord.setAttribute("flgAssegnaAlDestinatario", false); // non posso settare il check a false altrimenti non viene caricato successivamente  
					...
					if(mezzoTrasmissioneDestinatarioItem != null) {
						...
						mezzoTrasmissioneDestinatarioItem.setIndirizzoPECDestinatario("");
						mezzoTrasmissioneDestinatarioItem.setIndirizzoPEODestinatario("");			
						mezzoTrasmissioneDestinatarioItem.setIndirizzoMailDestinatario("");			
						mDynamicForm.update();
					}					
				...
				if(mezzoTrasmissioneDestinatarioItem != null) {
					mezzoTrasmissioneDestinatarioItem.markForRedraw();
				}
				showHideIndirizzo();
			}
		});
		...
		codRapidoDestinatarioItem.addChangedBlurHandler(new ChangedHandler() {
			...
			public void onChanged(final ChangedEvent event) {
				...
				if (showSelectOrganigramma()) {
					cercaSoggettoSelectOrganigramma((String) event.getValue());
...
//				mDynamicForm.setValue("organigrammaDestinatario", record.getAttributeAsString("id"));
			...
			protected void clearSelect() {
				...
				if(((DestinatarioProtUscitaItem)getItem()).isDimOrganigrammaNonStd()) {
...
//				}				
		...
		if(((DestinatarioProtUscitaItem) getItem()).isEscludiDestinatariNonEsterni()) {		
		...
		lookupOrganigrammaButton.setShowIfCondition(new FormItemIfFunction() {
			...
			public boolean execute(FormItem item, Object value, DynamicForm form) {
				...
				String tipoDestinatario = tipoDestinatarioItem.getValueAsString();				
				return tipoDestinatario != null && ("UP_UOI".equals(tipoDestinatario) || "UOI".equals(tipoDestinatario) || "UP".equals(tipoDestinatario));					
			}
		});
		...
		cercaInOrganigrammaButton.setShowIfCondition(new FormItemIfFunction() {
			...
			public boolean execute(FormItem item, Object value, DynamicForm form) {
				...
				String tipoDestinatario = tipoDestinatarioItem.getValueAsString();				
				return tipoDestinatario != null && ("UOI".equals(tipoDestinatario) || "UP".equals(tipoDestinatario));
			}
		});
		...
		flgPCItem.addChangedHandler(new ChangedHandler() {
			...
			@Override
			public void onChanged(ChangedEvent event) {
				if(!isInBozza() || (isTipoSoggettoInterno() || isTipoSoggettoLDInterno()) ) {
		...
		flgPCItem.setShowIfCondition(new FormItemIfFunction() {
			...
			public boolean execute(FormItem item, Object value, DynamicForm form) {
				if(AurigaLayout.getParametroDBAsBoolean("SHOW_CC_IN_DEST_REG")) {
					...
					if(isEffettuatoInvioInCC() && !isInBozza()) {
					...
					if(((DestinatarioProtItem) getItem()).isFlgAssegnazioneCondivisioneMutuamenteEsclusivi() && isEffettuataAssegnazione() && !isInBozza()) {
						item.setCanEdit(false);
					}
...
//		downloadFileExcelDestinatariItem.setEndRow(true);
		downloadFileExcelDestinatariItem.addIconClickHandler(new IconClickHandler() {
			...
			@Override
			public void onIconClick(IconClickEvent event) {
				String uriFileExcel = mDynamicForm.getValueAsString("uriFileExcel");
				Record lRecord = new Record();
...
//		downloadTemplateExcelDestinatariItem.setEndRow(true);
		...
		opzioniInvioPCButton.setShowIfCondition(new FormItemIfFunction() {
			...
			public boolean execute(FormItem item, Object value, DynamicForm form) {
				...
				if(!isTipoSoggettoInterno() && !isTipoSoggettoLDInterno()){
					return false;
				}
		...
		flgAssegnaAlDestinatarioItem.setShowIfCondition(new FormItemIfFunction() {
			...
			public boolean execute(FormItem item, Object value, DynamicForm form) {				
				...
				item.setCanEdit(getEditing());
				if(isEffettuataAssegnazione() && !isInBozza()) {
				...
				if(((DestinatarioProtItem) getItem()).isFlgAssegnazioneCondivisioneMutuamenteEsclusivi() && isEffettuatoInvioInCC() && !isInBozza()) {
					item.setCanEdit(false);
				}
...
//				if(/*!fromLoadDett && */!flgSelXAssegnazione && !isInBozza()) {
//					mDynamicForm.setValue("flgAssegnaAlDestinatario", false);
//					((DestinatarioProtItem) getItem()).manageChangedFlgAssegnaAlDestinatario(mDynamicForm.getValuesAsRecord());
//					item.setCanEdit(false);
//				}
		...
		mezzoTrasmissioneDestinatarioItem = new MezzoTrasmissioneDestinatarioItem() {
			...
			public boolean showMezzoTrasmissioneItem() {
				boolean isTipoDestinatarioXLS = tipoDestinatarioItem != null && 
						tipoDestinatarioItem.getValueAsString() != null && "XLS".equals(tipoDestinatarioItem.getValueAsString());
				return showDestinatariEstesi() && !isTipoDestinatarioXLS;
			}
			
			@Override
			public boolean isRequiredMezzoTrasmissioneItem() {
				return AurigaLayout.getParametroDBAsBoolean("MEZZO_TRASM_DEST_OBBL");
			}
			
			@Override
			public String getDefaultValueMezzoTrasmissioneItem() {
				return AurigaLayout.getParametroDB("COD_MEZZO_TRASM_USCITA_DEFAULT");
			}
			
			@Override
			public boolean isDestinatarioGruppo() {
				return tipoDestinatarioItem.getValueAsString() != null && "LD".equals(tipoDestinatarioItem.getValueAsString());
			}
			
			@Override
			public boolean getSoloMezzoTrasmissionePEC() {
				return ((DestinatarioProtUscitaItem) getItem()).getSoloMezzoTrasmissionePEC();
			}
			
			@Override
			public boolean showSelectIndirizzoItem() {
				return showDestinatariEstesi() && !showItemsIndirizzo();
			}			
			
			@Override
			public void manageOnChangedMezzoTrasmissione() {				
				manageOnChangedMezzoTrasmissioneDestinatario();
			}
		};
...
//								MessageType.INFO));
	...
	public boolean showFlgAssegnaAlDestinatarioItem() {
		if(isInBozza()) {
			return isTipoSoggettoInterno() || isTipoSoggettoLDInterno();
		}
	...
	public boolean isTipoSoggettoLDInterno() {
		
		Boolean isTipoSoggettoLD = tipoDestinatarioItem.getValueAsString() != null && "LD".equalsIgnoreCase(tipoDestinatarioItem.getValueAsString());
		
		Boolean isGruppoInterno = false;
		...
		if(idGruppoInterno != null && !"".equalsIgnoreCase(idGruppoInterno)) {
			isGruppoInterno = true;
		}
		
		return isTipoSoggettoLD && isGruppoInterno;
	}
	...
	public boolean showFlgPCItem() {
		if(AurigaLayout.getParametroDBAsBoolean("SHOW_CC_IN_DEST_REG")) {
			if(isInBozza()) {
				if(isTipoSoggettoInterno()) {
					return true;
				} else if(!AurigaLayout.getParametroDBAsBoolean("FLG_CC_SOLO_PER_DEST_IN_ORGANIGRAMMA")) {
					return true;
				}
			}
	...
	public boolean validate() {
		...
		if(mezzoTrasmissioneDestinatarioItem != null) {
			valid = mezzoTrasmissioneDestinatarioItem.validate() && valid;
		}
	...
	public void showHideIndirizzo() {
		try {						
			if(showItemsIndirizzo()) {
				if(((DestinatarioProtUscitaItem) getItem()).isProtInModalitaWizard()) {			
					...
					if(supportoOriginale != null && !"digitale".equalsIgnoreCase(supportoOriginale)) {							
						...
						boolean isDestinatarioInternoGruppoPreferito = tipo != null && ("UP".equals(tipo) || "UOI".equals(tipo) || "LD".equals(tipo) || "PREF".equals(tipo));
						setVisibleIndirizzo(!isDestinatarioInternoGruppoPreferito);
					} else {
						setVisibleIndirizzo(false);
					}				
				} else if(mezzoTrasmissioneDestinatarioItem != null) {
					String tipo = mDynamicForm.getValueAsString("tipoDestinatario");
					boolean isDestinatarioGruppo = tipo != null && "LD".equals(tipo);
					setVisibleIndirizzo(mezzoTrasmissioneDestinatarioItem.isMezzoTrasmissioneValorizzato() && !mezzoTrasmissioneDestinatarioItem.isEmailPECPEO() && mezzoTrasmissioneDestinatarioItem.showMezzoTrasmissioneItem() && !isDestinatarioGruppo, false);									
				}
			}
		} catch(Exception e) {}
	}
	...
	public class DestinatarioLookupSoggettiPopup extends LookupSoggettiPopup {
		...
		public String getFinalita() {	
			if (getItem() instanceof DestinatarioProtUscitaItem) {
				return ((DestinatarioProtUscitaItem) getItem()).getFinalitaLookup();
			} else {
				return null;
			}
		}
		...
		public String[] getTipiAmmessi() {
			if(((DestinatarioProtUscitaItem) getItem()).isEscludiDestinatariNonEsterni() || AurigaLayout.getParametroDBAsBoolean("DEST_USCITA_CERCA_IN_RUBR_SOLO_ESTERNI")) {
...
//		lGwtRestService.addParam("tipiAmmessi", AurigaLayout.getParametroDBAsBoolean("SHOW_AOOI_IN_MITT_DEST") ? "AOOI,UOI,UP,PA,PF,PG" : "UOI,UP,PA,PF,PG");			
	...
	protected void cercaSoggettoSelectOrganigramma(final String codRapido) {
		...
		mDynamicForm.clearErrors(true);
		if (codRapido != null && !"".equals(codRapido)) {
				...
				public void execute(DSResponse response, Object rawData, DSRequest request) {
					...
					if (data.getLength() > 0) {
						for (int i = 0; i < data.getLength(); i++) {
							...
							if ("UO".equals(typeNodo) && codRapido.equals(codice) && (flgSelXFinalita == null || "1".equals(flgSelXFinalita))) {
	...
	public boolean isTipoSoggettoInterno(String tipoDestinatario) {
		if(tipoDestinatario != null && (tipoDestinatario.equals("UOI") || tipoDestinatario.equals("UP") || tipoDestinatario.equals("UP_UOI"))) {						
			return true;
		}
		return false;
	}
...
//			mDynamicForm.setValue("flgAssegnaAlDestinatario", false); // non posso settare il check a false altrimenti non viene caricato successivamente  
	...
	public void setFormValuesFromRecordRubrica(Record record) {
		...
		if (tipoDestinatario != null) {
			...
			mDynamicForm.setValue("indirizzoMailDestinatario", record.getAttribute("email"));												
			setIndirizzoCompletoFromRecordRubrica(record);
			if(mezzoTrasmissioneDestinatarioItem != null) {
				mezzoTrasmissioneDestinatarioItem.refreshFilteredSelectIndirizzoDestinatario(record.getAttribute("idSoggetto"));
				mezzoTrasmissioneDestinatarioItem.setIndirizzoPECDestinatario(record.getAttribute("emailPec"));
				mezzoTrasmissioneDestinatarioItem.setIndirizzoPEODestinatario(record.getAttribute("emailPeo"));
				mezzoTrasmissioneDestinatarioItem.setIndirizzoMailDestinatario(record.getAttribute("email"));
				mDynamicForm.update();
			}			
			showHideIndirizzo();
	...
	public void editRecord(Record record) {
		...
		if (mezzoTrasmissioneDestinatarioItem != null) {
			if (idSoggetto != null && !idSoggetto.equalsIgnoreCase("") && (indirizzoDestinatario == null || "".equalsIgnoreCase(indirizzoDestinatario) || descrizioneIndirizzo == null || "".equalsIgnoreCase(descrizioneIndirizzo))){
				mezzoTrasmissioneDestinatarioItem.refreshFilteredSelectIndirizzoDestinatario(idSoggetto);				
			} else {
	...
	public abstract class CercaSoggettoServiceCallback extends ServiceCallback<Record> {
		...
		public void execute(Record object) {
			...
			if (tipoDestinatario != null) {
				mDynamicForm.setValue("tipoDestinatario", tipoDestinatario);				
				if (showSelectOrganigramma()) {					
					cercaSoggettoSelectOrganigramma(object.getAttribute("codRapidoSoggetto"));
				} else {
					// Pulisco i dati del soggetto
					mDynamicForm.setValue("idDestinatario", "");
					mDynamicForm.setValue("idSoggetto", "");
					...
					if (object.getAttribute("indirizzoMailSoggetto") != null && !object.getAttribute("indirizzoMailSoggetto").equalsIgnoreCase("")) {
						mDynamicForm.setValue("indirizzoMailDestinatario", object.getAttribute("indirizzoMailSoggetto"));
					}					
					...
					if(mezzoTrasmissioneDestinatarioItem != null) {
						mezzoTrasmissioneDestinatarioItem.refreshFilteredSelectIndirizzoDestinatario(idSoggetto);
						if (object.getAttribute("indirizzoPecSoggetto") != null && !object.getAttribute("indirizzoPecSoggetto").equalsIgnoreCase("")) {
							mezzoTrasmissioneDestinatarioItem.setIndirizzoPECDestinatario(object.getAttribute("indirizzoPecSoggetto"));
							mDynamicForm.update();
						}
						if (object.getAttribute("indirizzoPeoSoggetto") != null && !object.getAttribute("indirizzoPeoSoggetto").equalsIgnoreCase("")) {
							mezzoTrasmissioneDestinatarioItem.setIndirizzoPEODestinatario(object.getAttribute("indirizzoPeoSoggetto"));
							mDynamicForm.update();
						}
						if (object.getAttribute("indirizzoMailSoggetto") != null && !object.getAttribute("indirizzoMailSoggetto").equalsIgnoreCase("")) {
							mezzoTrasmissioneDestinatarioItem.setIndirizzoMailDestinatario(object.getAttribute("indirizzoMailSoggetto"));
							mDynamicForm.update();
						}
					}
					showHideIndirizzo();
	...
	public boolean showDestinatariEstesi() {
		return ((DestinatarioProtUscitaItem) getItem()).getShowDestinatariEstesi();
	}
	...
	public boolean showOpenIndirizzo() {
		return ((DestinatarioProtUscitaItem) getItem()).showOpenIndirizzo();
	}
	...
	public void manageOnChangedMezzoTrasmissioneDestinatario() {
		showHideIndirizzo();
	}
	
	public void manageOnChangedUoProtocollante() {
		if(mezzoTrasmissioneDestinatarioItem != null) {
			mezzoTrasmissioneDestinatarioItem.manageOnChangedUoProtocollante();
		}
		showHideIndirizzo();
	}
