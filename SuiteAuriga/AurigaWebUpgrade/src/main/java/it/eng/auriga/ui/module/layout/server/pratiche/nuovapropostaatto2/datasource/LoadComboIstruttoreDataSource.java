...
@Datasource(id="LoadComboIstruttoreDataSource")
public class LoadComboIstruttoreDataSource extends SelectDataSource<ScrivaniaOrganigrammaBean> {
	...
	public PaginatorBean<ScrivaniaOrganigrammaBean> realFetch(AdvancedCriteria criteria, Integer startRow,
		...
		String altriParamComboEstensori = ParametriDBUtil.getParametroDB(getSession(), "ALTRI_PARAM_COMBO_ISTRUTTORI");
		...
		if(StringUtils.isNotBlank(getExtraparams().get("altriParamLoadCombo"))) {
			...
			if(StringUtils.isNotBlank(altriParamComboEstensori)) {
				altriParametri += altriParamComboEstensori;
			}
			lDmpkLoadComboDmfn_load_comboBean.setAltriparametriin(altriParametri);							
		}
