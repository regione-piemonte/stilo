...
<xsl:stylesheet version="1.0"
		...
		<xsl:element namespace="{$namespace}" name="{$tableElement}">
			...
			<xsl:choose>
				<xsl:when test='name()="table:table"'>
					<xsl:variable name="value" select="$globalData/all-doc-styles/style[@style:name = current()/@table:style-name]/*/@table:border-model" />
					<xsl:choose>
						<xsl:when test="$value!=''">
			...
			<xsl:attribute name="name">
				<xsl:value-of select="current()/@table:name"/>
			</xsl:attribute>			
	...
	<xsl:template name="set-styles">
		...
		<xsl:attribute name="width">
			<xsl:for-each select="//style:style[@style:name = $currentTableColumn/@table:style-name]">
				<xsl:for-each select="current()/style:table-column-properties">
					<xsl:if test="current()/@style:column-width">
						<xsl:call-template name="convert2px">
							<xsl:with-param name="value" select="current()/@style:column-width"/>
						</xsl:call-template>	
					</xsl:if>
				</xsl:for-each>
			</xsl:for-each>
		...
		<xsl:attribute name="height">
			<xsl:for-each select="//style:style[@style:name = current()/../@table:style-name]">
				<xsl:for-each select="current()/style:table-row-properties">
					<xsl:if test="current()/@style:min-row-height">
						<xsl:call-template name="convert2px">
							<xsl:with-param name="value" select="current()/@style:min-row-height"/>
						</xsl:call-template>	
					</xsl:if>
				</xsl:for-each>
			</xsl:for-each>
