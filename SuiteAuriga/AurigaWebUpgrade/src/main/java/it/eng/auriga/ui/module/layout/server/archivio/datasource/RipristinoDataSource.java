...
import it.eng.client.DmpkCoreRecovertosezarealav;
...
@Datasource(id = "RipristinoDataSource")
public class RipristinoDataSource extends AbstractDataSource<OperazioneMassivaArchivioBean, OperazioneMassivaArchivioBean>{	
	...
	public OperazioneMassivaArchivioBean add(OperazioneMassivaArchivioBean bean) throws Exception {		
		...
		for(ArchivioBean udFolder : bean.getListaRecord()) {
			DmpkCoreRecovertosezarealavBean input = new DmpkCoreRecovertosezarealavBean();
			input.setCodidconnectiontokenin(token);
			input.setIduserlavoroin(StringUtils.isNotBlank(idUserLavoro) ? new BigDecimal(idUserLavoro) : null);
			input.setCisezionearealavin(udFolder.getEliminatoDa());
			input.setFlgtipoobjin(udFolder.getFlgUdFolder());
			input.setIdobjin(new BigDecimal(udFolder.getIdUdFolder()));
			
			DmpkCoreRecovertosezarealav dmpkCoreRecovertosezarealav = new DmpkCoreRecovertosezarealav();
			StoreResultBean<DmpkCoreRecovertosezarealavBean> output = dmpkCoreRecovertosezarealav.execute(getLocale(),loginBean, input);
