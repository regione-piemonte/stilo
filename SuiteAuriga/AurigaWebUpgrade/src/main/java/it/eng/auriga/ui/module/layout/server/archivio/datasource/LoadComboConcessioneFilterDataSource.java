...
import it.eng.auriga.ui.module.layout.server.archivio.datasource.bean.ConcessioneComboXmlBeanDeserializationHelper;
...
@Datasource(id="LoadComboConcessioneFilterDataSource")
public class LoadComboConcessioneFilterDataSource extends OptionFetchDataSource<ConcessioneComboXmlBean> {
	
	@Override
	public PaginatorBean<ConcessioneComboXmlBean> fetch(AdvancedCriteria criteria, Integer startRow, Integer endRow, List<OrderByBean> orderby) throws Exception {
		...
		if(StringUtils.isNotBlank(xmlLista)) {			
			ConcessioneComboXmlBeanDeserializationHelper helper = new ConcessioneComboXmlBeanDeserializationHelper(new HashMap<String, String>());
			lListResult = XmlListaUtility.recuperaLista(xmlLista, ConcessioneComboXmlBean.class, helper);
		}	
