...
@Datasource(id = "InCaricoSelectDataSource")
public class InCaricoSelectDataSource extends OptionFetchDataSource<UtenteBean> {
...
//		String valid = getExtraparams().get("soloValidi");
		...
		if (!"true".equals(skip)) {
			...
			UtenteBean lBeanNone = new UtenteBean();

			lBeanNone.setCodice("#NONE");
			lBeanNone.setCognomeNome("<i>nessuno</i>");
			lBeanNone.setIdUtente("#NONE");
			...
			lBeanAny.setCognomeNome("<i>a me</i>");
			lBeanAny.setIdUtente("#ME");

			resultList.add(lBeanAny);

			UtenteBean lBeanOthers = new UtenteBean();
			lBeanOthers.setCodice("#OTHERS");
			lBeanOthers.setCognomeNome("<i>ad altri</i>");
			lBeanOthers.setIdUtente("#OTHERS");

			resultList.add(lBeanOthers);
		}
