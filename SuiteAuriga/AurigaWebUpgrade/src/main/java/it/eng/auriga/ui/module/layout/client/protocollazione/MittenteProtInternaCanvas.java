...
public class MittenteProtInternaCanvas extends IndirizzoCanvas {
	...
	public MittenteProtInternaCanvas(ReplicableItem item) {
		super(item);
	}
	...
	public String getTipoAssegnatari() {
		if (isInBozza() && ((MittenteProtItem)getItem()).getFlgSoloInOrganigramma() && (!AurigaLayout.getParametroDBAsBoolean("SHOW_UP_IN_MITT_REG"))) {
			return "UO";
		}else if (isInBozza() && ((MittenteProtItem)getItem()).getFlgSoloInOrganigramma() && (AurigaLayout.getParametroDBAsBoolean("SHOW_UP_IN_MITT_REG"))) {
			return "UO;SV";
	...
	public void buildMainForm() {
		...
		LinkedHashMap<String,String> styleMap = getTipoMittenteMap();
		if (getAllowOnlyUnitaDiPersonale()) {
			tipoMittenteItem.setAllowEmptyValue(false);
			tipoMittenteItem.setDefaultValue("UP");
		} else {
...
//					lRecord.setAttribute("flgAssegnaAlMittente", false); // non posso settare il check a false altrimenti non viene caricato successivamente  
					...
					((MittenteProtInternaItem)getItem()).manageOnChanged();
				}
			}
		});
						...
						public void executeOnError(boolean trovatiSoggMultipliInRicerca) {
							...
							if(((MittenteProtItem) getItem()).getFlgSoloInOrganigramma()) {
								...
								mDynamicForm.setFieldErrors("codRapidoMittente", I18NUtil.getMessages()
										.protocollazione_detail_codrapidosoggetto_esitoValidazione_KO("mittente", "interna"));
							}
						}
					});
				}
		...
		organigrammaMittenteDS.addParam("finalita", getFinalitaOrganigramma());		
...
//				mDynamicForm.setValue("organigrammaMittente", record.getAttributeAsString("id"));
			...
			protected void clearSelect() {
				...
				if(((MittenteProtInternaItem)getItem()).isDimOrganigrammaNonStd()) {
...
//				}				
				...
				organigrammaMittenteDS.addParam("idUd", ((MittenteProtItem) getItem()).getIdUdProtocollo());
				organigrammaMittenteDS.addParam("tipoAssegnatari", getTipoAssegnatari());
				organigrammaMittenteDS.addParam("uoProtocollante", ((MittenteProtInternaItem) getItem()).getUoProtocollante());
				organigrammaMittenteItem.setOptionDataSource(organigrammaMittenteDS);
		...
		denominazioneMittenteItem.addChangeHandler(new ChangeHandler() {
			...
			@Override
			public void onChange(ChangeEvent event) {
				((MittenteProtInternaItem) getItem()).setCercaInRubricaAfterChanged(true);
			}
		});
		...
		lookupRubricaButton.addIconClickHandler(new IconClickHandler() {
			...
			public void onIconClick(IconClickEvent event) {
				((MittenteProtInternaItem) getItem()).setCercaInRubricaAfterChanged(false);	
		...
		lookupOrganigrammaButton.setShowIfCondition(new FormItemIfFunction() {
			...
			@Override
			public boolean execute(FormItem item, Object value, DynamicForm form) {
				if(((MittenteProtItem) getItem()).isRichiestaPubblicazioneAlbo()) return true;
				if(showSelectOrganigramma() && isInBozza()) return true;
				if(showSelectOrganigramma()) return false;
				String tipoMittente = tipoMittenteItem.getValueAsString();
		...
		salvaInRubricaButton.addIconClickHandler(new IconClickHandler() {
			...
			public void onIconClick(IconClickEvent event) {
				...
				SalvaInRubricaPopup salvaInRubricaPopup = new SalvaInRubricaPopup("SEL_SOGG_INT", recordMittente, recordIndirizzo) {
		...
		flgAssegnaAlMittenteItem.setShowIfCondition(new FormItemIfFunction() {
			...
			@Override
			public boolean execute(FormItem item, Object value, DynamicForm form) {
				if(((MittenteProtItem) getItem()).isRichiestaPubblicazioneAlbo() || isInBozza()) {
...
//			mDynamicForm.setValue("flgAssegnaAlMittente", false); // non posso settare il check a false altrimenti non viene caricato successivamente  
	...
	public class MittenteLookupRubrica extends LookupSoggettiPopup {
		...
		public String getFinalita() {
			if (getItem() instanceof MittenteProtInternaItem) {
				return ((MittenteProtInternaItem) getItem()).getFinalitaLookupRubrica();
			} else {
				return "SEL_SOGG_INT";
			}
		}
	...
	public class MittenteLookupOrganigramma extends LookupOrganigrammaPopup {
		...
		public String getFinalita() {	
			if (getItem() instanceof MittenteProtInternaItem) {
				return ((MittenteProtInternaItem) getItem()).getFinalitaLookupOrganigramma();	
			} else {
	...
	protected void cercaInRubricaAfterChangedField(final Record record) {
		if(AurigaLayout.getParametroDBAsBoolean("ATTIVA_CERCA_IN_RUBRICA")) {
			Timer t1 = new Timer() {
				public void run() {
					if(((MittenteProtInternaItem) getItem()).isCercaInRubricaAfterChanged()) {
						if(((MittenteProtItem) getItem()).getFlgSoloInOrganigramma()) {							
	...
	public void setFormValuesFromRecordRubrica(Record record) {
		...
		if (tipoMittente != null) {
			...
			mDynamicForm.setFieldErrors("codRapidoMittente",
					I18NUtil.getMessages().protocollazione_detail_codrapidosoggetto_esitoValidazione_KO("mittente", "interna"));
		}
	}
	...
	public boolean isMittenteValorizzato() {
		...
		String cognome = cognomeMittenteItem.isVisible() ? cognomeMittenteItem.getValueAsString() : null;
		String nome = nomeMittenteItem.isVisible() ? nomeMittenteItem.getValueAsString() : null;
		String organigramma = organigrammaMittenteItem.isVisible() ? organigrammaMittenteItem.getValueAsString() : null;
		return ((denominazione != null && !"".equals(denominazione)) || ((cognome != null && !"".equals(cognome)) && (nome != null && !"".equals(nome))) || (organigramma != null && !"".equals(organigramma)));
	}
	...
	public boolean showItemsIndirizzo() {
		return ((MittenteProtInternaItem) getItem()).getShowItemsIndirizzo();
	}
	
	public boolean showItemSelTipoMittente() {
		return ((MittenteProtInternaItem) getItem()).getShowItemSelTipoMittente();
	}
	
	public boolean getAllowOnlyUnitaDiPersonale() {
		return ((MittenteProtInternaItem) getItem()).getAllowOnlyUnitaDiPersonale();
	}
	
	public boolean isInBozza() {
		return ((MittenteProtItem) getItem()).isInBozza();
	}
	
	public String getFinalitaOrganigramma() {
		if (AurigaLayout.getParametroDBAsBoolean("ESTENDI_SCELTA_MITT_BOZZA") && isInBozza() && ((MittenteProtItem) getItem()).getFlgSoloInOrganigramma()) {
			return "ASS";
		} else if (((MittenteProtInternaItem) getItem()).getFinalitaOrganigramma() != null ){
			return ((MittenteProtInternaItem)getItem()).getFinalitaOrganigramma();
		} else {
			return "MITTENTE_REG";
		}
	}
