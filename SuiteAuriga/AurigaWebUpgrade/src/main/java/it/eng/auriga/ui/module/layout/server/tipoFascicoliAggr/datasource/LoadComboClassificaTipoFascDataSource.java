...
package it.eng.auriga.ui.module.layout.server.tipoFascicoliAggr.datasource;
 ...
 *
 */
@Datasource(id = "LoadComboClassificaTipoFascDataSource")
public class LoadComboClassificaTipoFascDataSource extends SelectDataSource<ClassificaBean> {
	
	private static Logger mLogger = Logger.getLogger(LoadComboClassificaTipoFascDataSource.class);

	@Override
	public PaginatorBean<ClassificaBean> realFetch(AdvancedCriteria criteria, Integer startRow, Integer endRow, List<OrderByBean> orderby) throws Exception {
		...
		if (!"".equals(descrizione) || !"".equals(indice)) {
			...
			lDmpkLoadComboDmfn_load_comboBean.setAltriparametriin("ID_USER_LAVORO|*|" + idUserLavoro + "|*|INDICE|*|" + indice + "|*|STR_IN_DES|*|"
					+ descrizione + "|*|CI_TO_ADD|*|" + idClassifica);
