...
@Datasource(id = "VisurePersonaliDatasource")
public class VisurePersonaliDatasource extends AurigaAbstractFetchDatasource<VisureBean> {

	private static Logger mLogger = Logger.getLogger(VisurePersonaliDatasource.class);
	
	@Override
	public String getNomeEntita() {
		return "visure_personali";
	}
	...
	public PaginatorBean<VisureBean> fetch(AdvancedCriteria criteria, Integer startRow, Integer endRow,
		...
		for (VisureXmlBean lVisureXmlBean : lListResult) {
			VisureBean lVisureBean = new VisureBean();
			BeanUtilsBean2.getInstance().copyProperties(lVisureBean, lVisureXmlBean);
			if(lVisureXmlBean.getRichAttiDiFabbrica() != null && 
					"1".equalsIgnoreCase(lVisureXmlBean.getRichAttiDiFabbrica())) {
				lVisureBean.setRichAttiDiFabbrica("SI");
			} else {
				lVisureBean.setRichAttiDiFabbrica("NO");
			}
			if(lVisureXmlBean.getRichModifiche() != null && 
					"1".equalsIgnoreCase(lVisureXmlBean.getRichModifiche())) {
