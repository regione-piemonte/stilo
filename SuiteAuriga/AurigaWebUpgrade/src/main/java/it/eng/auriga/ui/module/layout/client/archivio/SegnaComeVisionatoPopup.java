...
public abstract class SegnaComeVisionatoPopup extends ModalWindow {
	...
	public SegnaComeVisionatoPopup(String titolo, String noteOld, final RecordList listaUoDestNotifiche) {
		
		super("segnaComeVisionatoPopup", true);
		...
		flgDestinatarioItem = new RadioGroupItem("flgDestinatario", "Segna come visionato");   
		...
		if("UT".equalsIgnoreCase(AurigaLayout.getParametroDB("DEFAULT_OPZ_SEGNA_VISIONATO"))) {
			flgDestinatarioItem.setDefaultValue(_SOLO_PER_ME);
		} else {
			if(listaUoDestNotifiche != null && listaUoDestNotifiche.getLength() > 0) {
				if("UOUT".equalsIgnoreCase(AurigaLayout.getParametroDB("DEFAULT_OPZ_SEGNA_VISIONATO"))) {
