...
@Datasource(id = "LoadComboCdCAdottanteDataSource")
public class LoadComboCdCAdottanteDataSource extends AbstractFetchDataSource<SimpleKeyValueBean> {
	...
	public PaginatorBean<SimpleKeyValueBean> fetch(AdvancedCriteria criteria, Integer startRow, Integer endRow, List<OrderByBean> orderby) throws Exception {
		...
		String adottante = StringUtils.isNotBlank(getExtraparams().get("adottante")) ? getExtraparams().get("adottante") : "";
		if (adottante.startsWith("SV")) {
			adottante = adottante.substring(2);
		}
		...
		lDmpkLoadComboDmfn_load_comboBean.setAltriparametriin("ID_SV_ADOTTANTE|*|" + adottante);
