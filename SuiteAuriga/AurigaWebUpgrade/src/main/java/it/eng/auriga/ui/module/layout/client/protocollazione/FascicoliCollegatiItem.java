...
public class FascicoliCollegatiItem extends ReplicableItem {

	@Override
	public ReplicableCanvas getCanvasToReply() {
		FascicoloCollegatoCanvas lFascicoloCollegatoCanvas = new FascicoloCollegatoCanvas(this);
		return lFascicoloCollegatoCanvas;
	}
	...
	protected ImgButton[] createAddButtons() {
		...
		addButtons[1].addClickHandler(new ClickHandler() {	
			@Override
			public void onClick(ClickEvent event) {
				FascicoloCollegatoMultiLookupArchivio lookupArchivioPopup = new FascicoloCollegatoMultiLookupArchivio(null);				
	...
	public class FascicoloCollegatoMultiLookupArchivio extends LookupArchivioPopup {

			private List<FascicoloCollegatoCanvas> multiLookupList = new ArrayList<FascicoloCollegatoCanvas>(); 
			
			public FascicoloCollegatoMultiLookupArchivio(Record record) {
				super(record, null, false);			
			}
			
			@Override
			public String getWindowTitle() {
				return "Seleziona fascicoli da collegare";
			}
			...
			public void manageMultiLookupBack(Record record) {
				FascicoloCollegatoCanvas lastCanvas = (FascicoloCollegatoCanvas) getLastCanvas();
				if(lastCanvas != null && !lastCanvas.isChanged()) {
					lastCanvas.setFormValuesFromRecordArchivio(record);
					multiLookupList.add(lastCanvas);
				} else {
					FascicoloCollegatoCanvas canvas = (FascicoloCollegatoCanvas) onClickNewButton();
			...
			public void manageMultiLookupUndo(Record record) {	
				for(FascicoloCollegatoCanvas canvas : multiLookupList) {
					Record values = canvas.getFormValuesAsRecord();
					if(values.getAttribute("idFolderCollegata").equals(record.getAttribute("id"))) {
					...
					if(lVLayoutMember instanceof HLayout) {				
						...
						for(Canvas lHLayoutMember : ((HLayout)lVLayoutMember).getMembers()) {
							if(lHLayoutMember instanceof ImgButton) {
								...
								FascicoloCollegatoCanvas lFascicoloCollegatoCanvas = (FascicoloCollegatoCanvas)lHLayoutMember;
								lFascicoloCollegatoCanvas.setCanEdit(canEdit);
							}							
						}	
					}			
				}
