...
public class DelegatoItem extends ReplicableItem {
	...
	public ReplicableCanvas getCanvasToReply() {
		DelegatoCanvas lDelegatoCanvas = new DelegatoCanvas(this);
		return lDelegatoCanvas;
	}
	...
	protected ImgButton[] createAddButtons() {
		...
		addButtons[1].addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				...
				DelegatoMultiLookupRubrica lookupRubricaPopup = new DelegatoMultiLookupRubrica(null);
	...
	public class DelegatoMultiLookupRubrica extends LookupSoggettiPopup {

		private List<DelegatoCanvas> multiLookupList = new ArrayList<DelegatoCanvas>();

		public DelegatoMultiLookupRubrica(Record record) {
		...
		public void manageMultiLookupBack(Record record) {
			DelegatoCanvas lastCanvas = (DelegatoCanvas) getLastCanvas();
			if (lastCanvas != null && !lastCanvas.isChangedAndValid()) {
				...
				DelegatoCanvas canvas = (DelegatoCanvas) onClickNewButton();
		...
		public void manageMultiLookupUndo(Record record) {
			for (DelegatoCanvas canvas : multiLookupList) {
