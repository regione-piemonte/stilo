...
public class DocFascAssegnatiStrutturaVisCanvas extends ReplicableCanvas {
	...
	private void buildItems() {		
		...
		modificaButton.addIconClickHandler(new IconClickHandler() {
			...
			public void onIconClick(IconClickEvent event) {
				...
				String mode = ((VisualizzaDocumentiFascicoliStrutturaItem)getItem()).getMode();	
	...
	public boolean isAbilToModUoAssociateUtente() {
		return Layout.isPrivilegioAttivo("SIC/SO;M") && (((VisualizzaDocumentiFascicoliStrutturaItem)getItem()).isEditMode() || 
				((VisualizzaDocumentiFascicoliStrutturaItem)getItem()).isNewMode());
	}
	
	public boolean isAbilToDelUoAssociateUtente() {
		return Layout.isPrivilegioAttivo("SIC/SO;FC") && (((VisualizzaDocumentiFascicoliStrutturaItem)getItem()).isEditMode() || 
				((VisualizzaDocumentiFascicoliStrutturaItem)getItem()).isNewMode());
	}
