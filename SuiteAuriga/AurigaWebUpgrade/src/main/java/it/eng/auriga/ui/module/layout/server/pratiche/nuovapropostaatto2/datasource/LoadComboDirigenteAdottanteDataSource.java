...
@Datasource(id = "LoadComboDirigenteAdottanteDataSource")
public class LoadComboDirigenteAdottanteDataSource extends SelectDataSource<ScrivaniaOrganigrammaBean> {
	...
	public PaginatorBean<ScrivaniaOrganigrammaBean> realFetch(AdvancedCriteria criteria, Integer startRow,
		...
		if(StringUtils.isNotBlank(getExtraparams().get("altriParamLoadCombo"))) {		
			...
			String altriParametri = "ID_UO|*|" + uoProponente + "|*|FLG_INCL_UOSUP|*|1|*|DES_RUOLO|*|$DIRIGENTE$|*|STR_IN_DES|*|" + descrizione + "|*|CI_TO_ADD|*|" + idSv;
			if(StringUtils.isNotBlank(idUdAttoDaAnn)) {
				altriParametri += "|*|ID_UD_ATTO_DA_ANN|*|" + idUdAttoDaAnn;
			}
			lDmpkLoadComboDmfn_load_comboBean.setAltriparametriin(altriParametri);								
		}
