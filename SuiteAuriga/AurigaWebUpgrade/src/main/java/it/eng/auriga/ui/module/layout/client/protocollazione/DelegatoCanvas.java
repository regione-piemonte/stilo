...
public class DelegatoCanvas extends IndirizzoCanvas {
	...
	public DelegatoCanvas(ReplicableItem item) {
		super(item);		
	}
	...
	public void buildMainForm() {
		...
		tipoSoggettoItem.addChangedHandler(new ChangedHandler() {
			...
			public void onChanged(ChangedEvent event) {
				...
				if (!("".equals(oldValue) && toShowDenominazione(value)) && !(toShowDenominazione(oldValue) && "".equals(value))
					...
					((DelegatoItem) getItem()).manageOnChanged();
				}
			}
		});
		...
		denominazioneItem.setValidators(new RequiredIfValidator(new RequiredIfFunction() {
			...
			public boolean execute(FormItem formItem, Object value) {
				return toShowDenominazione(tipoSoggettoItem.getValueAsString()) && (((DelegatoItem) getItem()).isRequiredDenominazione(mDynamicForm.hasValue()));
			}
		}));
		...
		denominazioneItem.addChangeHandler(new ChangeHandler() {
			...
			@Override
			public void onChange(ChangeEvent event) {
				((DelegatoItem) getItem()).setCercaInRubricaAfterChanged(true);
			}
		});
		...
		cognomeItem.setValidators(new RequiredIfValidator(new RequiredIfFunction() {
			...
			@Override
			public boolean execute(FormItem formItem, Object value) {
				return isPersonaFisica(tipoSoggettoItem.getValueAsString()) && (((DelegatoItem) getItem()).isRequiredDenominazione(mDynamicForm.hasValue()));
			}
		}));
		...
		emailItem.setShowIfCondition(new FormItemIfFunction() {
			...
			@Override
			public boolean execute(FormItem item, Object value, DynamicForm form) {
				return ((DelegatoItem)getItem()).getShowEmail();
			}
		});
		emailItem.setValidators(new CustomValidator() {
			...
			@Override
			protected boolean condition(Object value) {
				if(((DelegatoItem)getItem()).getShowEmail()) {
		...
		telItem.setShowIfCondition(new FormItemIfFunction() {			
			@Override
			public boolean execute(FormItem item, Object value, DynamicForm form) {
				return ((DelegatoItem)getItem()).getShowTelefono();
			}
		});			
		...
		lookupRubricaButton.addIconClickHandler(new IconClickHandler() {
			...
			public void onIconClick(IconClickEvent event) {
				((DelegatoItem) getItem()).setCercaInRubricaAfterChanged(false);			
				DelegatoLookupSoggettiPopup lookupRubricaPopup = new DelegatoLookupSoggettiPopup(null, tipoSoggettoItem.getValueAsString());
		...
		salvaInRubricaButton.addIconClickHandler(new IconClickHandler() {
			...
			public void onIconClick(IconClickEvent event) {
				...
				Record recordDelegato = new Record();
				recordDelegato.setAttribute("tipo", tipo);
				recordDelegato.setAttribute("codRapido", codRapido);
				recordDelegato.setAttribute("denominazione", denominazione);
				recordDelegato.setAttribute("cognome", cognome);
				recordDelegato.setAttribute("nome", nome);
				recordDelegato.setAttribute("codiceFiscale", codiceFiscale);
				Record recordIndirizzo = getFormValuesAsRecord();
				SalvaInRubricaPopup salvaInRubricaPopup = new SalvaInRubricaPopup(null, recordDelegato, recordIndirizzo) {
		...
		visualizzaInRubricaButton = new ImgButtonItem("visualizzaInRubricaDelegato", "buttons/detail.png", I18NUtil.getMessages()
	...
	public class DelegatoLookupSoggettiPopup extends LookupSoggettiPopup {

		public DelegatoLookupSoggettiPopup(Record record, String tipoSoggetto) {
	...
	protected boolean isPersonaGiuridica(String tipoSoggetto) {
		return ((DelegatoItem) getItem()).isPersonaGiuridica(tipoSoggetto);
	}

	protected boolean isPersonaFisica(String tipoSoggetto) {
		return ((DelegatoItem) getItem()).isPersonaFisica(tipoSoggetto);
	}
	...
	protected void cercaInRubricaAfterChangedField(final Record record) {
		if(AurigaLayout.getParametroDBAsBoolean("ATTIVA_CERCA_IN_RUBRICA")) {
			Timer t1 = new Timer() {
				public void run() {
					if(((DelegatoItem) getItem()).isCercaInRubricaAfterChanged()) {
		...
		cercaSoggetto(record, new CercaSoggettoServiceCallback() {
			...
			public void executeOnError(boolean trovatiSoggMultipliInRicerca) {
				if(showLookupWithNoResults || trovatiSoggMultipliInRicerca) {
					DelegatoLookupSoggettiPopup lookupRubricaPopup = new DelegatoLookupSoggettiPopup(record, tipoSoggettoItem.getValueAsString());
					lookupRubricaPopup.show();
				}
			}
		});
