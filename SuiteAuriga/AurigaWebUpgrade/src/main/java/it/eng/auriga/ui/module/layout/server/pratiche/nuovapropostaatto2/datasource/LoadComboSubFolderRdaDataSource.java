...
@Datasource(id = "LoadComboSubFolderRdaDataSource")
public class LoadComboSubFolderRdaDataSource extends OptionFetchDataSource<SimpleKeyValueBean> {
	...
	public PaginatorBean<SimpleKeyValueBean> fetch(AdvancedCriteria criteria, Integer startRow, Integer endRow,
		...
		inputParamComboBean.setTipocomboin("SUBFOLDER_PREDEFINITI");
		...
		for (XmlListaSimpleBean lXmlListaSimpleBean : listaXML) {// key = colonna 1/NomeFolder | value = colonna 2/TipoFoler -- Per il momento, folder non lo usiamo - AURIGA-708
			SimpleKeyValueBean lBean = new SimpleKeyValueBean();
			lBean.setKey(lXmlListaSimpleBean.getKey());
			lBean.setValue(lXmlListaSimpleBean.getKey());
