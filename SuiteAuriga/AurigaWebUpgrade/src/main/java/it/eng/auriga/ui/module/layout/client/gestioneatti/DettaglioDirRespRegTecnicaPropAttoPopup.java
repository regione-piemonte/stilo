...
public class DettaglioDirRespRegTecnicaPropAttoPopup extends ModalWindow {

	private DettaglioDirRespRegTecnicaPropAttoPopup window;
	...
	private CustomLayout fromLayout;
	
	public DettaglioDirRespRegTecnicaPropAttoPopup(CustomLayout fromLayout, Record record, String title, final boolean isModificaTesto) {
		
		super("dettaglio_dir_resp_reg_tecnica_prop_atto", true);
		...
		setIcon("attiInLavorazione/dirRespRegTecnicaAttoProp.png");
		...
		if (idProcess != null && !"".equals(idProcess)) {
			callExecAtt(recordCallExecAtt, new ServiceCallback<Record>() {
				...
				public void execute(final Record lRecordEvento) {		
					...
					String tabIDIniziale = NuovaPropostaAtto2CompletaDetail._TAB_DATI_SCHEDA_ID;
				...
				if (response.getStatus() == DSResponse.STATUS_SUCCESS) {
					...
					if (savedRecord.getAttribute("flgIgnoreWarning") == null || savedRecord.getAttributeAsInt("flgIgnoreWarning") != 1) {
						try {
							...
							if(getFromLayout() != null) {
								getFromLayout().reloadList();
							}
							manageOnCloseClick();
						} catch (Exception e) {
							Layout.hideWaitPopup();
	...
	protected void realSave(final Record record, final DSCallback callback) {
		...
		try {
			...
			lNuovaPropostaAtto2DataSource.executecustom("modificaAltriRespRegTecnicaCompleta", record, new DSCallback() {
		...
		lNuovaPropostaAtto2DataSource.getData(lRecordToLoad, new DSCallback() {
			...
			public void execute(final DSResponse response, Object rawData, DSRequest request) {
				...
				if (response.getStatus() == DSResponse.STATUS_SUCCESS) {
					...
					detail = new NuovaPropostaAtto2CompletaDetail("dettaglio_dir_resp_reg_tecnica_prop_atto", tabs) {
						
						@Override
						public void build() {
							
							Record parametriTipoAttoNew = recordEvento != null ? recordEvento.getAttributeAsRecord("parametriTipoAtto") : null;
							
							if (parametriTipoAttoNew!=null){
								RecordList lAttributiCustomCablati = parametriTipoAttoNew != null ? parametriTipoAttoNew.getAttributeAsRecordList("attributiCustomCablati") : null;								
								if(lAttributiCustomCablati != null) {
									RecordList attributiCustomCablatiNew = new RecordList();
									for(int i = 0; i < lAttributiCustomCablati.getLength(); i++) {
										Record lRecordAttributiCustomCablati = lAttributiCustomCablati.get(i);
										if (lRecordAttributiCustomCablati.getAttribute("attrName")!=null && !lRecordAttributiCustomCablati.getAttribute("attrName").equalsIgnoreCase("")){
											// Mostra la lista degli altri dirigenti resp. reg. tecnica (listaAltriDirRespRegTecnicaItem)
											if (lRecordAttributiCustomCablati.getAttribute("attrName").equalsIgnoreCase("ID_SV_ALTRI_DIR_REG_TECNICA")){
												attributiCustomCablatiNew.add(lRecordAttributiCustomCablati);
											}
											if (lRecordAttributiCustomCablati.getAttribute("attrName").equalsIgnoreCase("ID_SV_SOSTITUTO_ALTRI_DIR_REG_TECNICA")){
												attributiCustomCablatiNew.add(lRecordAttributiCustomCablati);
											}
											if (lRecordAttributiCustomCablati.getAttribute("attrName").equalsIgnoreCase("PROVV_SOSTITUZIONE_ALTRI_DIR_REG_TECNICA")){
												attributiCustomCablatiNew.add(lRecordAttributiCustomCablati);
											}
											// Mostra la lista dei dirigenti resp. reg. tecnica (listaDirRespRegTecnicaItem)
											if (lRecordAttributiCustomCablati.getAttribute("attrName").equalsIgnoreCase("ID_SV_DIR_RESP_REG_TECNICA")){
												attributiCustomCablatiNew.add(lRecordAttributiCustomCablati);		
											}
											if (lRecordAttributiCustomCablati.getAttribute("attrName").equalsIgnoreCase("ID_SV_SOSTITUTO_DIR_RESP_REG_TECNICA")){
												attributiCustomCablatiNew.add(lRecordAttributiCustomCablati);		
											}
											if (lRecordAttributiCustomCablati.getAttribute("attrName").equalsIgnoreCase("PROVV_SOSTITUZIONE_DIR_RESP_REG_TECNICA")){
												attributiCustomCablatiNew.add(lRecordAttributiCustomCablati);		
											}
										}
									}	
									parametriTipoAttoNew.setAttribute("attributiCustomCablati", attributiCustomCablatiNew);
									recordEvento.setAttribute("parametriTipoAtto", parametriTipoAttoNew);
								}
							}
	...
	public CustomLayout getFromLayout() {
		return fromLayout;
	}
	
	public void setFromLayout(CustomLayout fromLayout) {
		this.fromLayout = fromLayout;
	}
}