...
public class DettaglioDatiPubblAttoPopup extends ModalWindow {

	private DettaglioDatiPubblAttoPopup window;
	...
	public DettaglioDatiPubblAttoPopup(Record record, String title) {
	
		super("dettaglio_dati_pubbl_atto", true);
		...
		if (idProcess != null && !"".equals(idProcess)) {
			callExecAtt(recordCallExecAtt, new ServiceCallback<Record>() {
				...
				public void execute(final Record lRecordEvento) {		
					...
					String tabIDIniziale = NuovaPropostaAtto2CompletaDetail._TAB_DATI_PUBBL_ID;
	...
	protected void realSave(final Record record, final DSCallback callback) {
		...
		try {
			...
			lNuovaPropostaAtto2DataSource.executecustom("modificaDatiPubblicazioneNotifiche", record, new DSCallback() {
		...
		lNuovaPropostaAtto2DataSource.getData(lRecordToLoad, new DSCallback() {
			...
			public void execute(final DSResponse response, Object rawData, DSRequest request) {
				...
				if (response.getStatus() == DSResponse.STATUS_SUCCESS) {
					...
					detail = new NuovaPropostaAtto2CompletaDetail("dettaglio_dati_pubbl_atto", tabs) {
