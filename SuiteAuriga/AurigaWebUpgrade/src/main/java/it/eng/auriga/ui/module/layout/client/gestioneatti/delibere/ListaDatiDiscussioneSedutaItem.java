...
import it.eng.auriga.ui.module.layout.client.postaElettronica.ModificaDatiExtraIterRegProtAssociatoWindow;
...
public class ListaDatiDiscussioneSedutaItem extends GridItem {
	...
	protected ControlListGridField scaricaVerModificabileButtonField;
	...
	protected ControlListGridField presenzeButtonField;
	protected ControlListGridField dichiarazioniDiVotoButtonField;
	...
	public ListaDatiDiscussioneSedutaItem(String name, String organoCollegiale, String codCircoscrizione) {
		...
		esitoDiscussione.setCanEdit(!isOdGChiuso());
		...
		firmeDaAcquisire = new ListGridField("firmeDaAcquisire", showVistoSegretario() ? "Firme/visti da acquisire" : "Firme da acquisire");
		firmeDaAcquisire.setCanSort(true);
		
		firmeApposte = new ListGridField("firmeApposte", showVistoSegretario() ? "Firme/visti apposti" : "Firme apposte");
		firmeApposte.setCanSort(true);
		
		attoDaFirmare = new ListGridField("attoDaFirmare", showVistoSegretario() ? "Atto da firmare/vistare dall’utente" : "Atto da firmare dall'utente");
		...
		attoDaFirmare.setHoverCustomizer(new HoverCustomizer() {
			...
			public String hoverHTML(Object value, ListGridRecord record, int rowNum, int colNum) {
				if(record.getAttribute("attoDaFirmare") != null && "1".equals(record.getAttribute("attoDaFirmare"))) {
					return showVistoSegretario() ? "atto in attesa di firma/visto" : "atto in attesa di firma";
				}						
				return null;
			}
		});
	...
	public ListGridRecord[] getGridSelectedRecords() {
	...
	protected void setCanEditForEachGridField(ListGridField field) {
		if("esitoDiscussione".equalsIgnoreCase(field.getName())) {
			field.setCanEdit(!isOdGChiuso());
		} else {
			field.setCanEdit(false);
		}
	}	
	
	public void setCanEditEsitoDiscussioneGridField() {
		setCanEditForEachGridField(esitoDiscussione);
	}
	...
	protected List<ControlListGridField> getButtonsFields() {
		...
		if(scaricaVerModificabileButtonField == null) {
			scaricaVerModificabileButtonField = buildScaricaVerModificabileButtonFieldButtonField();
		}
		...
		if(presenzeButtonField == null) { 
			presenzeButtonField = buildPresenzeButtonField();	
		}
		if(dichiarazioniDiVotoButtonField == null) { 
			dichiarazioniDiVotoButtonField = buildDichiarazioniDiVotoButtonField();	
		}
		...
		buttonsList.add(scaricaVerModificabileButtonField);
		...
		if(isButtonEnabled() && !fromStoricoSeduta()) {
			if ("COMMISSIONE".equalsIgnoreCase(organoCollegiale)) {
				buttonsList.add(presenzeButtonField);
				buttonsList.add(dichiarazioniDiVotoButtonField);
			} else {
	...
	protected Boolean isOdGChiuso() {
		return getStatoSeduta() != null && "lavori_conclusi".equalsIgnoreCase(getStatoSeduta());
	}
	...
	protected ControlListGridField buildModificaTestoButtonField() {
		...
		modificaTestoButtonField.setCellFormatter(new CellFormatter() {
			...
			public String format(Object value, ListGridRecord record, int rowNum, int colNum) {	
				if(fromStoricoSeduta()) {
					if(isRecordAbilToModificaTestoExtraIter(record) && !isCommissione()){
						return buildImgButtonHtml("file/editDoc.png");
					}
				} else if((isRecordAbilToModificaTesto(record) || isRecordAbilToModificaTestoExtraIter(record)) && !isCommissione()) {				
					return buildImgButtonHtml("file/editDoc.png");
				}
				return null;
			}
		...
		modificaTestoButtonField.addRecordClickHandler(new RecordClickHandler() {				
			...
			public void onRecordClick(RecordClickEvent event) {
				event.cancel();
				if(fromStoricoSeduta()) {
					if(isRecordAbilToModificaTestoExtraIter(event.getRecord()) && !isCommissione()) {
						onClickModificaTestoExtraIterButton(event.getRecord());
					}
				} else {
					if(isRecordAbilToModificaTesto(event.getRecord()) && !isCommissione()) {			
						onClickModificaTestoButton(event.getRecord());
					} else if(isRecordAbilToModificaTestoExtraIter(event.getRecord()) && !isCommissione()) {
						onClickModificaTestoExtraIterButton(event.getRecord());
					}
				}
			}
	...
	protected boolean isRecordAbilToModificaTesto(ListGridRecord record) {
		if(record.getAttributeAsString("idUd") != null && !"".equals(record.getAttributeAsString("idUd"))) {						
			if(record.getAttributeAsString("idProcessoAuriga") != null && !"".equals(record.getAttributeAsString("idProcessoAuriga"))) {			
				...
				boolean isFirmatoParzialmente = record.getAttribute("statoRevisioneTesto") != null && "firmato_parzialmente".equalsIgnoreCase(record.getAttribute("statoRevisioneTesto"));
				return !isFirmato && !isFirmatoParzialmente;
			}
		}
		return false;
	}
	
	protected boolean isRecordAbilToModificaTestoExtraIter(ListGridRecord record) {
		if(record.getAttributeAsString("idUd") != null && !"".equals(record.getAttributeAsString("idUd"))) {						
			if(record.getAttributeAsString("idProcessoAuriga") != null && !"".equals(record.getAttributeAsString("idProcessoAuriga"))) {			
				boolean isFirmato = record.getAttribute("statoRevisioneTesto") != null && "firmato".equalsIgnoreCase(record.getAttribute("statoRevisioneTesto"));
				boolean isFirmatoParzialmente = record.getAttribute("statoRevisioneTesto") != null && "firmato_parzialmente".equalsIgnoreCase(record.getAttribute("statoRevisioneTesto"));
	...
	public void onClickModificaTestoButton(final ListGridRecord record) {
		...
		lRecord.setAttribute("stato", getStatoSeduta());
		new DettaglioPropostaDeliberaWindow(lRecord, "Modifica testo " + record.getAttributeAsString("estremiPropostaUD"), true, true);
	}
	
	public void onClickModificaTestoExtraIterButton(final ListGridRecord record) {
		Record lRecord = new Record();
		lRecord.setAttribute("idUd", record.getAttributeAsString("idUd"));
		lRecord.setAttribute("idProcess", record.getAttributeAsString("idProcessoAuriga"));
		lRecord.setAttribute("activityName", "#POST_DISCUSSIONE_AULA_" + organoCollegiale);
		...
		lRecord.setAttribute("idInstProcFlow", record.getAttributeAsString("idIstanzaFlussoActiviti"));
		lRecord.setAttribute("stato", getStatoSeduta());
		new ModificaDatiExtraIterRegProtAssociatoWindow(lRecord, "Modifica testo " + record.getAttributeAsString("estremiPropostaUD"));
	}
	...
	protected boolean isRecordAbilToScaricaVerModificabile(ListGridRecord record) {
		return isRecordAbilToAnteprimaVersDefinitiva(record) && AurigaLayout.getParametroDBAsBoolean("ATTIVA_SCARICO_ATTO_VERS_MODIFICABILE");
	}
	...
	protected ControlListGridField buildAnteprimaVersDefinitivaButtonField() {
		...
		anteprimaVersDefinitivaButtonField.setCellFormatter(new CellFormatter() {
			...
			@Override
			public String format(Object value, ListGridRecord record, int rowNum, int colNum) {			
				if(isRecordAbilToAnteprimaVersDefinitiva(record) && !isCommissione()) {								
		...
		anteprimaVersDefinitivaButtonField.addRecordClickHandler(new RecordClickHandler() {				
			...
			public void onRecordClick(RecordClickEvent event) {
				event.cancel();
				if(isRecordAbilToAnteprimaVersDefinitiva(event.getRecord()) && !isCommissione()) {			
	...
	public void onClickAnteprimaVersDefinitiva(final ListGridRecord record) {
		...
		if (idProcess != null && !"".equals(idProcess)) {
			callExecAtt(recordCallExecAtt, new ServiceCallback<Record>() {
				...
				public void execute(final Record lRecordEvento) {		
					...
					final String nomeFileUnione = lRecordEvento != null ? lRecordEvento.getAttribute("unioneFileNomeFile") : null;
					final String nomeFileUnioneOmissis = lRecordEvento != null ? lRecordEvento.getAttribute("unioneFileNomeFileOmissis") : null;
					final Record impostazioniUnioneFile = lRecordEvento != null ? lRecordEvento.getAttributeAsRecord("impostazioniUnioneFile") : null;					
						...
						lNuovaPropostaAtto2CompletaDataSource.getData(lRecordToLoad, new DSCallback() {
							...
							public void execute(DSResponse response, Object rawData, DSRequest request) {
								if (response.getStatus() == DSResponse.STATUS_SUCCESS) {
									...
									detailRecord.setAttribute("impostazioniUnioneFile", impostazioniUnioneFile);
									
									detailRecord.setAttribute("idProcess", idProcess);
									detailRecord.setAttribute("idModCopertina", lRecordEvento != null ? lRecordEvento.getAttribute("idModCopertina") : "");
									detailRecord.setAttribute("nomeModCopertina", lRecordEvento != null ? lRecordEvento.getAttribute("nomeModCopertina") : "");
									detailRecord.setAttribute("idModCopertinaFinale", lRecordEvento != null ? lRecordEvento.getAttribute("idModCopertinaFinale") : "");
									detailRecord.setAttribute("nomeModCopertinaFinale", lRecordEvento != null ? lRecordEvento.getAttribute("nomeModCopertinaFinale") : "");
									detailRecord.setAttribute("idModAllegatiParteIntSeparati", lRecordEvento != null ? lRecordEvento.getAttribute("idModAllegatiParteIntSeparati") : "");
									detailRecord.setAttribute("nomeModAllegatiParteIntSeparati", lRecordEvento != null ? lRecordEvento.getAttribute("nomeModAllegatiParteIntSeparati") : "");
									detailRecord.setAttribute("uriModAllegatiParteIntSeparati", lRecordEvento != null ? lRecordEvento.getAttribute("uriModAllegatiParteIntSeparati") : "");
									detailRecord.setAttribute("tipoModAllegatiParteIntSeparati", lRecordEvento != null ? lRecordEvento.getAttribute("tipoModAllegatiParteIntSeparati") : "");
									detailRecord.setAttribute("idModAllegatiParteIntSeparatiXPubbl", lRecordEvento != null ? lRecordEvento.getAttribute("idModAllegatiParteIntSeparatiXPubbl") : "");
									...
									detailRecord.setAttribute("nomeModello", lRecordEvento != null ? lRecordEvento.getAttribute("nomeModAssDocTask") : "");
									detailRecord.setAttribute("displayFilenameModello", lRecordEvento != null ? lRecordEvento.getAttribute("displayFilenameModAssDocTask") : "");
									...
									lNuovaPropostaAtto2CompletaDataSource.executecustom("unioneFile", detailRecord, new DSCallback() {
										...
										public void execute(DSResponse response, Object rawData, DSRequest request) {
											...
											if (response.getStatus() == DSResponse.STATUS_SUCCESS) {
												...
												final Record recordPreview = recordUnioneFile.getAttribute("uriVersIntegrale") != null && !"".equals(recordUnioneFile.getAttribute("uriVersIntegrale")) ? new Record() : null;												
												if(recordPreview != null) {
													recordPreview.setAttribute("uri", recordUnioneFile.getAttribute("uriVersIntegrale"));
													recordPreview.setAttribute("nomeFile", recordUnioneFile.getAttribute("nomeFileVersIntegrale"));
													recordPreview.setAttribute("infoFile", recordUnioneFile.getAttributeAsRecord("infoFileVersIntegrale"));
													
												}
												final Record recordPreviewOmissis = recordUnioneFile.getAttribute("uri") != null && !"".equals(recordUnioneFile.getAttribute("uri")) ? new Record() : null;
												if(recordPreviewOmissis != null) {													
													recordPreviewOmissis.setAttribute("uri", recordUnioneFile.getAttribute("uri"));
													recordPreviewOmissis.setAttribute("nomeFile", recordUnioneFile.getAttribute("nomeFile"));
													recordPreviewOmissis.setAttribute("infoFile", recordUnioneFile.getAttributeAsRecord("infoFile"));													
												}
												if(recordPreview != null && recordPreviewOmissis != null) {
	...
	protected ControlListGridField buildPresenzeButtonField() {
		
		ControlListGridField presenzeButtonField = new ControlListGridField("presenzeButton");  
		presenzeButtonField.setAttribute("custom", true);	
		presenzeButtonField.setShowHover(true);		
		presenzeButtonField.setCanReorder(false);
		presenzeButtonField.setCellFormatter(new CellFormatter() {
			
			@Override
			public String format(Object value, ListGridRecord record, int rowNum, int colNum) {			
				
				return buildImgButtonHtml("delibere/presenze.png");
			}
		});
		presenzeButtonField.setHoverCustomizer(new HoverCustomizer() {	
		...
		presenzeButtonField.addRecordClickHandler(new RecordClickHandler() {	
			
			@Override
			public void onRecordClick(RecordClickEvent event) {
				event.cancel();
				onClickPresenzeButton(event.getRecord());				
			}
		});		
		return presenzeButtonField;
	}

	protected ControlListGridField buildDichiarazioniDiVotoButtonField() {
		
		ControlListGridField dichiarazioniDiVotoButtonField = new ControlListGridField("presenzeVotiButton");  
		dichiarazioniDiVotoButtonField.setAttribute("custom", true);	
		dichiarazioniDiVotoButtonField.setShowHover(true);		
		dichiarazioniDiVotoButtonField.setCanReorder(false);
		dichiarazioniDiVotoButtonField.setCellFormatter(new CellFormatter() {
			
			@Override
			public String format(Object value, ListGridRecord record, int rowNum, int colNum) {			
				
				return buildImgButtonHtml("delibere/dichiarazioni_di_voto.png");
			}
		});
		dichiarazioniDiVotoButtonField.setHoverCustomizer(new HoverCustomizer() {	
			
			@Override
			public String hoverHTML(Object value, ListGridRecord record, int rowNum, int colNum) {
		
				return "Dichiarazioni di voto";
			}
		});		
		dichiarazioniDiVotoButtonField.addRecordClickHandler(new RecordClickHandler() {	
			
			@Override
			public void onRecordClick(RecordClickEvent event) {
				event.cancel();
				onClickDichiarazioneDiVotoButton(event.getRecord());				
			}
		});		
		return dichiarazioniDiVotoButtonField;
	}
	
	protected ControlListGridField buildScaricaVerModificabileButtonFieldButtonField() {
		ControlListGridField scaricaVerModificabileButtonField = new ControlListGridField("scaricaVerModificabileButton");  
		scaricaVerModificabileButtonField.setAttribute("custom", true);	
		scaricaVerModificabileButtonField.setShowHover(true);		
		scaricaVerModificabileButtonField.setCanReorder(false);
		scaricaVerModificabileButtonField.setCellFormatter(new CellFormatter() {
			
			@Override
			public String format(Object value, ListGridRecord record, int rowNum, int colNum) {			
				if(isRecordAbilToScaricaVerModificabile(record)) {	
					return buildImgButtonHtml("delibere/scarica_ver_modificabile.png");
				}
				return null;
			}
		});
		scaricaVerModificabileButtonField.setHoverCustomizer(new HoverCustomizer() {	
			
			@Override
			public String hoverHTML(Object value, ListGridRecord record, int rowNum, int colNum) {
				if(isRecordAbilToScaricaVerModificabile(record)) {
					return "Scarica versione modificabile testo proposta/atto";
				}
				return null;
			}
		});		
		scaricaVerModificabileButtonField.addRecordClickHandler(new RecordClickHandler() {	
			
			@Override
			public void onRecordClick(RecordClickEvent event) {
				event.cancel();
				if(isRecordAbilToScaricaVerModificabile(event.getRecord())) {
					onClickScaricaVerModificabileButtonField(event.getRecord());	
				}
			}
		});		
		return scaricaVerModificabileButtonField;
	}
		...
		lGWTRestDataSource.executecustom("getPresenzeVotiSuContOdG", recordToSave, new DSCallback() {
			...
			public void execute(DSResponse dsResponse, Object data, DSRequest dsRequest) {
				...
				if (dsResponse.getStatus() == DSResponse.STATUS_SUCCESS) {
					...
					String title = null;
					if(isAbilHideVotazioneOrgColl()) {
						title = "Presenze su "+ record.getAttributeAsString("tipo") +" / " + record.getAttributeAsString("nrOrdineOdg");
					} else {
						title = "Presenze e voti su "+ record.getAttributeAsString("tipo") +" / " + record.getAttributeAsString("nrOrdineOdg");
					}
					PresenzaVotoArgomentoWindow lPresenzePrimaConvocazioneWindow = new PresenzaVotoArgomentoWindow("presenze_prima_convocazione_window",title,recordToGet,organoCollegiale, codCircoscrizione);
	...
	public void onClickPresenzeButton(final ListGridRecord record) {
		
		GWTRestService<Record, Record> lGWTRestDataSource = new GWTRestService<Record, Record>("DiscussioneSedutaDataSource");
		Record recordToSave = new Record();
		recordToSave.setAttribute("idSeduta", getIdSeduta());
		recordToSave.setAttribute("idUdArgomentoOdG", record.getAttribute("idUd"));
		recordToSave.setAttribute("flgPresenzeVotiIn", "P");
		lGWTRestDataSource.executecustom("getPresenzeVotiSuContOdG", recordToSave, new DSCallback() {
			...
			public void execute(DSResponse dsResponse, Object data, DSRequest dsRequest) {
				...
				if (dsResponse.getStatus() == DSResponse.STATUS_SUCCESS) {
					...
					Record recordToGet = dsResponse.getData()[0];
					String title = "Presenze su "+ record.getAttributeAsString("tipo") +" - " + record.getAttributeAsString("estremiPropostaUD");
					PresenzaArgomentoWindow lPresenzePrimaConvocazioneWindow = new PresenzaArgomentoWindow("presenza_argomento_window",title,recordToGet,organoCollegiale);
					lPresenzePrimaConvocazioneWindow.show();
				}
			}
		});
	...
	public void onClickDichiarazioneDiVotoButton(final ListGridRecord record) {
	
		GWTRestService<Record, Record> lGWTRestDataSource = new GWTRestService<Record, Record>("DiscussioneSedutaDataSource");
		Record recordToSave = new Record();
		recordToSave.setAttribute("idSeduta", getIdSeduta());
		recordToSave.setAttribute("idUdArgomentoOdG", record.getAttribute("idUd"));
		recordToSave.setAttribute("flgPresenzeVotiIn", "V");
		lGWTRestDataSource.executecustom("getPresenzeVotiSuContOdG", recordToSave, new DSCallback() {
			...
			public void execute(DSResponse dsResponse, Object data, DSRequest dsRequest) {
				...
				if (dsResponse.getStatus() == DSResponse.STATUS_SUCCESS) {
					...
					Record recordToGet = dsResponse.getData()[0];
					String title = "Dichiarazioni di voto su "+ record.getAttributeAsString("tipo") +" - " + record.getAttributeAsString("estremiPropostaUD");
					DichiarazioneDiVotoArgomentoWindow lPresenzePrimaConvocazioneWindow = new DichiarazioneDiVotoArgomentoWindow("dichiarazione_di_voto_argomento_window",title,recordToGet,organoCollegiale);
					lPresenzePrimaConvocazioneWindow.show();
				}
			}
		});
	...
	public void onClickScaricaVerModificabileButtonField(final ListGridRecord record) {
	...
	public void generaDispositivoDaModelloVersModificabile(Record record, final ServiceCallback<Record> callback) {			
		record.setAttribute("flgMostraDatiSensibili", true);
		record.setAttribute("flgAnteprima", false);		
		record.setAttribute("flgVersioneFinale", false);		
		
		Layout.showWaitPopup("Generazione versione modificabile in corso...");
		...
		if(this.getCanvas() != null) {
			for(Canvas member : toolStrip.getMembers()) {
				if (member.getPrefix() != null && member.getPrefix().equalsIgnoreCase("aggiunta_proposta")) {
					if(showButtons() && !fromStoricoSeduta() && !isStatoLavoriConclusi()) {
						member.show();											
					} else {
						member.hide();						
					}
	...
	private Boolean isAbilHideVotazioneOrgColl() {
		String param = null;
		if(codCircoscrizione != null && !"".equalsIgnoreCase(codCircoscrizione)) {
			param = "HIDE_VOTAZIONE_ORG_COLL_" + organoCollegiale + "_CIRC";
		} else {
			param = "HIDE_VOTAZIONE_ORG_COLL_" + organoCollegiale;
		}
