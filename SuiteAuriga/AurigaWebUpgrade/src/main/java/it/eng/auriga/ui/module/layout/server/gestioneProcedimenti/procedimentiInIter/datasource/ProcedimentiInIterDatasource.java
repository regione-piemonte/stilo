...
import it.eng.auriga.ui.module.layout.server.gestioneProcedimenti.procedimentiInIter.bean.ProcedimentiInIterXmlBean;
import it.eng.auriga.ui.module.layout.server.gestioneProcedimenti.procedimentiInIter.bean.ProcedimentiInIterXmlBeanDeserializationHelper;
...
@Datasource(id = "ProcedimentiInIterDatasource")
public class ProcedimentiInIterDatasource extends AurigaAbstractFetchDatasource<ProcedimentiInIterBean> {

	private static Logger mLogger = Logger.getLogger(ProcedimentiInIterDatasource.class);

	@Override
	public PaginatorBean<ProcedimentiInIterBean> fetch(AdvancedCriteria criteria, Integer startRow, Integer endRow, List<OrderByBean> orderby) throws Exception {
		...
		List<ProcedimentiInIterXmlBean> lListResult = XmlListaUtility.recuperaLista(xmlOut, ProcedimentiInIterXmlBean.class);
		
		getSession().setAttribute(FETCH_SESSION_KEY, lListResult);
		
		List<ProcedimentiInIterBean> lList = new ArrayList<ProcedimentiInIterBean>(lListResult.size());		
		for (ProcedimentiInIterXmlBean lProcedimentiInIterXmlBean : lListResult){
			ProcedimentiInIterBean lProcedimentiInIterBean = new ProcedimentiInIterBean();
			BeanUtilsBean2.getInstance().copyProperties(lProcedimentiInIterBean, lProcedimentiInIterXmlBean);
			lProcedimentiInIterBean.setOggettoProcedimento(HtmlNormalizeUtil.getPlainXmlWithCarriageReturn(lProcedimentiInIterBean.getOggettoProcedimento()));
			lProcedimentiInIterBean.setFirmatoRispostaCedAutotutele(lProcedimentiInIterXmlBean.getFlgFirmatoRispostaCedAutotutele() != null && Flag.SETTED.toString().equalsIgnoreCase(lProcedimentiInIterXmlBean.getFlgFirmatoRispostaCedAutotutele()));
			lProcedimentiInIterBean.setConvertibileRispostaCedAutotutele(StringUtils.isNotBlank(lProcedimentiInIterXmlBean.getMimetypeRispostaCedAutotutele()) ? FormatiUtil.isConvertibile(getSession(), lProcedimentiInIterXmlBean.getMimetypeRispostaCedAutotutele()) : false);
			lProcedimentiInIterBean.setFlgPrevistaNumerazione(lProcedimentiInIterXmlBean.getFlgPrevistaNumerazione());
			lList.add(lProcedimentiInIterBean);
		}
		
		PaginatorBean<ProcedimentiInIterBean> lPaginatorBean = new PaginatorBean<ProcedimentiInIterBean>();
	...
	private DmpkWfTrovalistalavoroBean createInputBean(boolean forNroRecordTot,AdvancedCriteria criteria, AurigaLoginBean loginBean) throws Exception {
		...
		ProcedimentiInIterBean filter = new ProcedimentiInIterBean(); 
...
//			HashSet<String> setNumColonneOrdinabili = new HashSet<String>(Arrays.asList("3","5","13"));
			String[] colAndFlgDescOrderBy = getColAndFlgDescOrderBy(ProcedimentiInIterXmlBean.class, setNumColonneOrdinabili);
	...
	@Override
	public Map<String, ErrorBean> validate(ProcedimentiInIterBean bean) throws Exception {
		return null;
	}
	...
	@Override
	public ProcedimentiInIterBean get(ProcedimentiInIterBean bean) throws Exception {
		return bean;
	}

	@Override
	public ProcedimentiInIterBean add(ProcedimentiInIterBean bean) throws Exception {
		return bean;
	}

	@Override
	public ProcedimentiInIterBean remove(ProcedimentiInIterBean bean) throws Exception {
		return bean;
	}

	@Override
	public ProcedimentiInIterBean update(ProcedimentiInIterBean bean, ProcedimentiInIterBean oldvalue)
			throws Exception {
		return bean;
	}
	...
	protected ExportBean asyncExport(ExportBean bean) throws Exception {
		...
		saveParameters(loginBean, bean, jobId, loginBean.getIdUserLavoro(), ProcedimentiInIterXmlBean.class.getName());

		saveRemapInformations(loginBean, jobId, createRemapConditionsMap(), ProcedimentiInIterXmlBeanDeserializationHelper.class);
