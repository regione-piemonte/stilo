...
public class DettaglioTermineRaccoltaSottoscrizioniPropAttoPopup extends ModalWindow {

	private DettaglioTermineRaccoltaSottoscrizioniPropAttoPopup window;
	...
	public DettaglioTermineRaccoltaSottoscrizioniPropAttoPopup(Record record, String title, final boolean isModificaTesto) {
		
		super("dettaglio_termine_raccolta_sottoscrizioni_prop_atto", true);
		...
		setIcon("attiInLavorazione/termineRaccoltaSottoscrizioniPropAtto.png");
	...
	protected void realSave(final Record record, final DSCallback callback) {
		...
		try {
			...
			lNuovaPropostaAtto2DataSource.executecustom("modificaTermineTermineFirmeConsiglieri", record, new DSCallback() {
		...
		lNuovaPropostaAtto2DataSource.getData(lRecordToLoad, new DSCallback() {
			...
			public void execute(final DSResponse response, Object rawData, DSRequest request) {
				...
				if (response.getStatus() == DSResponse.STATUS_SUCCESS) {
					...
					detail = new NuovaPropostaAtto2CompletaDetail("dettaglio_termine_raccolta_sottoscrizioni_prop_atto", tabs) {
								...
								if(lAttributiCustomCablati != null) {
									...
									for(int i = 0; i < lAttributiCustomCablati.getLength(); i++) {
										...
										if (lRecordAttributiCustomCablati.getAttribute("attrName")!=null && !lRecordAttributiCustomCablati.getAttribute("attrName").equalsIgnoreCase("")){											
											if (lRecordAttributiCustomCablati.getAttribute("attrName").equalsIgnoreCase("DT_TERM_FIRME_CONSIGLIERI_COPROP")){
