...
public abstract class ListaSceltaItem extends CanvasItem {
	...
	public ListaSceltaItem(String title, String keyFieldName, String displayFieldName, GWTRestDataSource datasource) {	
		this.title = title;
			...
			public void onInit(FormItem item) {
				...
				addShowValueHandler(new ShowValueHandler() {
					...
					public void onShowValue(final ShowValueEvent event) {
						try {
							String valore = (String) event.getDataValue();
							valueToSelect = valore != null ? valore : "";
							if(grid != null && grid.getDataAsRecordList() != null && grid.getDataAsRecordList().getLength() > 0) {
								if(valore != null && !"".equals(valore)) {
									for(int i = 0; i < grid.getDataAsRecordList().getLength(); i++) {
										Record record = grid.getDataAsRecordList().get(i);
										if(record.getAttribute(keyFieldName).equals(valore)) {
	...
	public void buildGrid() {
		...
		grid.setHeaderBaseStyle(null);	
		grid.setHeaderTitleStyle(null);	
