...
@Controller
@RequestMapping("/servletCreaPassword")
public class CreaPasswordServlet {
	
	private static Logger mLogger = Logger.getLogger(CreaPasswordServlet.class);
	...
	public ResponseEntity<String> verifica(@RequestParam("j_username") String username,HttpSession session, HttpServletRequest servletrequest, HttpServletResponse servletresponse, ModelMap model) throws Exception {
		...
		try {
			lResetPasswordBean = lAurigaCambioPasswordDataSource.creaPasswordLogin(lUtenteBean, lLoginBean, session);
		} catch (Exception e) {
			mLogger.warn(e);
