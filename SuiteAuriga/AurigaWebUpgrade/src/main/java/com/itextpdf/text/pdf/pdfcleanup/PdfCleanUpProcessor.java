...
public class PdfCleanUpProcessor {
    ...
    public boolean cleanUp(boolean fromPecette) throws IOException, DocumentException {
		boolean tryToCleanText = false;

    	for (Map.Entry<Integer, List<PdfCleanUpLocation>> entry : pdfCleanUpLocations.entrySet()) {
            if (fromPecette) {
            	if (!tryToCleanText) {
                	tryToCleanText = cleanUpPagePecette(entry.getKey(), entry.getValue());
            	} else {
    ...
    private boolean cleanUpPagePecette(int pageNum, List<PdfCleanUpLocation> cleanUpLocations) throws IOException, DocumentException {
        boolean tryToCleanText = false;
        ...
        tryToCleanText = contentProcessor.processContentPecette(pageContentInput, page.getAsDict(PdfName.RESOURCES));
