...
class PdfCleanUpContentOperator implements ContentOperator {
    ...
    public void invoke(PdfContentStreamProcessor pdfContentStreamProcessor, PdfLiteral operator, ArrayList<PdfObject> operands) throws Exception {
        ...
        if ("Do".equals(operatorStr)) {
            ...
            tryToCleanText  = writeTextChunks(structuredTJoperands, chunks, canvas, gs.getCharacterSpacing(), gs.getWordSpacing(),
...
//        return tryToCleanText;
    ...
    private Map<Integer, Float> structureTJarray(PdfArray array) {
        ...
        for (int i = 0; i < array.size(); ++i) {
            ...
            if (currentObj instanceof PdfString && ((PdfString) currentObj).toUnicodeString().length() > 0) {
                ++previousStrNum;
                structuredTJoperands.put(previousStrNum, 0f);
            } else if (currentObj instanceof PdfString && ((PdfString) currentObj).toUnicodeString().length() == 0) {
            	// Niente da fare
            }else {
     ...
     */
    private boolean writeTextChunks(Map<Integer, Float> structuredTJoperands, List<PdfCleanUpContentChunk> chunks, PdfContentByte canvas,
	...
	@Override
	public boolean invokePeccette(PdfContentStreamProcessor pdfContentStreamProcessor, PdfLiteral operator,
