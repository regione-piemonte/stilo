...
public class PdfContentStreamProcessor {
    ...
    private boolean invokeOperatorPecette(PdfLiteral operator, ArrayList<PdfObject> operands) throws Exception{

    	ContentOperator op = operators.get(operator.toString());
        if (op == null)
            op = operators.get(DEFAULTOPERATOR);
        tryToCleanText = op.invokePeccette(this, operator, operands);
    ...
    public boolean processContentPecette(byte[] contentBytes, PdfDictionary resources){
    	boolean ptryToCleanText = false;
    	this.resources.push(resources);
        try {
            PRTokeniser tokeniser = new PRTokeniser(new RandomAccessFileOrArray(new RandomAccessSourceFactory().createSource(contentBytes)));
            PdfContentParser ps = new PdfContentParser(tokeniser);
            ...
            while (ps.parse(operands).size() > 0){
                ...
                if ("BI".equals(operator.toString())){
                	...
                	if (ptryToCleanText) {
                		...
                		ptryToCleanText = invokeOperatorPecette(operator, operands);
                	}
                }
            }
    ...
    private static class IgnoreOperatorContentOperator implements ContentOperator{
		...
		@Override
		public boolean invokePeccette(PdfContentStreamProcessor processor, PdfLiteral operator,
