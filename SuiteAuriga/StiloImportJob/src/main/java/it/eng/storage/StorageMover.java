...
public class StorageMover {
	...
	/**
	 * Metodo che, dati in input il file corrente e il path di destinazione, ritorna il file contenente il path di destinazione CON alberatura.
	 * 
	 * @param currentFile
	...
	public static File getMoveDirectoryConAlberatura(File currentFile, File destinationFile) throws Exception {
		...
		if (destinationFile != null) {
			...
			/*
			 * Assegno il path del file di destinazione. Se il path è già risolto allora andrà in eccezione nel try e la variabile pathResolved conterrà già il
			 * valore corretto, altrimenti verrà riassegnato il valore del path risolto
			 */
			String pathResolved = destinationFile.getPath();

			try {
				/*
				 * Risolvo l'indirizzo per identificare dove si trova la root dove creare l'alberatura e spostare il file
				 */
				pathResolved = resolveStoragePath(destinationFile);
			} catch (AurigaJobException e) {
				if (logger.isInfoEnabled()) {
					logger.info("Non è necessario risolvere il path tramite StorageUtil");
				}
			} catch (StorageException e) {
				logger.info("Eccezione nel recupero dello storage");
				throw e;
			}

			/*
			 * directoryPrimoLivello conterrà il path comprensivo della cartella in cui si trova il file da spostare directorySecondoLivello conterrà il path
			 * comprensivo della cartella in cui si trova la directory directoryPrimoLivello directoryTerzoLivello conterrà il path comprensivo della cartella
			 * in cui si trova la directory directorySecondoLivello
			 */
			...
			if (!directory.exists()) {
				if (!directory.mkdirs()) {
					throw new Exception(String.format("Non è stato possibile creare la directory %1$s per spostare il file %2$s", directory.getName(),
	...
	/**
	 * Metodo che, dati in input il file corrente e il path di destinazione, ritorna il file contenente il path di destinazione SENZA alberatura.
	 * 
	 * @param currentFile
	 * @param destinationFile
	 * @return
	...
	/**
	 * Metodo che, dato in input un file, risolve lo storage e ritorna il path fisico del file
	 * 
	 * @param destinationFile
	...
	public static String resolveStoragePath(File destinationFile) throws AurigaJobException, StorageException {
		...
		logger.info("Result storage - length: " + lStrResult.length);
		...
		try {
			...
			logger.error("Eccezione nel recupero della cartella di destinazione dallo storage", e);
			throw e;
		}
	}
	 ...
	 */
	private static String addSeparatorToPath(String directoryName) {
