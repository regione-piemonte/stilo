...
public class MultiLookupListGrid extends ListGrid {

	private CustomLayout layout;
		
	public MultiLookupListGrid(CustomLayout pLayout) {
...
//		});		
		...
		ListGridField nome = new ListGridField("nome", getNomeFieldTitle());
		...
		setFields(/*icona,*/ nome, removeButton);
		
	}	
	
	public String getNomeFieldTitle() {
		return null;
	}
	...
	public boolean addRecord(Record record) {
		...
		for(int i = 0; i < getDataAsRecordList().getLength(); i++) {
			...
			if(listRecord.getAttributeAsString("id").equals(record.getAttributeAsString("id"))) {
				trovato = true;
				break;
			}
		}
