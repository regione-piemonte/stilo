...
import it.eng.auriga.database.store.dmpk_core_2.store.impl.InviarimuovidoclibrofirmaImpl;
 ...
 */
@Service(name="DmpkCore2Inviarimuovidoclibrofirma")
public class Inviarimuovidoclibrofirma {
		
	private DmpkCore2InviarimuovidoclibrofirmaBean bean;
		  
	public void setBean(DmpkCore2InviarimuovidoclibrofirmaBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkCore2InviarimuovidoclibrofirmaBean> execute(AurigaLoginBean lLoginBean, DmpkCore2InviarimuovidoclibrofirmaBean pBean) throws Exception{
	   final InviarimuovidoclibrofirmaImpl lInviarimuovidoclibrofirma = new InviarimuovidoclibrofirmaImpl();
	   setBean(pBean);
	   lInviarimuovidoclibrofirma.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lInviarimuovidoclibrofirma.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkCore2InviarimuovidoclibrofirmaBean> result = new StoreResultBean<DmpkCore2InviarimuovidoclibrofirmaBean>();
