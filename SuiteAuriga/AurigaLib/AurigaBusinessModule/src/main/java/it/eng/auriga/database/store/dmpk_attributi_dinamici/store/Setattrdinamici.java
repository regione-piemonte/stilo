...
import it.eng.auriga.database.store.dmpk_attributi_dinamici.store.impl.SetattrdinamiciImpl;
 ...
 */
@Service(name="DmpkAttributiDinamiciSetattrdinamici")
public class Setattrdinamici {
		
	private DmpkAttributiDinamiciSetattrdinamiciBean bean;
		  
	public void setBean(DmpkAttributiDinamiciSetattrdinamiciBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkAttributiDinamiciSetattrdinamiciBean> execute(AurigaLoginBean lLoginBean, DmpkAttributiDinamiciSetattrdinamiciBean pBean) throws Exception{
	   final SetattrdinamiciImpl lSetattrdinamici = new SetattrdinamiciImpl();
	   setBean(pBean);
	   lSetattrdinamici.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lSetattrdinamici.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkAttributiDinamiciSetattrdinamiciBean> result = new StoreResultBean<DmpkAttributiDinamiciSetattrdinamiciBean>();
