...
import it.eng.auriga.database.store.dmpk_order.bean.DmpkOrderSetattributedettordiniBean;
import it.eng.auriga.database.store.dmpk_order.store.impl.SetattributedettordiniImpl;
 ...
 */
@Service(name="DmpkOrderSetattributedettordini")
public class Setattributedettordini {
		
	private DmpkOrderSetattributedettordiniBean bean;
		  
	public void setBean(DmpkOrderSetattributedettordiniBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkOrderSetattributedettordiniBean> execute(AurigaLoginBean lLoginBean, DmpkOrderSetattributedettordiniBean pBean) throws Exception{
	   final SetattributedettordiniImpl lSetattributedettordini = new SetattributedettordiniImpl();
	   setBean(pBean);
	   lSetattributedettordini.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lSetattributedettordini.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkOrderSetattributedettordiniBean> result = new StoreResultBean<DmpkOrderSetattributedettordiniBean>();
