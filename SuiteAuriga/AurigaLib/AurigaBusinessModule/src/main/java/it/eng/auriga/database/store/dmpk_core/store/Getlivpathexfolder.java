...
import it.eng.auriga.database.store.dmpk_core.store.impl.GetlivpathexfolderImpl;
 ...
 */
@Service(name="DmpkCoreGetlivpathexfolder")
public class Getlivpathexfolder {
		
	private DmpkCoreGetlivpathexfolderBean bean;
		  
	public void setBean(DmpkCoreGetlivpathexfolderBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkCoreGetlivpathexfolderBean> execute(AurigaLoginBean lLoginBean, DmpkCoreGetlivpathexfolderBean pBean) throws Exception{
	   final GetlivpathexfolderImpl lGetlivpathexfolder = new GetlivpathexfolderImpl();
	   setBean(pBean);
	   lGetlivpathexfolder.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lGetlivpathexfolder.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkCoreGetlivpathexfolderBean> result = new StoreResultBean<DmpkCoreGetlivpathexfolderBean>();
