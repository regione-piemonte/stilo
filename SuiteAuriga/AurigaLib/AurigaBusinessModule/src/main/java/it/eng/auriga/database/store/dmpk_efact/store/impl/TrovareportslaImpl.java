...
import it.eng.auriga.database.store.dmpk_efact.bean.DmpkEfactTrovareportslaBean;
...
public class TrovareportslaImpl  {
		
	private DmpkEfactTrovareportslaBean bean;
		  
	public void setBean(DmpkEfactTrovareportslaBean bean){
		this.bean = bean;
	}
	...
	public void execute(Connection connection) throws SQLException {
		...
		try{
			...
			call = connection.prepareCall("{? = call DMPK_EFACT.TROVAREPORTSLA(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}");			
			...
			util.settingParameterOnStore(call,bean,wrapperBean,"tiposlain",4,Types.VARCHAR,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"maxslaorein",5,Types.DECIMAL,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"filtriio",6,Types.CLOB,connection); 	
			...
			util.settingParameterOnStore(call,bean,wrapperBean,"flgbatchsearchin",16,Types.INTEGER,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"coltoreturnin",17,Types.VARCHAR,connection); 	
			...
			util.settinParameterOnBean(call,bean,wrapperBean,"filtriio",6,Types.CLOB); 
