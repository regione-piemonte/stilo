...
import it.eng.auriga.database.store.dmpk_efact.bean.DmpkEfactTrovaaltriallegatifatturadettBean;
import it.eng.auriga.database.store.dmpk_efact.store.impl.TrovaaltriallegatifatturadettImpl;
 ...
 */
@Service(name="DmpkEfactTrovaaltriallegatifatturadett")
public class Trovaaltriallegatifatturadett {
		
	private DmpkEfactTrovaaltriallegatifatturadettBean bean;
		  
	public void setBean(DmpkEfactTrovaaltriallegatifatturadettBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkEfactTrovaaltriallegatifatturadettBean> execute(AurigaLoginBean lLoginBean, DmpkEfactTrovaaltriallegatifatturadettBean pBean) throws Exception{
	   final TrovaaltriallegatifatturadettImpl lTrovaaltriallegatifatturadett = new TrovaaltriallegatifatturadettImpl();
	   setBean(pBean);
	   lTrovaaltriallegatifatturadett.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lTrovaaltriallegatifatturadett.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkEfactTrovaaltriallegatifatturadettBean> result = new StoreResultBean<DmpkEfactTrovaaltriallegatifatturadettBean>();
