...
import it.eng.auriga.database.store.dmpk_repository_gui.store.impl.LoadsoggestdocImpl;
 ...
 */
@Service(name="DmpkRepositoryGuiLoadsoggestdoc")
public class Loadsoggestdoc {
		
	private DmpkRepositoryGuiLoadsoggestdocBean bean;
		  
	public void setBean(DmpkRepositoryGuiLoadsoggestdocBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkRepositoryGuiLoadsoggestdocBean> execute(AurigaLoginBean lLoginBean, DmpkRepositoryGuiLoadsoggestdocBean pBean) throws Exception{
	   final LoadsoggestdocImpl lLoadsoggestdoc = new LoadsoggestdocImpl();
	   setBean(pBean);
	   lLoadsoggestdoc.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lLoadsoggestdoc.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkRepositoryGuiLoadsoggestdocBean> result = new StoreResultBean<DmpkRepositoryGuiLoadsoggestdocBean>();
