...
import it.eng.auriga.database.store.dmpk_ws.store.impl.GetresponseaddupdudImpl;
 ...
 */
@Service(name="DmpkWsGetresponseaddupdud")
public class Getresponseaddupdud {
		
	private DmpkWsGetresponseaddupdudBean bean;
		  
	public void setBean(DmpkWsGetresponseaddupdudBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkWsGetresponseaddupdudBean> execute(AurigaLoginBean lLoginBean, DmpkWsGetresponseaddupdudBean pBean) throws Exception{
	   final GetresponseaddupdudImpl lGetresponseaddupdud = new GetresponseaddupdudImpl();
	   setBean(pBean);
	   lGetresponseaddupdud.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lGetresponseaddupdud.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkWsGetresponseaddupdudBean> result = new StoreResultBean<DmpkWsGetresponseaddupdudBean>();
