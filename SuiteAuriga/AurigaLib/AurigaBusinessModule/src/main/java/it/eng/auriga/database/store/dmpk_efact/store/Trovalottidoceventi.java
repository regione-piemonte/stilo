...
import it.eng.auriga.database.store.dmpk_efact.bean.DmpkEfactTrovalottidoceventiBean;
import it.eng.auriga.database.store.dmpk_efact.store.impl.TrovalottidoceventiImpl;
 ...
 */
@Service(name="DmpkEfactTrovalottidoceventi")
public class Trovalottidoceventi {
		
	private DmpkEfactTrovalottidoceventiBean bean;
		  
	public void setBean(DmpkEfactTrovalottidoceventiBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkEfactTrovalottidoceventiBean> execute(AurigaLoginBean lLoginBean, DmpkEfactTrovalottidoceventiBean pBean) throws Exception{
	   final TrovalottidoceventiImpl lTrovalottidoceventi = new TrovalottidoceventiImpl();
	   setBean(pBean);
	   lTrovalottidoceventi.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lTrovalottidoceventi.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkEfactTrovalottidoceventiBean> result = new StoreResultBean<DmpkEfactTrovalottidoceventiBean>();
