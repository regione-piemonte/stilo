...
import it.eng.auriga.database.store.dmpk_int_mgo_email.store.impl.CollegaregtoemailImpl;
 ...
 */
@Service(name="DmpkIntMgoEmailCollegaregtoemail")
public class Collegaregtoemail {
		
	private DmpkIntMgoEmailCollegaregtoemailBean bean;
		  
	public void setBean(DmpkIntMgoEmailCollegaregtoemailBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkIntMgoEmailCollegaregtoemailBean> execute(AurigaLoginBean lLoginBean, DmpkIntMgoEmailCollegaregtoemailBean pBean) throws Exception{
	   final CollegaregtoemailImpl lCollegaregtoemail = new CollegaregtoemailImpl();
	   setBean(pBean);
	   lCollegaregtoemail.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lCollegaregtoemail.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkIntMgoEmailCollegaregtoemailBean> result = new StoreResultBean<DmpkIntMgoEmailCollegaregtoemailBean>();
