...
import it.eng.auriga.database.store.dmpk_ws.store.impl.TrovaclassiftitolarioImpl;
 ...
 */
@Service(name="DmpkWsTrovaclassiftitolario")
public class Trovaclassiftitolario {
		
	private DmpkWsTrovaclassiftitolarioBean bean;
		  
	public void setBean(DmpkWsTrovaclassiftitolarioBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkWsTrovaclassiftitolarioBean> execute(AurigaLoginBean lLoginBean, DmpkWsTrovaclassiftitolarioBean pBean) throws Exception{
	   final TrovaclassiftitolarioImpl lTrovaclassiftitolario = new TrovaclassiftitolarioImpl();
	   setBean(pBean);
	   lTrovaclassiftitolario.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lTrovaclassiftitolario.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkWsTrovaclassiftitolarioBean> result = new StoreResultBean<DmpkWsTrovaclassiftitolarioBean>();
