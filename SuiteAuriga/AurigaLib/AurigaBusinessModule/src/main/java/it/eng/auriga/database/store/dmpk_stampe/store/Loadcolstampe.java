...
package it.eng.auriga.database.store.dmpk_stampe.store;

import it.eng.auriga.database.store.dmpk_stampe.bean.DmpkStampeLoadcolstampeBean;
import it.eng.auriga.database.store.dmpk_stampe.store.impl.LoadcolstampeImpl;
 ...
 */
@Service(name="DmpkStampeLoadcolstampe")
public class Loadcolstampe {
		
	private DmpkStampeLoadcolstampeBean bean;
		  
	public void setBean(DmpkStampeLoadcolstampeBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkStampeLoadcolstampeBean> execute(AurigaLoginBean lLoginBean, DmpkStampeLoadcolstampeBean pBean) throws Exception{
	   final LoadcolstampeImpl lLoadcolstampe = new LoadcolstampeImpl();
	   setBean(pBean);
	   lLoadcolstampe.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lLoadcolstampe.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkStampeLoadcolstampeBean> result = new StoreResultBean<DmpkStampeLoadcolstampeBean>();
