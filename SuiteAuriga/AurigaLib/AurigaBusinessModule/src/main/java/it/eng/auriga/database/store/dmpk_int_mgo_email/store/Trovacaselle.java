...
import it.eng.auriga.database.store.dmpk_int_mgo_email.store.impl.TrovacaselleImpl;
 ...
 */
@Service(name="DmpkIntMgoEmailTrovacaselle")
public class Trovacaselle {
		
	private DmpkIntMgoEmailTrovacaselleBean bean;
		  
	public void setBean(DmpkIntMgoEmailTrovacaselleBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkIntMgoEmailTrovacaselleBean> execute(AurigaLoginBean lLoginBean, DmpkIntMgoEmailTrovacaselleBean pBean) throws Exception{
	   final TrovacaselleImpl lTrovacaselle = new TrovacaselleImpl();
	   setBean(pBean);
	   lTrovacaselle.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lTrovacaselle.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkIntMgoEmailTrovacaselleBean> result = new StoreResultBean<DmpkIntMgoEmailTrovacaselleBean>();
