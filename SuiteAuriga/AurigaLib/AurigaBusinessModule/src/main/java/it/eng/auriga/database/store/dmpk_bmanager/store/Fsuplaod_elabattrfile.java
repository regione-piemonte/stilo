...
import it.eng.auriga.database.store.dmpk_bmanager.bean.DmpkBmanagerFsuplaod_elabattrfileBean;
import it.eng.auriga.database.store.dmpk_bmanager.store.impl.Fsuplaod_elabattrfileImpl;
 ...
 */
@Service(name="DmpkBmanagerFsuplaod_elabattrfile")
public class Fsuplaod_elabattrfile {
		
	private DmpkBmanagerFsuplaod_elabattrfileBean bean;
		  
	public void setBean(DmpkBmanagerFsuplaod_elabattrfileBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkBmanagerFsuplaod_elabattrfileBean> execute(AurigaLoginBean lLoginBean, DmpkBmanagerFsuplaod_elabattrfileBean pBean) throws Exception{
	   final Fsuplaod_elabattrfileImpl lFsuplaod_elabattrfile = new Fsuplaod_elabattrfileImpl();
	   setBean(pBean);
	   lFsuplaod_elabattrfile.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lFsuplaod_elabattrfile.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkBmanagerFsuplaod_elabattrfileBean> result = new StoreResultBean<DmpkBmanagerFsuplaod_elabattrfileBean>();
