 ...
 */
@Service(name="DmpkWsAddud")
public class Addud {
		
	private DmpkWsAddudBean bean;
		  
	public void setBean(DmpkWsAddudBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkWsAddudBean> execute(AurigaLoginBean lLoginBean, DmpkWsAddudBean pBean) throws Exception{
	   final AddudImpl lAddud = new AddudImpl();
	   setBean(pBean);
	   lAddud.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lAddud.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkWsAddudBean> result = new StoreResultBean<DmpkWsAddudBean>();
