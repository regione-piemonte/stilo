...
import it.eng.auriga.database.store.dmpk_int_mgo_email.store.impl.AnnullaarchiviazioneemailImpl;
 ...
 */
@Service(name="DmpkIntMgoEmailAnnullaarchiviazioneemail")
public class Annullaarchiviazioneemail {
		
	private DmpkIntMgoEmailAnnullaarchiviazioneemailBean bean;
		  
	public void setBean(DmpkIntMgoEmailAnnullaarchiviazioneemailBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkIntMgoEmailAnnullaarchiviazioneemailBean> execute(AurigaLoginBean lLoginBean, DmpkIntMgoEmailAnnullaarchiviazioneemailBean pBean) throws Exception{
	   final AnnullaarchiviazioneemailImpl lAnnullaarchiviazioneemail = new AnnullaarchiviazioneemailImpl();
	   setBean(pBean);
	   lAnnullaarchiviazioneemail.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lAnnullaarchiviazioneemail.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkIntMgoEmailAnnullaarchiviazioneemailBean> result = new StoreResultBean<DmpkIntMgoEmailAnnullaarchiviazioneemailBean>();
