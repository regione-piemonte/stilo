...
import it.eng.auriga.database.store.dmpk_fatture.bean.DmpkFattureUpdflginviatoreportBean;
import it.eng.auriga.database.store.dmpk_fatture.store.impl.UpdflginviatoreportImpl;
 ...
 */
@Service(name="DmpkFattureUpdflginviatoreport")
public class Updflginviatoreport {
		
	private DmpkFattureUpdflginviatoreportBean bean;
		  
	public void setBean(DmpkFattureUpdflginviatoreportBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkFattureUpdflginviatoreportBean> execute(AurigaLoginBean lLoginBean, DmpkFattureUpdflginviatoreportBean pBean) throws Exception{
	   final UpdflginviatoreportImpl lUpdflginviatoreport = new UpdflginviatoreportImpl();
	   setBean(pBean);
	   lUpdflginviatoreport.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lUpdflginviatoreport.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkFattureUpdflginviatoreportBean> result = new StoreResultBean<DmpkFattureUpdflginviatoreportBean>();
