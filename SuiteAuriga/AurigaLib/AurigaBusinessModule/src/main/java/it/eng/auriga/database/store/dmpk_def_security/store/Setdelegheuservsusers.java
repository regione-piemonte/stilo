...
import it.eng.auriga.database.store.dmpk_def_security.store.impl.SetdelegheuservsusersImpl;
 ...
 */
@Service(name="DmpkDefSecuritySetdelegheuservsusers")
public class Setdelegheuservsusers {
		
	private DmpkDefSecuritySetdelegheuservsusersBean bean;
		  
	public void setBean(DmpkDefSecuritySetdelegheuservsusersBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkDefSecuritySetdelegheuservsusersBean> execute(AurigaLoginBean lLoginBean, DmpkDefSecuritySetdelegheuservsusersBean pBean) throws Exception{
	   final SetdelegheuservsusersImpl lSetdelegheuservsusers = new SetdelegheuservsusersImpl();
	   setBean(pBean);
	   lSetdelegheuservsusers.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lSetdelegheuservsusers.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkDefSecuritySetdelegheuservsusersBean> result = new StoreResultBean<DmpkDefSecuritySetdelegheuservsusersBean>();
