...
import it.eng.auriga.database.store.dmpk_int_mgo_email.store.impl.PreparainvioudxemailImpl;
 ...
 */
@Service(name="DmpkIntMgoEmailPreparainvioudxemail")
public class Preparainvioudxemail {
		
	private DmpkIntMgoEmailPreparainvioudxemailBean bean;
		  
	public void setBean(DmpkIntMgoEmailPreparainvioudxemailBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkIntMgoEmailPreparainvioudxemailBean> execute(AurigaLoginBean lLoginBean, DmpkIntMgoEmailPreparainvioudxemailBean pBean) throws Exception{
	   final PreparainvioudxemailImpl lPreparainvioudxemail = new PreparainvioudxemailImpl();
	   setBean(pBean);
	   lPreparainvioudxemail.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lPreparainvioudxemail.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkIntMgoEmailPreparainvioudxemailBean> result = new StoreResultBean<DmpkIntMgoEmailPreparainvioudxemailBean>();
