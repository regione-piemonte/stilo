...
import it.eng.auriga.database.store.dmpk_corrispettivi.bean.DmpkCorrispettiviAddupdfileinvBean;
import it.eng.auriga.database.store.dmpk_corrispettivi.store.impl.AddupdfileinvImpl;
 ...
 */
@Service(name="DmpkCorrispettiviAddupdfileinv")
public class Addupdfileinv {
		
	private DmpkCorrispettiviAddupdfileinvBean bean;
		  
	public void setBean(DmpkCorrispettiviAddupdfileinvBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkCorrispettiviAddupdfileinvBean> execute(AurigaLoginBean lLoginBean, DmpkCorrispettiviAddupdfileinvBean pBean) throws Exception{
	   final AddupdfileinvImpl lAddupdfileinv = new AddupdfileinvImpl();
	   setBean(pBean);
	   lAddupdfileinv.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lAddupdfileinv.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkCorrispettiviAddupdfileinvBean> result = new StoreResultBean<DmpkCorrispettiviAddupdfileinvBean>();
