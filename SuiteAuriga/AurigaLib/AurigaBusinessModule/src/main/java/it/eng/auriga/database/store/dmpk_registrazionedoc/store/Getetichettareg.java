...
import it.eng.auriga.database.store.dmpk_registrazionedoc.store.impl.GetetichettaregImpl;
 ...
 */
@Service(name="DmpkRegistrazionedocGetetichettareg")
public class Getetichettareg {
	...
	public StoreResultBean<DmpkRegistrazionedocGetetichettaregBean> execute(AurigaLoginBean lLoginBean, DmpkRegistrazionedocGetetichettaregBean pBean) throws Exception{
	   final GetetichettaregImpl lGetetichettareg = new GetetichettaregImpl();
	   setBean(pBean);
	   lGetetichettareg.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lGetetichettareg.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkRegistrazionedocGetetichettaregBean> result = new StoreResultBean<DmpkRegistrazionedocGetetichettaregBean>();
