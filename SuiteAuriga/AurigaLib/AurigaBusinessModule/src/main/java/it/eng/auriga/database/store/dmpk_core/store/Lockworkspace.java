...
import it.eng.auriga.database.store.dmpk_core.bean.DmpkCoreLockworkspaceBean;
import it.eng.auriga.database.store.dmpk_core.store.impl.LockworkspaceImpl;
 ...
 */
@Service(name="DmpkCoreLockworkspace")
public class Lockworkspace {
		
	private DmpkCoreLockworkspaceBean bean;
		  
	public void setBean(DmpkCoreLockworkspaceBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkCoreLockworkspaceBean> execute(AurigaLoginBean lLoginBean, DmpkCoreLockworkspaceBean pBean) throws Exception{
	   final LockworkspaceImpl lLockworkspace = new LockworkspaceImpl();
	   setBean(pBean);
	   lLockworkspace.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lLockworkspace.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkCoreLockworkspaceBean> result = new StoreResultBean<DmpkCoreLockworkspaceBean>();
