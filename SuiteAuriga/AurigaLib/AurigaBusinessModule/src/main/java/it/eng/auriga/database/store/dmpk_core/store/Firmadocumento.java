...
import it.eng.auriga.database.store.dmpk_core.store.impl.FirmadocumentoImpl;
 ...
 */
@Service(name="DmpkCoreFirmadocumento")
public class Firmadocumento {
		
	private DmpkCoreFirmadocumentoBean bean;
		  
	public void setBean(DmpkCoreFirmadocumentoBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkCoreFirmadocumentoBean> execute(AurigaLoginBean lLoginBean, DmpkCoreFirmadocumentoBean pBean) throws Exception{
	   final FirmadocumentoImpl lFirmadocumento = new FirmadocumentoImpl();
	   setBean(pBean);
	   lFirmadocumento.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lFirmadocumento.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkCoreFirmadocumentoBean> result = new StoreResultBean<DmpkCoreFirmadocumentoBean>();
