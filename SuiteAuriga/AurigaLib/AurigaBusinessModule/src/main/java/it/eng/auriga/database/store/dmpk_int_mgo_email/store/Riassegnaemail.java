...
import it.eng.auriga.database.store.dmpk_int_mgo_email.store.impl.RiassegnaemailImpl;
 ...
 */
@Service(name="DmpkIntMgoEmailRiassegnaemail")
public class Riassegnaemail {
	...
	public StoreResultBean<DmpkIntMgoEmailRiassegnaemailBean> execute(AurigaLoginBean lLoginBean, DmpkIntMgoEmailRiassegnaemailBean pBean) throws Exception{
	   final RiassegnaemailImpl lRiassegnaemail = new RiassegnaemailImpl();
	   setBean(pBean);
	   lRiassegnaemail.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lRiassegnaemail.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkIntMgoEmailRiassegnaemailBean> result = new StoreResultBean<DmpkIntMgoEmailRiassegnaemailBean>();
