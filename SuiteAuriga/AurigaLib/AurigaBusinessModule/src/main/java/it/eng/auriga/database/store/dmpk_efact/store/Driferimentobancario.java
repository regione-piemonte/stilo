...
import it.eng.auriga.database.store.dmpk_efact.bean.DmpkEfactDriferimentobancarioBean;
import it.eng.auriga.database.store.dmpk_efact.store.impl.DriferimentobancarioImpl;
 ...
 */
@Service(name="DmpkEfactDriferimentobancario")
public class Driferimentobancario {
		
	private DmpkEfactDriferimentobancarioBean bean;
		  
	public void setBean(DmpkEfactDriferimentobancarioBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkEfactDriferimentobancarioBean> execute(AurigaLoginBean lLoginBean, DmpkEfactDriferimentobancarioBean pBean) throws Exception{
	   final DriferimentobancarioImpl lDriferimentobancario = new DriferimentobancarioImpl();
	   setBean(pBean);
	   lDriferimentobancario.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lDriferimentobancario.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkEfactDriferimentobancarioBean> result = new StoreResultBean<DmpkEfactDriferimentobancarioBean>();
