...
import it.eng.auriga.database.store.dmpk_core.bean.DmpkCoreSetaclfolderBean;
...
public class SetaclfolderImpl  {
		
	private DmpkCoreSetaclfolderBean bean;
		  
	public void setBean(DmpkCoreSetaclfolderBean bean){
		this.bean = bean;
	}
	...
	public void execute(Connection connection) throws SQLException {
		...
		try{
			...
			call = connection.prepareCall("{? = call DMPK_CORE.SETACLFOLDER(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}");			
			...
			util.settingParameterOnStore(call,bean,wrapperBean,"flgereditarietaconsentitain",5,Types.INTEGER,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"idfolderereditadain",6,Types.DECIMAL,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"flgpropagazioneconsentitain",7,Types.INTEGER,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"flgmodaclin",8,Types.VARCHAR,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"aclxmlin",9,Types.CLOB,connection); 	
			...
			util.settingParameterOnStore(call,bean,wrapperBean,"flgcallbyguiin",18,Types.INTEGER,connection); 	
			...
			util.settinParameterOnBean(call,bean,wrapperBean,"urixaggcontainerout",10,Types.VARCHAR); 
