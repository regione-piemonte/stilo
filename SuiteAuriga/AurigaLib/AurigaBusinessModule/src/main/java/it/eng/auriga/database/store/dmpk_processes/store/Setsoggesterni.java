...
import it.eng.auriga.database.store.dmpk_processes.store.impl.SetsoggesterniImpl;
 ...
 */
@Service(name="DmpkProcessesSetsoggesterni")
public class Setsoggesterni {
	...
	public StoreResultBean<DmpkProcessesSetsoggesterniBean> execute(AurigaLoginBean lLoginBean, DmpkProcessesSetsoggesterniBean pBean) throws Exception{
	   final SetsoggesterniImpl lSetsoggesterni = new SetsoggesterniImpl();
	   setBean(pBean);
	   lSetsoggesterni.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lSetsoggesterni.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkProcessesSetsoggesterniBean> result = new StoreResultBean<DmpkProcessesSetsoggesterniBean>();
