...
import it.eng.auriga.database.store.dmpk_ws.store.impl.DelapplicazioneesternaImpl;
 ...
 */
@Service(name="DmpkWsDelapplicazioneesterna")
public class Delapplicazioneesterna {
		
	private DmpkWsDelapplicazioneesternaBean bean;
		  
	public void setBean(DmpkWsDelapplicazioneesternaBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkWsDelapplicazioneesternaBean> execute(AurigaLoginBean lLoginBean, DmpkWsDelapplicazioneesternaBean pBean) throws Exception{
	   final DelapplicazioneesternaImpl lDelapplicazioneesterna = new DelapplicazioneesternaImpl();
	   setBean(pBean);
	   lDelapplicazioneesterna.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lDelapplicazioneesterna.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkWsDelapplicazioneesternaBean> result = new StoreResultBean<DmpkWsDelapplicazioneesternaBean>();
