...
import it.eng.auriga.database.store.dmpk_gestore.bean.DmpkGestoreSavedatispecattBean;
import it.eng.auriga.database.store.dmpk_gestore.store.impl.SavedatispecattImpl;
 ...
 */
@Service(name="DmpkGestoreSavedatispecatt")
public class Savedatispecatt {
		
	private DmpkGestoreSavedatispecattBean bean;
		  
	public void setBean(DmpkGestoreSavedatispecattBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkGestoreSavedatispecattBean> execute(AurigaLoginBean lLoginBean, DmpkGestoreSavedatispecattBean pBean) throws Exception{
	   final SavedatispecattImpl lSavedatispecatt = new SavedatispecattImpl();
	   setBean(pBean);
	   lSavedatispecatt.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lSavedatispecatt.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkGestoreSavedatispecattBean> result = new StoreResultBean<DmpkGestoreSavedatispecattBean>();
