...
public class TrovadictvaluesfordictentryImpl  {
	...
	public void execute(Connection connection) throws SQLException {
		...
		try{
			...
			call = connection.prepareCall("{? = call DMPK_DIZIONARIO.TROVADICTVALUESFORDICTENTRY(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}");			
			...
			util.settingParameterOnStore(call,bean,wrapperBean,"strinvaluein",6,Types.VARCHAR,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"strincodvaluein",7,Types.VARCHAR,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"valuegenvincoloin",8,Types.VARCHAR,connection); 	
			...
			util.settingParameterOnStore(call,bean,wrapperBean,"restringiadizdiuoin",22,Types.CLOB,connection); 	
