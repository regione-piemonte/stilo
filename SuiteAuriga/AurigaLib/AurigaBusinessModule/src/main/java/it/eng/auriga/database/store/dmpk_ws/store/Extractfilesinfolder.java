...
import it.eng.auriga.database.store.dmpk_ws.bean.DmpkWsExtractfilesinfolderBean;
import it.eng.auriga.database.store.dmpk_ws.store.impl.ExtractfilesinfolderImpl;
 ...
 */
@Service(name="DmpkWsExtractfilesinfolder")
public class Extractfilesinfolder {
		
	private DmpkWsExtractfilesinfolderBean bean;
		  
	public void setBean(DmpkWsExtractfilesinfolderBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkWsExtractfilesinfolderBean> execute(AurigaLoginBean lLoginBean, DmpkWsExtractfilesinfolderBean pBean) throws Exception{
	   final ExtractfilesinfolderImpl lExtractfilesinfolder = new ExtractfilesinfolderImpl();
	   setBean(pBean);
	   lExtractfilesinfolder.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lExtractfilesinfolder.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkWsExtractfilesinfolderBean> result = new StoreResultBean<DmpkWsExtractfilesinfolderBean>();
