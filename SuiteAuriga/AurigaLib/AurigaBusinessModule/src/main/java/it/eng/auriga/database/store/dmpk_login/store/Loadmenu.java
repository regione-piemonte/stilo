...
import it.eng.auriga.database.store.dmpk_login.bean.DmpkLoginLoadmenuBean;
import it.eng.auriga.database.store.dmpk_login.store.impl.LoadmenuImpl;
 ...
 */
@Service(name="DmpkLoginLoadmenu")
public class Loadmenu {
		
	private DmpkLoginLoadmenuBean bean;
		  
	public void setBean(DmpkLoginLoadmenuBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkLoginLoadmenuBean> execute(AurigaLoginBean lLoginBean, DmpkLoginLoadmenuBean pBean) throws Exception{
	   final LoadmenuImpl lLoadmenu = new LoadmenuImpl();
	   setBean(pBean);
	   lLoadmenu.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lLoadmenu.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkLoginLoadmenuBean> result = new StoreResultBean<DmpkLoginLoadmenuBean>();
