...
import it.eng.auriga.database.store.dmpk_int_mgo_email.store.impl.LoaddettemailImpl;
 ...
 */
@Service(name="DmpkIntMgoEmailLoaddettemail")
public class Loaddettemail {
		
	private DmpkIntMgoEmailLoaddettemailBean bean;
		  
	public void setBean(DmpkIntMgoEmailLoaddettemailBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkIntMgoEmailLoaddettemailBean> execute(AurigaLoginBean lLoginBean, DmpkIntMgoEmailLoaddettemailBean pBean) throws Exception{
	   final LoaddettemailImpl lLoaddettemail = new LoaddettemailImpl();
	   setBean(pBean);
	   lLoaddettemail.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lLoaddettemail.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkIntMgoEmailLoaddettemailBean> result = new StoreResultBean<DmpkIntMgoEmailLoaddettemailBean>();
