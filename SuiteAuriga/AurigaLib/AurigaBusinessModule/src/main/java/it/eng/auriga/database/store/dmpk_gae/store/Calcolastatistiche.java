...
import it.eng.auriga.database.store.dmpk_gae.bean.DmpkGaeCalcolastatisticheBean;
import it.eng.auriga.database.store.dmpk_gae.store.impl.CalcolastatisticheImpl;
 ...
 */
@Service(name="DmpkGaeCalcolastatistiche")
public class Calcolastatistiche {
		
	private DmpkGaeCalcolastatisticheBean bean;
		  
	public void setBean(DmpkGaeCalcolastatisticheBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkGaeCalcolastatisticheBean> execute(AurigaLoginBean lLoginBean, DmpkGaeCalcolastatisticheBean pBean) throws Exception{
	   final CalcolastatisticheImpl lCalcolastatistiche = new CalcolastatisticheImpl();
	   setBean(pBean);
	   lCalcolastatistiche.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lCalcolastatistiche.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkGaeCalcolastatisticheBean> result = new StoreResultBean<DmpkGaeCalcolastatisticheBean>();
