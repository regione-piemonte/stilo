...
package it.eng.auriga.database.store.dmpk_fonti.store;

import it.eng.auriga.database.store.dmpk_fonti.bean.DmpkFontiLoaddettfonteBean;
import it.eng.auriga.database.store.dmpk_fonti.store.impl.LoaddettfonteImpl;
 ...
 */
@Service(name="DmpkFontiLoaddettfonte")
public class Loaddettfonte {
		
	private DmpkFontiLoaddettfonteBean bean;
		  
	public void setBean(DmpkFontiLoaddettfonteBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkFontiLoaddettfonteBean> execute(AurigaLoginBean lLoginBean, DmpkFontiLoaddettfonteBean pBean) throws Exception{
	   final LoaddettfonteImpl lLoaddettfonte = new LoaddettfonteImpl();
	   setBean(pBean);
	   lLoaddettfonte.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lLoaddettfonte.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkFontiLoaddettfonteBean> result = new StoreResultBean<DmpkFontiLoaddettfonteBean>();
