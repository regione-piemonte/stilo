...
import it.eng.auriga.database.store.dmpk_process_types.store.impl.DeventtypeImpl;
 ...
 */
@Service(name="DmpkProcessTypesDeventtype")
public class Deventtype {
		
	private DmpkProcessTypesDeventtypeBean bean;
		  
	public void setBean(DmpkProcessTypesDeventtypeBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkProcessTypesDeventtypeBean> execute(AurigaLoginBean lLoginBean, DmpkProcessTypesDeventtypeBean pBean) throws Exception{
	   final DeventtypeImpl lDeventtype = new DeventtypeImpl();
	   setBean(pBean);
	   lDeventtype.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lDeventtype.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkProcessTypesDeventtypeBean> result = new StoreResultBean<DmpkProcessTypesDeventtypeBean>();
