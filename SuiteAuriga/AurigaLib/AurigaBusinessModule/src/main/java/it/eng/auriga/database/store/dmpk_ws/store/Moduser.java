...
import it.eng.auriga.database.store.dmpk_ws.store.impl.ModuserImpl;
 ...
 */
@Service(name="DmpkWsModuser")
public class Moduser {
		
	private DmpkWsModuserBean bean;
		  
	public void setBean(DmpkWsModuserBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkWsModuserBean> execute(AurigaLoginBean lLoginBean, DmpkWsModuserBean pBean) throws Exception{
	   final ModuserImpl lModuser = new ModuserImpl();
	   setBean(pBean);
	   lModuser.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lModuser.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkWsModuserBean> result = new StoreResultBean<DmpkWsModuserBean>();
