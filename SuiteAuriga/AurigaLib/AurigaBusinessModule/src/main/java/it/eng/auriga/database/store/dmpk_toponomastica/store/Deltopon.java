...
import it.eng.auriga.database.store.dmpk_toponomastica.bean.DmpkToponomasticaDeltoponBean;
import it.eng.auriga.database.store.dmpk_toponomastica.store.impl.DeltoponImpl;
 ...
 */
@Service(name="DmpkToponomasticaDeltopon")
public class Deltopon {
		
	private DmpkToponomasticaDeltoponBean bean;
		  
	public void setBean(DmpkToponomasticaDeltoponBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkToponomasticaDeltoponBean> execute(AurigaLoginBean lLoginBean, DmpkToponomasticaDeltoponBean pBean) throws Exception{
	   final DeltoponImpl lDeltopon = new DeltoponImpl();
	   setBean(pBean);
	   lDeltopon.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lDeltopon.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkToponomasticaDeltoponBean> result = new StoreResultBean<DmpkToponomasticaDeltoponBean>();
