...
package it.eng.auriga.database.store.dmpk_tipi_reg_num_ud.store;

import it.eng.auriga.database.store.dmpk_tipi_reg_num_ud.bean.DmpkTipiRegNumUdDtiporegnumBean;
import it.eng.auriga.database.store.dmpk_tipi_reg_num_ud.store.impl.DtiporegnumImpl;
 ...
 */
@Service(name="DmpkTipiRegNumUdDtiporegnum")
public class Dtiporegnum {
		
	private DmpkTipiRegNumUdDtiporegnumBean bean;
		  
	public void setBean(DmpkTipiRegNumUdDtiporegnumBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkTipiRegNumUdDtiporegnumBean> execute(AurigaLoginBean lLoginBean, DmpkTipiRegNumUdDtiporegnumBean pBean) throws Exception{
	   final DtiporegnumImpl lDtiporegnum = new DtiporegnumImpl();
	   setBean(pBean);
	   lDtiporegnum.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lDtiporegnum.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkTipiRegNumUdDtiporegnumBean> result = new StoreResultBean<DmpkTipiRegNumUdDtiporegnumBean>();
