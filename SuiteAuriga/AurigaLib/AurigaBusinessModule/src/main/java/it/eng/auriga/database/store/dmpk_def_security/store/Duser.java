...
import it.eng.auriga.database.store.dmpk_def_security.store.impl.DuserImpl;
 ...
 */
@Service(name="DmpkDefSecurityDuser")
public class Duser {
		
	private DmpkDefSecurityDuserBean bean;
		  
	public void setBean(DmpkDefSecurityDuserBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkDefSecurityDuserBean> execute(AurigaLoginBean lLoginBean, DmpkDefSecurityDuserBean pBean) throws Exception{
	   final DuserImpl lDuser = new DuserImpl();
	   setBean(pBean);
	   lDuser.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lDuser.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkDefSecurityDuserBean> result = new StoreResultBean<DmpkDefSecurityDuserBean>();
