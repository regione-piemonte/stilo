...
import it.eng.auriga.database.store.dmpk_registrazionedoc.store.impl.CtrludxregistrazioneImpl;
 ...
 */
@Service(name="DmpkRegistrazionedocCtrludxregistrazione")
public class Ctrludxregistrazione {
	...
	public StoreResultBean<DmpkRegistrazionedocCtrludxregistrazioneBean> execute(AurigaLoginBean lLoginBean, DmpkRegistrazionedocCtrludxregistrazioneBean pBean) throws Exception{
	   final CtrludxregistrazioneImpl lCtrludxregistrazione = new CtrludxregistrazioneImpl();
	   setBean(pBean);
	   lCtrludxregistrazione.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lCtrludxregistrazione.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkRegistrazionedocCtrludxregistrazioneBean> result = new StoreResultBean<DmpkRegistrazionedocCtrludxregistrazioneBean>();
