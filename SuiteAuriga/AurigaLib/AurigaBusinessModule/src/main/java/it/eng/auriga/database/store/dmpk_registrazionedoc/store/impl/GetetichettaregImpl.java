...
public class GetetichettaregImpl  {
		
	private DmpkRegistrazionedocGetetichettaregBean bean;
		  
	public void setBean(DmpkRegistrazionedocGetetichettaregBean bean){
		this.bean = bean;
	}
	...
	public void execute(Connection connection) throws SQLException {
		...
		try{
			...
			call = connection.prepareCall("{? = call DMPK_REGISTRAZIONEDOC.GETETICHETTAREG(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}");			
			...
			util.settingParameterOnStore(call,bean,wrapperBean,"numregin",8,Types.INTEGER,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"nroallegatoin",9,Types.INTEGER,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"xmlopzionistampain",10,Types.CLOB,connection); 	
			...
			util.settinParameterOnBean(call,bean,wrapperBean,"contenutobarcodeout",11,Types.VARCHAR); 
			util.settinParameterOnBean(call,bean,wrapperBean,"testoinchiaroout",12,Types.VARCHAR); 
			util.settinParameterOnBean(call,bean,wrapperBean,"testoinchiarobarcodeout",13,Types.VARCHAR); 
			util.settinParameterOnBean(call,bean,wrapperBean,"testoinchiaro2aregout",14,Types.VARCHAR); 
