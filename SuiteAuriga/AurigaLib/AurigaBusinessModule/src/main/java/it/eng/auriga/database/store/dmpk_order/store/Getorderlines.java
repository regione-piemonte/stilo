...
import it.eng.auriga.database.store.dmpk_order.bean.DmpkOrderGetorderlinesBean;
import it.eng.auriga.database.store.dmpk_order.store.impl.GetorderlinesImpl;
 ...
 */
@Service(name="DmpkOrderGetorderlines")
public class Getorderlines {
		
	private DmpkOrderGetorderlinesBean bean;
		  
	public void setBean(DmpkOrderGetorderlinesBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkOrderGetorderlinesBean> execute(AurigaLoginBean lLoginBean, DmpkOrderGetorderlinesBean pBean) throws Exception{
	   final GetorderlinesImpl lGetorderlines = new GetorderlinesImpl();
	   setBean(pBean);
	   lGetorderlines.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lGetorderlines.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkOrderGetorderlinesBean> result = new StoreResultBean<DmpkOrderGetorderlinesBean>();
