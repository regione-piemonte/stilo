...
import it.eng.auriga.database.store.dmpk_int_mgo_email.store.impl.CtrlutenzaabilitatainvioImpl;
 ...
 */
@Service(name="DmpkIntMgoEmailCtrlutenzaabilitatainvio")
public class Ctrlutenzaabilitatainvio {
		
	private DmpkIntMgoEmailCtrlutenzaabilitatainvioBean bean;
		  
	public void setBean(DmpkIntMgoEmailCtrlutenzaabilitatainvioBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkIntMgoEmailCtrlutenzaabilitatainvioBean> execute(AurigaLoginBean lLoginBean, DmpkIntMgoEmailCtrlutenzaabilitatainvioBean pBean) throws Exception{
	   final CtrlutenzaabilitatainvioImpl lCtrlutenzaabilitatainvio = new CtrlutenzaabilitatainvioImpl();
	   setBean(pBean);
	   lCtrlutenzaabilitatainvio.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lCtrlutenzaabilitatainvio.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkIntMgoEmailCtrlutenzaabilitatainvioBean> result = new StoreResultBean<DmpkIntMgoEmailCtrlutenzaabilitatainvioBean>();
