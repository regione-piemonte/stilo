...
public class SavefileasclobImpl  {
	...
	public void execute(Connection connection) throws SQLException {
		...
		try{
			...
			call = connection.prepareCall("{? = call DMPK_REGISTRAZIONEDOC.SAVEFILEASCLOB(?,?,?,?,?,?,?,?,?,?,?,?)}");			
			...
			util.settingParameterOnStore(call,bean,wrapperBean,"filecontentin",5,Types.CLOB,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"flgparsecontentin",6,Types.INTEGER,connection); 	
			...
			util.settingParameterOnStore(call,bean,wrapperBean,"nomefilein",13,Types.VARCHAR,connection); 	
