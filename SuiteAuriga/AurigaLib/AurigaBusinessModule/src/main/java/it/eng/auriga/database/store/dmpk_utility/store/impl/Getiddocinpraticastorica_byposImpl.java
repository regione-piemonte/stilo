...
public class Getiddocinpraticastorica_byposImpl  {
	...
	public void execute(Connection connection) throws SQLException {
		...
		try{
			...
			call = connection.prepareCall("{? = call DMPK_UTILITY.GETIDDOCINPRATICASTORICA_BYPOS(?,?,?,?,?,?,?,?,?,?,?)}");			
			...
			util.settingParameterOnStore(call,bean,wrapperBean,"sezionepraticain",8,Types.VARCHAR,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"nroposdocinpraticain",9,Types.INTEGER,connection); 	
			...
			util.settinParameterOnBean(call,bean,wrapperBean,"nroposizioneultimaudout",10,Types.INTEGER); 
			util.settinParameterOnBean(call,bean,wrapperBean,"iddocout",11,Types.DECIMAL); 
			util.settinParameterOnBean(call,bean,wrapperBean,"idfolderout",12,Types.DECIMAL); 
