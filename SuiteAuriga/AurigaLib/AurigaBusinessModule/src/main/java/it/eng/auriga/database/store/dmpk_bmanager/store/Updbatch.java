...
import it.eng.auriga.database.store.dmpk_bmanager.store.impl.UpdbatchImpl;
 ...
 */
@Service(name="DmpkBmanagerUpdbatch")
public class Updbatch {
	...
	public StoreResultBean<DmpkBmanagerUpdbatchBean> execute(AurigaLoginBean lLoginBean, DmpkBmanagerUpdbatchBean pBean) throws Exception{
	   final UpdbatchImpl lUpdbatch = new UpdbatchImpl();
	   setBean(pBean);
	   lUpdbatch.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lUpdbatch.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkBmanagerUpdbatchBean> result = new StoreResultBean<DmpkBmanagerUpdbatchBean>();
