...
import it.eng.auriga.database.store.dmpk_ws.bean.DmpkWsRevokeprivilegiBean;
import it.eng.auriga.database.store.dmpk_ws.store.impl.RevokeprivilegiImpl;
 ...
 */
@Service(name="DmpkWsRevokeprivilegi")
public class Revokeprivilegi {
		
	private DmpkWsRevokeprivilegiBean bean;
		  
	public void setBean(DmpkWsRevokeprivilegiBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkWsRevokeprivilegiBean> execute(AurigaLoginBean lLoginBean, DmpkWsRevokeprivilegiBean pBean) throws Exception{
	   final RevokeprivilegiImpl lRevokeprivilegi = new RevokeprivilegiImpl();
	   setBean(pBean);
	   lRevokeprivilegi.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lRevokeprivilegi.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkWsRevokeprivilegiBean> result = new StoreResultBean<DmpkWsRevokeprivilegiBean>();
