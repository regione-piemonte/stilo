...
import it.eng.auriga.database.store.dmpk_efact.bean.DmpkEfactDemaildestfpBean;
import it.eng.auriga.database.store.dmpk_efact.store.impl.DemaildestfpImpl;
 ...
 */
@Service(name="DmpkEfactDemaildestfp")
public class Demaildestfp {
		
	private DmpkEfactDemaildestfpBean bean;
		  
	public void setBean(DmpkEfactDemaildestfpBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkEfactDemaildestfpBean> execute(AurigaLoginBean lLoginBean, DmpkEfactDemaildestfpBean pBean) throws Exception{
	   final DemaildestfpImpl lDemaildestfp = new DemaildestfpImpl();
	   setBean(pBean);
	   lDemaildestfp.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lDemaildestfp.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkEfactDemaildestfpBean> result = new StoreResultBean<DmpkEfactDemaildestfpBean>();
