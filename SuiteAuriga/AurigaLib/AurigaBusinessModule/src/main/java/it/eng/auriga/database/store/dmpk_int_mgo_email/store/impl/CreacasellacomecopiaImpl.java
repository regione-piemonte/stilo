...
public class CreacasellacomecopiaImpl  {
	...
	public void execute(Connection connection) throws SQLException {
		...
		try{
			...
			call = connection.prepareCall("{? = call DMPK_INT_MGO_EMAIL.CREACASELLACOMECOPIA(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}");			
			...
			util.settingParameterOnStore(call,bean,wrapperBean,"idcaselladacopiarein",4,Types.VARCHAR,connection); 	
			...
			util.settingParameterOnStore(call,bean,wrapperBean,"accountcasellain",8,Types.VARCHAR,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"xmlparametriin",9,Types.CLOB,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"flgattivascaricoin",10,Types.INTEGER,connection); 	
			...
			util.settinParameterOnBean(call,bean,wrapperBean,"idcasellanewout",11,Types.VARCHAR); 
