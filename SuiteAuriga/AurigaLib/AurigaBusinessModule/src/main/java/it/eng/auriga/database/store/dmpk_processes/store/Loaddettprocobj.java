...
import it.eng.auriga.database.store.dmpk_processes.bean.DmpkProcessesLoaddettprocobjBean;
import it.eng.auriga.database.store.dmpk_processes.store.impl.LoaddettprocobjImpl;
 ...
 */
@Service(name="DmpkProcessesLoaddettprocobj")
public class Loaddettprocobj {
		
	private DmpkProcessesLoaddettprocobjBean bean;
		  
	public void setBean(DmpkProcessesLoaddettprocobjBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkProcessesLoaddettprocobjBean> execute(AurigaLoginBean lLoginBean, DmpkProcessesLoaddettprocobjBean pBean) throws Exception{
	   final LoaddettprocobjImpl lLoaddettprocobj = new LoaddettprocobjImpl();
	   setBean(pBean);
	   lLoaddettprocobj.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lLoaddettprocobj.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkProcessesLoaddettprocobjBean> result = new StoreResultBean<DmpkProcessesLoaddettprocobjBean>();
