...
public class Addrelevtty_proctyImpl  {
	...
	public void execute(Connection connection) throws SQLException {
		...
		try{
			...
			call = connection.prepareCall("{? = call DMPK_PROCESS_TYPES.ADDRELEVTTY_PROCTY(?,?,?,?,?,?,?,?,?,?,?,?,?)}");			
			...
			util.settingParameterOnStore(call,bean,wrapperBean,"deseventtypein",5,Types.VARCHAR,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"processtypesxmlin",6,Types.CLOB,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"flglockedin",7,Types.INTEGER,connection); 	
