...
import it.eng.auriga.database.store.dmpk_processes.bean.DmpkProcessesGetlistascadenzeBean;
import it.eng.auriga.database.store.dmpk_processes.store.impl.GetlistascadenzeImpl;
 ...
 */
@Service(name="DmpkProcessesGetlistascadenze")
public class Getlistascadenze {
		
	private DmpkProcessesGetlistascadenzeBean bean;
		  
	public void setBean(DmpkProcessesGetlistascadenzeBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkProcessesGetlistascadenzeBean> execute(AurigaLoginBean lLoginBean, DmpkProcessesGetlistascadenzeBean pBean) throws Exception{
	   final GetlistascadenzeImpl lGetlistascadenze = new GetlistascadenzeImpl();
	   setBean(pBean);
	   lGetlistascadenze.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lGetlistascadenze.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkProcessesGetlistascadenzeBean> result = new StoreResultBean<DmpkProcessesGetlistascadenzeBean>();
