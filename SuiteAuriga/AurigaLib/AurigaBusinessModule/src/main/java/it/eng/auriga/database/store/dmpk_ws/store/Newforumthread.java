...
import it.eng.auriga.database.store.dmpk_ws.bean.DmpkWsNewforumthreadBean;
import it.eng.auriga.database.store.dmpk_ws.store.impl.NewforumthreadImpl;
 ...
 */
@Service(name="DmpkWsNewforumthread")
public class Newforumthread {
		
	private DmpkWsNewforumthreadBean bean;
		  
	public void setBean(DmpkWsNewforumthreadBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkWsNewforumthreadBean> execute(AurigaLoginBean lLoginBean, DmpkWsNewforumthreadBean pBean) throws Exception{
	   final NewforumthreadImpl lNewforumthread = new NewforumthreadImpl();
	   setBean(pBean);
	   lNewforumthread.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lNewforumthread.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkWsNewforumthreadBean> result = new StoreResultBean<DmpkWsNewforumthreadBean>();
