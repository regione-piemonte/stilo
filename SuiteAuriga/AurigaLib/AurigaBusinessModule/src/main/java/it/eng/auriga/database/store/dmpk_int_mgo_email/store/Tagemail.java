...
import it.eng.auriga.database.store.dmpk_int_mgo_email.store.impl.TagemailImpl;
 ...
 */
@Service(name="DmpkIntMgoEmailTagemail")
public class Tagemail {
		
	private DmpkIntMgoEmailTagemailBean bean;
		  
	public void setBean(DmpkIntMgoEmailTagemailBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkIntMgoEmailTagemailBean> execute(AurigaLoginBean lLoginBean, DmpkIntMgoEmailTagemailBean pBean) throws Exception{
	   final TagemailImpl lTagemail = new TagemailImpl();
	   setBean(pBean);
	   lTagemail.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lTagemail.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkIntMgoEmailTagemailBean> result = new StoreResultBean<DmpkIntMgoEmailTagemailBean>();
