...
public class FindudImpl  {
	...
	private DmpkCoreFindudBean bean;
		  
	public void setBean(DmpkCoreFindudBean bean){
		this.bean = bean;
	}
	...
	public void execute(Connection connection) throws SQLException {
		...
		try{
			...
			call = connection.prepareCall("{? = call DMPK_CORE.FINDUD(?,?,?,?,?,?,?,?,?,?,?,?,?,?)}");			
			...
			util.settingParameterOnStore(call,bean,wrapperBean,"accesstypein",9,Types.VARCHAR,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"iddoctypein",15,Types.DECIMAL,connection); 	
			...
			util.settinParameterOnBean(call,bean,wrapperBean,"iddocprimarioout",10,Types.DECIMAL); 
