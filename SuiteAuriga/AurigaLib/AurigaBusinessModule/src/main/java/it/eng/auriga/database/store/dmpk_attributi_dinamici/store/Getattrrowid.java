...
import it.eng.auriga.database.store.dmpk_attributi_dinamici.bean.DmpkAttributiDinamiciGetattrrowidBean;
import it.eng.auriga.database.store.dmpk_attributi_dinamici.store.impl.GetattrrowidImpl;
 ...
 */
@Service(name="DmpkAttributiDinamiciGetattrrowid")
public class Getattrrowid {
		
	private DmpkAttributiDinamiciGetattrrowidBean bean;
		  
	public void setBean(DmpkAttributiDinamiciGetattrrowidBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkAttributiDinamiciGetattrrowidBean> execute(AurigaLoginBean lLoginBean, DmpkAttributiDinamiciGetattrrowidBean pBean) throws Exception{
	   final GetattrrowidImpl lGetattrrowid = new GetattrrowidImpl();
	   setBean(pBean);
	   lGetattrrowid.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lGetattrrowid.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkAttributiDinamiciGetattrrowidBean> result = new StoreResultBean<DmpkAttributiDinamiciGetattrrowidBean>();
