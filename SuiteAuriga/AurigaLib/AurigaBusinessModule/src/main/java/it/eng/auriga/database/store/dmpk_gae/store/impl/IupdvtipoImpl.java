...
public class IupdvtipoImpl  {
	...
	public void execute(Connection connection) throws SQLException {
		...
		try{
			...
			call = connection.prepareCall("{? = call DMPK_GAE.IUPDVTIPO(?,?,?,?,?,?,?,?,?,?,?,?)}");			
			...
			util.settingParameterOnStore(call,bean,wrapperBean,"idpdvtipoio",4,Types.VARCHAR,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"tipooperazionein",5,Types.VARCHAR,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"xmldatipdvtipoin",6,Types.CLOB,connection); 	
			...
			util.settinParameterOnBean(call,bean,wrapperBean,"idpdvtipoio",4,Types.VARCHAR); 
