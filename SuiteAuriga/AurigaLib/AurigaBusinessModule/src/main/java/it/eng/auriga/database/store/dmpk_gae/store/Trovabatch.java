...
import it.eng.auriga.database.store.dmpk_gae.bean.DmpkGaeTrovabatchBean;
import it.eng.auriga.database.store.dmpk_gae.store.impl.TrovabatchImpl;
 ...
 */
@Service(name="DmpkGaeTrovabatch")
public class Trovabatch {
		
	private DmpkGaeTrovabatchBean bean;
		  
	public void setBean(DmpkGaeTrovabatchBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkGaeTrovabatchBean> execute(AurigaLoginBean lLoginBean, DmpkGaeTrovabatchBean pBean) throws Exception{
	   final TrovabatchImpl lTrovabatch = new TrovabatchImpl();
	   setBean(pBean);
	   lTrovabatch.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lTrovabatch.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkGaeTrovabatchBean> result = new StoreResultBean<DmpkGaeTrovabatchBean>();
