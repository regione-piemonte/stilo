...
import it.eng.auriga.database.store.dmpk_def_security.bean.DmpkDefSecurityDgrupposoggintBean;
import it.eng.auriga.database.store.dmpk_def_security.store.impl.DgrupposoggintImpl;
 ...
 */
@Service(name="DmpkDefSecurityDgrupposoggint")
public class Dgrupposoggint {
		
	private DmpkDefSecurityDgrupposoggintBean bean;
		  
	public void setBean(DmpkDefSecurityDgrupposoggintBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkDefSecurityDgrupposoggintBean> execute(AurigaLoginBean lLoginBean, DmpkDefSecurityDgrupposoggintBean pBean) throws Exception{
	   final DgrupposoggintImpl lDgrupposoggint = new DgrupposoggintImpl();
	   setBean(pBean);
	   lDgrupposoggint.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lDgrupposoggint.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkDefSecurityDgrupposoggintBean> result = new StoreResultBean<DmpkDefSecurityDgrupposoggintBean>();
