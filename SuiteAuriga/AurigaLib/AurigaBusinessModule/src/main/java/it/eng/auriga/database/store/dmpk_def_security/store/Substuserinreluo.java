...
import it.eng.auriga.database.store.dmpk_def_security.store.impl.SubstuserinreluoImpl;
 ...
 */
@Service(name="DmpkDefSecuritySubstuserinreluo")
public class Substuserinreluo {
	...
	public StoreResultBean<DmpkDefSecuritySubstuserinreluoBean> execute(AurigaLoginBean lLoginBean, DmpkDefSecuritySubstuserinreluoBean pBean) throws Exception{
	   final SubstuserinreluoImpl lSubstuserinreluo = new SubstuserinreluoImpl();
	   setBean(pBean);
	   lSubstuserinreluo.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lSubstuserinreluo.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkDefSecuritySubstuserinreluoBean> result = new StoreResultBean<DmpkDefSecuritySubstuserinreluoBean>();
