...
/**
 * The persistent class for the T_FILE_DOC_TO_SIGN database table.
 * 
 */
@Entity
@Table(name="T_FILE_DOC_TO_SIGN")
@NamedQuery(name="TFileDocToSign.findAll", query="SELECT t FROM TFileDocToSign t")
public class TFileDocToSign implements Serializable {
	...
	//bi-directional many-to-one association to TDocToSign
	@ManyToOne
	@JoinColumn(name="DOC_REC_ID")
	private TDocToSign TDocToSign;

	public TFileDocToSign() {
	}
	...
	public TDocToSign getTDocToSign() {
		return this.TDocToSign;
	}

	public void setTDocToSign(TDocToSign TDocToSign) {
		this.TDocToSign = TDocToSign;
	}
