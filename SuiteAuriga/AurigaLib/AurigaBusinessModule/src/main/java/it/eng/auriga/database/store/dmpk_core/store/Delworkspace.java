...
import it.eng.auriga.database.store.dmpk_core.bean.DmpkCoreDelworkspaceBean;
import it.eng.auriga.database.store.dmpk_core.store.impl.DelworkspaceImpl;
 ...
 */
@Service(name="DmpkCoreDelworkspace")
public class Delworkspace {
		
	private DmpkCoreDelworkspaceBean bean;
		  
	public void setBean(DmpkCoreDelworkspaceBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkCoreDelworkspaceBean> execute(AurigaLoginBean lLoginBean, DmpkCoreDelworkspaceBean pBean) throws Exception{
	   final DelworkspaceImpl lDelworkspace = new DelworkspaceImpl();
	   setBean(pBean);
	   lDelworkspace.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lDelworkspace.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkCoreDelworkspaceBean> result = new StoreResultBean<DmpkCoreDelworkspaceBean>();
