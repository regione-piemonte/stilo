...
import it.eng.auriga.database.store.dmpk_def_security.store.impl.DuoImpl;
 ...
 */
@Service(name="DmpkDefSecurityDuo")
public class Duo {
	...
	public StoreResultBean<DmpkDefSecurityDuoBean> execute(AurigaLoginBean lLoginBean, DmpkDefSecurityDuoBean pBean) throws Exception{
	   final DuoImpl lDuo = new DuoImpl();
	   setBean(pBean);
	   lDuo.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lDuo.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkDefSecurityDuoBean> result = new StoreResultBean<DmpkDefSecurityDuoBean>();
