...
import it.eng.auriga.database.store.dmpk_efact.store.impl.IulottoImpl;
 ...
 */
@Service(name="DmpkEfactIulotto")
public class Iulotto {
	...
	public StoreResultBean<DmpkEfactIulottoBean> execute(AurigaLoginBean lLoginBean, DmpkEfactIulottoBean pBean) throws Exception{
	   final IulottoImpl lIulotto = new IulottoImpl();
	   setBean(pBean);
	   lIulotto.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lIulotto.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkEfactIulottoBean> result = new StoreResultBean<DmpkEfactIulottoBean>();
