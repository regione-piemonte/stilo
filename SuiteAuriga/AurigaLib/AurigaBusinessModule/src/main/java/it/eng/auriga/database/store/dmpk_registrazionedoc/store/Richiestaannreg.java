...
import it.eng.auriga.database.store.dmpk_registrazionedoc.store.impl.RichiestaannregImpl;
 ...
 */
@Service(name="DmpkRegistrazionedocRichiestaannreg")
public class Richiestaannreg {
		
	private DmpkRegistrazionedocRichiestaannregBean bean;
		  
	public void setBean(DmpkRegistrazionedocRichiestaannregBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkRegistrazionedocRichiestaannregBean> execute(AurigaLoginBean lLoginBean, DmpkRegistrazionedocRichiestaannregBean pBean) throws Exception{
	   final RichiestaannregImpl lRichiestaannreg = new RichiestaannregImpl();
	   setBean(pBean);
	   lRichiestaannreg.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lRichiestaannreg.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkRegistrazionedocRichiestaannregBean> result = new StoreResultBean<DmpkRegistrazionedocRichiestaannregBean>();
