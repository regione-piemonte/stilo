...
package it.eng.auriga.database.store.dmpk_efact2.store;

import it.eng.auriga.database.store.dmpk_efact2.bean.DmpkEfact2IuusermassivoBean;
import it.eng.auriga.database.store.dmpk_efact2.store.impl.IuusermassivoImpl;
 ...
 */
@Service(name="DmpkEfact2Iuusermassivo")
public class Iuusermassivo {
		
	private DmpkEfact2IuusermassivoBean bean;
		  
	public void setBean(DmpkEfact2IuusermassivoBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkEfact2IuusermassivoBean> execute(AurigaLoginBean lLoginBean, DmpkEfact2IuusermassivoBean pBean) throws Exception{
	   final IuusermassivoImpl lIuusermassivo = new IuusermassivoImpl();
	   setBean(pBean);
	   lIuusermassivo.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lIuusermassivo.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkEfact2IuusermassivoBean> result = new StoreResultBean<DmpkEfact2IuusermassivoBean>();
