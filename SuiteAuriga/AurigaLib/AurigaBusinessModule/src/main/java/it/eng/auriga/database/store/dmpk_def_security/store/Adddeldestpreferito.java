...
import it.eng.auriga.database.store.dmpk_def_security.store.impl.AdddeldestpreferitoImpl;
 ...
 */
@Service(name="DmpkDefSecurityAdddeldestpreferito")
public class Adddeldestpreferito {
		
	private DmpkDefSecurityAdddeldestpreferitoBean bean;
		  
	public void setBean(DmpkDefSecurityAdddeldestpreferitoBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkDefSecurityAdddeldestpreferitoBean> execute(AurigaLoginBean lLoginBean, DmpkDefSecurityAdddeldestpreferitoBean pBean) throws Exception{
	   final AdddeldestpreferitoImpl lAdddeldestpreferito = new AdddeldestpreferitoImpl();
	   setBean(pBean);
	   lAdddeldestpreferito.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lAdddeldestpreferito.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkDefSecurityAdddeldestpreferitoBean> result = new StoreResultBean<DmpkDefSecurityAdddeldestpreferitoBean>();
