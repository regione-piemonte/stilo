...
public class LoginapplicazioneImpl  {
		
	private DmpkLoginLoginapplicazioneBean bean;
		  
	public void setBean(DmpkLoginLoginapplicazioneBean bean){
		this.bean = bean;
	}
	...
	public void execute(Connection connection) throws SQLException {
		...
		try{
			...
			call = connection.prepareCall("{? = call DMPK_LOGIN.LOGINAPPLICAZIONE(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}");			
			...
			util.settingParameterOnStore(call,bean,wrapperBean,"useridapplicazionein",2,Types.VARCHAR,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"passwordin",3,Types.VARCHAR,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"iddominioautout",5,Types.DECIMAL,connection); 	
			...
			util.settinParameterOnBean(call,bean,wrapperBean,"desuserout",8,Types.VARCHAR); 
