...
import it.eng.auriga.database.store.dmpk_bmanager.store.impl.Fsuplaod_verifyfileImpl;
 ...
 */
@Service(name="DmpkBmanagerFsuplaod_verifyfile")
public class Fsuplaod_verifyfile {
	...
	public StoreResultBean<DmpkBmanagerFsuplaod_verifyfileBean> execute(AurigaLoginBean lLoginBean, DmpkBmanagerFsuplaod_verifyfileBean pBean) throws Exception{
	   final Fsuplaod_verifyfileImpl lFsuplaod_verifyfile = new Fsuplaod_verifyfileImpl();
	   setBean(pBean);
	   lFsuplaod_verifyfile.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lFsuplaod_verifyfile.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkBmanagerFsuplaod_verifyfileBean> result = new StoreResultBean<DmpkBmanagerFsuplaod_verifyfileBean>();
