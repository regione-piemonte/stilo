...
import it.eng.auriga.database.store.dmpk_core.bean.DmpkCoreFindlibraryBean;
import it.eng.auriga.database.store.dmpk_core.store.impl.FindlibraryImpl;
 ...
 */
@Service(name="DmpkCoreFindlibrary")
public class Findlibrary {
		
	private DmpkCoreFindlibraryBean bean;
		  
	public void setBean(DmpkCoreFindlibraryBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkCoreFindlibraryBean> execute(AurigaLoginBean lLoginBean, DmpkCoreFindlibraryBean pBean) throws Exception{
	   final FindlibraryImpl lFindlibrary = new FindlibraryImpl();
	   setBean(pBean);
	   lFindlibrary.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lFindlibrary.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkCoreFindlibraryBean> result = new StoreResultBean<DmpkCoreFindlibraryBean>();
