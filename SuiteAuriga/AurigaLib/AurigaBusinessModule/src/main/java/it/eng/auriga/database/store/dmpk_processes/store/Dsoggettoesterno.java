...
import it.eng.auriga.database.store.dmpk_processes.bean.DmpkProcessesDsoggettoesternoBean;
import it.eng.auriga.database.store.dmpk_processes.store.impl.DsoggettoesternoImpl;
 ...
 */
@Service(name="DmpkProcessesDsoggettoesterno")
public class Dsoggettoesterno {
		
	private DmpkProcessesDsoggettoesternoBean bean;
		  
	public void setBean(DmpkProcessesDsoggettoesternoBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkProcessesDsoggettoesternoBean> execute(AurigaLoginBean lLoginBean, DmpkProcessesDsoggettoesternoBean pBean) throws Exception{
	   final DsoggettoesternoImpl lDsoggettoesterno = new DsoggettoesternoImpl();
	   setBean(pBean);
	   lDsoggettoesterno.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lDsoggettoesterno.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkProcessesDsoggettoesternoBean> result = new StoreResultBean<DmpkProcessesDsoggettoesternoBean>();
