...
import it.eng.auriga.database.store.dmpk_def_security.store.impl.IuuserImpl;
 ...
 */
@Service(name="DmpkDefSecurityIuuser")
public class Iuuser {
	...
	public StoreResultBean<DmpkDefSecurityIuuserBean> execute(AurigaLoginBean lLoginBean, DmpkDefSecurityIuuserBean pBean) throws Exception{
	   final IuuserImpl lIuuser = new IuuserImpl();
	   setBean(pBean);
	   lIuuser.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lIuuser.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkDefSecurityIuuserBean> result = new StoreResultBean<DmpkDefSecurityIuuserBean>();
