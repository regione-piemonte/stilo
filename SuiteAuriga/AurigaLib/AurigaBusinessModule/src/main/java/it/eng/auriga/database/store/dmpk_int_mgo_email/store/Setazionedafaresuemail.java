...
import it.eng.auriga.database.store.dmpk_int_mgo_email.store.impl.SetazionedafaresuemailImpl;
 ...
 */
@Service(name="DmpkIntMgoEmailSetazionedafaresuemail")
public class Setazionedafaresuemail {
	...
	public StoreResultBean<DmpkIntMgoEmailSetazionedafaresuemailBean> execute(AurigaLoginBean lLoginBean, DmpkIntMgoEmailSetazionedafaresuemailBean pBean) throws Exception{
	   final SetazionedafaresuemailImpl lSetazionedafaresuemail = new SetazionedafaresuemailImpl();
	   setBean(pBean);
	   lSetazionedafaresuemail.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lSetazionedafaresuemail.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkIntMgoEmailSetazionedafaresuemailBean> result = new StoreResultBean<DmpkIntMgoEmailSetazionedafaresuemailBean>();
