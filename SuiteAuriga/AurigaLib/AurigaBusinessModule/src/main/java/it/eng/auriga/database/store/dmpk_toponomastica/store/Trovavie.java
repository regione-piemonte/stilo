...
import it.eng.auriga.database.store.dmpk_toponomastica.store.impl.TrovavieImpl;
 ...
 */
@Service(name="DmpkToponomasticaTrovavie")
public class Trovavie {
		
	private DmpkToponomasticaTrovavieBean bean;
		  
	public void setBean(DmpkToponomasticaTrovavieBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkToponomasticaTrovavieBean> execute(AurigaLoginBean lLoginBean, DmpkToponomasticaTrovavieBean pBean) throws Exception{
	   final TrovavieImpl lTrovavie = new TrovavieImpl();
	   setBean(pBean);
	   lTrovavie.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lTrovavie.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkToponomasticaTrovavieBean> result = new StoreResultBean<DmpkToponomasticaTrovavieBean>();
