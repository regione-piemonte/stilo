...
import it.eng.auriga.database.store.dmpk_servizi_digibox.bean.DmpkServiziDigiboxRicercadocumentidigiboxBean;
import it.eng.auriga.database.store.dmpk_servizi_digibox.store.impl.RicercadocumentidigiboxImpl;
 ...
 */
@Service(name="DmpkServiziDigiboxRicercadocumentidigibox")
public class Ricercadocumentidigibox {
		
	private DmpkServiziDigiboxRicercadocumentidigiboxBean bean;
		  
	public void setBean(DmpkServiziDigiboxRicercadocumentidigiboxBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkServiziDigiboxRicercadocumentidigiboxBean> execute(AurigaLoginBean lLoginBean, DmpkServiziDigiboxRicercadocumentidigiboxBean pBean) throws Exception{
	   final RicercadocumentidigiboxImpl lRicercadocumentidigibox = new RicercadocumentidigiboxImpl();
	   setBean(pBean);
	   lRicercadocumentidigibox.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lRicercadocumentidigibox.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkServiziDigiboxRicercadocumentidigiboxBean> result = new StoreResultBean<DmpkServiziDigiboxRicercadocumentidigiboxBean>();
