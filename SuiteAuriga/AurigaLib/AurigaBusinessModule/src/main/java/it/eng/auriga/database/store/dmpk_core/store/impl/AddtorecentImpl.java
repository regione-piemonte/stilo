...
public class AddtorecentImpl  {
	...
	public void execute(Connection connection) throws SQLException {
		...
		try{
			...
			call = connection.prepareCall("{? = call DMPK_CORE.ADDTORECENT(?,?,?,?,?,?,?,?,?,?,?,?,?,?)}");			
			...
			util.settingParameterOnStore(call,bean,wrapperBean,"flgvlin",7,Types.VARCHAR,connection); 	
			...
			util.settingParameterOnStore(call,bean,wrapperBean,"iddocdownloadedin",14,Types.DECIMAL,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"nroverdownloadedin",15,Types.INTEGER,connection); 	
