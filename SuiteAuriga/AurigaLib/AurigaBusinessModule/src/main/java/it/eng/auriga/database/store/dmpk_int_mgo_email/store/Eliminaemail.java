...
import it.eng.auriga.database.store.dmpk_int_mgo_email.store.impl.EliminaemailImpl;
 ...
 */
@Service(name="DmpkIntMgoEmailEliminaemail")
public class Eliminaemail {
		
	private DmpkIntMgoEmailEliminaemailBean bean;
		  
	public void setBean(DmpkIntMgoEmailEliminaemailBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkIntMgoEmailEliminaemailBean> execute(AurigaLoginBean lLoginBean, DmpkIntMgoEmailEliminaemailBean pBean) throws Exception{
	   final EliminaemailImpl lEliminaemail = new EliminaemailImpl();
	   setBean(pBean);
	   lEliminaemail.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lEliminaemail.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkIntMgoEmailEliminaemailBean> result = new StoreResultBean<DmpkIntMgoEmailEliminaemailBean>();
