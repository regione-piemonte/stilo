...
import it.eng.auriga.database.store.dmpk_process_types.store.impl.TrovascadenzeproctypeImpl;
 ...
 */
@Service(name="DmpkProcessTypesTrovascadenzeproctype")
public class Trovascadenzeproctype {
		
	private DmpkProcessTypesTrovascadenzeproctypeBean bean;
		  
	public void setBean(DmpkProcessTypesTrovascadenzeproctypeBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkProcessTypesTrovascadenzeproctypeBean> execute(AurigaLoginBean lLoginBean, DmpkProcessTypesTrovascadenzeproctypeBean pBean) throws Exception{
	   final TrovascadenzeproctypeImpl lTrovascadenzeproctype = new TrovascadenzeproctypeImpl();
	   setBean(pBean);
	   lTrovascadenzeproctype.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lTrovascadenzeproctype.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkProcessTypesTrovascadenzeproctypeBean> result = new StoreResultBean<DmpkProcessTypesTrovascadenzeproctypeBean>();
