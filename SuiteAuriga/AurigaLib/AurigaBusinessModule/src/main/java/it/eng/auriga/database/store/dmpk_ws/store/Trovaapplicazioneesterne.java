...
import it.eng.auriga.database.store.dmpk_ws.store.impl.TrovaapplicazioneesterneImpl;
 ...
 */
@Service(name="DmpkWsTrovaapplicazioneesterne")
public class Trovaapplicazioneesterne {
		
	private DmpkWsTrovaapplicazioneesterneBean bean;
		  
	public void setBean(DmpkWsTrovaapplicazioneesterneBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkWsTrovaapplicazioneesterneBean> execute(AurigaLoginBean lLoginBean, DmpkWsTrovaapplicazioneesterneBean pBean) throws Exception{
	   final TrovaapplicazioneesterneImpl lTrovaapplicazioneesterne = new TrovaapplicazioneesterneImpl();
	   setBean(pBean);
	   lTrovaapplicazioneesterne.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lTrovaapplicazioneesterne.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkWsTrovaapplicazioneesterneBean> result = new StoreResultBean<DmpkWsTrovaapplicazioneesterneBean>();
