...
import it.eng.auriga.database.store.dmpk_int_mgo_email.bean.DmpkIntMgoEmailGetassegnatariemailBean;
import it.eng.auriga.database.store.dmpk_int_mgo_email.store.impl.GetassegnatariemailImpl;
 ...
 */
@Service(name="DmpkIntMgoEmailGetassegnatariemail")
public class Getassegnatariemail {
		
	private DmpkIntMgoEmailGetassegnatariemailBean bean;
		  
	public void setBean(DmpkIntMgoEmailGetassegnatariemailBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkIntMgoEmailGetassegnatariemailBean> execute(AurigaLoginBean lLoginBean, DmpkIntMgoEmailGetassegnatariemailBean pBean) throws Exception{
	   final GetassegnatariemailImpl lGetassegnatariemail = new GetassegnatariemailImpl();
	   setBean(pBean);
	   lGetassegnatariemail.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lGetassegnatariemail.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkIntMgoEmailGetassegnatariemailBean> result = new StoreResultBean<DmpkIntMgoEmailGetassegnatariemailBean>();
