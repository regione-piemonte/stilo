...
import it.eng.auriga.database.store.dmpk_modelli_doc.store.impl.LoaddettmodelloImpl;
 ...
 */
@Service(name="DmpkModelliDocLoaddettmodello")
public class Loaddettmodello {
	...
	public StoreResultBean<DmpkModelliDocLoaddettmodelloBean> execute(AurigaLoginBean lLoginBean, DmpkModelliDocLoaddettmodelloBean pBean) throws Exception{
	   final LoaddettmodelloImpl lLoaddettmodello = new LoaddettmodelloImpl();
	   setBean(pBean);
	   lLoaddettmodello.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lLoaddettmodello.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkModelliDocLoaddettmodelloBean> result = new StoreResultBean<DmpkModelliDocLoaddettmodelloBean>();
