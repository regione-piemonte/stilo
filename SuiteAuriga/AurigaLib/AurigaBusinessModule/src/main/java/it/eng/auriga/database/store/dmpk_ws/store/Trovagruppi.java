...
import it.eng.auriga.database.store.dmpk_ws.store.impl.TrovagruppiImpl;
 ...
 */
@Service(name="DmpkWsTrovagruppi")
public class Trovagruppi {
		
	private DmpkWsTrovagruppiBean bean;
		  
	public void setBean(DmpkWsTrovagruppiBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkWsTrovagruppiBean> execute(AurigaLoginBean lLoginBean, DmpkWsTrovagruppiBean pBean) throws Exception{
	   final TrovagruppiImpl lTrovagruppi = new TrovagruppiImpl();
	   setBean(pBean);
	   lTrovagruppi.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lTrovagruppi.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkWsTrovagruppiBean> result = new StoreResultBean<DmpkWsTrovagruppiBean>();
