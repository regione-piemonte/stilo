...
import it.eng.auriga.database.store.dmpk_gae.bean.DmpkGaeTrovadoclottibatchBean;
import it.eng.auriga.database.store.dmpk_gae.store.impl.TrovadoclottibatchImpl;
 ...
 */
@Service(name="DmpkGaeTrovadoclottibatch")
public class Trovadoclottibatch {
		
	private DmpkGaeTrovadoclottibatchBean bean;
		  
	public void setBean(DmpkGaeTrovadoclottibatchBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkGaeTrovadoclottibatchBean> execute(AurigaLoginBean lLoginBean, DmpkGaeTrovadoclottibatchBean pBean) throws Exception{
	   final TrovadoclottibatchImpl lTrovadoclottibatch = new TrovadoclottibatchImpl();
	   setBean(pBean);
	   lTrovadoclottibatch.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lTrovadoclottibatch.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkGaeTrovadoclottibatchBean> result = new StoreResultBean<DmpkGaeTrovadoclottibatchBean>();
