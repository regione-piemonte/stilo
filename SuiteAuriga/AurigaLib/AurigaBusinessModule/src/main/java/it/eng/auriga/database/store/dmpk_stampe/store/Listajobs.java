...
import it.eng.auriga.database.store.dmpk_stampe.bean.DmpkStampeListajobsBean;
import it.eng.auriga.database.store.dmpk_stampe.store.impl.ListajobsImpl;
 ...
 */
@Service(name="DmpkStampeListajobs")
public class Listajobs {
		
	private DmpkStampeListajobsBean bean;
		  
	public void setBean(DmpkStampeListajobsBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkStampeListajobsBean> execute(AurigaLoginBean lLoginBean, DmpkStampeListajobsBean pBean) throws Exception{
	   final ListajobsImpl lListajobs = new ListajobsImpl();
	   setBean(pBean);
	   lListajobs.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lListajobs.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkStampeListajobsBean> result = new StoreResultBean<DmpkStampeListajobsBean>();
