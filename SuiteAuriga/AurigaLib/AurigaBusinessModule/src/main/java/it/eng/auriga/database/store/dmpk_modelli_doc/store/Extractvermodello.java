...
import it.eng.auriga.database.store.dmpk_modelli_doc.store.impl.ExtractvermodelloImpl;
 ...
 */
@Service(name="DmpkModelliDocExtractvermodello")
public class Extractvermodello {
	...
	public StoreResultBean<DmpkModelliDocExtractvermodelloBean> execute(AurigaLoginBean lLoginBean, DmpkModelliDocExtractvermodelloBean pBean) throws Exception{
	   final ExtractvermodelloImpl lExtractvermodello = new ExtractvermodelloImpl();
	   setBean(pBean);
	   lExtractvermodello.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lExtractvermodello.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkModelliDocExtractvermodelloBean> result = new StoreResultBean<DmpkModelliDocExtractvermodelloBean>();
