...
import it.eng.auriga.database.store.dmpk_repository_gui.bean.DmpkRepositoryGuiLoaddestassudBean;
import it.eng.auriga.database.store.dmpk_repository_gui.store.impl.LoaddestassudImpl;
 ...
 */
@Service(name="DmpkRepositoryGuiLoaddestassud")
public class Loaddestassud {
		
	private DmpkRepositoryGuiLoaddestassudBean bean;
		  
	public void setBean(DmpkRepositoryGuiLoaddestassudBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkRepositoryGuiLoaddestassudBean> execute(AurigaLoginBean lLoginBean, DmpkRepositoryGuiLoaddestassudBean pBean) throws Exception{
	   final LoaddestassudImpl lLoaddestassud = new LoaddestassudImpl();
	   setBean(pBean);
	   lLoaddestassud.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lLoaddestassud.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkRepositoryGuiLoaddestassudBean> result = new StoreResultBean<DmpkRepositoryGuiLoaddestassudBean>();
