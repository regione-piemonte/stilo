...
import it.eng.auriga.database.store.dmpk_login.store.impl.StartsessioneindelegaImpl;
 ...
 */
@Service(name="DmpkLoginStartsessioneindelega")
public class Startsessioneindelega {
		
	private DmpkLoginStartsessioneindelegaBean bean;
		  
	public void setBean(DmpkLoginStartsessioneindelegaBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkLoginStartsessioneindelegaBean> execute(AurigaLoginBean lLoginBean, DmpkLoginStartsessioneindelegaBean pBean) throws Exception{
	   final StartsessioneindelegaImpl lStartsessioneindelega = new StartsessioneindelegaImpl();
	   setBean(pBean);
	   lStartsessioneindelega.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lStartsessioneindelega.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkLoginStartsessioneindelegaBean> result = new StoreResultBean<DmpkLoginStartsessioneindelegaBean>();
