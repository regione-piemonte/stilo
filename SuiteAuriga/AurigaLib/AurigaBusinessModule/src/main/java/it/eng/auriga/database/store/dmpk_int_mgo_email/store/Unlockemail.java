...
import it.eng.auriga.database.store.dmpk_int_mgo_email.store.impl.UnlockemailImpl;
 ...
 */
@Service(name="DmpkIntMgoEmailUnlockemail")
public class Unlockemail {
		
	private DmpkIntMgoEmailUnlockemailBean bean;
		  
	public void setBean(DmpkIntMgoEmailUnlockemailBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkIntMgoEmailUnlockemailBean> execute(AurigaLoginBean lLoginBean, DmpkIntMgoEmailUnlockemailBean pBean) throws Exception{
	   final UnlockemailImpl lUnlockemail = new UnlockemailImpl();
	   setBean(pBean);
	   lUnlockemail.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lUnlockemail.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkIntMgoEmailUnlockemailBean> result = new StoreResultBean<DmpkIntMgoEmailUnlockemailBean>();
