...
package it.eng.auriga.database.store.dmpk_order.store;

import it.eng.auriga.database.store.dmpk_order.bean.DmpkOrderSetattributesdettordiniBean;
import it.eng.auriga.database.store.dmpk_order.store.impl.SetattributesdettordiniImpl;
 ...
 */
@Service(name="DmpkOrderSetattributesdettordini")
public class Setattributesdettordini {
		
	private DmpkOrderSetattributesdettordiniBean bean;
		  
	public void setBean(DmpkOrderSetattributesdettordiniBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkOrderSetattributesdettordiniBean> execute(AurigaLoginBean lLoginBean, DmpkOrderSetattributesdettordiniBean pBean) throws Exception{
	   final SetattributesdettordiniImpl lSetattributesdettordini = new SetattributesdettordiniImpl();
	   setBean(pBean);
	   lSetattributesdettordini.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lSetattributesdettordini.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkOrderSetattributesdettordiniBean> result = new StoreResultBean<DmpkOrderSetattributesdettordiniBean>();
