...
import it.eng.auriga.database.store.dmpk_int_mgo_email.store.impl.GenerasegnaturaxmlImpl;
 ...
 */
@Service(name="DmpkIntMgoEmailGenerasegnaturaxml")
public class Generasegnaturaxml {
		
	private DmpkIntMgoEmailGenerasegnaturaxmlBean bean;
		  
	public void setBean(DmpkIntMgoEmailGenerasegnaturaxmlBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkIntMgoEmailGenerasegnaturaxmlBean> execute(AurigaLoginBean lLoginBean, DmpkIntMgoEmailGenerasegnaturaxmlBean pBean) throws Exception{
	   final GenerasegnaturaxmlImpl lGenerasegnaturaxml = new GenerasegnaturaxmlImpl();
	   setBean(pBean);
	   lGenerasegnaturaxml.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lGenerasegnaturaxml.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkIntMgoEmailGenerasegnaturaxmlBean> result = new StoreResultBean<DmpkIntMgoEmailGenerasegnaturaxmlBean>();
