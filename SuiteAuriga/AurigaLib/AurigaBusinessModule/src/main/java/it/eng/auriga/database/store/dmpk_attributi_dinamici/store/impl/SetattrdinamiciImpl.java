...
public class SetattrdinamiciImpl  {
	...
	public void execute(Connection connection) throws SQLException {
		...
		try{
			...
			call = connection.prepareCall("{? = call DMPK_ATTRIBUTI_DINAMICI.SETATTRDINAMICI(?,?,?,?,?,?,?,?,?,?,?,?,?,?)}");			
			...
			util.settingParameterOnStore(call,bean,wrapperBean,"attrvaluesxmlin",6,Types.CLOB,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"flgignoreattrnonprevistiin",7,Types.INTEGER,connection); 	
			...
			util.settingParameterOnStore(call,bean,wrapperBean,"flgignoraobbligin",13,Types.INTEGER,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"activitynamewfin",14,Types.VARCHAR,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"esitoactivitywfin",15,Types.VARCHAR,connection); 	
