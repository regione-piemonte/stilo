...
import it.eng.auriga.database.store.dmpk_corrispettivi.bean.DmpkCorrispettiviTrovafileinvBean;
import it.eng.auriga.database.store.dmpk_corrispettivi.store.impl.TrovafileinvImpl;
 ...
 */
@Service(name="DmpkCorrispettiviTrovafileinv")
public class Trovafileinv {
		
	private DmpkCorrispettiviTrovafileinvBean bean;
		  
	public void setBean(DmpkCorrispettiviTrovafileinvBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkCorrispettiviTrovafileinvBean> execute(AurigaLoginBean lLoginBean, DmpkCorrispettiviTrovafileinvBean pBean) throws Exception{
	   final TrovafileinvImpl lTrovafileinv = new TrovafileinvImpl();
	   setBean(pBean);
	   lTrovafileinv.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lTrovafileinv.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkCorrispettiviTrovafileinvBean> result = new StoreResultBean<DmpkCorrispettiviTrovafileinvBean>();
