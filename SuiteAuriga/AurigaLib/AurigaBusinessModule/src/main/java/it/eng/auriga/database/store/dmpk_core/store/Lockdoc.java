...
import it.eng.auriga.database.store.dmpk_core.store.impl.LockdocImpl;
 ...
 */
@Service(name="DmpkCoreLockdoc")
public class Lockdoc {
		
	private DmpkCoreLockdocBean bean;
		  
	public void setBean(DmpkCoreLockdocBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkCoreLockdocBean> execute(AurigaLoginBean lLoginBean, DmpkCoreLockdocBean pBean) throws Exception{
	   final LockdocImpl lLockdoc = new LockdocImpl();
	   setBean(pBean);
	   lLockdoc.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lLockdoc.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkCoreLockdocBean> result = new StoreResultBean<DmpkCoreLockdocBean>();
