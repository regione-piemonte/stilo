...
import it.eng.auriga.database.store.dmpk_ws.store.impl.AddupdattrcustomImpl;
 ...
 */
@Service(name="DmpkWsAddupdattrcustom")
public class Addupdattrcustom {
		
	private DmpkWsAddupdattrcustomBean bean;
		  
	public void setBean(DmpkWsAddupdattrcustomBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkWsAddupdattrcustomBean> execute(AurigaLoginBean lLoginBean, DmpkWsAddupdattrcustomBean pBean) throws Exception{
	   final AddupdattrcustomImpl lAddupdattrcustom = new AddupdattrcustomImpl();
	   setBean(pBean);
	   lAddupdattrcustom.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lAddupdattrcustom.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkWsAddupdattrcustomBean> result = new StoreResultBean<DmpkWsAddupdattrcustomBean>();
