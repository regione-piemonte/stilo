...
import it.eng.auriga.database.store.dmpk_process_types.store.impl.DprocesstypeImpl;
 ...
 */
@Service(name="DmpkProcessTypesDprocesstype")
public class Dprocesstype {
		
	private DmpkProcessTypesDprocesstypeBean bean;
		  
	public void setBean(DmpkProcessTypesDprocesstypeBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkProcessTypesDprocesstypeBean> execute(AurigaLoginBean lLoginBean, DmpkProcessTypesDprocesstypeBean pBean) throws Exception{
	   final DprocesstypeImpl lDprocesstype = new DprocesstypeImpl();
	   setBean(pBean);
	   lDprocesstype.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lDprocesstype.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkProcessTypesDprocesstypeBean> result = new StoreResultBean<DmpkProcessTypesDprocesstypeBean>();
