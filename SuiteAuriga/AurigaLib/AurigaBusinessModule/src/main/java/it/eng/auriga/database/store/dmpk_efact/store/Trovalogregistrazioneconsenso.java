...
import it.eng.auriga.database.store.dmpk_efact.bean.DmpkEfactTrovalogregistrazioneconsensoBean;
import it.eng.auriga.database.store.dmpk_efact.store.impl.TrovalogregistrazioneconsensoImpl;
 ...
 */
@Service(name="DmpkEfactTrovalogregistrazioneconsenso")
public class Trovalogregistrazioneconsenso {
		
	private DmpkEfactTrovalogregistrazioneconsensoBean bean;
		  
	public void setBean(DmpkEfactTrovalogregistrazioneconsensoBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkEfactTrovalogregistrazioneconsensoBean> execute(AurigaLoginBean lLoginBean, DmpkEfactTrovalogregistrazioneconsensoBean pBean) throws Exception{
	   final TrovalogregistrazioneconsensoImpl lTrovalogregistrazioneconsenso = new TrovalogregistrazioneconsensoImpl();
	   setBean(pBean);
	   lTrovalogregistrazioneconsenso.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lTrovalogregistrazioneconsenso.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkEfactTrovalogregistrazioneconsensoBean> result = new StoreResultBean<DmpkEfactTrovalogregistrazioneconsensoBean>();
