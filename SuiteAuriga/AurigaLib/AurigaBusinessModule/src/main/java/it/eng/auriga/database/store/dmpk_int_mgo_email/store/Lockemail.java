...
import it.eng.auriga.database.store.dmpk_int_mgo_email.store.impl.LockemailImpl;
 ...
 */
@Service(name="DmpkIntMgoEmailLockemail")
public class Lockemail {
	...
	public StoreResultBean<DmpkIntMgoEmailLockemailBean> execute(AurigaLoginBean lLoginBean, DmpkIntMgoEmailLockemailBean pBean) throws Exception{
	   final LockemailImpl lLockemail = new LockemailImpl();
	   setBean(pBean);
	   lLockemail.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lLockemail.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkIntMgoEmailLockemailBean> result = new StoreResultBean<DmpkIntMgoEmailLockemailBean>();
