...
import it.eng.auriga.database.store.dmpk_ws.store.impl.GetlistanotificheudImpl;
 ...
 */
@Service(name="DmpkWsGetlistanotificheud")
public class Getlistanotificheud {
		
	private DmpkWsGetlistanotificheudBean bean;
		  
	public void setBean(DmpkWsGetlistanotificheudBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkWsGetlistanotificheudBean> execute(AurigaLoginBean lLoginBean, DmpkWsGetlistanotificheudBean pBean) throws Exception{
	   final GetlistanotificheudImpl lGetlistanotificheud = new GetlistanotificheudImpl();
	   setBean(pBean);
	   lGetlistanotificheud.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lGetlistanotificheud.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkWsGetlistanotificheudBean> result = new StoreResultBean<DmpkWsGetlistanotificheudBean>();
