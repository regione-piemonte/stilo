...
import it.eng.auriga.database.store.dmpk_collaboration.bean.DmpkCollaborationSetstatofolderBean;
import it.eng.auriga.database.store.dmpk_collaboration.store.impl.SetstatofolderImpl;
 ...
 */
@Service(name="DmpkCollaborationSetstatofolder")
public class Setstatofolder {
		
	private DmpkCollaborationSetstatofolderBean bean;
		  
	public void setBean(DmpkCollaborationSetstatofolderBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkCollaborationSetstatofolderBean> execute(AurigaLoginBean lLoginBean, DmpkCollaborationSetstatofolderBean pBean) throws Exception{
	   final SetstatofolderImpl lSetstatofolder = new SetstatofolderImpl();
	   setBean(pBean);
	   lSetstatofolder.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lSetstatofolder.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkCollaborationSetstatofolderBean> result = new StoreResultBean<DmpkCollaborationSetstatofolderBean>();
