...
import it.eng.auriga.database.store.dmpk_credential_vault.bean.DmpkCredentialVaultApritisesamoBean;
import it.eng.auriga.database.store.dmpk_credential_vault.store.impl.ApritisesamoImpl;
 ...
 */
@Service(name="DmpkCredentialVaultApritisesamo")
public class Apritisesamo {
		
	private DmpkCredentialVaultApritisesamoBean bean;
		  
	public void setBean(DmpkCredentialVaultApritisesamoBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkCredentialVaultApritisesamoBean> execute(AurigaLoginBean lLoginBean, DmpkCredentialVaultApritisesamoBean pBean) throws Exception{
	   final ApritisesamoImpl lApritisesamo = new ApritisesamoImpl();
	   setBean(pBean);
	   lApritisesamo.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lApritisesamo.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkCredentialVaultApritisesamoBean> result = new StoreResultBean<DmpkCredentialVaultApritisesamoBean>();
