...
import it.eng.auriga.database.store.dmpk_core_2.bean.DmpkCore2AddverdocBean;
import it.eng.auriga.database.store.dmpk_core_2.store.impl.AddverdocImpl;
 ...
 */
@Service(name="DmpkCore2Addverdoc")
public class Addverdoc {
		
	private DmpkCore2AddverdocBean bean;
		  
	public void setBean(DmpkCore2AddverdocBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkCore2AddverdocBean> execute(AurigaLoginBean lLoginBean, DmpkCore2AddverdocBean pBean) throws Exception{
	   final AddverdocImpl lAddverdoc = new AddverdocImpl();
	   setBean(pBean);
	   lAddverdoc.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lAddverdoc.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkCore2AddverdocBean> result = new StoreResultBean<DmpkCore2AddverdocBean>();
