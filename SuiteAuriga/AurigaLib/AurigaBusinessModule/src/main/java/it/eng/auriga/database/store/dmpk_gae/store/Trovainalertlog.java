...
import it.eng.auriga.database.store.dmpk_gae.bean.DmpkGaeTrovainalertlogBean;
import it.eng.auriga.database.store.dmpk_gae.store.impl.TrovainalertlogImpl;
 ...
 */
@Service(name="DmpkGaeTrovainalertlog")
public class Trovainalertlog {
		
	private DmpkGaeTrovainalertlogBean bean;
		  
	public void setBean(DmpkGaeTrovainalertlogBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkGaeTrovainalertlogBean> execute(AurigaLoginBean lLoginBean, DmpkGaeTrovainalertlogBean pBean) throws Exception{
	   final TrovainalertlogImpl lTrovainalertlog = new TrovainalertlogImpl();
	   setBean(pBean);
	   lTrovainalertlog.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lTrovainalertlog.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkGaeTrovainalertlogBean> result = new StoreResultBean<DmpkGaeTrovainalertlogBean>();
