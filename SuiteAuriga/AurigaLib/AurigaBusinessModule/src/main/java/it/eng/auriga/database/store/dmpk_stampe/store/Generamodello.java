...
import it.eng.auriga.database.store.dmpk_stampe.bean.DmpkStampeGeneramodelloBean;
import it.eng.auriga.database.store.dmpk_stampe.store.impl.GeneramodelloImpl;
 ...
 */
@Service(name="DmpkStampeGeneramodello")
public class Generamodello {
		
	private DmpkStampeGeneramodelloBean bean;
		  
	public void setBean(DmpkStampeGeneramodelloBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkStampeGeneramodelloBean> execute(AurigaLoginBean lLoginBean, DmpkStampeGeneramodelloBean pBean) throws Exception{
	   final GeneramodelloImpl lGeneramodello = new GeneramodelloImpl();
	   setBean(pBean);
	   lGeneramodello.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lGeneramodello.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkStampeGeneramodelloBean> result = new StoreResultBean<DmpkStampeGeneramodelloBean>();
