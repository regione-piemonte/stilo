...
import it.eng.auriga.database.store.dmpk_core.store.impl.SetacludImpl;
 ...
 */
@Service(name="DmpkCoreSetaclud")
public class Setaclud {
	...
	public StoreResultBean<DmpkCoreSetacludBean> execute(AurigaLoginBean lLoginBean, DmpkCoreSetacludBean pBean) throws Exception{
	   final SetacludImpl lSetaclud = new SetacludImpl();
	   setBean(pBean);
	   lSetaclud.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lSetaclud.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkCoreSetacludBean> result = new StoreResultBean<DmpkCoreSetacludBean>();
