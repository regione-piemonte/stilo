...
import it.eng.auriga.database.store.dmpk_modelli_doc.store.impl.DmodelloImpl;
 ...
 */
@Service(name="DmpkModelliDocDmodello")
public class Dmodello {
		
	private DmpkModelliDocDmodelloBean bean;
		  
	public void setBean(DmpkModelliDocDmodelloBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkModelliDocDmodelloBean> execute(AurigaLoginBean lLoginBean, DmpkModelliDocDmodelloBean pBean) throws Exception{
	   final DmodelloImpl lDmodello = new DmodelloImpl();
	   setBean(pBean);
	   lDmodello.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lDmodello.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkModelliDocDmodelloBean> result = new StoreResultBean<DmpkModelliDocDmodelloBean>();
