...
import it.eng.auriga.database.store.dmpk_login.store.impl.EndsessioneindelegaImpl;
 ...
 */
@Service(name="DmpkLoginEndsessioneindelega")
public class Endsessioneindelega {
		
	private DmpkLoginEndsessioneindelegaBean bean;
		  
	public void setBean(DmpkLoginEndsessioneindelegaBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkLoginEndsessioneindelegaBean> execute(AurigaLoginBean lLoginBean, DmpkLoginEndsessioneindelegaBean pBean) throws Exception{
	   final EndsessioneindelegaImpl lEndsessioneindelega = new EndsessioneindelegaImpl();
	   setBean(pBean);
	   lEndsessioneindelega.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lEndsessioneindelega.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkLoginEndsessioneindelegaBean> result = new StoreResultBean<DmpkLoginEndsessioneindelegaBean>();
