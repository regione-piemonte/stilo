...
import it.eng.auriga.database.store.dmpk_efact.bean.DmpkEfactHasfileelettroniciuploadedudBean;
import it.eng.auriga.database.store.dmpk_efact.store.impl.HasfileelettroniciuploadedudImpl;
 ...
 */
@Service(name="DmpkEfactHasfileelettroniciuploadedud")
public class Hasfileelettroniciuploadedud {
		
	private DmpkEfactHasfileelettroniciuploadedudBean bean;
		  
	public void setBean(DmpkEfactHasfileelettroniciuploadedudBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkEfactHasfileelettroniciuploadedudBean> execute(AurigaLoginBean lLoginBean, DmpkEfactHasfileelettroniciuploadedudBean pBean) throws Exception{
	   final HasfileelettroniciuploadedudImpl lHasfileelettroniciuploadedud = new HasfileelettroniciuploadedudImpl();
	   setBean(pBean);
	   lHasfileelettroniciuploadedud.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lHasfileelettroniciuploadedud.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkEfactHasfileelettroniciuploadedudBean> result = new StoreResultBean<DmpkEfactHasfileelettroniciuploadedudBean>();
