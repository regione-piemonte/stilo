...
import it.eng.auriga.database.store.dmpk_utility.store.impl.IusezionaleImpl;
 ...
 */
@Service(name="DmpkUtilityIusezionale")
public class Iusezionale {
	...
	public StoreResultBean<DmpkUtilityIusezionaleBean> execute(AurigaLoginBean lLoginBean, DmpkUtilityIusezionaleBean pBean) throws Exception{
	   final IusezionaleImpl lIusezionale = new IusezionaleImpl();
	   setBean(pBean);
	   lIusezionale.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lIusezionale.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkUtilityIusezionaleBean> result = new StoreResultBean<DmpkUtilityIusezionaleBean>();
