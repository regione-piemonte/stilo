...
import it.eng.auriga.database.store.dmpk_formati_el_ammessi.bean.DmpkFormatiElAmmessiDformatoelBean;
import it.eng.auriga.database.store.dmpk_formati_el_ammessi.store.impl.DformatoelImpl;
 ...
 */
@Service(name="DmpkFormatiElAmmessiDformatoel")
public class Dformatoel {
		
	private DmpkFormatiElAmmessiDformatoelBean bean;
		  
	public void setBean(DmpkFormatiElAmmessiDformatoelBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkFormatiElAmmessiDformatoelBean> execute(AurigaLoginBean lLoginBean, DmpkFormatiElAmmessiDformatoelBean pBean) throws Exception{
	   final DformatoelImpl lDformatoel = new DformatoelImpl();
	   setBean(pBean);
	   lDformatoel.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lDformatoel.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkFormatiElAmmessiDformatoelBean> result = new StoreResultBean<DmpkFormatiElAmmessiDformatoelBean>();
