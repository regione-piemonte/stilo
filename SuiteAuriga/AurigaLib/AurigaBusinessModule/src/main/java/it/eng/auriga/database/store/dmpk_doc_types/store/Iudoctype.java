...
import it.eng.auriga.database.store.dmpk_doc_types.store.impl.IudoctypeImpl;
 ...
 */
@Service(name="DmpkDocTypesIudoctype")
public class Iudoctype {
	...
	public StoreResultBean<DmpkDocTypesIudoctypeBean> execute(AurigaLoginBean lLoginBean, DmpkDocTypesIudoctypeBean pBean) throws Exception{
	   final IudoctypeImpl lIudoctype = new IudoctypeImpl();
	   setBean(pBean);
	   lIudoctype.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lIudoctype.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkDocTypesIudoctypeBean> result = new StoreResultBean<DmpkDocTypesIudoctypeBean>();
