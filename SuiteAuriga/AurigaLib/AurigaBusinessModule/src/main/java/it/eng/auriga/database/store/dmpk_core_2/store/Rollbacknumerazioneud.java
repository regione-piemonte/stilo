...
import it.eng.auriga.database.store.dmpk_core_2.store.impl.RollbacknumerazioneudImpl;
 ...
 */
@Service(name="DmpkCore2Rollbacknumerazioneud")
public class Rollbacknumerazioneud {
		
	private DmpkCore2RollbacknumerazioneudBean bean;
		  
	public void setBean(DmpkCore2RollbacknumerazioneudBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkCore2RollbacknumerazioneudBean> execute(AurigaLoginBean lLoginBean, DmpkCore2RollbacknumerazioneudBean pBean) throws Exception{
	   final RollbacknumerazioneudImpl lRollbacknumerazioneud = new RollbacknumerazioneudImpl();
	   setBean(pBean);
	   lRollbacknumerazioneud.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lRollbacknumerazioneud.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkCore2RollbacknumerazioneudBean> result = new StoreResultBean<DmpkCore2RollbacknumerazioneudBean>();
