...
package it.eng.auriga.database.store.dmpk_bt_clienti.store;

import it.eng.auriga.database.store.dmpk_bt_clienti.bean.DmpkBtClientiUploadmassivoclientiBean;
import it.eng.auriga.database.store.dmpk_bt_clienti.store.impl.UploadmassivoclientiImpl;
 ...
 */
@Service(name="DmpkBtClientiUploadmassivoclienti")
public class Uploadmassivoclienti {
		
	private DmpkBtClientiUploadmassivoclientiBean bean;
		  
	public void setBean(DmpkBtClientiUploadmassivoclientiBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkBtClientiUploadmassivoclientiBean> execute(AurigaLoginBean lLoginBean, DmpkBtClientiUploadmassivoclientiBean pBean) throws Exception{
	   final UploadmassivoclientiImpl lUploadmassivoclienti = new UploadmassivoclientiImpl();
	   setBean(pBean);
	   lUploadmassivoclienti.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lUploadmassivoclienti.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkBtClientiUploadmassivoclientiBean> result = new StoreResultBean<DmpkBtClientiUploadmassivoclientiBean>();
