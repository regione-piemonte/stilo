...
import it.eng.auriga.database.store.dmpk_efact.bean.DmpkEfactTrovafatturaeventiBean;
import it.eng.auriga.database.store.dmpk_efact.store.impl.TrovafatturaeventiImpl;
 ...
 */
@Service(name="DmpkEfactTrovafatturaeventi")
public class Trovafatturaeventi {
		
	private DmpkEfactTrovafatturaeventiBean bean;
		  
	public void setBean(DmpkEfactTrovafatturaeventiBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkEfactTrovafatturaeventiBean> execute(AurigaLoginBean lLoginBean, DmpkEfactTrovafatturaeventiBean pBean) throws Exception{
	   final TrovafatturaeventiImpl lTrovafatturaeventi = new TrovafatturaeventiImpl();
	   setBean(pBean);
	   lTrovafatturaeventi.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lTrovafatturaeventi.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkEfactTrovafatturaeventiBean> result = new StoreResultBean<DmpkEfactTrovafatturaeventiBean>();
