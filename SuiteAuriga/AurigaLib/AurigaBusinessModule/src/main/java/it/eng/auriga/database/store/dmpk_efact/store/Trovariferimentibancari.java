...
import it.eng.auriga.database.store.dmpk_efact.bean.DmpkEfactTrovariferimentibancariBean;
import it.eng.auriga.database.store.dmpk_efact.store.impl.TrovariferimentibancariImpl;
 ...
 */
@Service(name="DmpkEfactTrovariferimentibancari")
public class Trovariferimentibancari {
		
	private DmpkEfactTrovariferimentibancariBean bean;
		  
	public void setBean(DmpkEfactTrovariferimentibancariBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkEfactTrovariferimentibancariBean> execute(AurigaLoginBean lLoginBean, DmpkEfactTrovariferimentibancariBean pBean) throws Exception{
	   final TrovariferimentibancariImpl lTrovariferimentibancari = new TrovariferimentibancariImpl();
	   setBean(pBean);
	   lTrovariferimentibancari.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lTrovariferimentibancari.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkEfactTrovariferimentibancariBean> result = new StoreResultBean<DmpkEfactTrovariferimentibancariBean>();
