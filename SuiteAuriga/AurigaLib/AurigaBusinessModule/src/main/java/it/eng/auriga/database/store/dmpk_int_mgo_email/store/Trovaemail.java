...
import it.eng.auriga.database.store.dmpk_int_mgo_email.store.impl.TrovaemailImpl;
 ...
 */
@Service(name="DmpkIntMgoEmailTrovaemail")
public class Trovaemail {
		
	private DmpkIntMgoEmailTrovaemailBean bean;
		  
	public void setBean(DmpkIntMgoEmailTrovaemailBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkIntMgoEmailTrovaemailBean> execute(AurigaLoginBean lLoginBean, DmpkIntMgoEmailTrovaemailBean pBean) throws Exception{
	   final TrovaemailImpl lTrovaemail = new TrovaemailImpl();
	   setBean(pBean);
	   lTrovaemail.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lTrovaemail.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkIntMgoEmailTrovaemailBean> result = new StoreResultBean<DmpkIntMgoEmailTrovaemailBean>();
