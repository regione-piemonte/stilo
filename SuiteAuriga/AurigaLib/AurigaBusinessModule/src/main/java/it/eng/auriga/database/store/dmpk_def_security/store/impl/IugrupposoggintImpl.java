...
import it.eng.auriga.database.store.dmpk_def_security.bean.DmpkDefSecurityIugrupposoggintBean;
...
public class IugrupposoggintImpl  {
		
	private DmpkDefSecurityIugrupposoggintBean bean;
		  
	public void setBean(DmpkDefSecurityIugrupposoggintBean bean){
		this.bean = bean;
	}
	...
	public void execute(Connection connection) throws SQLException {
		...
		try{
			...
			call = connection.prepareCall("{? = call DMPK_DEF_SECURITY.IUGRUPPOSOGGINT(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}");			
			...
			util.settingParameterOnStore(call,bean,wrapperBean,"flgdidefaultinaclin",6,Types.INTEGER,connection); 	
			...
			util.settingParameterOnStore(call,bean,wrapperBean,"ciprovgruppoin",9,Types.VARCHAR,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"flglockedin",10,Types.INTEGER,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"flgmodmembriin",12,Types.VARCHAR,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"xmlmembriin",13,Types.CLOB,connection); 	
