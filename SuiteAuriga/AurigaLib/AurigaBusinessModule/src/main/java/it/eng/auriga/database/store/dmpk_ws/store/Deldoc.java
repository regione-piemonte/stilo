...
import it.eng.auriga.database.store.dmpk_ws.store.impl.DeldocImpl;
 ...
 */
@Service(name="DmpkWsDeldoc")
public class Deldoc {
		
	private DmpkWsDeldocBean bean;
		  
	public void setBean(DmpkWsDeldocBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkWsDeldocBean> execute(AurigaLoginBean lLoginBean, DmpkWsDeldocBean pBean) throws Exception{
	   final DeldocImpl lDeldoc = new DeldocImpl();
	   setBean(pBean);
	   lDeldoc.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lDeldoc.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkWsDeldocBean> result = new StoreResultBean<DmpkWsDeldocBean>();
