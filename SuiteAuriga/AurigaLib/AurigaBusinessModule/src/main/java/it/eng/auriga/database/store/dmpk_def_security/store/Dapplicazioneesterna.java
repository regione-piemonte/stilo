...
import it.eng.auriga.database.store.dmpk_def_security.store.impl.DapplicazioneesternaImpl;
 ...
 */
@Service(name="DmpkDefSecurityDapplicazioneesterna")
public class Dapplicazioneesterna {
		
	private DmpkDefSecurityDapplicazioneesternaBean bean;
		  
	public void setBean(DmpkDefSecurityDapplicazioneesternaBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkDefSecurityDapplicazioneesternaBean> execute(AurigaLoginBean lLoginBean, DmpkDefSecurityDapplicazioneesternaBean pBean) throws Exception{
	   final DapplicazioneesternaImpl lDapplicazioneesterna = new DapplicazioneesternaImpl();
	   setBean(pBean);
	   lDapplicazioneesterna.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lDapplicazioneesterna.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkDefSecurityDapplicazioneesternaBean> result = new StoreResultBean<DmpkDefSecurityDapplicazioneesternaBean>();
