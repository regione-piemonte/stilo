...
import it.eng.auriga.database.store.dmpk_repository_gui.bean.DmpkRepositoryGuiLoaddettdocBean;
import it.eng.auriga.database.store.dmpk_repository_gui.store.impl.LoaddettdocImpl;
 ...
 */
@Service(name="DmpkRepositoryGuiLoaddettdoc")
public class Loaddettdoc {
		
	private DmpkRepositoryGuiLoaddettdocBean bean;
		  
	public void setBean(DmpkRepositoryGuiLoaddettdocBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkRepositoryGuiLoaddettdocBean> execute(AurigaLoginBean lLoginBean, DmpkRepositoryGuiLoaddettdocBean pBean) throws Exception{
	   final LoaddettdocImpl lLoaddettdoc = new LoaddettdocImpl();
	   setBean(pBean);
	   lLoaddettdoc.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lLoaddettdoc.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkRepositoryGuiLoaddettdocBean> result = new StoreResultBean<DmpkRepositoryGuiLoaddettdocBean>();
