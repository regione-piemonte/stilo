...
import it.eng.auriga.database.store.dmpk_efact.bean.DmpkEfactTrovaclienticc_bdbBean;
import it.eng.auriga.database.store.dmpk_efact.store.impl.Trovaclienticc_bdbImpl;
 ...
 */
@Service(name="DmpkEfactTrovaclienticc_bdb")
public class Trovaclienticc_bdb {
		
	private DmpkEfactTrovaclienticc_bdbBean bean;
		  
	public void setBean(DmpkEfactTrovaclienticc_bdbBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkEfactTrovaclienticc_bdbBean> execute(AurigaLoginBean lLoginBean, DmpkEfactTrovaclienticc_bdbBean pBean) throws Exception{
	   final Trovaclienticc_bdbImpl lTrovaclienticc_bdb = new Trovaclienticc_bdbImpl();
	   setBean(pBean);
	   lTrovaclienticc_bdb.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lTrovaclienticc_bdb.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkEfactTrovaclienticc_bdbBean> result = new StoreResultBean<DmpkEfactTrovaclienticc_bdbBean>();
