...
import it.eng.auriga.database.store.dmpk_registrazionedoc.bean.DmpkRegistrazionedocGeneraprogressivoBean;
import it.eng.auriga.database.store.dmpk_registrazionedoc.store.impl.GeneraprogressivoImpl;
 ...
 */
@Service(name="DmpkRegistrazionedocGeneraprogressivo")
public class Generaprogressivo {
		
	private DmpkRegistrazionedocGeneraprogressivoBean bean;
		  
	public void setBean(DmpkRegistrazionedocGeneraprogressivoBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkRegistrazionedocGeneraprogressivoBean> execute(AurigaLoginBean lLoginBean, DmpkRegistrazionedocGeneraprogressivoBean pBean) throws Exception{
	   final GeneraprogressivoImpl lGeneraprogressivo = new GeneraprogressivoImpl();
	   setBean(pBean);
	   lGeneraprogressivo.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lGeneraprogressivo.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkRegistrazionedocGeneraprogressivoBean> result = new StoreResultBean<DmpkRegistrazionedocGeneraprogressivoBean>();
