...
import it.eng.auriga.database.store.dmpk_efact.bean.DmpkEfactTrovareportkpiBean;
import it.eng.auriga.database.store.dmpk_efact.store.impl.TrovareportkpiImpl;
 ...
 */
@Service(name="DmpkEfactTrovareportkpi")
public class Trovareportkpi {
		
	private DmpkEfactTrovareportkpiBean bean;
		  
	public void setBean(DmpkEfactTrovareportkpiBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkEfactTrovareportkpiBean> execute(AurigaLoginBean lLoginBean, DmpkEfactTrovareportkpiBean pBean) throws Exception{
	   final TrovareportkpiImpl lTrovareportkpi = new TrovareportkpiImpl();
	   setBean(pBean);
	   lTrovareportkpi.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lTrovareportkpi.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkEfactTrovareportkpiBean> result = new StoreResultBean<DmpkEfactTrovareportkpiBean>();
