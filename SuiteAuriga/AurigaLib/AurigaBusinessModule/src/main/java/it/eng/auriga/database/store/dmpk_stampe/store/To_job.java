...
import it.eng.auriga.database.store.dmpk_stampe.store.impl.To_jobImpl;
...
public class To_job {
	...
	public StoreResultBean<DmpkStampeTo_jobBean> execute(AurigaLoginBean lLoginBean, DmpkStampeTo_jobBean pBean) throws Exception{
	   final To_jobImpl lTo_job = new To_jobImpl();
	   setBean(pBean);
	   lTo_job.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lTo_job.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkStampeTo_jobBean> result = new StoreResultBean<DmpkStampeTo_jobBean>();
