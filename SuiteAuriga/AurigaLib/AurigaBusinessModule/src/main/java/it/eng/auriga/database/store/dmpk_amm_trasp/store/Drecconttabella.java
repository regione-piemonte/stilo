...
import it.eng.auriga.database.store.dmpk_amm_trasp.store.impl.DrecconttabellaImpl;
 ...
 */
@Service(name="DmpkAmmTraspDrecconttabella")
public class Drecconttabella {
		
	private DmpkAmmTraspDrecconttabellaBean bean;
		  
	public void setBean(DmpkAmmTraspDrecconttabellaBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkAmmTraspDrecconttabellaBean> execute(AurigaLoginBean lLoginBean, DmpkAmmTraspDrecconttabellaBean pBean) throws Exception{
	   final DrecconttabellaImpl lDrecconttabella = new DrecconttabellaImpl();
	   setBean(pBean);
	   lDrecconttabella.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lDrecconttabella.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkAmmTraspDrecconttabellaBean> result = new StoreResultBean<DmpkAmmTraspDrecconttabellaBean>();
