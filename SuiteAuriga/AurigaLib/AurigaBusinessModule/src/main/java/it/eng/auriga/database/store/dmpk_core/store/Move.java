...
import it.eng.auriga.database.store.dmpk_core.store.impl.MoveImpl;
...
public class Move {
	...
	public StoreResultBean<DmpkCoreMoveBean> execute(AurigaLoginBean lLoginBean, DmpkCoreMoveBean pBean) throws Exception{
	   final MoveImpl lMove = new MoveImpl();
	   setBean(pBean);
	   lMove.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lMove.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkCoreMoveBean> result = new StoreResultBean<DmpkCoreMoveBean>();
