...
import it.eng.auriga.database.store.dmpk_gestore.bean.DmpkGestoreLoad_messaggioattBean;
import it.eng.auriga.database.store.dmpk_gestore.store.impl.Load_messaggioattImpl;
 ...
 */
@Service(name="DmpkGestoreLoad_messaggioatt")
public class Load_messaggioatt {
		
	private DmpkGestoreLoad_messaggioattBean bean;
		  
	public void setBean(DmpkGestoreLoad_messaggioattBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkGestoreLoad_messaggioattBean> execute(AurigaLoginBean lLoginBean, DmpkGestoreLoad_messaggioattBean pBean) throws Exception{
	   final Load_messaggioattImpl lLoad_messaggioatt = new Load_messaggioattImpl();
	   setBean(pBean);
	   lLoad_messaggioatt.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lLoad_messaggioatt.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkGestoreLoad_messaggioattBean> result = new StoreResultBean<DmpkGestoreLoad_messaggioattBean>();
