...
import it.eng.auriga.database.store.dmpk_ws.store.impl.AddupdapplicazioneesternaImpl;
 ...
 */
@Service(name="DmpkWsAddupdapplicazioneesterna")
public class Addupdapplicazioneesterna {
		
	private DmpkWsAddupdapplicazioneesternaBean bean;
		  
	public void setBean(DmpkWsAddupdapplicazioneesternaBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkWsAddupdapplicazioneesternaBean> execute(AurigaLoginBean lLoginBean, DmpkWsAddupdapplicazioneesternaBean pBean) throws Exception{
	   final AddupdapplicazioneesternaImpl lAddupdapplicazioneesterna = new AddupdapplicazioneesternaImpl();
	   setBean(pBean);
	   lAddupdapplicazioneesterna.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lAddupdapplicazioneesterna.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkWsAddupdapplicazioneesternaBean> result = new StoreResultBean<DmpkWsAddupdapplicazioneesternaBean>();
