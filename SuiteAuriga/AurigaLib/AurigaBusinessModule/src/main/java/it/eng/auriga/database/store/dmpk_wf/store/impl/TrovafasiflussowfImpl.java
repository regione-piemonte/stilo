...
import it.eng.auriga.database.store.dmpk_wf.bean.DmpkWfTrovafasiflussowfBean;
...
public class TrovafasiflussowfImpl  {
		
	private DmpkWfTrovafasiflussowfBean bean;
		  
	public void setBean(DmpkWfTrovafasiflussowfBean bean){
		this.bean = bean;
	}
	...
	public void execute(Connection connection) throws SQLException {
		...
		try{
			...
			call = connection.prepareCall("{? = call DMPK_WF.TROVAFASIFLUSSOWF(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}");			
			...
			util.settingParameterOnStore(call,bean,wrapperBean,"strinnomefasein",5,Types.VARCHAR,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"cifasein",6,Types.VARCHAR,connection); 	
			...
			util.settingParameterOnStore(call,bean,wrapperBean,"cifasetoaddin",9,Types.VARCHAR,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"nomefasetoaddin",10,Types.VARCHAR,connection); 	
