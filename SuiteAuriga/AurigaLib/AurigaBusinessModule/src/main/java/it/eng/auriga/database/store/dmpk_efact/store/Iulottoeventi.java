...
import it.eng.auriga.database.store.dmpk_efact.bean.DmpkEfactIulottoeventiBean;
import it.eng.auriga.database.store.dmpk_efact.store.impl.IulottoeventiImpl;
 ...
 */
@Service(name="DmpkEfactIulottoeventi")
public class Iulottoeventi {
		
	private DmpkEfactIulottoeventiBean bean;
		  
	public void setBean(DmpkEfactIulottoeventiBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkEfactIulottoeventiBean> execute(AurigaLoginBean lLoginBean, DmpkEfactIulottoeventiBean pBean) throws Exception{
	   final IulottoeventiImpl lIulottoeventi = new IulottoeventiImpl();
	   setBean(pBean);
	   lIulottoeventi.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lIulottoeventi.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkEfactIulottoeventiBean> result = new StoreResultBean<DmpkEfactIulottoeventiBean>();
