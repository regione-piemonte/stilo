...
import it.eng.auriga.database.store.dmpk_efact.bean.DmpkEfactGetstatoemailudBean;
import it.eng.auriga.database.store.dmpk_efact.store.impl.GetstatoemailudImpl;
 ...
 */
@Service(name="DmpkEfactGetstatoemailud")
public class Getstatoemailud {
		
	private DmpkEfactGetstatoemailudBean bean;
		  
	public void setBean(DmpkEfactGetstatoemailudBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkEfactGetstatoemailudBean> execute(AurigaLoginBean lLoginBean, DmpkEfactGetstatoemailudBean pBean) throws Exception{
	   final GetstatoemailudImpl lGetstatoemailud = new GetstatoemailudImpl();
	   setBean(pBean);
	   lGetstatoemailud.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lGetstatoemailud.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkEfactGetstatoemailudBean> result = new StoreResultBean<DmpkEfactGetstatoemailudBean>();
