...
import it.eng.auriga.database.store.dmpk_int_mgo_email.store.impl.AnnullaassegnazioneemailImpl;
 ...
 */
@Service(name="DmpkIntMgoEmailAnnullaassegnazioneemail")
public class Annullaassegnazioneemail {
		
	private DmpkIntMgoEmailAnnullaassegnazioneemailBean bean;
		  
	public void setBean(DmpkIntMgoEmailAnnullaassegnazioneemailBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkIntMgoEmailAnnullaassegnazioneemailBean> execute(AurigaLoginBean lLoginBean, DmpkIntMgoEmailAnnullaassegnazioneemailBean pBean) throws Exception{
	   final AnnullaassegnazioneemailImpl lAnnullaassegnazioneemail = new AnnullaassegnazioneemailImpl();
	   setBean(pBean);
	   lAnnullaassegnazioneemail.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lAnnullaassegnazioneemail.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkIntMgoEmailAnnullaassegnazioneemailBean> result = new StoreResultBean<DmpkIntMgoEmailAnnullaassegnazioneemailBean>();
