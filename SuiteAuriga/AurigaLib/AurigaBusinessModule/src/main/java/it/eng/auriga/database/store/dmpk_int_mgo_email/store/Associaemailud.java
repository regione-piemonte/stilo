...
import it.eng.auriga.database.store.dmpk_int_mgo_email.store.impl.AssociaemailudImpl;
 ...
 */
@Service(name="DmpkIntMgoEmailAssociaemailud")
public class Associaemailud {
	...
	public StoreResultBean<DmpkIntMgoEmailAssociaemailudBean> execute(AurigaLoginBean lLoginBean, DmpkIntMgoEmailAssociaemailudBean pBean) throws Exception{
	   final AssociaemailudImpl lAssociaemailud = new AssociaemailudImpl();
	   setBean(pBean);
	   lAssociaemailud.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lAssociaemailud.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkIntMgoEmailAssociaemailudBean> result = new StoreResultBean<DmpkIntMgoEmailAssociaemailudBean>();
