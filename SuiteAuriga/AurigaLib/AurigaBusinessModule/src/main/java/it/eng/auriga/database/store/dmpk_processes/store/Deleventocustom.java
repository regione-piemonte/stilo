...
import it.eng.auriga.database.store.dmpk_processes.bean.DmpkProcessesDeleventocustomBean;
import it.eng.auriga.database.store.dmpk_processes.store.impl.DeleventocustomImpl;
 ...
 */
@Service(name="DmpkProcessesDeleventocustom")
public class Deleventocustom {
		
	private DmpkProcessesDeleventocustomBean bean;
		  
	public void setBean(DmpkProcessesDeleventocustomBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkProcessesDeleventocustomBean> execute(AurigaLoginBean lLoginBean, DmpkProcessesDeleventocustomBean pBean) throws Exception{
	   final DeleventocustomImpl lDeleventocustom = new DeleventocustomImpl();
	   setBean(pBean);
	   lDeleventocustom.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lDeleventocustom.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkProcessesDeleventocustomBean> result = new StoreResultBean<DmpkProcessesDeleventocustomBean>();
