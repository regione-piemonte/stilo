...
import it.eng.auriga.database.store.dmpk_efact.bean.DmpkEfactTrovaemaildestfpBean;
import it.eng.auriga.database.store.dmpk_efact.store.impl.TrovaemaildestfpImpl;
 ...
 */
@Service(name="DmpkEfactTrovaemaildestfp")
public class Trovaemaildestfp {
		
	private DmpkEfactTrovaemaildestfpBean bean;
		  
	public void setBean(DmpkEfactTrovaemaildestfpBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkEfactTrovaemaildestfpBean> execute(AurigaLoginBean lLoginBean, DmpkEfactTrovaemaildestfpBean pBean) throws Exception{
	   final TrovaemaildestfpImpl lTrovaemaildestfp = new TrovaemaildestfpImpl();
	   setBean(pBean);
	   lTrovaemaildestfp.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lTrovaemaildestfp.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkEfactTrovaemaildestfpBean> result = new StoreResultBean<DmpkEfactTrovaemaildestfpBean>();
