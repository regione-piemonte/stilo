...
import it.eng.auriga.database.store.dmpk_credential_vault.bean.DmpkCredentialVaultGetusernameextBean;
import it.eng.auriga.database.store.dmpk_credential_vault.store.impl.GetusernameextImpl;
 ...
 */
@Service(name="DmpkCredentialVaultGetusernameext")
public class Getusernameext {
		
	private DmpkCredentialVaultGetusernameextBean bean;
		  
	public void setBean(DmpkCredentialVaultGetusernameextBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkCredentialVaultGetusernameextBean> execute(AurigaLoginBean lLoginBean, DmpkCredentialVaultGetusernameextBean pBean) throws Exception{
	   final GetusernameextImpl lGetusernameext = new GetusernameextImpl();
	   setBean(pBean);
	   lGetusernameext.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lGetusernameext.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkCredentialVaultGetusernameextBean> result = new StoreResultBean<DmpkCredentialVaultGetusernameextBean>();
