...
import it.eng.auriga.database.store.dmpk_core.store.impl.LoadacludImpl;
 ...
 */
@Service(name="DmpkCoreLoadaclud")
public class Loadaclud {
	...
	public StoreResultBean<DmpkCoreLoadacludBean> execute(AurigaLoginBean lLoginBean, DmpkCoreLoadacludBean pBean) throws Exception{
	   final LoadacludImpl lLoadaclud = new LoadacludImpl();
	   setBean(pBean);
	   lLoadaclud.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lLoadaclud.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkCoreLoadacludBean> result = new StoreResultBean<DmpkCoreLoadacludBean>();
