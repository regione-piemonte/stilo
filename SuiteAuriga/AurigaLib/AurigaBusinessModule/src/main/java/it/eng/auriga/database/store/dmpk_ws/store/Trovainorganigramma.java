...
import it.eng.auriga.database.store.dmpk_ws.store.impl.TrovainorganigrammaImpl;
 ...
 */
@Service(name="DmpkWsTrovainorganigramma")
public class Trovainorganigramma {
		
	private DmpkWsTrovainorganigrammaBean bean;
		  
	public void setBean(DmpkWsTrovainorganigrammaBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkWsTrovainorganigrammaBean> execute(AurigaLoginBean lLoginBean, DmpkWsTrovainorganigrammaBean pBean) throws Exception{
	   final TrovainorganigrammaImpl lTrovainorganigramma = new TrovainorganigrammaImpl();
	   setBean(pBean);
	   lTrovainorganigramma.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lTrovainorganigramma.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkWsTrovainorganigrammaBean> result = new StoreResultBean<DmpkWsTrovainorganigrammaBean>();
