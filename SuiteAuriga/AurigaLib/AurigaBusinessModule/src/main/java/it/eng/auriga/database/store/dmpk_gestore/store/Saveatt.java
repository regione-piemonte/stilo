...
import it.eng.auriga.database.store.dmpk_gestore.store.impl.SaveattImpl;
 ...
 */
@Service(name="DmpkGestoreSaveatt")
public class Saveatt {
	...
	public StoreResultBean<DmpkGestoreSaveattBean> execute(AurigaLoginBean lLoginBean, DmpkGestoreSaveattBean pBean) throws Exception{
	   final SaveattImpl lSaveatt = new SaveattImpl();
	   setBean(pBean);
	   lSaveatt.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lSaveatt.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkGestoreSaveattBean> result = new StoreResultBean<DmpkGestoreSaveattBean>();
