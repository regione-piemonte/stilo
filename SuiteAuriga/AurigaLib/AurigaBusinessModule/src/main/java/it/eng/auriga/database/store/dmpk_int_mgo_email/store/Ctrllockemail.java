...
import it.eng.auriga.database.store.dmpk_int_mgo_email.store.impl.CtrllockemailImpl;
 ...
 */
@Service(name="DmpkIntMgoEmailCtrllockemail")
public class Ctrllockemail {
		
	private DmpkIntMgoEmailCtrllockemailBean bean;
		  
	public void setBean(DmpkIntMgoEmailCtrllockemailBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkIntMgoEmailCtrllockemailBean> execute(AurigaLoginBean lLoginBean, DmpkIntMgoEmailCtrllockemailBean pBean) throws Exception{
	   final CtrllockemailImpl lCtrllockemail = new CtrllockemailImpl();
	   setBean(pBean);
	   lCtrllockemail.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lCtrllockemail.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkIntMgoEmailCtrllockemailBean> result = new StoreResultBean<DmpkIntMgoEmailCtrllockemailBean>();
