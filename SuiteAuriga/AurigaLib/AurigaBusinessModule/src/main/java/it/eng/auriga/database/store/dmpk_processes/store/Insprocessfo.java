...
import it.eng.auriga.database.store.dmpk_processes.store.impl.InsprocessfoImpl;
 ...
 */
@Service(name="DmpkProcessesInsprocessfo")
public class Insprocessfo {
	...
	public StoreResultBean<DmpkProcessesInsprocessfoBean> execute(AurigaLoginBean lLoginBean, DmpkProcessesInsprocessfoBean pBean) throws Exception{
	   final InsprocessfoImpl lInsprocessfo = new InsprocessfoImpl();
	   setBean(pBean);
	   lInsprocessfo.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lInsprocessfo.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkProcessesInsprocessfoBean> result = new StoreResultBean<DmpkProcessesInsprocessfoBean>();
