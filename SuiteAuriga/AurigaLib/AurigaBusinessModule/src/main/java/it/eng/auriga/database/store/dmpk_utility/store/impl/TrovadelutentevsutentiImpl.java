...
import it.eng.auriga.database.store.dmpk_utility.bean.DmpkUtilityTrovadelutentevsutentiBean;
...
public class TrovadelutentevsutentiImpl  {
		
	private DmpkUtilityTrovadelutentevsutentiBean bean;
		  
	public void setBean(DmpkUtilityTrovadelutentevsutentiBean bean){
		this.bean = bean;
	}
	...
	public void execute(Connection connection) throws SQLException {
		...
		try{
			...
			call = connection.prepareCall("{? = call DMPK_UTILITY.TROVADELUTENTEVSUTENTI(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}");			
			...
			util.settingParameterOnStore(call,bean,wrapperBean,"iduserdelegatoin",3,Types.DECIMAL,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"tsrifin",4,Types.VARCHAR,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"strindesuserin",5,Types.VARCHAR,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"strinusernamein",6,Types.VARCHAR,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"strinnromatricolain",7,Types.VARCHAR,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"flgsolouservldin",8,Types.INTEGER,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"strinnaturadelin",9,Types.VARCHAR,connection); 	
