...
import it.eng.auriga.database.store.dmpk_repository_gui.bean.DmpkRepositoryGuiLoadfilefatturaBean;
import it.eng.auriga.database.store.dmpk_repository_gui.store.impl.LoadfilefatturaImpl;
 ...
 */
@Service(name="DmpkRepositoryGuiLoadfilefattura")
public class Loadfilefattura {
		
	private DmpkRepositoryGuiLoadfilefatturaBean bean;
		  
	public void setBean(DmpkRepositoryGuiLoadfilefatturaBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkRepositoryGuiLoadfilefatturaBean> execute(AurigaLoginBean lLoginBean, DmpkRepositoryGuiLoadfilefatturaBean pBean) throws Exception{
	   final LoadfilefatturaImpl lLoadfilefattura = new LoadfilefatturaImpl();
	   setBean(pBean);
	   lLoadfilefattura.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lLoadfilefattura.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkRepositoryGuiLoadfilefatturaBean> result = new StoreResultBean<DmpkRepositoryGuiLoadfilefatturaBean>();
