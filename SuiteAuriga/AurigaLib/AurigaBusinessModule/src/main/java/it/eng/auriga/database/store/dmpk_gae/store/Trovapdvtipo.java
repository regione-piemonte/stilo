...
import it.eng.auriga.database.store.dmpk_gae.bean.DmpkGaeTrovapdvtipoBean;
import it.eng.auriga.database.store.dmpk_gae.store.impl.TrovapdvtipoImpl;
 ...
 */
@Service(name="DmpkGaeTrovapdvtipo")
public class Trovapdvtipo {
		
	private DmpkGaeTrovapdvtipoBean bean;
		  
	public void setBean(DmpkGaeTrovapdvtipoBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkGaeTrovapdvtipoBean> execute(AurigaLoginBean lLoginBean, DmpkGaeTrovapdvtipoBean pBean) throws Exception{
	   final TrovapdvtipoImpl lTrovapdvtipo = new TrovapdvtipoImpl();
	   setBean(pBean);
	   lTrovapdvtipo.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lTrovapdvtipo.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkGaeTrovapdvtipoBean> result = new StoreResultBean<DmpkGaeTrovapdvtipoBean>();
