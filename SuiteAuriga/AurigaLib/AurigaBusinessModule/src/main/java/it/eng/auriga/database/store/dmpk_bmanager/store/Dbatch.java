...
import it.eng.auriga.database.store.dmpk_bmanager.bean.DmpkBmanagerDbatchBean;
import it.eng.auriga.database.store.dmpk_bmanager.store.impl.DbatchImpl;
 ...
 */
@Service(name="DmpkBmanagerDbatch")
public class Dbatch {
		
	private DmpkBmanagerDbatchBean bean;
		  
	public void setBean(DmpkBmanagerDbatchBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkBmanagerDbatchBean> execute(AurigaLoginBean lLoginBean, DmpkBmanagerDbatchBean pBean) throws Exception{
	   final DbatchImpl lDbatch = new DbatchImpl();
	   setBean(pBean);
	   lDbatch.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lDbatch.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkBmanagerDbatchBean> result = new StoreResultBean<DmpkBmanagerDbatchBean>();
