...
import it.eng.auriga.database.store.dmpk_process_types.store.impl.DattributoproctypeImpl;
 ...
 */
@Service(name="DmpkProcessTypesDattributoproctype")
public class Dattributoproctype {
	...
	public StoreResultBean<DmpkProcessTypesDattributoproctypeBean> execute(AurigaLoginBean lLoginBean, DmpkProcessTypesDattributoproctypeBean pBean) throws Exception{
	   final DattributoproctypeImpl lDattributoproctype = new DattributoproctypeImpl();
	   setBean(pBean);
	   lDattributoproctype.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lDattributoproctype.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkProcessTypesDattributoproctypeBean> result = new StoreResultBean<DmpkProcessTypesDattributoproctypeBean>();
