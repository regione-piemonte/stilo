...
public class IuassociazioneudvsprocImpl  {
		
	private DmpkProcessesIuassociazioneudvsprocBean bean;
		  
	public void setBean(DmpkProcessesIuassociazioneudvsprocBean bean){
		this.bean = bean;
	}
	...
	public void execute(Connection connection) throws SQLException {
		...
		try{
			...
			call = connection.prepareCall("{? = call DMPK_PROCESSES.IUASSOCIAZIONEUDVSPROC(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}");			
			...
			util.settingParameterOnStore(call,bean,wrapperBean,"nroordinevsprocin",6,Types.INTEGER,connection); 	
			...
			util.settingParameterOnStore(call,bean,wrapperBean,"codstatoudinprocin",9,Types.VARCHAR,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"noteassociazionein",10,Types.VARCHAR,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"flglockedin",11,Types.INTEGER,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"attributiaddin",12,Types.CLOB,connection); 	
