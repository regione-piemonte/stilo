...
import it.eng.auriga.database.store.dmpk_efact.bean.DmpkEfactTrovagruppidoc_bdbBean;
import it.eng.auriga.database.store.dmpk_efact.store.impl.Trovagruppidoc_bdbImpl;
 ...
 */
@Service(name="DmpkEfactTrovagruppidoc_bdb")
public class Trovagruppidoc_bdb {
		
	private DmpkEfactTrovagruppidoc_bdbBean bean;
		  
	public void setBean(DmpkEfactTrovagruppidoc_bdbBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkEfactTrovagruppidoc_bdbBean> execute(AurigaLoginBean lLoginBean, DmpkEfactTrovagruppidoc_bdbBean pBean) throws Exception{
	   final Trovagruppidoc_bdbImpl lTrovagruppidoc_bdb = new Trovagruppidoc_bdbImpl();
	   setBean(pBean);
	   lTrovagruppidoc_bdb.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lTrovagruppidoc_bdb.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkEfactTrovagruppidoc_bdbBean> result = new StoreResultBean<DmpkEfactTrovagruppidoc_bdbBean>();
