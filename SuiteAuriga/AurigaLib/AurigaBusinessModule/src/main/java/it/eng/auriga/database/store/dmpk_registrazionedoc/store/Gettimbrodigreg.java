...
import it.eng.auriga.database.store.dmpk_registrazionedoc.store.impl.GettimbrodigregImpl;
 ...
 */
@Service(name="DmpkRegistrazionedocGettimbrodigreg")
public class Gettimbrodigreg {
		
	private DmpkRegistrazionedocGettimbrodigregBean bean;
		  
	public void setBean(DmpkRegistrazionedocGettimbrodigregBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkRegistrazionedocGettimbrodigregBean> execute(AurigaLoginBean lLoginBean, DmpkRegistrazionedocGettimbrodigregBean pBean) throws Exception{
	   final GettimbrodigregImpl lGettimbrodigreg = new GettimbrodigregImpl();
	   setBean(pBean);
	   lGettimbrodigreg.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lGettimbrodigreg.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkRegistrazionedocGettimbrodigregBean> result = new StoreResultBean<DmpkRegistrazionedocGettimbrodigregBean>();
