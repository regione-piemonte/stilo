...
public class DelfolderImpl  {
		
	private DmpkCoreDelfolderBean bean;
		  
	public void setBean(DmpkCoreDelfolderBean bean){
		this.bean = bean;
	}
	...
	public void execute(Connection connection) throws SQLException {
		...
		try{
			...
			call = connection.prepareCall("{? = call DMPK_CORE.DELFOLDER(?,?,?,?,?,?,?,?,?,?,?,?,?)}");			
			...
			util.settingParameterOnStore(call,bean,wrapperBean,"flgcancfisicain",8,Types.INTEGER,connection); 	
			...
			util.settinParameterOnBean(call,bean,wrapperBean,"uriout",9,Types.VARCHAR); 
