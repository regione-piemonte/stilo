...
import it.eng.auriga.database.store.dmpk_core.store.impl.LockfolderImpl;
 ...
 */
@Service(name="DmpkCoreLockfolder")
public class Lockfolder {
		
	private DmpkCoreLockfolderBean bean;
		  
	public void setBean(DmpkCoreLockfolderBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkCoreLockfolderBean> execute(AurigaLoginBean lLoginBean, DmpkCoreLockfolderBean pBean) throws Exception{
	   final LockfolderImpl lLockfolder = new LockfolderImpl();
	   setBean(pBean);
	   lLockfolder.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lLockfolder.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkCoreLockfolderBean> result = new StoreResultBean<DmpkCoreLockfolderBean>();
