...
import it.eng.auriga.database.store.dmpk_int_mgo_email.store.impl.ArchiviaemailImpl;
 ...
 */
@Service(name="DmpkIntMgoEmailArchiviaemail")
public class Archiviaemail {
		
	private DmpkIntMgoEmailArchiviaemailBean bean;
		  
	public void setBean(DmpkIntMgoEmailArchiviaemailBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkIntMgoEmailArchiviaemailBean> execute(AurigaLoginBean lLoginBean, DmpkIntMgoEmailArchiviaemailBean pBean) throws Exception{
	   final ArchiviaemailImpl lArchiviaemail = new ArchiviaemailImpl();
	   setBean(pBean);
	   lArchiviaemail.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lArchiviaemail.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkIntMgoEmailArchiviaemailBean> result = new StoreResultBean<DmpkIntMgoEmailArchiviaemailBean>();
