...
import it.eng.auriga.database.store.dmpk_attributi_dinamici.bean.DmpkAttributiDinamiciComponicriterisuattrdinBean;
import it.eng.auriga.database.store.dmpk_attributi_dinamici.store.impl.ComponicriterisuattrdinImpl;
 ...
 */
@Service(name="DmpkAttributiDinamiciComponicriterisuattrdin")
public class Componicriterisuattrdin {
		
	private DmpkAttributiDinamiciComponicriterisuattrdinBean bean;
		  
	public void setBean(DmpkAttributiDinamiciComponicriterisuattrdinBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkAttributiDinamiciComponicriterisuattrdinBean> execute(AurigaLoginBean lLoginBean, DmpkAttributiDinamiciComponicriterisuattrdinBean pBean) throws Exception{
	   final ComponicriterisuattrdinImpl lComponicriterisuattrdin = new ComponicriterisuattrdinImpl();
	   setBean(pBean);
	   lComponicriterisuattrdin.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lComponicriterisuattrdin.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkAttributiDinamiciComponicriterisuattrdinBean> result = new StoreResultBean<DmpkAttributiDinamiciComponicriterisuattrdinBean>();
