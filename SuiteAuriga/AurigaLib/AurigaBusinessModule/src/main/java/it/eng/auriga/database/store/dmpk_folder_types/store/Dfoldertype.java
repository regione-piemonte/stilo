...
package it.eng.auriga.database.store.dmpk_folder_types.store;

import it.eng.auriga.database.store.dmpk_folder_types.bean.DmpkFolderTypesDfoldertypeBean;
import it.eng.auriga.database.store.dmpk_folder_types.store.impl.DfoldertypeImpl;
 ...
 */
@Service(name="DmpkFolderTypesDfoldertype")
public class Dfoldertype {
		
	private DmpkFolderTypesDfoldertypeBean bean;
		  
	public void setBean(DmpkFolderTypesDfoldertypeBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkFolderTypesDfoldertypeBean> execute(AurigaLoginBean lLoginBean, DmpkFolderTypesDfoldertypeBean pBean) throws Exception{
	   final DfoldertypeImpl lDfoldertype = new DfoldertypeImpl();
	   setBean(pBean);
	   lDfoldertype.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lDfoldertype.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkFolderTypesDfoldertypeBean> result = new StoreResultBean<DmpkFolderTypesDfoldertypeBean>();
