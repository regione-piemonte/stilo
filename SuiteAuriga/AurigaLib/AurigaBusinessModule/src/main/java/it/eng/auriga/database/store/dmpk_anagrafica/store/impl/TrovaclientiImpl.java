...
public class TrovaclientiImpl  {
	...
	public void execute(Connection connection) throws SQLException {
		...
		try{
			...
			call = connection.prepareCall("{? = call DMPK_ANAGRAFICA.TROVACLIENTI(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}");			
			...
			util.settingParameterOnStore(call,bean,wrapperBean,"rigatagnamein",52,Types.VARCHAR,connection); 	
