...
public class TrovasoggesterniprocImpl  {
	...
	public void execute(Connection connection) throws SQLException {
		...
		try{
			...
			call = connection.prepareCall("{? = call DMPK_GESTORE.TROVASOGGESTERNIPROC(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}");			
			...
			util.settingParameterOnStore(call,bean,wrapperBean,"tsvldin",4,Types.VARCHAR,connection); 	
			...
			util.settinParameterOnBean(call,bean,wrapperBean,"listaxmlout",14,Types.CLOB); 
			util.settinParameterOnBean(call,bean,wrapperBean,"noteout",15,Types.VARCHAR); 
