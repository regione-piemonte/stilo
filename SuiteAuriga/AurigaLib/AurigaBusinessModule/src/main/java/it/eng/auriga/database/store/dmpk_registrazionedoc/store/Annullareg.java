...
import it.eng.auriga.database.store.dmpk_registrazionedoc.store.impl.AnnullaregImpl;
 ...
 */
@Service(name="DmpkRegistrazionedocAnnullareg")
public class Annullareg {
		
	private DmpkRegistrazionedocAnnullaregBean bean;
		  
	public void setBean(DmpkRegistrazionedocAnnullaregBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkRegistrazionedocAnnullaregBean> execute(AurigaLoginBean lLoginBean, DmpkRegistrazionedocAnnullaregBean pBean) throws Exception{
	   final AnnullaregImpl lAnnullareg = new AnnullaregImpl();
	   setBean(pBean);
	   lAnnullareg.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lAnnullareg.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkRegistrazionedocAnnullaregBean> result = new StoreResultBean<DmpkRegistrazionedocAnnullaregBean>();
