...
import it.eng.auriga.database.store.dmpk_ws.store.impl.DeluserImpl;
 ...
 */
@Service(name="DmpkWsDeluser")
public class Deluser {
		
	private DmpkWsDeluserBean bean;
		  
	public void setBean(DmpkWsDeluserBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkWsDeluserBean> execute(AurigaLoginBean lLoginBean, DmpkWsDeluserBean pBean) throws Exception{
	   final DeluserImpl lDeluser = new DeluserImpl();
	   setBean(pBean);
	   lDeluser.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lDeluser.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkWsDeluserBean> result = new StoreResultBean<DmpkWsDeluserBean>();
