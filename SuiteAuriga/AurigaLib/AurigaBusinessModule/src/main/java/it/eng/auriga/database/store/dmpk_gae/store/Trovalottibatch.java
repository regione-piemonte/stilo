...
import it.eng.auriga.database.store.dmpk_gae.bean.DmpkGaeTrovalottibatchBean;
import it.eng.auriga.database.store.dmpk_gae.store.impl.TrovalottibatchImpl;
 ...
 */
@Service(name="DmpkGaeTrovalottibatch")
public class Trovalottibatch {
		
	private DmpkGaeTrovalottibatchBean bean;
		  
	public void setBean(DmpkGaeTrovalottibatchBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkGaeTrovalottibatchBean> execute(AurigaLoginBean lLoginBean, DmpkGaeTrovalottibatchBean pBean) throws Exception{
	   final TrovalottibatchImpl lTrovalottibatch = new TrovalottibatchImpl();
	   setBean(pBean);
	   lTrovalottibatch.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lTrovalottibatch.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkGaeTrovalottibatchBean> result = new StoreResultBean<DmpkGaeTrovalottibatchBean>();
