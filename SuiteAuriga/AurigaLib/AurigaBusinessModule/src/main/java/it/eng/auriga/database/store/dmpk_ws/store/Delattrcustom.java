...
import it.eng.auriga.database.store.dmpk_ws.store.impl.DelattrcustomImpl;
 ...
 */
@Service(name="DmpkWsDelattrcustom")
public class Delattrcustom {
		
	private DmpkWsDelattrcustomBean bean;
		  
	public void setBean(DmpkWsDelattrcustomBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkWsDelattrcustomBean> execute(AurigaLoginBean lLoginBean, DmpkWsDelattrcustomBean pBean) throws Exception{
	   final DelattrcustomImpl lDelattrcustom = new DelattrcustomImpl();
	   setBean(pBean);
	   lDelattrcustom.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lDelattrcustom.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkWsDelattrcustomBean> result = new StoreResultBean<DmpkWsDelattrcustomBean>();
