...
public class TestshowfoldercontentallowedImpl  {
	...
	public void execute(Connection connection) throws SQLException {
		...
		try{
			...
			call = connection.prepareCall("{? = call DMPK_TITOLARIO.TESTSHOWFOLDERCONTENTALLOWED(?,?,?,?,?,?,?,?,?,?,?)}");			
			...
			util.settingParameterOnStore(call,bean,wrapperBean,"idpianoclassifin",9,Types.DECIMAL,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"tsrifin",10,Types.VARCHAR,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"flgsoloattivein",11,Types.INTEGER,connection); 	
			...
			util.settinParameterOnBean(call,bean,wrapperBean,"flgshowcontentallowedout",5,Types.INTEGER); 
