...
import it.eng.auriga.database.store.dmpk_core.bean.DmpkCoreGetdisplaynameverdocBean;
import it.eng.auriga.database.store.dmpk_core.store.impl.GetdisplaynameverdocImpl;
 ...
 */
@Service(name="DmpkCoreGetdisplaynameverdoc")
public class Getdisplaynameverdoc {
		
	private DmpkCoreGetdisplaynameverdocBean bean;
		  
	public void setBean(DmpkCoreGetdisplaynameverdocBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkCoreGetdisplaynameverdocBean> execute(AurigaLoginBean lLoginBean, DmpkCoreGetdisplaynameverdocBean pBean) throws Exception{
	   final GetdisplaynameverdocImpl lGetdisplaynameverdoc = new GetdisplaynameverdocImpl();
	   setBean(pBean);
	   lGetdisplaynameverdoc.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lGetdisplaynameverdoc.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkCoreGetdisplaynameverdocBean> result = new StoreResultBean<DmpkCoreGetdisplaynameverdocBean>();
