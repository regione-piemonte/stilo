...
public class UpdfolderImpl  {
		
	private DmpkCoreUpdfolderBean bean;
		  
	public void setBean(DmpkCoreUpdfolderBean bean){
		this.bean = bean;
	}
	...
	public void execute(Connection connection) throws SQLException {
		...
		try{
			...
			call = connection.prepareCall("{? = call DMPK_CORE.UPDFOLDER(?,?,?,?,?,?,?,?,?,?,?,?,?)}");			
			...
			util.settingParameterOnStore(call,bean,wrapperBean,"idlibraryin",5,Types.DECIMAL,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"nomelibraryin",6,Types.VARCHAR,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"folderpathin",7,Types.VARCHAR,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"attributixmlin",8,Types.CLOB,connection); 	
			...
			util.settinParameterOnBean(call,bean,wrapperBean,"urixaggcontainerout",9,Types.VARCHAR); 
