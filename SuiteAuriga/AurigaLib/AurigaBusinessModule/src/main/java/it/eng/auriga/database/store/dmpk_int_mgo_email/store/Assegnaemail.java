...
import it.eng.auriga.database.store.dmpk_int_mgo_email.store.impl.AssegnaemailImpl;
 ...
 */
@Service(name="DmpkIntMgoEmailAssegnaemail")
public class Assegnaemail {
		
	private DmpkIntMgoEmailAssegnaemailBean bean;
		  
	public void setBean(DmpkIntMgoEmailAssegnaemailBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkIntMgoEmailAssegnaemailBean> execute(AurigaLoginBean lLoginBean, DmpkIntMgoEmailAssegnaemailBean pBean) throws Exception{
	   final AssegnaemailImpl lAssegnaemail = new AssegnaemailImpl();
	   setBean(pBean);
	   lAssegnaemail.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lAssegnaemail.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkIntMgoEmailAssegnaemailBean> result = new StoreResultBean<DmpkIntMgoEmailAssegnaemailBean>();
