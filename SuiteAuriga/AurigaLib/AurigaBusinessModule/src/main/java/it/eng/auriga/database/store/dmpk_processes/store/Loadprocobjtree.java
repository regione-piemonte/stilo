...
import it.eng.auriga.database.store.dmpk_processes.bean.DmpkProcessesLoadprocobjtreeBean;
import it.eng.auriga.database.store.dmpk_processes.store.impl.LoadprocobjtreeImpl;
 ...
 */
@Service(name="DmpkProcessesLoadprocobjtree")
public class Loadprocobjtree {
		
	private DmpkProcessesLoadprocobjtreeBean bean;
		  
	public void setBean(DmpkProcessesLoadprocobjtreeBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkProcessesLoadprocobjtreeBean> execute(AurigaLoginBean lLoginBean, DmpkProcessesLoadprocobjtreeBean pBean) throws Exception{
	   final LoadprocobjtreeImpl lLoadprocobjtree = new LoadprocobjtreeImpl();
	   setBean(pBean);
	   lLoadprocobjtree.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lLoadprocobjtree.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkProcessesLoadprocobjtreeBean> result = new StoreResultBean<DmpkProcessesLoadprocobjtreeBean>();
