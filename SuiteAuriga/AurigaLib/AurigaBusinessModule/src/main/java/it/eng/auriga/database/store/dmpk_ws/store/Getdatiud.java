...
import it.eng.auriga.database.store.dmpk_ws.store.impl.GetdatiudImpl;
 ...
 */
@Service(name="DmpkWsGetdatiud")
public class Getdatiud {
		
	private DmpkWsGetdatiudBean bean;
		  
	public void setBean(DmpkWsGetdatiudBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkWsGetdatiudBean> execute(AurigaLoginBean lLoginBean, DmpkWsGetdatiudBean pBean) throws Exception{
	   final GetdatiudImpl lGetdatiud = new GetdatiudImpl();
	   setBean(pBean);
	   lGetdatiud.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lGetdatiud.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkWsGetdatiudBean> result = new StoreResultBean<DmpkWsGetdatiudBean>();
