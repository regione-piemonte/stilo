...
import it.eng.auriga.database.store.dmpk_efact.bean.DmpkEfactTrovadoclottoeventiBean;
import it.eng.auriga.database.store.dmpk_efact.store.impl.TrovadoclottoeventiImpl;
 ...
 */
@Service(name="DmpkEfactTrovadoclottoeventi")
public class Trovadoclottoeventi {
		
	private DmpkEfactTrovadoclottoeventiBean bean;
		  
	public void setBean(DmpkEfactTrovadoclottoeventiBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkEfactTrovadoclottoeventiBean> execute(AurigaLoginBean lLoginBean, DmpkEfactTrovadoclottoeventiBean pBean) throws Exception{
	   final TrovadoclottoeventiImpl lTrovadoclottoeventi = new TrovadoclottoeventiImpl();
	   setBean(pBean);
	   lTrovadoclottoeventi.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lTrovadoclottoeventi.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkEfactTrovadoclottoeventiBean> result = new StoreResultBean<DmpkEfactTrovadoclottoeventiBean>();
