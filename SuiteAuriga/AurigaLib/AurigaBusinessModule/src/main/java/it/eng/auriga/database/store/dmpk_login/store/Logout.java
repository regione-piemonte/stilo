...
import it.eng.auriga.database.store.dmpk_login.store.impl.LogoutImpl;
 ...
 */
@Service(name="DmpkLoginLogout")
public class Logout {
		
	private DmpkLoginLogoutBean bean;
		  
	public void setBean(DmpkLoginLogoutBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkLoginLogoutBean> execute(AurigaLoginBean lLoginBean, DmpkLoginLogoutBean pBean) throws Exception{
	   final LogoutImpl lLogout = new LogoutImpl();
	   setBean(pBean);
	   lLogout.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lLogout.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkLoginLogoutBean> result = new StoreResultBean<DmpkLoginLogoutBean>();
