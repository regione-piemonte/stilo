...
import it.eng.auriga.database.store.dmpk_wf.bean.DmpkWfGeturlfromnameattBean;
import it.eng.auriga.database.store.dmpk_wf.store.impl.GeturlfromnameattImpl;
 ...
 */
@Service(name="DmpkWfGeturlfromnameatt")
public class Geturlfromnameatt {
		
	private DmpkWfGeturlfromnameattBean bean;
		  
	public void setBean(DmpkWfGeturlfromnameattBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkWfGeturlfromnameattBean> execute(AurigaLoginBean lLoginBean, DmpkWfGeturlfromnameattBean pBean) throws Exception{
	   final GeturlfromnameattImpl lGeturlfromnameatt = new GeturlfromnameattImpl();
	   setBean(pBean);
	   lGeturlfromnameatt.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lGeturlfromnameatt.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkWfGeturlfromnameattBean> result = new StoreResultBean<DmpkWfGeturlfromnameattBean>();
