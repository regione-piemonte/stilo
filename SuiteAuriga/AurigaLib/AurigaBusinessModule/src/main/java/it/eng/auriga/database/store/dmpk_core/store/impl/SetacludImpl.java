...
public class SetacludImpl  {
		
	private DmpkCoreSetacludBean bean;
		  
	public void setBean(DmpkCoreSetacludBean bean){
		this.bean = bean;
	}
	...
	public void execute(Connection connection) throws SQLException {
		...
		try{
			...
			call = connection.prepareCall("{? = call DMPK_CORE.SETACLUD(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}");			
			...
			util.settingParameterOnStore(call,bean,wrapperBean,"flgtipooggereditadain",6,Types.VARCHAR,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"idoggereditadain",7,Types.DECIMAL,connection); 	
			...
			util.settinParameterOnBean(call,bean,wrapperBean,"urixmlout",10,Types.CLOB); 
