...
public class ToglidaImpl  {
		
	private DmpkCoreToglidaBean bean;
		  
	public void setBean(DmpkCoreToglidaBean bean){
		this.bean = bean;
	}
	...
	public void execute(Connection connection) throws SQLException {
		...
		try{
			...
			call = connection.prepareCall("{? = call DMPK_CORE.TOGLIDA(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}");			
			...
			util.settingParameterOnStore(call,bean,wrapperBean,"flgtpobjouterin",7,Types.VARCHAR,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"idobjouterin",8,Types.DECIMAL,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"pathnomeobjouterin",9,Types.VARCHAR,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"idlibraryin",10,Types.DECIMAL,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"nomelibraryin",11,Types.VARCHAR,connection); 	
			...
			util.settinParameterOnBean(call,bean,wrapperBean,"urixmlout",12,Types.CLOB); 
