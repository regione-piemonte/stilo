...
import it.eng.auriga.database.store.dmpk_core.bean.DmpkCoreAddworkspaceBean;
import it.eng.auriga.database.store.dmpk_core.store.impl.AddworkspaceImpl;
 ...
 */
@Service(name="DmpkCoreAddworkspace")
public class Addworkspace {
		
	private DmpkCoreAddworkspaceBean bean;
		  
	public void setBean(DmpkCoreAddworkspaceBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkCoreAddworkspaceBean> execute(AurigaLoginBean lLoginBean, DmpkCoreAddworkspaceBean pBean) throws Exception{
	   final AddworkspaceImpl lAddworkspace = new AddworkspaceImpl();
	   setBean(pBean);
	   lAddworkspace.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lAddworkspace.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkCoreAddworkspaceBean> result = new StoreResultBean<DmpkCoreAddworkspaceBean>();
