...
import it.eng.auriga.database.store.dmpk_order.store.impl.GetcodiciintermediariImpl;
 ...
 */
@Service(name="DmpkOrderGetcodiciintermediari")
public class Getcodiciintermediari {
	...
	public StoreResultBean<DmpkOrderGetcodiciintermediariBean> execute(AurigaLoginBean lLoginBean, DmpkOrderGetcodiciintermediariBean pBean) throws Exception{
	   final GetcodiciintermediariImpl lGetcodiciintermediari = new GetcodiciintermediariImpl();
	   setBean(pBean);
	   lGetcodiciintermediari.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lGetcodiciintermediari.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkOrderGetcodiciintermediariBean> result = new StoreResultBean<DmpkOrderGetcodiciintermediariBean>();
