...
import it.eng.auriga.database.store.dmpk_efact.bean.DmpkEfactIudocumentolottoBean;
import it.eng.auriga.database.store.dmpk_efact.store.impl.IudocumentolottoImpl;
 ...
 */
@Service(name="DmpkEfactIudocumentolotto")
public class Iudocumentolotto {
		
	private DmpkEfactIudocumentolottoBean bean;
		  
	public void setBean(DmpkEfactIudocumentolottoBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkEfactIudocumentolottoBean> execute(AurigaLoginBean lLoginBean, DmpkEfactIudocumentolottoBean pBean) throws Exception{
	   final IudocumentolottoImpl lIudocumentolotto = new IudocumentolottoImpl();
	   setBean(pBean);
	   lIudocumentolotto.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lIudocumentolotto.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkEfactIudocumentolottoBean> result = new StoreResultBean<DmpkEfactIudocumentolottoBean>();
