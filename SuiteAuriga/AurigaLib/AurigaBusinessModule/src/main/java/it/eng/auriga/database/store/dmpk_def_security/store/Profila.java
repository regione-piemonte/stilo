...
import it.eng.auriga.database.store.dmpk_def_security.store.impl.ProfilaImpl;
 ...
 */
@Service(name="DmpkDefSecurityProfila")
public class Profila {
	...
	public StoreResultBean<DmpkDefSecurityProfilaBean> execute(AurigaLoginBean lLoginBean, DmpkDefSecurityProfilaBean pBean) throws Exception{
	   final ProfilaImpl lProfila = new ProfilaImpl();
	   setBean(pBean);
	   lProfila.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lProfila.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkDefSecurityProfilaBean> result = new StoreResultBean<DmpkDefSecurityProfilaBean>();
