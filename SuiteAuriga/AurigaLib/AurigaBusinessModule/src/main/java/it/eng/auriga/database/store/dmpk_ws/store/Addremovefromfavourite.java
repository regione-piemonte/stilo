...
import it.eng.auriga.database.store.dmpk_ws.store.impl.AddremovefromfavouriteImpl;
 ...
 */
@Service(name="DmpkWsAddremovefromfavourite")
public class Addremovefromfavourite {
		
	private DmpkWsAddremovefromfavouriteBean bean;
		  
	public void setBean(DmpkWsAddremovefromfavouriteBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkWsAddremovefromfavouriteBean> execute(AurigaLoginBean lLoginBean, DmpkWsAddremovefromfavouriteBean pBean) throws Exception{
	   final AddremovefromfavouriteImpl lAddremovefromfavourite = new AddremovefromfavouriteImpl();
	   setBean(pBean);
	   lAddremovefromfavourite.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lAddremovefromfavourite.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkWsAddremovefromfavouriteBean> result = new StoreResultBean<DmpkWsAddremovefromfavouriteBean>();
