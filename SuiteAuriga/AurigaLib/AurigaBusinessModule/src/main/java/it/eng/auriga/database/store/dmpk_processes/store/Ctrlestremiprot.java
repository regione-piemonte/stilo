...
import it.eng.auriga.database.store.dmpk_processes.bean.DmpkProcessesCtrlestremiprotBean;
import it.eng.auriga.database.store.dmpk_processes.store.impl.CtrlestremiprotImpl;
 ...
 */
@Service(name="DmpkProcessesCtrlestremiprot")
public class Ctrlestremiprot {
		
	private DmpkProcessesCtrlestremiprotBean bean;
		  
	public void setBean(DmpkProcessesCtrlestremiprotBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkProcessesCtrlestremiprotBean> execute(AurigaLoginBean lLoginBean, DmpkProcessesCtrlestremiprotBean pBean) throws Exception{
	   final CtrlestremiprotImpl lCtrlestremiprot = new CtrlestremiprotImpl();
	   setBean(pBean);
	   lCtrlestremiprot.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lCtrlestremiprot.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkProcessesCtrlestremiprotBean> result = new StoreResultBean<DmpkProcessesCtrlestremiprotBean>();
