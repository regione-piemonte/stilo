...
package it.eng.auriga.database.store.dmpk_toponomastica.store;

import it.eng.auriga.database.store.dmpk_toponomastica.bean.DmpkToponomasticaListaciviciBean;
import it.eng.auriga.database.store.dmpk_toponomastica.store.impl.ListaciviciImpl;
 ...
 */
@Service(name="DmpkToponomasticaListacivici")
public class Listacivici {
		
	private DmpkToponomasticaListaciviciBean bean;
		  
	public void setBean(DmpkToponomasticaListaciviciBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkToponomasticaListaciviciBean> execute(AurigaLoginBean lLoginBean, DmpkToponomasticaListaciviciBean pBean) throws Exception{
	   final ListaciviciImpl lListacivici = new ListaciviciImpl();
	   setBean(pBean);
	   lListacivici.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lListacivici.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkToponomasticaListaciviciBean> result = new StoreResultBean<DmpkToponomasticaListaciviciBean>();
