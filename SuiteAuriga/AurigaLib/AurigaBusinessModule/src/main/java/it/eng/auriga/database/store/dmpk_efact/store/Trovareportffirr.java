...
import it.eng.auriga.database.store.dmpk_efact.bean.DmpkEfactTrovareportffirrBean;
import it.eng.auriga.database.store.dmpk_efact.store.impl.TrovareportffirrImpl;
 ...
 */
@Service(name="DmpkEfactTrovareportffirr")
public class Trovareportffirr {
		
	private DmpkEfactTrovareportffirrBean bean;
		  
	public void setBean(DmpkEfactTrovareportffirrBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkEfactTrovareportffirrBean> execute(AurigaLoginBean lLoginBean, DmpkEfactTrovareportffirrBean pBean) throws Exception{
	   final TrovareportffirrImpl lTrovareportffirr = new TrovareportffirrImpl();
	   setBean(pBean);
	   lTrovareportffirr.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lTrovareportffirr.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkEfactTrovareportffirrBean> result = new StoreResultBean<DmpkEfactTrovareportffirrBean>();
