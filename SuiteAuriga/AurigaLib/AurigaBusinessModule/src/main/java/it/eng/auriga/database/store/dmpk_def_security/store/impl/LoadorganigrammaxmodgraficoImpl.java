...
public class LoadorganigrammaxmodgraficoImpl  {
	...
	public void execute(Connection connection) throws SQLException {
		...
		try{
			...
			call = connection.prepareCall("{? = call DMPK_DEF_SECURITY.LOADORGANIGRAMMAXMODGRAFICO(?,?,?,?,?,?,?,?,?,?,?)}");			
			...
			util.settingParameterOnStore(call,bean,wrapperBean,"iduorootin",6,Types.VARCHAR,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"livellomaxin",7,Types.INTEGER,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"codtipiuoin",8,Types.VARCHAR,connection); 	
			...
			util.settinParameterOnBean(call,bean,wrapperBean,"organigrammaxmlout",9,Types.CLOB); 
