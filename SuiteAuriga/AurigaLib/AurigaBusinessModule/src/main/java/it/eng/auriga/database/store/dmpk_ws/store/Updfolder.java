...
import it.eng.auriga.database.store.dmpk_ws.store.impl.UpdfolderImpl;
 ...
 */
@Service(name="DmpkWsUpdfolder")
public class Updfolder {
		
	private DmpkWsUpdfolderBean bean;
		  
	public void setBean(DmpkWsUpdfolderBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkWsUpdfolderBean> execute(AurigaLoginBean lLoginBean, DmpkWsUpdfolderBean pBean) throws Exception{
	   final UpdfolderImpl lUpdfolder = new UpdfolderImpl();
	   setBean(pBean);
	   lUpdfolder.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lUpdfolder.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkWsUpdfolderBean> result = new StoreResultBean<DmpkWsUpdfolderBean>();
