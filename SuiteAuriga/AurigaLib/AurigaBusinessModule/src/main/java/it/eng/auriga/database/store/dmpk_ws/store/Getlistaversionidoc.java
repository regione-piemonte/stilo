...
import it.eng.auriga.database.store.dmpk_ws.store.impl.GetlistaversionidocImpl;
 ...
 */
@Service(name="DmpkWsGetlistaversionidoc")
public class Getlistaversionidoc {
		
	private DmpkWsGetlistaversionidocBean bean;
		  
	public void setBean(DmpkWsGetlistaversionidocBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkWsGetlistaversionidocBean> execute(AurigaLoginBean lLoginBean, DmpkWsGetlistaversionidocBean pBean) throws Exception{
	   final GetlistaversionidocImpl lGetlistaversionidoc = new GetlistaversionidocImpl();
	   setBean(pBean);
	   lGetlistaversionidoc.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lGetlistaversionidoc.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkWsGetlistaversionidocBean> result = new StoreResultBean<DmpkWsGetlistaversionidocBean>();
