...
import it.eng.auriga.database.store.dmpk_gestore.store.impl.Submit_soggesterniprocImpl;
 ...
 */
@Service(name="DmpkGestoreSubmit_soggesterniproc")
public class Submit_soggesterniproc {
	...
	public StoreResultBean<DmpkGestoreSubmit_soggesterniprocBean> execute(AurigaLoginBean lLoginBean, DmpkGestoreSubmit_soggesterniprocBean pBean) throws Exception{
	   final Submit_soggesterniprocImpl lSubmit_soggesterniproc = new Submit_soggesterniprocImpl();
	   setBean(pBean);
	   lSubmit_soggesterniproc.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lSubmit_soggesterniproc.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkGestoreSubmit_soggesterniprocBean> result = new StoreResultBean<DmpkGestoreSubmit_soggesterniprocBean>();
