...
import it.eng.auriga.database.store.dmpk_core.bean.DmpkCoreGetmaindataverdocBean;
...
public class GetmaindataverdocImpl  {
		
	private DmpkCoreGetmaindataverdocBean bean;
		  
	public void setBean(DmpkCoreGetmaindataverdocBean bean){
		this.bean = bean;
	}
	...
	public void execute(Connection connection) throws SQLException {
		...
		try{
			...
			call = connection.prepareCall("{? = call DMPK_CORE.GETMAINDATAVERDOC(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}");			
			...
			util.settingParameterOnStore(call,bean,wrapperBean,"iddocin",2,Types.DECIMAL,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"nroprogrverio",3,Types.DECIMAL,connection); 	
			...
			util.settinParameterOnBean(call,bean,wrapperBean,"nroprogrverio",3,Types.DECIMAL); 
			util.settinParameterOnBean(call,bean,wrapperBean,"codverout",4,Types.VARCHAR); 
			util.settinParameterOnBean(call,bean,wrapperBean,"flgvldout",5,Types.INTEGER); 
			...
			util.settinParameterOnBean(call,bean,wrapperBean,"mimetypeverout",12,Types.DECIMAL); 
			util.settinParameterOnBean(call,bean,wrapperBean,"flgfirmataverout",13,Types.INTEGER); 
			util.settinParameterOnBean(call,bean,wrapperBean,"noteverout",14,Types.VARCHAR); 
