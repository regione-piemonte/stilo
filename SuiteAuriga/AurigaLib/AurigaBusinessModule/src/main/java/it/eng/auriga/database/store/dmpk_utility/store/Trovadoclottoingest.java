...
import it.eng.auriga.database.store.dmpk_utility.bean.DmpkUtilityTrovadoclottoingestBean;
import it.eng.auriga.database.store.dmpk_utility.store.impl.TrovadoclottoingestImpl;
 ...
 */
@Service(name="DmpkUtilityTrovadoclottoingest")
public class Trovadoclottoingest {
		
	private DmpkUtilityTrovadoclottoingestBean bean;
		  
	public void setBean(DmpkUtilityTrovadoclottoingestBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkUtilityTrovadoclottoingestBean> execute(AurigaLoginBean lLoginBean, DmpkUtilityTrovadoclottoingestBean pBean) throws Exception{
	   final TrovadoclottoingestImpl lTrovadoclottoingest = new TrovadoclottoingestImpl();
	   setBean(pBean);
	   lTrovadoclottoingest.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lTrovadoclottoingest.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkUtilityTrovadoclottoingestBean> result = new StoreResultBean<DmpkUtilityTrovadoclottoingestBean>();
