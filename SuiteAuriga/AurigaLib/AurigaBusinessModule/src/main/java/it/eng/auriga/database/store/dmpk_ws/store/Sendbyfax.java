...
import it.eng.auriga.database.store.dmpk_ws.store.impl.SendbyfaxImpl;
 ...
 */
@Service(name="DmpkWsSendbyfax")
public class Sendbyfax {
	...
	public StoreResultBean<DmpkWsSendbyfaxBean> execute(AurigaLoginBean lLoginBean, DmpkWsSendbyfaxBean pBean) throws Exception{
	   final SendbyfaxImpl lSendbyfax = new SendbyfaxImpl();
	   setBean(pBean);
	   lSendbyfax.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lSendbyfax.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkWsSendbyfaxBean> result = new StoreResultBean<DmpkWsSendbyfaxBean>();
