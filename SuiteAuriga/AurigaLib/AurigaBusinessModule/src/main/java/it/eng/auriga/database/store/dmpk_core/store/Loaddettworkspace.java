...
import it.eng.auriga.database.store.dmpk_core.store.impl.LoaddettworkspaceImpl;
 ...
 */
@Service(name="DmpkCoreLoaddettworkspace")
public class Loaddettworkspace {
	...
	public StoreResultBean<DmpkCoreLoaddettworkspaceBean> execute(AurigaLoginBean lLoginBean, DmpkCoreLoaddettworkspaceBean pBean) throws Exception{
	   final LoaddettworkspaceImpl lLoaddettworkspace = new LoaddettworkspaceImpl();
	   setBean(pBean);
	   lLoaddettworkspace.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lLoaddettworkspace.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkCoreLoaddettworkspaceBean> result = new StoreResultBean<DmpkCoreLoaddettworkspaceBean>();
