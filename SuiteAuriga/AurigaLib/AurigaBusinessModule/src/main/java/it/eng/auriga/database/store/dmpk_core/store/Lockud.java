...
import it.eng.auriga.database.store.dmpk_core.store.impl.LockudImpl;
 ...
 */
@Service(name="DmpkCoreLockud")
public class Lockud {
		
	private DmpkCoreLockudBean bean;
		  
	public void setBean(DmpkCoreLockudBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkCoreLockudBean> execute(AurigaLoginBean lLoginBean, DmpkCoreLockudBean pBean) throws Exception{
	   final LockudImpl lLockud = new LockudImpl();
	   setBean(pBean);
	   lLockud.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lLockud.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkCoreLockudBean> result = new StoreResultBean<DmpkCoreLockudBean>();
