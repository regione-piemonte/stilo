...
import it.eng.auriga.database.store.dmpk_ws.store.impl.TrovaregistrinumerazioneImpl;
 ...
 */
@Service(name="DmpkWsTrovaregistrinumerazione")
public class Trovaregistrinumerazione {
		
	private DmpkWsTrovaregistrinumerazioneBean bean;
		  
	public void setBean(DmpkWsTrovaregistrinumerazioneBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkWsTrovaregistrinumerazioneBean> execute(AurigaLoginBean lLoginBean, DmpkWsTrovaregistrinumerazioneBean pBean) throws Exception{
	   final TrovaregistrinumerazioneImpl lTrovaregistrinumerazione = new TrovaregistrinumerazioneImpl();
	   setBean(pBean);
	   lTrovaregistrinumerazione.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lTrovaregistrinumerazione.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkWsTrovaregistrinumerazioneBean> result = new StoreResultBean<DmpkWsTrovaregistrinumerazioneBean>();
