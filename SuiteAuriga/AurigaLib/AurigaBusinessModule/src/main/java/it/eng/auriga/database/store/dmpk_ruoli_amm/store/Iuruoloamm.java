...
import it.eng.auriga.database.store.dmpk_ruoli_amm.store.impl.IuruoloammImpl;
 ...
 */
@Service(name="DmpkRuoliAmmIuruoloamm")
public class Iuruoloamm {
	...
	public StoreResultBean<DmpkRuoliAmmIuruoloammBean> execute(AurigaLoginBean lLoginBean, DmpkRuoliAmmIuruoloammBean pBean) throws Exception{
	   final IuruoloammImpl lIuruoloamm = new IuruoloammImpl();
	   setBean(pBean);
	   lIuruoloamm.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lIuruoloamm.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkRuoliAmmIuruoloammBean> result = new StoreResultBean<DmpkRuoliAmmIuruoloammBean>();
