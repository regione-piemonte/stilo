...
import it.eng.auriga.database.store.dmpk_toponomastica.bean.DmpkToponomasticaIuciviciBean;
import it.eng.auriga.database.store.dmpk_toponomastica.store.impl.IuciviciImpl;
 ...
 */
@Service(name="DmpkToponomasticaIucivici")
public class Iucivici {
		
	private DmpkToponomasticaIuciviciBean bean;
		  
	public void setBean(DmpkToponomasticaIuciviciBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkToponomasticaIuciviciBean> execute(AurigaLoginBean lLoginBean, DmpkToponomasticaIuciviciBean pBean) throws Exception{
	   final IuciviciImpl lIucivici = new IuciviciImpl();
	   setBean(pBean);
	   lIucivici.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lIucivici.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkToponomasticaIuciviciBean> result = new StoreResultBean<DmpkToponomasticaIuciviciBean>();
