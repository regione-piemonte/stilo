...
import it.eng.auriga.database.store.dmpk_wf.bean.DmpkWfGetnamefasecorrenteBean;
import it.eng.auriga.database.store.dmpk_wf.store.impl.GetnamefasecorrenteImpl;
 ...
 */
@Service(name="DmpkWfGetnamefasecorrente")
public class Getnamefasecorrente {
		
	private DmpkWfGetnamefasecorrenteBean bean;
		  
	public void setBean(DmpkWfGetnamefasecorrenteBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkWfGetnamefasecorrenteBean> execute(AurigaLoginBean lLoginBean, DmpkWfGetnamefasecorrenteBean pBean) throws Exception{
	   final GetnamefasecorrenteImpl lGetnamefasecorrente = new GetnamefasecorrenteImpl();
	   setBean(pBean);
	   lGetnamefasecorrente.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lGetnamefasecorrente.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkWfGetnamefasecorrenteBean> result = new StoreResultBean<DmpkWfGetnamefasecorrenteBean>();
