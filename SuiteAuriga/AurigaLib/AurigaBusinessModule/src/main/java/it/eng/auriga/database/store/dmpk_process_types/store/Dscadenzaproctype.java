...
import it.eng.auriga.database.store.dmpk_process_types.bean.DmpkProcessTypesDscadenzaproctypeBean;
import it.eng.auriga.database.store.dmpk_process_types.store.impl.DscadenzaproctypeImpl;
 ...
 */
@Service(name="DmpkProcessTypesDscadenzaproctype")
public class Dscadenzaproctype {
		
	private DmpkProcessTypesDscadenzaproctypeBean bean;
		  
	public void setBean(DmpkProcessTypesDscadenzaproctypeBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkProcessTypesDscadenzaproctypeBean> execute(AurigaLoginBean lLoginBean, DmpkProcessTypesDscadenzaproctypeBean pBean) throws Exception{
	   final DscadenzaproctypeImpl lDscadenzaproctype = new DscadenzaproctypeImpl();
	   setBean(pBean);
	   lDscadenzaproctype.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lDscadenzaproctype.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkProcessTypesDscadenzaproctypeBean> result = new StoreResultBean<DmpkProcessTypesDscadenzaproctypeBean>();
