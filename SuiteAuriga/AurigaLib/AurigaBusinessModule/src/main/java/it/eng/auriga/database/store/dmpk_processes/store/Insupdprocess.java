...
import it.eng.auriga.database.store.dmpk_processes.store.impl.InsupdprocessImpl;
 ...
 */
@Service(name="DmpkProcessesInsupdprocess")
public class Insupdprocess {
	...
	public StoreResultBean<DmpkProcessesInsupdprocessBean> execute(AurigaLoginBean lLoginBean, DmpkProcessesInsupdprocessBean pBean) throws Exception{
	   final InsupdprocessImpl lInsupdprocess = new InsupdprocessImpl();
	   setBean(pBean);
	   lInsupdprocess.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lInsupdprocess.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkProcessesInsupdprocessBean> result = new StoreResultBean<DmpkProcessesInsupdprocessBean>();
