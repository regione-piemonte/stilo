...
package it.eng.auriga.database.store.dmpk_elenchi_albi.store.impl;
...
public class IurecelencoalboImpl  {
	...
	public void execute(Connection connection) throws SQLException {
		...
		try{
			...
			call = connection.prepareCall("{? = call DMPK_ELENCHI_ALBI.IURECELENCOALBO(?,?,?,?,?,?,?,?,?,?,?,?,?)}");			
			...
			util.settingParameterOnStore(call,bean,wrapperBean,"idrecelencoalboio",4,Types.DECIMAL,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"idelencoalbotypein",5,Types.DECIMAL,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"attributirecxmlin",6,Types.CLOB,connection); 	
			...
			util.settinParameterOnBean(call,bean,wrapperBean,"idrecelencoalboio",4,Types.DECIMAL); 
			util.settinParameterOnBean(call,bean,wrapperBean,"rowidout",7,Types.VARCHAR); 
