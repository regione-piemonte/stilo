...
import it.eng.auriga.database.store.dmpk_efact.store.impl.TrovacidbaImpl;
 ...
 */
@Service(name="DmpkEfactTrovacidba")
public class Trovacidba {
	...
	public StoreResultBean<DmpkEfactTrovacidbaBean> execute(AurigaLoginBean lLoginBean, DmpkEfactTrovacidbaBean pBean) throws Exception{
	   final TrovacidbaImpl lTrovacidba = new TrovacidbaImpl();
	   setBean(pBean);
	   lTrovacidba.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lTrovacidba.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkEfactTrovacidbaBean> result = new StoreResultBean<DmpkEfactTrovacidbaBean>();
