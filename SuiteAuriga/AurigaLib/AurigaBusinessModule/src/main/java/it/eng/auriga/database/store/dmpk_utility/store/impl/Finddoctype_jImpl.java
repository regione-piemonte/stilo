...
public class Finddoctype_jImpl  {
	...
	public void execute(Connection connection) throws SQLException {
		...
		try{
			...
			call = connection.prepareCall("{? = call DMPK_UTILITY.FINDDOCTYPE_J(?,?,?,?,?,?,?)}");			
			...
			util.settingParameterOnStore(call,bean,wrapperBean,"iddoctypein",3,Types.DECIMAL,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"nomedoctypein",4,Types.VARCHAR,connection); 	
			...
			util.settinParameterOnBean(call,bean,wrapperBean,"iddoctypeout",8,Types.DECIMAL); 
