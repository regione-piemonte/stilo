...
import it.eng.auriga.database.store.dmpk_utility.bean.DmpkUtilityDsezionaleBean;
import it.eng.auriga.database.store.dmpk_utility.store.impl.DsezionaleImpl;
 ...
 */
@Service(name="DmpkUtilityDsezionale")
public class Dsezionale {
		
	private DmpkUtilityDsezionaleBean bean;
		  
	public void setBean(DmpkUtilityDsezionaleBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkUtilityDsezionaleBean> execute(AurigaLoginBean lLoginBean, DmpkUtilityDsezionaleBean pBean) throws Exception{
	   final DsezionaleImpl lDsezionale = new DsezionaleImpl();
	   setBean(pBean);
	   lDsezionale.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lDsezionale.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkUtilityDsezionaleBean> result = new StoreResultBean<DmpkUtilityDsezionaleBean>();
