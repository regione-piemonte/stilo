 ...
 */
@Service(name="DmpkWsUpdud")
public class Updud {
		
	private DmpkWsUpdudBean bean;
		  
	public void setBean(DmpkWsUpdudBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkWsUpdudBean> execute(AurigaLoginBean lLoginBean, DmpkWsUpdudBean pBean) throws Exception{
	   final UpdudImpl lUpdud = new UpdudImpl();
	   setBean(pBean);
	   lUpdud.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lUpdud.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkWsUpdudBean> result = new StoreResultBean<DmpkWsUpdudBean>();
