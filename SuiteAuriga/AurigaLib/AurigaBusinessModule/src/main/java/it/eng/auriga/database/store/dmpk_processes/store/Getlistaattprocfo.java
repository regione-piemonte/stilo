...
import it.eng.auriga.database.store.dmpk_processes.store.impl.GetlistaattprocfoImpl;
 ...
 */
@Service(name="DmpkProcessesGetlistaattprocfo")
public class Getlistaattprocfo {
		
	private DmpkProcessesGetlistaattprocfoBean bean;
		  
	public void setBean(DmpkProcessesGetlistaattprocfoBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkProcessesGetlistaattprocfoBean> execute(AurigaLoginBean lLoginBean, DmpkProcessesGetlistaattprocfoBean pBean) throws Exception{
	   final GetlistaattprocfoImpl lGetlistaattprocfo = new GetlistaattprocfoImpl();
	   setBean(pBean);
	   lGetlistaattprocfo.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lGetlistaattprocfo.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkProcessesGetlistaattprocfoBean> result = new StoreResultBean<DmpkProcessesGetlistaattprocfoBean>();
