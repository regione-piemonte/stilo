...
import it.eng.auriga.database.store.dmpk_fonti.bean.DmpkFontiTrovafontiBean;
import it.eng.auriga.database.store.dmpk_fonti.store.impl.TrovafontiImpl;
 ...
 */
@Service(name="DmpkFontiTrovafonti")
public class Trovafonti {
		
	private DmpkFontiTrovafontiBean bean;
		  
	public void setBean(DmpkFontiTrovafontiBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkFontiTrovafontiBean> execute(AurigaLoginBean lLoginBean, DmpkFontiTrovafontiBean pBean) throws Exception{
	   final TrovafontiImpl lTrovafonti = new TrovafontiImpl();
	   setBean(pBean);
	   lTrovafonti.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lTrovafonti.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkFontiTrovafontiBean> result = new StoreResultBean<DmpkFontiTrovafontiBean>();
