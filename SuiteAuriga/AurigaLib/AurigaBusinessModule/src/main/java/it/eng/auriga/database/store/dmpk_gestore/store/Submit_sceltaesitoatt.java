...
package it.eng.auriga.database.store.dmpk_gestore.store;

import it.eng.auriga.database.store.dmpk_gestore.bean.DmpkGestoreSubmit_sceltaesitoattBean;
import it.eng.auriga.database.store.dmpk_gestore.store.impl.Submit_sceltaesitoattImpl;
 ...
 */
@Service(name="DmpkGestoreSubmit_sceltaesitoatt")
public class Submit_sceltaesitoatt {
		
	private DmpkGestoreSubmit_sceltaesitoattBean bean;
		  
	public void setBean(DmpkGestoreSubmit_sceltaesitoattBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkGestoreSubmit_sceltaesitoattBean> execute(AurigaLoginBean lLoginBean, DmpkGestoreSubmit_sceltaesitoattBean pBean) throws Exception{
	   final Submit_sceltaesitoattImpl lSubmit_sceltaesitoatt = new Submit_sceltaesitoattImpl();
	   setBean(pBean);
	   lSubmit_sceltaesitoatt.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lSubmit_sceltaesitoatt.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkGestoreSubmit_sceltaesitoattBean> result = new StoreResultBean<DmpkGestoreSubmit_sceltaesitoattBean>();
