...
import it.eng.auriga.database.store.dmpk_ws.store.impl.CheckinImpl;
 ...
 */
@Service(name="DmpkWsCheckin")
public class Checkin {
		
	private DmpkWsCheckinBean bean;
		  
	public void setBean(DmpkWsCheckinBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkWsCheckinBean> execute(AurigaLoginBean lLoginBean, DmpkWsCheckinBean pBean) throws Exception{
	   final CheckinImpl lCheckin = new CheckinImpl();
	   setBean(pBean);
	   lCheckin.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lCheckin.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkWsCheckinBean> result = new StoreResultBean<DmpkWsCheckinBean>();
