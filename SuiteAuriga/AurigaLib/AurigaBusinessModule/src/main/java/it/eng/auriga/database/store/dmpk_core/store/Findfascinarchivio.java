...
import it.eng.auriga.database.store.dmpk_core.store.impl.FindfascinarchivioImpl;
 ...
 */
@Service(name="DmpkCoreFindfascinarchivio")
public class Findfascinarchivio {
		
	private DmpkCoreFindfascinarchivioBean bean;
		  
	public void setBean(DmpkCoreFindfascinarchivioBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkCoreFindfascinarchivioBean> execute(AurigaLoginBean lLoginBean, DmpkCoreFindfascinarchivioBean pBean) throws Exception{
	   final FindfascinarchivioImpl lFindfascinarchivio = new FindfascinarchivioImpl();
	   setBean(pBean);
	   lFindfascinarchivio.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lFindfascinarchivio.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkCoreFindfascinarchivioBean> result = new StoreResultBean<DmpkCoreFindfascinarchivioBean>();
