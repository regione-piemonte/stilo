...
import it.eng.auriga.database.store.dmpk_core.bean.DmpkCoreRecoverworkspaceBean;
import it.eng.auriga.database.store.dmpk_core.store.impl.RecoverworkspaceImpl;
 ...
 */
@Service(name="DmpkCoreRecoverworkspace")
public class Recoverworkspace {
		
	private DmpkCoreRecoverworkspaceBean bean;
		  
	public void setBean(DmpkCoreRecoverworkspaceBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkCoreRecoverworkspaceBean> execute(AurigaLoginBean lLoginBean, DmpkCoreRecoverworkspaceBean pBean) throws Exception{
	   final RecoverworkspaceImpl lRecoverworkspace = new RecoverworkspaceImpl();
	   setBean(pBean);
	   lRecoverworkspace.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lRecoverworkspace.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkCoreRecoverworkspaceBean> result = new StoreResultBean<DmpkCoreRecoverworkspaceBean>();
