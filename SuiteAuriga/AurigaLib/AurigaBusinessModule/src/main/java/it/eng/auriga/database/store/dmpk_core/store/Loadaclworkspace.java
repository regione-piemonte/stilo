...
import it.eng.auriga.database.store.dmpk_core.bean.DmpkCoreLoadaclworkspaceBean;
import it.eng.auriga.database.store.dmpk_core.store.impl.LoadaclworkspaceImpl;
 ...
 */
@Service(name="DmpkCoreLoadaclworkspace")
public class Loadaclworkspace {
		
	private DmpkCoreLoadaclworkspaceBean bean;
		  
	public void setBean(DmpkCoreLoadaclworkspaceBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkCoreLoadaclworkspaceBean> execute(AurigaLoginBean lLoginBean, DmpkCoreLoadaclworkspaceBean pBean) throws Exception{
	   final LoadaclworkspaceImpl lLoadaclworkspace = new LoadaclworkspaceImpl();
	   setBean(pBean);
	   lLoadaclworkspace.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lLoadaclworkspace.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkCoreLoadaclworkspaceBean> result = new StoreResultBean<DmpkCoreLoadaclworkspaceBean>();
