...
import it.eng.auriga.database.store.dmpk_int_mgo_email.bean.DmpkIntMgoEmailRiprendiemailBean;
import it.eng.auriga.database.store.dmpk_int_mgo_email.store.impl.RiprendiemailImpl;
 ...
 */
@Service(name="DmpkIntMgoEmailRiprendiemail")
public class Riprendiemail {
		
	private DmpkIntMgoEmailRiprendiemailBean bean;
		  
	public void setBean(DmpkIntMgoEmailRiprendiemailBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkIntMgoEmailRiprendiemailBean> execute(AurigaLoginBean lLoginBean, DmpkIntMgoEmailRiprendiemailBean pBean) throws Exception{
	   final RiprendiemailImpl lRiprendiemail = new RiprendiemailImpl();
	   setBean(pBean);
	   lRiprendiemail.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lRiprendiemail.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkIntMgoEmailRiprendiemailBean> result = new StoreResultBean<DmpkIntMgoEmailRiprendiemailBean>();
