...
public class AdddocImpl  {
		
	private DmpkCoreAdddocBean bean;
		  
	public void setBean(DmpkCoreAdddocBean bean){
		this.bean = bean;
	}
	...
	public void execute(Connection connection) throws SQLException {
		...
		try{
			...
			call = connection.prepareCall("{? = call DMPK_CORE.ADDDOC(?,?,?,?,?,?,?,?,?,?,?)}");			
			...
			util.settingParameterOnStore(call,bean,wrapperBean,"attributiuddocxmlin",4,Types.CLOB,connection); 	
			...
			util.settinParameterOnBean(call,bean,wrapperBean,"iddocout",6,Types.DECIMAL); 
