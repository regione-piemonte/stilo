...
import it.eng.auriga.database.store.dmpk_ws.store.impl.AdduserImpl;
 ...
 */
@Service(name="DmpkWsAdduser")
public class Adduser {
		
	private DmpkWsAdduserBean bean;
		  
	public void setBean(DmpkWsAdduserBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkWsAdduserBean> execute(AurigaLoginBean lLoginBean, DmpkWsAdduserBean pBean) throws Exception{
	   final AdduserImpl lAdduser = new AdduserImpl();
	   setBean(pBean);
	   lAdduser.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lAdduser.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkWsAdduserBean> result = new StoreResultBean<DmpkWsAdduserBean>();
