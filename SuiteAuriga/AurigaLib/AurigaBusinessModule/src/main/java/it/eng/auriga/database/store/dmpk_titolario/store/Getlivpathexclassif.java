...
import it.eng.auriga.database.store.dmpk_titolario.store.impl.GetlivpathexclassifImpl;
 ...
 */
@Service(name="DmpkTitolarioGetlivpathexclassif")
public class Getlivpathexclassif {
		
	private DmpkTitolarioGetlivpathexclassifBean bean;
		  
	public void setBean(DmpkTitolarioGetlivpathexclassifBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkTitolarioGetlivpathexclassifBean> execute(AurigaLoginBean lLoginBean, DmpkTitolarioGetlivpathexclassifBean pBean) throws Exception{
	   final GetlivpathexclassifImpl lGetlivpathexclassif = new GetlivpathexclassifImpl();
	   setBean(pBean);
	   lGetlivpathexclassif.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lGetlivpathexclassif.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkTitolarioGetlivpathexclassifBean> result = new StoreResultBean<DmpkTitolarioGetlivpathexclassifBean>();
