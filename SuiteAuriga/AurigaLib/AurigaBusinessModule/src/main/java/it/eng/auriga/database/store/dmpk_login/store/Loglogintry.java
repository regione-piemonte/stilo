...
import it.eng.auriga.database.store.dmpk_login.store.impl.LoglogintryImpl;
 ...
 */
@Service(name="DmpkLoginLoglogintry")
public class Loglogintry {
	...
	public StoreResultBean<DmpkLoginLoglogintryBean> execute(AurigaLoginBean lLoginBean, DmpkLoginLoglogintryBean pBean) throws Exception{
	   final LoglogintryImpl lLoglogintry = new LoglogintryImpl();
	   setBean(pBean);
	   lLoglogintry.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lLoglogintry.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkLoginLoglogintryBean> result = new StoreResultBean<DmpkLoginLoglogintryBean>();
