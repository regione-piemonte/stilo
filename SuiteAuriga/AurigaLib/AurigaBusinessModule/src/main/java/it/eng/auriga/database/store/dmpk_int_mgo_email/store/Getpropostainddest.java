...
import it.eng.auriga.database.store.dmpk_int_mgo_email.store.impl.GetpropostainddestImpl;
 ...
 */
@Service(name="DmpkIntMgoEmailGetpropostainddest")
public class Getpropostainddest {
		
	private DmpkIntMgoEmailGetpropostainddestBean bean;
		  
	public void setBean(DmpkIntMgoEmailGetpropostainddestBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkIntMgoEmailGetpropostainddestBean> execute(AurigaLoginBean lLoginBean, DmpkIntMgoEmailGetpropostainddestBean pBean) throws Exception{
	   final GetpropostainddestImpl lGetpropostainddest = new GetpropostainddestImpl();
	   setBean(pBean);
	   lGetpropostainddest.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lGetpropostainddest.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkIntMgoEmailGetpropostainddestBean> result = new StoreResultBean<DmpkIntMgoEmailGetpropostainddestBean>();
