...
import it.eng.auriga.database.store.dmpk_core.store.impl.TrovavisibledocversionImpl;
 ...
 */
@Service(name="DmpkCoreTrovavisibledocversion")
public class Trovavisibledocversion {
		
	private DmpkCoreTrovavisibledocversionBean bean;
		  
	public void setBean(DmpkCoreTrovavisibledocversionBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkCoreTrovavisibledocversionBean> execute(AurigaLoginBean lLoginBean, DmpkCoreTrovavisibledocversionBean pBean) throws Exception{
	   final TrovavisibledocversionImpl lTrovavisibledocversion = new TrovavisibledocversionImpl();
	   setBean(pBean);
	   lTrovavisibledocversion.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lTrovavisibledocversion.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkCoreTrovavisibledocversionBean> result = new StoreResultBean<DmpkCoreTrovavisibledocversionBean>();
