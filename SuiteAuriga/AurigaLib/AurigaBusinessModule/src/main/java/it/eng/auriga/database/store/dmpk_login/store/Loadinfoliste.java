...
import it.eng.auriga.database.store.dmpk_login.bean.DmpkLoginLoadinfolisteBean;
import it.eng.auriga.database.store.dmpk_login.store.impl.LoadinfolisteImpl;
 ...
 */
@Service(name="DmpkLoginLoadinfoliste")
public class Loadinfoliste {
		
	private DmpkLoginLoadinfolisteBean bean;
		  
	public void setBean(DmpkLoginLoadinfolisteBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkLoginLoadinfolisteBean> execute(AurigaLoginBean lLoginBean, DmpkLoginLoadinfolisteBean pBean) throws Exception{
	   final LoadinfolisteImpl lLoadinfoliste = new LoadinfolisteImpl();
	   setBean(pBean);
	   lLoadinfoliste.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lLoadinfoliste.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkLoginLoadinfolisteBean> result = new StoreResultBean<DmpkLoginLoadinfolisteBean>();
