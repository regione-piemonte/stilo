...
import it.eng.auriga.database.store.dmpk_int_mgo_email.store.impl.DregolaregautoImpl;
 ...
 */
@Service(name="DmpkIntMgoEmailDregolaregauto")
public class Dregolaregauto {
		
	private DmpkIntMgoEmailDregolaregautoBean bean;
		  
	public void setBean(DmpkIntMgoEmailDregolaregautoBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkIntMgoEmailDregolaregautoBean> execute(AurigaLoginBean lLoginBean, DmpkIntMgoEmailDregolaregautoBean pBean) throws Exception{
	   final DregolaregautoImpl lDregolaregauto = new DregolaregautoImpl();
	   setBean(pBean);
	   lDregolaregauto.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lDregolaregauto.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkIntMgoEmailDregolaregautoBean> result = new StoreResultBean<DmpkIntMgoEmailDregolaregautoBean>();
