...
import it.eng.auriga.database.store.dmpk_repository_gui.bean.DmpkRepositoryGuiLoadfilefatturapaBean;
import it.eng.auriga.database.store.dmpk_repository_gui.store.impl.LoadfilefatturapaImpl;
 ...
 */
@Service(name="DmpkRepositoryGuiLoadfilefatturapa")
public class Loadfilefatturapa {
		
	private DmpkRepositoryGuiLoadfilefatturapaBean bean;
		  
	public void setBean(DmpkRepositoryGuiLoadfilefatturapaBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkRepositoryGuiLoadfilefatturapaBean> execute(AurigaLoginBean lLoginBean, DmpkRepositoryGuiLoadfilefatturapaBean pBean) throws Exception{
	   final LoadfilefatturapaImpl lLoadfilefatturapa = new LoadfilefatturapaImpl();
	   setBean(pBean);
	   lLoadfilefatturapa.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lLoadfilefatturapa.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkRepositoryGuiLoadfilefatturapaBean> result = new StoreResultBean<DmpkRepositoryGuiLoadfilefatturapaBean>();
