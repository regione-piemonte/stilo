...
import it.eng.auriga.database.store.dmpk_ws.store.impl.AddfolderImpl;
 ...
 */
@Service(name="DmpkWsAddfolder")
public class Addfolder {
		
	private DmpkWsAddfolderBean bean;
		  
	public void setBean(DmpkWsAddfolderBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkWsAddfolderBean> execute(AurigaLoginBean lLoginBean, DmpkWsAddfolderBean pBean) throws Exception{
	   final AddfolderImpl lAddfolder = new AddfolderImpl();
	   setBean(pBean);
	   lAddfolder.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lAddfolder.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkWsAddfolderBean> result = new StoreResultBean<DmpkWsAddfolderBean>();
