...
import it.eng.auriga.database.store.dmpk_ws.store.impl.ExtractfileudImpl;
 ...
 */
@Service(name="DmpkWsExtractfileud")
public class Extractfileud {
		
	private DmpkWsExtractfileudBean bean;
		  
	public void setBean(DmpkWsExtractfileudBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkWsExtractfileudBean> execute(AurigaLoginBean lLoginBean, DmpkWsExtractfileudBean pBean) throws Exception{
	   final ExtractfileudImpl lExtractfileud = new ExtractfileudImpl();
	   setBean(pBean);
	   lExtractfileud.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lExtractfileud.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkWsExtractfileudBean> result = new StoreResultBean<DmpkWsExtractfileudBean>();
