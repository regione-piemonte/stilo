...
import it.eng.auriga.database.store.dmpk_wf.bean.DmpkWfGetcifasecorrenteBean;
import it.eng.auriga.database.store.dmpk_wf.store.impl.GetcifasecorrenteImpl;
 ...
 */
@Service(name="DmpkWfGetcifasecorrente")
public class Getcifasecorrente {
		
	private DmpkWfGetcifasecorrenteBean bean;
		  
	public void setBean(DmpkWfGetcifasecorrenteBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkWfGetcifasecorrenteBean> execute(AurigaLoginBean lLoginBean, DmpkWfGetcifasecorrenteBean pBean) throws Exception{
	   final GetcifasecorrenteImpl lGetcifasecorrente = new GetcifasecorrenteImpl();
	   setBean(pBean);
	   lGetcifasecorrente.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lGetcifasecorrente.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkWfGetcifasecorrenteBean> result = new StoreResultBean<DmpkWfGetcifasecorrenteBean>();
