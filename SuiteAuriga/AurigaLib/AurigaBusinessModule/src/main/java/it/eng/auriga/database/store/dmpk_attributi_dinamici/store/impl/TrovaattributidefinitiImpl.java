...
public class TrovaattributidefinitiImpl  {
		
	private DmpkAttributiDinamiciTrovaattributidefinitiBean bean;
		  
	public void setBean(DmpkAttributiDinamiciTrovaattributidefinitiBean bean){
		this.bean = bean;
	}
	...
	public void execute(Connection connection) throws SQLException {
		...
		try{
			...
			call = connection.prepareCall("{? = call DMPK_ATTRIBUTI_DINAMICI.TROVAATTRIBUTIDEFINITI(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}");			
			...
			util.settingParameterOnStore(call,bean,wrapperBean,"nomeattrio",5,Types.VARCHAR,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"labelattrio",6,Types.VARCHAR,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"tipoattrio",7,Types.VARCHAR,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"desattrio",8,Types.VARCHAR,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"flginclannullatiio",9,Types.DECIMAL,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"flgescludisottoattrio",10,Types.DECIMAL,connection); 	
			...
			util.settingParameterOnStore(call,bean,wrapperBean,"nomeattrsupin",28,Types.VARCHAR,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"idmodellonotin",29,Types.DECIMAL,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"nomeattrtoforcein",30,Types.VARCHAR,connection); 	
			...
			util.settinParameterOnBean(call,bean,wrapperBean,"nomeattrio",5,Types.VARCHAR); 
			util.settinParameterOnBean(call,bean,wrapperBean,"labelattrio",6,Types.VARCHAR); 
			util.settinParameterOnBean(call,bean,wrapperBean,"tipoattrio",7,Types.VARCHAR); 
			util.settinParameterOnBean(call,bean,wrapperBean,"desattrio",8,Types.VARCHAR); 
			util.settinParameterOnBean(call,bean,wrapperBean,"flginclannullatiio",9,Types.DECIMAL); 
			util.settinParameterOnBean(call,bean,wrapperBean,"flgescludisottoattrio",10,Types.DECIMAL); 
			...
			util.settinParameterOnBean(call,bean,wrapperBean,"flgmostraaclout",24,Types.INTEGER); 
