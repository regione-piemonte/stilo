...
import it.eng.auriga.database.store.dmpk_anagrafica.bean.DmpkAnagraficaDdestinatariodocBean;
import it.eng.auriga.database.store.dmpk_anagrafica.store.impl.DdestinatariodocImpl;
 ...
 */
@Service(name="DmpkAnagraficaDdestinatariodoc")
public class Ddestinatariodoc {
		
	private DmpkAnagraficaDdestinatariodocBean bean;
		  
	public void setBean(DmpkAnagraficaDdestinatariodocBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkAnagraficaDdestinatariodocBean> execute(AurigaLoginBean lLoginBean, DmpkAnagraficaDdestinatariodocBean pBean) throws Exception{
	   final DdestinatariodocImpl lDdestinatariodoc = new DdestinatariodocImpl();
	   setBean(pBean);
	   lDdestinatariodoc.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lDdestinatariodoc.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkAnagraficaDdestinatariodocBean> result = new StoreResultBean<DmpkAnagraficaDdestinatariodocBean>();
