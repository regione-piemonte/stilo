...
import it.eng.auriga.database.store.dmpk_core_2.store.impl.FirmasudocinlibrofirmaImpl;
 ...
 */
@Service(name="DmpkCore2Firmasudocinlibrofirma")
public class Firmasudocinlibrofirma {
		
	private DmpkCore2FirmasudocinlibrofirmaBean bean;
		  
	public void setBean(DmpkCore2FirmasudocinlibrofirmaBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkCore2FirmasudocinlibrofirmaBean> execute(AurigaLoginBean lLoginBean, DmpkCore2FirmasudocinlibrofirmaBean pBean) throws Exception{
	   final FirmasudocinlibrofirmaImpl lFirmasudocinlibrofirma = new FirmasudocinlibrofirmaImpl();
	   setBean(pBean);
	   lFirmasudocinlibrofirma.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lFirmasudocinlibrofirma.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkCore2FirmasudocinlibrofirmaBean> result = new StoreResultBean<DmpkCore2FirmasudocinlibrofirmaBean>();
