...
import it.eng.auriga.database.store.dmpk_bt_clienti.bean.DmpkBtClientiLoadmassivoclientiBean;
import it.eng.auriga.database.store.dmpk_bt_clienti.store.impl.LoadmassivoclientiImpl;
 ...
 */
@Service(name="DmpkBtClientiLoadmassivoclienti")
public class Loadmassivoclienti {
		
	private DmpkBtClientiLoadmassivoclientiBean bean;
		  
	public void setBean(DmpkBtClientiLoadmassivoclientiBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkBtClientiLoadmassivoclientiBean> execute(AurigaLoginBean lLoginBean, DmpkBtClientiLoadmassivoclientiBean pBean) throws Exception{
	   final LoadmassivoclientiImpl lLoadmassivoclienti = new LoadmassivoclientiImpl();
	   setBean(pBean);
	   lLoadmassivoclienti.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lLoadmassivoclienti.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkBtClientiLoadmassivoclientiBean> result = new StoreResultBean<DmpkBtClientiLoadmassivoclientiBean>();
