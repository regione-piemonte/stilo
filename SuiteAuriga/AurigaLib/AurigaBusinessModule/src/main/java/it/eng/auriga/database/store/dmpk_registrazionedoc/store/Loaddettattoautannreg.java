...
import it.eng.auriga.database.store.dmpk_registrazionedoc.store.impl.LoaddettattoautannregImpl;
 ...
 */
@Service(name="DmpkRegistrazionedocLoaddettattoautannreg")
public class Loaddettattoautannreg {
		
	private DmpkRegistrazionedocLoaddettattoautannregBean bean;
		  
	public void setBean(DmpkRegistrazionedocLoaddettattoautannregBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkRegistrazionedocLoaddettattoautannregBean> execute(AurigaLoginBean lLoginBean, DmpkRegistrazionedocLoaddettattoautannregBean pBean) throws Exception{
	   final LoaddettattoautannregImpl lLoaddettattoautannreg = new LoaddettattoautannregImpl();
	   setBean(pBean);
	   lLoaddettattoautannreg.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lLoaddettattoautannreg.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkRegistrazionedocLoaddettattoautannregBean> result = new StoreResultBean<DmpkRegistrazionedocLoaddettattoautannregBean>();
