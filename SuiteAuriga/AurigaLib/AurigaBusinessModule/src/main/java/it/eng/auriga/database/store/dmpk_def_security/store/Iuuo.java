...
import it.eng.auriga.database.store.dmpk_def_security.store.impl.IuuoImpl;
 ...
 */
@Service(name="DmpkDefSecurityIuuo")
public class Iuuo {
	...
	public StoreResultBean<DmpkDefSecurityIuuoBean> execute(AurigaLoginBean lLoginBean, DmpkDefSecurityIuuoBean pBean) throws Exception{
	   final IuuoImpl lIuuo = new IuuoImpl();
	   setBean(pBean);
	   lIuuo.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lIuuo.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkDefSecurityIuuoBean> result = new StoreResultBean<DmpkDefSecurityIuuoBean>();
