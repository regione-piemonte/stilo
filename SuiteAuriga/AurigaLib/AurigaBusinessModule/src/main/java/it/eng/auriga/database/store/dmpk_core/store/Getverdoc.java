...
import it.eng.auriga.database.store.dmpk_core.bean.DmpkCoreGetverdocBean;
import it.eng.auriga.database.store.dmpk_core.store.impl.GetverdocImpl;
 ...
 */
@Service(name="DmpkCoreGetverdoc")
public class Getverdoc {
		
	private DmpkCoreGetverdocBean bean;
		  
	public void setBean(DmpkCoreGetverdocBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkCoreGetverdocBean> execute(AurigaLoginBean lLoginBean, DmpkCoreGetverdocBean pBean) throws Exception{
	   final GetverdocImpl lGetverdoc = new GetverdocImpl();
	   setBean(pBean);
	   lGetverdoc.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lGetverdoc.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkCoreGetverdocBean> result = new StoreResultBean<DmpkCoreGetverdocBean>();
