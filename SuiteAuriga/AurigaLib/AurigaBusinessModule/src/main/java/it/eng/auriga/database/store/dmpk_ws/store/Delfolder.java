...
import it.eng.auriga.database.store.dmpk_ws.store.impl.DelfolderImpl;
 ...
 */
@Service(name="DmpkWsDelfolder")
public class Delfolder {
		
	private DmpkWsDelfolderBean bean;
		  
	public void setBean(DmpkWsDelfolderBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkWsDelfolderBean> execute(AurigaLoginBean lLoginBean, DmpkWsDelfolderBean pBean) throws Exception{
	   final DelfolderImpl lDelfolder = new DelfolderImpl();
	   setBean(pBean);
	   lDelfolder.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lDelfolder.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkWsDelfolderBean> result = new StoreResultBean<DmpkWsDelfolderBean>();
