...
import it.eng.auriga.database.store.dmpk_fatture.bean.DmpkFattureLoadfileordinepaBean;
import it.eng.auriga.database.store.dmpk_fatture.store.impl.LoadfileordinepaImpl;
 ...
 */
@Service(name="DmpkFattureLoadfileordinepa")
public class Loadfileordinepa {
		
	private DmpkFattureLoadfileordinepaBean bean;
		  
	public void setBean(DmpkFattureLoadfileordinepaBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkFattureLoadfileordinepaBean> execute(AurigaLoginBean lLoginBean, DmpkFattureLoadfileordinepaBean pBean) throws Exception{
	   final LoadfileordinepaImpl lLoadfileordinepa = new LoadfileordinepaImpl();
	   setBean(pBean);
	   lLoadfileordinepa.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lLoadfileordinepa.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkFattureLoadfileordinepaBean> result = new StoreResultBean<DmpkFattureLoadfileordinepaBean>();
