...
public class GettimbrodigregImpl  {
	...
	public void execute(Connection connection) throws SQLException {
		...
		try{
			...
			call = connection.prepareCall("{? = call DMPK_REGISTRAZIONEDOC.GETTIMBRODIGREG(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}");			
			...
			util.settingParameterOnStore(call,bean,wrapperBean,"iddocin",16,Types.DECIMAL,connection); 	
			...
			util.settinParameterOnBean(call,bean,wrapperBean,"contenutobarcodeout",10,Types.VARCHAR); 
			util.settinParameterOnBean(call,bean,wrapperBean,"testoinchiaroout",11,Types.VARCHAR); 
