...
import it.eng.auriga.database.store.dmpk_utility.bean.DmpkUtilityTrovasezionaliBean;
import it.eng.auriga.database.store.dmpk_utility.store.impl.TrovasezionaliImpl;
 ...
 */
@Service(name="DmpkUtilityTrovasezionali")
public class Trovasezionali {
		
	private DmpkUtilityTrovasezionaliBean bean;
		  
	public void setBean(DmpkUtilityTrovasezionaliBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkUtilityTrovasezionaliBean> execute(AurigaLoginBean lLoginBean, DmpkUtilityTrovasezionaliBean pBean) throws Exception{
	   final TrovasezionaliImpl lTrovasezionali = new TrovasezionaliImpl();
	   setBean(pBean);
	   lTrovasezionali.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lTrovasezionali.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkUtilityTrovasezionaliBean> result = new StoreResultBean<DmpkUtilityTrovasezionaliBean>();
