...
import it.eng.auriga.database.store.dmpk_ws.store.impl.ExtractfilesudImpl;
 ...
 */
@Service(name="DmpkWsExtractfilesud")
public class Extractfilesud {
		
	private DmpkWsExtractfilesudBean bean;
		  
	public void setBean(DmpkWsExtractfilesudBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkWsExtractfilesudBean> execute(AurigaLoginBean lLoginBean, DmpkWsExtractfilesudBean pBean) throws Exception{
	   final ExtractfilesudImpl lExtractfilesud = new ExtractfilesudImpl();
	   setBean(pBean);
	   lExtractfilesud.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lExtractfilesud.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkWsExtractfilesudBean> result = new StoreResultBean<DmpkWsExtractfilesudBean>();
