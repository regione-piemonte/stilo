...
public class DuoImpl  {
		
	private DmpkDefSecurityDuoBean bean;
		  
	public void setBean(DmpkDefSecurityDuoBean bean){
		this.bean = bean;
	}
	...
	public void execute(Connection connection) throws SQLException {
		...
		try{
			...
			call = connection.prepareCall("{? = call DMPK_DEF_SECURITY.DUO(?,?,?,?,?,?,?,?,?,?,?,?,?,?)}");			
			...
			util.settingParameterOnStore(call,bean,wrapperBean,"denominazionein",6,Types.VARCHAR,connection); 	
			...
			util.settingParameterOnStore(call,bean,wrapperBean,"idorganigrammain",15,Types.DECIMAL,connection); 	
