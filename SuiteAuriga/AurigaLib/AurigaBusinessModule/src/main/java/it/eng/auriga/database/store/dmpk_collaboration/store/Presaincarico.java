...
import it.eng.auriga.database.store.dmpk_collaboration.store.impl.PresaincaricoImpl;
 ...
 */
@Service(name="DmpkCollaborationPresaincarico")
public class Presaincarico {
	...
	public StoreResultBean<DmpkCollaborationPresaincaricoBean> execute(AurigaLoginBean lLoginBean, DmpkCollaborationPresaincaricoBean pBean) throws Exception{
	   final PresaincaricoImpl lPresaincarico = new PresaincaricoImpl();
	   setBean(pBean);
	   lPresaincarico.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lPresaincarico.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkCollaborationPresaincaricoBean> result = new StoreResultBean<DmpkCollaborationPresaincaricoBean>();
