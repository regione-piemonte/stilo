...
import it.eng.auriga.database.store.dmpk_ws.store.impl.GetlistaversionidatiudImpl;
 ...
 */
@Service(name="DmpkWsGetlistaversionidatiud")
public class Getlistaversionidatiud {
		
	private DmpkWsGetlistaversionidatiudBean bean;
		  
	public void setBean(DmpkWsGetlistaversionidatiudBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkWsGetlistaversionidatiudBean> execute(AurigaLoginBean lLoginBean, DmpkWsGetlistaversionidatiudBean pBean) throws Exception{
	   final GetlistaversionidatiudImpl lGetlistaversionidatiud = new GetlistaversionidatiudImpl();
	   setBean(pBean);
	   lGetlistaversionidatiud.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lGetlistaversionidatiud.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkWsGetlistaversionidatiudBean> result = new StoreResultBean<DmpkWsGetlistaversionidatiudBean>();
