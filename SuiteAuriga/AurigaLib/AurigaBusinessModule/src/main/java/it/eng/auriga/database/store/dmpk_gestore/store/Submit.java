...
import it.eng.auriga.database.store.dmpk_gestore.store.impl.SubmitImpl;
 ...
 */
@Service(name="DmpkGestoreSubmit")
public class Submit {
	...
	public StoreResultBean<DmpkGestoreSubmitBean> execute(AurigaLoginBean lLoginBean, DmpkGestoreSubmitBean pBean) throws Exception{
	   final SubmitImpl lSubmit = new SubmitImpl();
	   setBean(pBean);
	   lSubmit.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lSubmit.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkGestoreSubmitBean> result = new StoreResultBean<DmpkGestoreSubmitBean>();
