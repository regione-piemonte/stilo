...
import it.eng.auriga.database.store.dmpk_registrazionedoc.store.impl.EliminazionerichannregImpl;
 ...
 */
@Service(name="DmpkRegistrazionedocEliminazionerichannreg")
public class Eliminazionerichannreg {
		
	private DmpkRegistrazionedocEliminazionerichannregBean bean;
		  
	public void setBean(DmpkRegistrazionedocEliminazionerichannregBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkRegistrazionedocEliminazionerichannregBean> execute(AurigaLoginBean lLoginBean, DmpkRegistrazionedocEliminazionerichannregBean pBean) throws Exception{
	   final EliminazionerichannregImpl lEliminazionerichannreg = new EliminazionerichannregImpl();
	   setBean(pBean);
	   lEliminazionerichannreg.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lEliminazionerichannreg.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkRegistrazionedocEliminazionerichannregBean> result = new StoreResultBean<DmpkRegistrazionedocEliminazionerichannregBean>();
