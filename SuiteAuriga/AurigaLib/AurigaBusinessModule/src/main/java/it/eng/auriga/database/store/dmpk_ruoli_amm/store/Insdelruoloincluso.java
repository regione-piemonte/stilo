...
package it.eng.auriga.database.store.dmpk_ruoli_amm.store;

import it.eng.auriga.database.store.dmpk_ruoli_amm.bean.DmpkRuoliAmmInsdelruoloinclusoBean;
import it.eng.auriga.database.store.dmpk_ruoli_amm.store.impl.InsdelruoloinclusoImpl;
 ...
 */
@Service(name="DmpkRuoliAmmInsdelruoloincluso")
public class Insdelruoloincluso {
		
	private DmpkRuoliAmmInsdelruoloinclusoBean bean;
		  
	public void setBean(DmpkRuoliAmmInsdelruoloinclusoBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkRuoliAmmInsdelruoloinclusoBean> execute(AurigaLoginBean lLoginBean, DmpkRuoliAmmInsdelruoloinclusoBean pBean) throws Exception{
	   final InsdelruoloinclusoImpl lInsdelruoloincluso = new InsdelruoloinclusoImpl();
	   setBean(pBean);
	   lInsdelruoloincluso.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lInsdelruoloincluso.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkRuoliAmmInsdelruoloinclusoBean> result = new StoreResultBean<DmpkRuoliAmmInsdelruoloinclusoBean>();
