...
import it.eng.auriga.database.store.dmpk_corrispettivi.bean.DmpkCorrispettiviLoaddettfileinvBean;
import it.eng.auriga.database.store.dmpk_corrispettivi.store.impl.LoaddettfileinvImpl;
 ...
 */
@Service(name="DmpkCorrispettiviLoaddettfileinv")
public class Loaddettfileinv {
		
	private DmpkCorrispettiviLoaddettfileinvBean bean;
		  
	public void setBean(DmpkCorrispettiviLoaddettfileinvBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkCorrispettiviLoaddettfileinvBean> execute(AurigaLoginBean lLoginBean, DmpkCorrispettiviLoaddettfileinvBean pBean) throws Exception{
	   final LoaddettfileinvImpl lLoaddettfileinv = new LoaddettfileinvImpl();
	   setBean(pBean);
	   lLoaddettfileinv.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lLoaddettfileinv.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkCorrispettiviLoaddettfileinvBean> result = new StoreResultBean<DmpkCorrispettiviLoaddettfileinvBean>();
