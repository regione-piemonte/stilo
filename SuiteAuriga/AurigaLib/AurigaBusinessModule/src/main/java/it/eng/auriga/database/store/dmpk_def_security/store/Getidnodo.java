...
import it.eng.auriga.database.store.dmpk_def_security.bean.DmpkDefSecurityGetidnodoBean;
import it.eng.auriga.database.store.dmpk_def_security.store.impl.GetidnodoImpl;
 ...
 */
@Service(name="DmpkDefSecurityGetidnodo")
public class Getidnodo {
		
	private DmpkDefSecurityGetidnodoBean bean;
		  
	public void setBean(DmpkDefSecurityGetidnodoBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkDefSecurityGetidnodoBean> execute(AurigaLoginBean lLoginBean, DmpkDefSecurityGetidnodoBean pBean) throws Exception{
	   final GetidnodoImpl lGetidnodo = new GetidnodoImpl();
	   setBean(pBean);
	   lGetidnodo.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lGetidnodo.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkDefSecurityGetidnodoBean> result = new StoreResultBean<DmpkDefSecurityGetidnodoBean>();
