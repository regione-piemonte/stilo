...
import it.eng.auriga.database.store.dmpk_ws.store.impl.DeludImpl;
 ...
 */
@Service(name="DmpkWsDelud")
public class Delud {
		
	private DmpkWsDeludBean bean;
		  
	public void setBean(DmpkWsDeludBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkWsDeludBean> execute(AurigaLoginBean lLoginBean, DmpkWsDeludBean pBean) throws Exception{
	   final DeludImpl lDelud = new DeludImpl();
	   setBean(pBean);
	   lDelud.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lDelud.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkWsDeludBean> result = new StoreResultBean<DmpkWsDeludBean>();
