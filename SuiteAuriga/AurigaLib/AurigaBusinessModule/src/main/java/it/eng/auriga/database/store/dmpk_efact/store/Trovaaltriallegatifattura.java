...
import it.eng.auriga.database.store.dmpk_efact.bean.DmpkEfactTrovaaltriallegatifatturaBean;
import it.eng.auriga.database.store.dmpk_efact.store.impl.TrovaaltriallegatifatturaImpl;
 ...
 */
@Service(name="DmpkEfactTrovaaltriallegatifattura")
public class Trovaaltriallegatifattura {
		
	private DmpkEfactTrovaaltriallegatifatturaBean bean;
		  
	public void setBean(DmpkEfactTrovaaltriallegatifatturaBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkEfactTrovaaltriallegatifatturaBean> execute(AurigaLoginBean lLoginBean, DmpkEfactTrovaaltriallegatifatturaBean pBean) throws Exception{
	   final TrovaaltriallegatifatturaImpl lTrovaaltriallegatifattura = new TrovaaltriallegatifatturaImpl();
	   setBean(pBean);
	   lTrovaaltriallegatifattura.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lTrovaaltriallegatifattura.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkEfactTrovaaltriallegatifatturaBean> result = new StoreResultBean<DmpkEfactTrovaaltriallegatifatturaBean>();
