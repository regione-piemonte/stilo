...
import it.eng.auriga.database.store.dmpk_anagrafica.bean.DmpkAnagraficaDindluogosoggBean;
import it.eng.auriga.database.store.dmpk_anagrafica.store.impl.DindluogosoggImpl;
 ...
 */
@Service(name="DmpkAnagraficaDindluogosogg")
public class Dindluogosogg {
		
	private DmpkAnagraficaDindluogosoggBean bean;
		  
	public void setBean(DmpkAnagraficaDindluogosoggBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkAnagraficaDindluogosoggBean> execute(AurigaLoginBean lLoginBean, DmpkAnagraficaDindluogosoggBean pBean) throws Exception{
	   final DindluogosoggImpl lDindluogosogg = new DindluogosoggImpl();
	   setBean(pBean);
	   lDindluogosogg.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lDindluogosogg.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkAnagraficaDindluogosoggBean> result = new StoreResultBean<DmpkAnagraficaDindluogosoggBean>();
