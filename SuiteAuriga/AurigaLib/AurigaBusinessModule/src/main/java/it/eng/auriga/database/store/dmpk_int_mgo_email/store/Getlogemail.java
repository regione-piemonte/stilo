...
import it.eng.auriga.database.store.dmpk_int_mgo_email.store.impl.GetlogemailImpl;
 ...
 */
@Service(name="DmpkIntMgoEmailGetlogemail")
public class Getlogemail {
		
	private DmpkIntMgoEmailGetlogemailBean bean;
		  
	public void setBean(DmpkIntMgoEmailGetlogemailBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkIntMgoEmailGetlogemailBean> execute(AurigaLoginBean lLoginBean, DmpkIntMgoEmailGetlogemailBean pBean) throws Exception{
	   final GetlogemailImpl lGetlogemail = new GetlogemailImpl();
	   setBean(pBean);
	   lGetlogemail.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lGetlogemail.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkIntMgoEmailGetlogemailBean> result = new StoreResultBean<DmpkIntMgoEmailGetlogemailBean>();
