...
import it.eng.auriga.database.store.dmpk_ws.store.impl.DelverdocImpl;
 ...
 */
@Service(name="DmpkWsDelverdoc")
public class Delverdoc {
		
	private DmpkWsDelverdocBean bean;
		  
	public void setBean(DmpkWsDelverdocBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkWsDelverdocBean> execute(AurigaLoginBean lLoginBean, DmpkWsDelverdocBean pBean) throws Exception{
	   final DelverdocImpl lDelverdoc = new DelverdocImpl();
	   setBean(pBean);
	   lDelverdoc.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lDelverdoc.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkWsDelverdocBean> result = new StoreResultBean<DmpkWsDelverdocBean>();
