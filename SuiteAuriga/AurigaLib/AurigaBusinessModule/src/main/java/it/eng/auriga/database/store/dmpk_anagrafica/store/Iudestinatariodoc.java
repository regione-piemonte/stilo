...
import it.eng.auriga.database.store.dmpk_anagrafica.bean.DmpkAnagraficaIudestinatariodocBean;
import it.eng.auriga.database.store.dmpk_anagrafica.store.impl.IudestinatariodocImpl;
 ...
 */
@Service(name="DmpkAnagraficaIudestinatariodoc")
public class Iudestinatariodoc {
		
	private DmpkAnagraficaIudestinatariodocBean bean;
		  
	public void setBean(DmpkAnagraficaIudestinatariodocBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkAnagraficaIudestinatariodocBean> execute(AurigaLoginBean lLoginBean, DmpkAnagraficaIudestinatariodocBean pBean) throws Exception{
	   final IudestinatariodocImpl lIudestinatariodoc = new IudestinatariodocImpl();
	   setBean(pBean);
	   lIudestinatariodoc.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lIudestinatariodoc.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkAnagraficaIudestinatariodocBean> result = new StoreResultBean<DmpkAnagraficaIudestinatariodocBean>();
