...
import it.eng.auriga.database.store.dmpk_collaboration.bean.DmpkCollaborationSetstatoudBean;
import it.eng.auriga.database.store.dmpk_collaboration.store.impl.SetstatoudImpl;
 ...
 */
@Service(name="DmpkCollaborationSetstatoud")
public class Setstatoud {
		
	private DmpkCollaborationSetstatoudBean bean;
		  
	public void setBean(DmpkCollaborationSetstatoudBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkCollaborationSetstatoudBean> execute(AurigaLoginBean lLoginBean, DmpkCollaborationSetstatoudBean pBean) throws Exception{
	   final SetstatoudImpl lSetstatoud = new SetstatoudImpl();
	   setBean(pBean);
	   lSetstatoud.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lSetstatoud.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkCollaborationSetstatoudBean> result = new StoreResultBean<DmpkCollaborationSetstatoudBean>();
