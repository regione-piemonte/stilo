...
public class GetprofattivitafasewfImpl  {
		
	private DmpkProcessTypesGetprofattivitafasewfBean bean;
		  
	public void setBean(DmpkProcessTypesGetprofattivitafasewfBean bean){
		this.bean = bean;
	}
	...
	public void execute(Connection connection) throws SQLException {
		...
		try{
			...
			call = connection.prepareCall("{? = call DMPK_PROCESS_TYPES.GETPROFATTIVITAFASEWF(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}");			
			...
			util.settinParameterOnBean(call,bean,wrapperBean,"nroordinevisout",7,Types.INTEGER); 
			util.settinParameterOnBean(call,bean,wrapperBean,"xmlaclout",8,Types.CLOB); 
			util.settinParameterOnBean(call,bean,wrapperBean,"flgassabilout",9,Types.INTEGER); 
			util.settinParameterOnBean(call,bean,wrapperBean,"controlliobblvsfaseout",10,Types.VARCHAR); 
			util.settinParameterOnBean(call,bean,wrapperBean,"xmldettxesitoout",11,Types.CLOB); 
			util.settinParameterOnBean(call,bean,wrapperBean,"xmldesttrasmout",12,Types.CLOB); 
			util.settinParameterOnBean(call,bean,wrapperBean,"nomiattraddeditabiliout",13,Types.CLOB); 
