...
import it.eng.auriga.database.store.dmpk_gae.bean.DmpkGaeDsottoscrizionenotificamailBean;
import it.eng.auriga.database.store.dmpk_gae.store.impl.DsottoscrizionenotificamailImpl;
 ...
 */
@Service(name="DmpkGaeDsottoscrizionenotificamail")
public class Dsottoscrizionenotificamail {
		
	private DmpkGaeDsottoscrizionenotificamailBean bean;
		  
	public void setBean(DmpkGaeDsottoscrizionenotificamailBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkGaeDsottoscrizionenotificamailBean> execute(AurigaLoginBean lLoginBean, DmpkGaeDsottoscrizionenotificamailBean pBean) throws Exception{
	   final DsottoscrizionenotificamailImpl lDsottoscrizionenotificamail = new DsottoscrizionenotificamailImpl();
	   setBean(pBean);
	   lDsottoscrizionenotificamail.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lDsottoscrizionenotificamail.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkGaeDsottoscrizionenotificamailBean> result = new StoreResultBean<DmpkGaeDsottoscrizionenotificamailBean>();
