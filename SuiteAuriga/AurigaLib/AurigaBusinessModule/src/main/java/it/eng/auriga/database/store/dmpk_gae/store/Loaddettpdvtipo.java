...
import it.eng.auriga.database.store.dmpk_gae.bean.DmpkGaeLoaddettpdvtipoBean;
import it.eng.auriga.database.store.dmpk_gae.store.impl.LoaddettpdvtipoImpl;
 ...
 */
@Service(name="DmpkGaeLoaddettpdvtipo")
public class Loaddettpdvtipo {
		
	private DmpkGaeLoaddettpdvtipoBean bean;
		  
	public void setBean(DmpkGaeLoaddettpdvtipoBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkGaeLoaddettpdvtipoBean> execute(AurigaLoginBean lLoginBean, DmpkGaeLoaddettpdvtipoBean pBean) throws Exception{
	   final LoaddettpdvtipoImpl lLoaddettpdvtipo = new LoaddettpdvtipoImpl();
	   setBean(pBean);
	   lLoaddettpdvtipo.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lLoaddettpdvtipo.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkGaeLoaddettpdvtipoBean> result = new StoreResultBean<DmpkGaeLoaddettpdvtipoBean>();
