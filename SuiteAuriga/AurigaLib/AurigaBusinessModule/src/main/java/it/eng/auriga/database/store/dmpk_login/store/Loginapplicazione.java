...
import it.eng.auriga.database.store.dmpk_login.store.impl.LoginapplicazioneImpl;
 ...
 */
@Service(name="DmpkLoginLoginapplicazione")
public class Loginapplicazione {
	...
	public StoreResultBean<DmpkLoginLoginapplicazioneBean> execute(AurigaLoginBean lLoginBean, DmpkLoginLoginapplicazioneBean pBean) throws Exception{
	   final LoginapplicazioneImpl lLoginapplicazione = new LoginapplicazioneImpl();
	   setBean(pBean);
	   lLoginapplicazione.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lLoginapplicazione.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkLoginLoginapplicazioneBean> result = new StoreResultBean<DmpkLoginLoginapplicazioneBean>();
