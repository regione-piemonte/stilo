...
import it.eng.auriga.database.store.dmpk_int_cs.store.impl.TrovapdvImpl;
 ...
 */
@Service(name="DmpkIntCsTrovapdv")
public class Trovapdv {
		
	private DmpkIntCsTrovapdvBean bean;
		  
	public void setBean(DmpkIntCsTrovapdvBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkIntCsTrovapdvBean> execute(AurigaLoginBean lLoginBean, DmpkIntCsTrovapdvBean pBean) throws Exception{
	   final TrovapdvImpl lTrovapdv = new TrovapdvImpl();
	   setBean(pBean);
	   lTrovapdv.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lTrovapdv.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkIntCsTrovapdvBean> result = new StoreResultBean<DmpkIntCsTrovapdvBean>();
