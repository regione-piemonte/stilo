...
import it.eng.auriga.database.store.dmpk_modelli_doc.store.impl.IumodelloImpl;
 ...
 */
@Service(name="DmpkModelliDocIumodello")
public class Iumodello {
	...
	public StoreResultBean<DmpkModelliDocIumodelloBean> execute(AurigaLoginBean lLoginBean, DmpkModelliDocIumodelloBean pBean) throws Exception{
	   final IumodelloImpl lIumodello = new IumodelloImpl();
	   setBean(pBean);
	   lIumodello.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lIumodello.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkModelliDocIumodelloBean> result = new StoreResultBean<DmpkModelliDocIumodelloBean>();
