...
import it.eng.auriga.database.store.dmpk_process_types.bean.DmpkProcessTypesDattoreproctypeBean;
import it.eng.auriga.database.store.dmpk_process_types.store.impl.DattoreproctypeImpl;
 ...
 */
@Service(name="DmpkProcessTypesDattoreproctype")
public class Dattoreproctype {
		
	private DmpkProcessTypesDattoreproctypeBean bean;
		  
	public void setBean(DmpkProcessTypesDattoreproctypeBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkProcessTypesDattoreproctypeBean> execute(AurigaLoginBean lLoginBean, DmpkProcessTypesDattoreproctypeBean pBean) throws Exception{
	   final DattoreproctypeImpl lDattoreproctype = new DattoreproctypeImpl();
	   setBean(pBean);
	   lDattoreproctype.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lDattoreproctype.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkProcessTypesDattoreproctypeBean> result = new StoreResultBean<DmpkProcessTypesDattoreproctypeBean>();
