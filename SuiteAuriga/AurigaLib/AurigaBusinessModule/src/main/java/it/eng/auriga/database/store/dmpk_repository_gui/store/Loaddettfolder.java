...
import it.eng.auriga.database.store.dmpk_repository_gui.store.impl.LoaddettfolderImpl;
 ...
 */
@Service(name="DmpkRepositoryGuiLoaddettfolder")
public class Loaddettfolder {
	...
	public StoreResultBean<DmpkRepositoryGuiLoaddettfolderBean> execute(AurigaLoginBean lLoginBean, DmpkRepositoryGuiLoaddettfolderBean pBean) throws Exception{
	   final LoaddettfolderImpl lLoaddettfolder = new LoaddettfolderImpl();
	   setBean(pBean);
	   lLoaddettfolder.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lLoaddettfolder.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkRepositoryGuiLoaddettfolderBean> result = new StoreResultBean<DmpkRepositoryGuiLoaddettfolderBean>();
