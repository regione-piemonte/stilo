...
import it.eng.auriga.database.store.dmpk_core.bean.DmpkCoreRecoverfolderBean;
import it.eng.auriga.database.store.dmpk_core.store.impl.RecoverfolderImpl;
 ...
 */
@Service(name="DmpkCoreRecoverfolder")
public class Recoverfolder {
		
	private DmpkCoreRecoverfolderBean bean;
		  
	public void setBean(DmpkCoreRecoverfolderBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkCoreRecoverfolderBean> execute(AurigaLoginBean lLoginBean, DmpkCoreRecoverfolderBean pBean) throws Exception{
	   final RecoverfolderImpl lRecoverfolder = new RecoverfolderImpl();
	   setBean(pBean);
	   lRecoverfolder.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lRecoverfolder.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkCoreRecoverfolderBean> result = new StoreResultBean<DmpkCoreRecoverfolderBean>();
