...
import it.eng.auriga.database.store.dmpk_dizionario.store.impl.DdictionaryvalueImpl;
 ...
 */
@Service(name="DmpkDizionarioDdictionaryvalue")
public class Ddictionaryvalue {
		
	private DmpkDizionarioDdictionaryvalueBean bean;
		  
	public void setBean(DmpkDizionarioDdictionaryvalueBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkDizionarioDdictionaryvalueBean> execute(AurigaLoginBean lLoginBean, DmpkDizionarioDdictionaryvalueBean pBean) throws Exception{
	   final DdictionaryvalueImpl lDdictionaryvalue = new DdictionaryvalueImpl();
	   setBean(pBean);
	   lDdictionaryvalue.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lDdictionaryvalue.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkDizionarioDdictionaryvalueBean> result = new StoreResultBean<DmpkDizionarioDdictionaryvalueBean>();
