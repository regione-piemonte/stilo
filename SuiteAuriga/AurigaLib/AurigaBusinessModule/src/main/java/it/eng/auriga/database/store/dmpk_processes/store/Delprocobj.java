...
import it.eng.auriga.database.store.dmpk_processes.bean.DmpkProcessesDelprocobjBean;
import it.eng.auriga.database.store.dmpk_processes.store.impl.DelprocobjImpl;
 ...
 */
@Service(name="DmpkProcessesDelprocobj")
public class Delprocobj {
		
	private DmpkProcessesDelprocobjBean bean;
		  
	public void setBean(DmpkProcessesDelprocobjBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkProcessesDelprocobjBean> execute(AurigaLoginBean lLoginBean, DmpkProcessesDelprocobjBean pBean) throws Exception{
	   final DelprocobjImpl lDelprocobj = new DelprocobjImpl();
	   setBean(pBean);
	   lDelprocobj.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lDelprocobj.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkProcessesDelprocobjBean> result = new StoreResultBean<DmpkProcessesDelprocobjBean>();
