...
import it.eng.auriga.database.store.dmpk_ws.store.impl.GetdatiapplicazioneesternaImpl;
 ...
 */
@Service(name="DmpkWsGetdatiapplicazioneesterna")
public class Getdatiapplicazioneesterna {
		
	private DmpkWsGetdatiapplicazioneesternaBean bean;
		  
	public void setBean(DmpkWsGetdatiapplicazioneesternaBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkWsGetdatiapplicazioneesternaBean> execute(AurigaLoginBean lLoginBean, DmpkWsGetdatiapplicazioneesternaBean pBean) throws Exception{
	   final GetdatiapplicazioneesternaImpl lGetdatiapplicazioneesterna = new GetdatiapplicazioneesternaImpl();
	   setBean(pBean);
	   lGetdatiapplicazioneesterna.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lGetdatiapplicazioneesterna.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkWsGetdatiapplicazioneesternaBean> result = new StoreResultBean<DmpkWsGetdatiapplicazioneesternaBean>();
