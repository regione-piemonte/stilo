...
import it.eng.auriga.database.store.dmpk_utility.bean.DmpkUtilityConverttofmtstddataBean;
import it.eng.auriga.database.store.dmpk_utility.store.impl.ConverttofmtstddataImpl;
 ...
 */
@Service(name="DmpkUtilityConverttofmtstddata")
public class Converttofmtstddata {
		
	private DmpkUtilityConverttofmtstddataBean bean;
		  
	public void setBean(DmpkUtilityConverttofmtstddataBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkUtilityConverttofmtstddataBean> execute(AurigaLoginBean lLoginBean, DmpkUtilityConverttofmtstddataBean pBean) throws Exception{
	   final ConverttofmtstddataImpl lConverttofmtstddata = new ConverttofmtstddataImpl();
	   setBean(pBean);
	   lConverttofmtstddata.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lConverttofmtstddata.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkUtilityConverttofmtstddataBean> result = new StoreResultBean<DmpkUtilityConverttofmtstddataBean>();
