...
import it.eng.auriga.database.store.dmpk_ws.store.impl.LockcheckoutImpl;
 ...
 */
@Service(name="DmpkWsLockcheckout")
public class Lockcheckout {
	...
	public StoreResultBean<DmpkWsLockcheckoutBean> execute(AurigaLoginBean lLoginBean, DmpkWsLockcheckoutBean pBean) throws Exception{
	   final LockcheckoutImpl lLockcheckout = new LockcheckoutImpl();
	   setBean(pBean);
	   lLockcheckout.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lLockcheckout.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkWsLockcheckoutBean> result = new StoreResultBean<DmpkWsLockcheckoutBean>();
