...
import it.eng.auriga.database.store.dmpk_def_security.store.impl.DreluouserImpl;
 ...
 */
@Service(name="DmpkDefSecurityDreluouser")
public class Dreluouser {
	...
	public StoreResultBean<DmpkDefSecurityDreluouserBean> execute(AurigaLoginBean lLoginBean, DmpkDefSecurityDreluouserBean pBean) throws Exception{
	   final DreluouserImpl lDreluouser = new DreluouserImpl();
	   setBean(pBean);
	   lDreluouser.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lDreluouser.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkDefSecurityDreluouserBean> result = new StoreResultBean<DmpkDefSecurityDreluouserBean>();
