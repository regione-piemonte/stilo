...
import it.eng.auriga.database.store.dmpk_process_types.bean.DmpkProcessTypesDeventoderivatoBean;
import it.eng.auriga.database.store.dmpk_process_types.store.impl.DeventoderivatoImpl;
 ...
 */
@Service(name="DmpkProcessTypesDeventoderivato")
public class Deventoderivato {
		
	private DmpkProcessTypesDeventoderivatoBean bean;
		  
	public void setBean(DmpkProcessTypesDeventoderivatoBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkProcessTypesDeventoderivatoBean> execute(AurigaLoginBean lLoginBean, DmpkProcessTypesDeventoderivatoBean pBean) throws Exception{
	   final DeventoderivatoImpl lDeventoderivato = new DeventoderivatoImpl();
	   setBean(pBean);
	   lDeventoderivato.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lDeventoderivato.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkProcessTypesDeventoderivatoBean> result = new StoreResultBean<DmpkProcessTypesDeventoderivatoBean>();
