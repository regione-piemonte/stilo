...
import it.eng.auriga.database.store.dmpk_wf.bean.DmpkWfDecodeesitoattivitaBean;
import it.eng.auriga.database.store.dmpk_wf.store.impl.DecodeesitoattivitaImpl;
 ...
 */
@Service(name="DmpkWfDecodeesitoattivita")
public class Decodeesitoattivita {
		
	private DmpkWfDecodeesitoattivitaBean bean;
		  
	public void setBean(DmpkWfDecodeesitoattivitaBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkWfDecodeesitoattivitaBean> execute(AurigaLoginBean lLoginBean, DmpkWfDecodeesitoattivitaBean pBean) throws Exception{
	   final DecodeesitoattivitaImpl lDecodeesitoattivita = new DecodeesitoattivitaImpl();
	   setBean(pBean);
	   lDecodeesitoattivita.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lDecodeesitoattivita.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkWfDecodeesitoattivitaBean> result = new StoreResultBean<DmpkWfDecodeesitoattivitaBean>();
