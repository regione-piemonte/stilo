...
import it.eng.auriga.database.store.dmpk_core.bean.DmpkCoreUpdworkspaceBean;
import it.eng.auriga.database.store.dmpk_core.store.impl.UpdworkspaceImpl;
 ...
 */
@Service(name="DmpkCoreUpdworkspace")
public class Updworkspace {
		
	private DmpkCoreUpdworkspaceBean bean;
		  
	public void setBean(DmpkCoreUpdworkspaceBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkCoreUpdworkspaceBean> execute(AurigaLoginBean lLoginBean, DmpkCoreUpdworkspaceBean pBean) throws Exception{
	   final UpdworkspaceImpl lUpdworkspace = new UpdworkspaceImpl();
	   setBean(pBean);
	   lUpdworkspace.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lUpdworkspace.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkCoreUpdworkspaceBean> result = new StoreResultBean<DmpkCoreUpdworkspaceBean>();
