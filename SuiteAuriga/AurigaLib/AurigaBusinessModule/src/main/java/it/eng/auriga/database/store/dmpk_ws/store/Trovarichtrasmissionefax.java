...
import it.eng.auriga.database.store.dmpk_ws.bean.DmpkWsTrovarichtrasmissionefaxBean;
import it.eng.auriga.database.store.dmpk_ws.store.impl.TrovarichtrasmissionefaxImpl;
 ...
 */
@Service(name="DmpkWsTrovarichtrasmissionefax")
public class Trovarichtrasmissionefax {
		
	private DmpkWsTrovarichtrasmissionefaxBean bean;
		  
	public void setBean(DmpkWsTrovarichtrasmissionefaxBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkWsTrovarichtrasmissionefaxBean> execute(AurigaLoginBean lLoginBean, DmpkWsTrovarichtrasmissionefaxBean pBean) throws Exception{
	   final TrovarichtrasmissionefaxImpl lTrovarichtrasmissionefax = new TrovarichtrasmissionefaxImpl();
	   setBean(pBean);
	   lTrovarichtrasmissionefax.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lTrovarichtrasmissionefax.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkWsTrovarichtrasmissionefaxBean> result = new StoreResultBean<DmpkWsTrovarichtrasmissionefaxBean>();
