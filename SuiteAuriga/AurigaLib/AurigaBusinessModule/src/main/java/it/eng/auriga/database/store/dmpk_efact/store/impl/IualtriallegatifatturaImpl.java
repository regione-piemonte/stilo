...
public class IualtriallegatifatturaImpl  {
	...
	public void execute(Connection connection) throws SQLException {
		...
		try{
			...
			call = connection.prepareCall("{? = call DMPK_EFACT.IUALTRIALLEGATIFATTURA(?,?,?,?,?,?,?,?,?,?,?,?)}");			
			...
			util.settingParameterOnStore(call,bean,wrapperBean,"idkeyio",4,Types.DECIMAL,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"attributixmlin",5,Types.CLOB,connection); 	
			util.settingParameterOnStore(call,bean,wrapperBean,"xmlvaloriin",6,Types.CLOB,connection); 	
			...
			util.settinParameterOnBean(call,bean,wrapperBean,"idkeyio",4,Types.DECIMAL); 
