...
package it.eng.auriga.database.store.dmpk_credential_vault.store;

import it.eng.auriga.database.store.dmpk_credential_vault.bean.DmpkCredentialVaultRegistrazionecredenzialiBean;
import it.eng.auriga.database.store.dmpk_credential_vault.store.impl.RegistrazionecredenzialiImpl;
 ...
 */
@Service(name="DmpkCredentialVaultRegistrazionecredenziali")
public class Registrazionecredenziali {
		
	private DmpkCredentialVaultRegistrazionecredenzialiBean bean;
		  
	public void setBean(DmpkCredentialVaultRegistrazionecredenzialiBean bean){
		this.bean = bean;
	}
	
	@Operation(name="execute")
	public StoreResultBean<DmpkCredentialVaultRegistrazionecredenzialiBean> execute(AurigaLoginBean lLoginBean, DmpkCredentialVaultRegistrazionecredenzialiBean pBean) throws Exception{
	   final RegistrazionecredenzialiImpl lRegistrazionecredenziali = new RegistrazionecredenzialiImpl();
	   setBean(pBean);
	   lRegistrazionecredenziali.setBean(bean);
       setCommit();
       setToken(lLoginBean);
		...
		try {
			...
			session.doWork(new Work() {
				...
				public void execute(Connection paramConnection) throws SQLException {
					...
					lRegistrazionecredenziali.execute(paramConnection);
				}
			});
			StoreResultBean<DmpkCredentialVaultRegistrazionecredenzialiBean> result = new StoreResultBean<DmpkCredentialVaultRegistrazionecredenzialiBean>();
