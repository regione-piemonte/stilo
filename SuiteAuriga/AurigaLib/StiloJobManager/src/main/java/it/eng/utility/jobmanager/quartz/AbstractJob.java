...
@Named
public abstract class AbstractJob<T> extends AbstractBatchlet {
...
//				(String) getAttribute(JOBATTRKEY_TOKENID), (String) getAttribute(JOBATTRKEY_UUIDTRANSACTION));
	...
	/**
	 * Crea la chiave di ricerca tramite la quale poter gestire l'mbean associato al
	 * job, utilizzando il name specificato nel file di configurazione
	 * 
	 * @param job
	 * @return
	 */
	...
	/**
	 * Esegue le operazioni di chiusura del job, anche se il suo execute è terminato
	 * con errore
	 * 
	 * @param obj
	 */
	 ...
	 * 
	 * @param key la chiave dell'attributo
	...
	public ObjectName getMBeanIdentifier() {
		...
		try {
			name = new ObjectName("it.csi:type=" + jobType);
		} catch (Exception e) {
			log.error(
	...
	/**
	 * Permette di richiedere lo shutdown automatico del job, una volta che tutti i
	 * thread hanno completato la loro esecuzione, ovvero tutti quanti hanno
	...
	/**
	 * Settaggio iniziale di alcune variabili per evitare un nullPointerException
	 * nella fase di login
	 * 
	 * Nel caso in cui la business NON sia integrata questo passaggio non serve
	 * percha l'inizializzazione del Subject viene eseguita automaticamente dalla
	 * Business. Nel caso in cui la Business è integrata questo passaggio di
	 * inizializzazione è necessario per evitare un nullPointerException.
	 * 
	 * Questo passaggio deve essere posto in un punto diverso dal login percha a
	 * volte si interagisce con il DB direttamente, senza passare appunto per il
	 * login; ponendolo nel load del job in un caso o nell'altro si riesce ad
	 * eseguire il login richiesto.
	 */
...
//
//			/*
//			 * Settaggio del subject. Se nessuno dei parametri precedenti a stato fornito allora il subject viene settato come nullo.
//			 */
...
//	}
	...
	/**
	 * metodo generico del protocollo JSR-352 per essere eseguito sotto jboss
	 */
	@Override
