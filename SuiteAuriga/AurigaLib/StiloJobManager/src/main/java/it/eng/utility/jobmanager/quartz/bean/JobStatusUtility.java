...
public class JobStatusUtility {
	 ...
	 * @param jobType 
	 */
	public static synchronized void createMBean(String jobName, String jobType) {
		...
		try {
			JobExecutionStatus mbean = new JobExecutionStatus(jobName, jobType);
	...
	public static synchronized ObjectName createName(String jobName) {
		...
		try {
			name = new ObjectName("it.csi.ade.job:name=" + jobName);
		} catch (Exception e) {
			log.error(
