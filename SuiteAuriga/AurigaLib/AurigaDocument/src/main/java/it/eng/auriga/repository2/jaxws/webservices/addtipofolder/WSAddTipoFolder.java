...
package it.eng.auriga.repository2.jaxws.webservices.addtipofolder;

import it.eng.auriga.database.store.dmpk_ws.bean.DmpkWsAddtipofolderBean;
import it.eng.auriga.database.store.dmpk_ws.store.Addtipofolder;
...
@WebService(targetNamespace = "http://addtipofolder.webservices.repository2.auriga.eng.it",  endpointInterface="it.eng.auriga.repository2.jaxws.webservices.addtipofolder.WSIAddTipoFolder", name = "WSAddTipoFolder")
@MTOM(enabled = true, threshold = 0)

public class WSAddTipoFolder extends JAXWSAbstractAurigaService implements WSIAddTipoFolder{

    private final String K_SAVEPOINTNAME = "INIZIOWSADDTIPOFOLDER";
   
    static Logger aLogger = Logger.getLogger(WSAddTipoFolder.class.getName());    
    
    public WSAddTipoFolder() {
	super();
    ...
    try {
    	 aLogger.info("Inizio WSAddTipoFolder");
		 ...
		 **************************************************************************/
	     ...
	     aLogger.info("Fine WSAddTipoFolder");
	    
	     return risposta;
    }
  
    catch (Exception excptn) {
        aLogger.error("WSAddTipoFolder: " + excptn.getMessage(), excptn);
	...
	finally
	{
	    ...
	    aLogger.info("Fine WSAddTipoFolder serviceImplementation");
	}

    }
        
    private String callWS(AurigaLoginBean loginBean, String xmlIn) throws Exception {
    	aLogger.debug("Eseguo il WS DMPK_WS->AddTipoFolder.");
    	
    	String result = null;
    	    		
    	// Inizializzo l'INPUT    		
    	DmpkWsAddtipofolderBean input = new DmpkWsAddtipofolderBean();
    	input.setCodidconnectiontokenin(loginBean.getToken());
    	input.setXmlin(xmlIn);
	  
    	// Eseguo il servizio
    	Addtipofolder service = new Addtipofolder();
    	StoreResultBean<DmpkWsAddtipofolderBean> output = service.execute(loginBean, input);
    	...
    	if (result== null || result.equalsIgnoreCase(""))
		  throw new Exception("La store procedure ha ritornato IdFolder nullo");
