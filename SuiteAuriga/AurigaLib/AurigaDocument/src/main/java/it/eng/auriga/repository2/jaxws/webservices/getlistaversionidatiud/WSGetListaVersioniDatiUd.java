...
package it.eng.auriga.repository2.jaxws.webservices.getlistaversionidatiud;

import it.eng.auriga.database.store.dmpk_ws.bean.DmpkWsGetlistaversionidatiudBean;
import it.eng.auriga.database.store.dmpk_ws.store.Getlistaversionidatiud;
...
@WebService(targetNamespace = "http://getlistaversionidatiud.webservices.repository2.auriga.eng.it",  endpointInterface="it.eng.auriga.repository2.jaxws.webservices.getlistaversionidatiud.WSIGetListaVersioniDatiUd", name = "WSGetListaVersioniDatiUd")
@MTOM(enabled = true, threshold = 0)

public class WSGetListaVersioniDatiUd extends JAXWSAbstractAurigaService implements WSIGetListaVersioniDatiUd{

    private final String K_SAVEPOINTNAME = "INIZIO_WSGetListaVerDatiUd";
   
    static Logger aLogger = Logger.getLogger(WSGetListaVersioniDatiUd.class.getName());    
    
    public WSGetListaVersioniDatiUd() {
	super();
    ...
    try {
    	 aLogger.info("Inizio WSGetListaVersioniDatiUd");
		 ...
		 **************************************************************************/
	     ...
	     aLogger.info("Fine WSGetListaVersioniDatiUd");
	    
	     return risposta;
    }
  
    catch (Exception excptn) {
        aLogger.error("WSGetListaVersioniDatiUd: " + excptn.getMessage(), excptn);
	...
	finally
	{
	    ...
	    aLogger.info("Fine WSGetListaVersioniDatiUd serviceImplementation");
	}
    }

        
    private String callWS(AurigaLoginBean loginBean, String xmlIn) throws Exception {    	    
    	aLogger.debug("Eseguo il WS DMPK_WS->GetListaVersioniDatiUd.");
    	
    	String result = null;
    	
    	// Inizializzo l'INPUT    		
    	DmpkWsGetlistaversionidatiudBean input = new DmpkWsGetlistaversionidatiudBean();
    	input.setCodidconnectiontokenin(loginBean.getToken());
    	input.setXmlin(xmlIn);
	  
    	// Eseguo il servizio
    	Getlistaversionidatiud service = new Getlistaversionidatiud();
    	StoreResultBean<DmpkWsGetlistaversionidatiudBean> output = service.execute(loginBean, input);
