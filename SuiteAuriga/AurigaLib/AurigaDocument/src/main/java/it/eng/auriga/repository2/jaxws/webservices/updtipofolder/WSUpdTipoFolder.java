...
package it.eng.auriga.repository2.jaxws.webservices.updtipofolder;

import it.eng.auriga.database.store.dmpk_ws.bean.DmpkWsUpdtipofolderBean;
import it.eng.auriga.database.store.dmpk_ws.store.Updtipofolder;
...
@WebService(targetNamespace = "http://updtipofolder.webservices.repository2.auriga.eng.it", endpointInterface="it.eng.auriga.repository2.jaxws.webservices.updtipofolder.WSIUpdTipoFolder", name = "WSUpdTipoFolder")
@MTOM(enabled = true, threshold = 0)

public class WSUpdTipoFolder extends JAXWSAbstractAurigaService implements WSIUpdTipoFolder{

    private final String K_SAVEPOINTNAME = "INIZIOWSUPDTIPOFOLDER";
   
    static Logger aLogger = Logger.getLogger(WSUpdTipoFolder.class.getName());    
    
   
    public WSUpdTipoFolder() {
	super();
    ...
    try {
    	 aLogger.info("Inizio WSUpdTipoFolder");
		 ...
		 **************************************************************************/
	     ...
	     aLogger.info("Fine WSUpdTipoFolder");
	    
	     return risposta;
    }
  
    catch (Exception excptn) {
        aLogger.error("WSUpdTipoFolder: " + excptn.getMessage(), excptn);
	...
	finally
	{
	    ...
	    aLogger.info("Fine WSUpdTipoFolder serviceImplementation");
	}
    }

        
    private String callWS(AurigaLoginBean loginBean, String xmlIn) throws Exception {
    	aLogger.debug("Eseguo il WS DMPK_WS->UpdTipoFolder.");
    	
    	String result = null;    	
    	
    	// Inizializzo l'INPUT    		
    	DmpkWsUpdtipofolderBean input = new DmpkWsUpdtipofolderBean();
    	input.setCodidconnectiontokenin(loginBean.getToken());
    	input.setXmlin(xmlIn);
	  
    	// Eseguo il servizio
    	Updtipofolder service = new Updtipofolder();
    	StoreResultBean<DmpkWsUpdtipofolderBean> output = service.execute(loginBean, input);
