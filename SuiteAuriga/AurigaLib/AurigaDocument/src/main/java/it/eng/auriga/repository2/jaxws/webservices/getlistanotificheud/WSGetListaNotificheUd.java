...
package it.eng.auriga.repository2.jaxws.webservices.getlistanotificheud;

import it.eng.auriga.database.store.dmpk_ws.bean.DmpkWsGetlistanotificheudBean;
import it.eng.auriga.database.store.dmpk_ws.store.Getlistanotificheud;
...
@WebService(targetNamespace = "http://getlistanotificheud.webservices.repository2.auriga.eng.it",  endpointInterface="it.eng.auriga.repository2.jaxws.webservices.getlistanotificheud.WSIGetListaNotificheUd", name = "WSGetListaNotificheUd")
@MTOM(enabled = true, threshold = 0)

public class WSGetListaNotificheUd extends JAXWSAbstractAurigaService implements WSIGetListaNotificheUd{

    private final String K_SAVEPOINTNAME = "INIZIO_WSGetListaNotificheUd";
   
    static Logger aLogger = Logger.getLogger(WSGetListaNotificheUd.class.getName());    
    
    public WSGetListaNotificheUd() {
	super();
    ...
    try {
    	 aLogger.info("Inizio WSGetListaNotificheUd");
		 ...
		 **************************************************************************/
	     ...
	     aLogger.info("Fine WSGetListaNotificheUd");
	    
	     return risposta;
    }
  
    catch (Exception excptn) {
        aLogger.error("WSGetListaNotificheUd: " + excptn.getMessage(), excptn);
	...
	finally
	{
	    ...
	    aLogger.info("Fine WSGetListaNotificheUd serviceImplementation");
	}
    }

        
    private String callWS(AurigaLoginBean loginBean, String xmlIn) throws Exception {    	    
    	aLogger.debug("Eseguo il WS DMPK_WS->GetListaNotificheUd.");
    	
    	String result = null;    	
    	    		
	    // Inizializzo l'INPUT    		
	    DmpkWsGetlistanotificheudBean input = new DmpkWsGetlistanotificheudBean();
	    input.setCodidconnectiontokenin(loginBean.getToken());
	    input.setXmlin(xmlIn);
	  
	    // Eseguo il servizio
	    Getlistanotificheud service = new Getlistanotificheud();
	    StoreResultBean<DmpkWsGetlistanotificheudBean> output = service.execute(loginBean, input);
