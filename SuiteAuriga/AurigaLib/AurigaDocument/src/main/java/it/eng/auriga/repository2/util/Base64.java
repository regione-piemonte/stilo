...
public class Base64
    ...
    * logger	 
    */
   static Logger aLogger = Logger.getLogger(Base64.class.getName());    
   ...
   public  String encode(byte[] b)
      ...
      if ( outputLength != sb.length() )
         {
         aLogger.debug("oops: minor program flaw: output length mis-estimated");
         aLogger.debug("estimate:" + outputLength);
         aLogger.debug("actual:" + sb.length());
         }
      return sb.toString();
      }// end encode
   ...
   public static void show (byte[] b)
      ...
      for ( int i=0; i<b.length; i++ )
         {
         aLogger.debug( Integer.toHexString(b[i] & 0xff) + " ");
         }
      aLogger.debug("");
      }
