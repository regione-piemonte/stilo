...
package it.eng.auriga.repository2.jaxws.webservices.addudtofavourite;
...
@WebService(targetNamespace = "http://addudtofavourite.webservices.repository2.auriga.eng.it",  endpointInterface="it.eng.auriga.repository2.jaxws.webservices.addudtofavourite.WSIAddUdToFavourite", name = "WSAddUdToFavourite")
@MTOM(enabled = true, threshold = 0)

public class WSAddUdToFavourite extends JAXWSAbstractAurigaService implements WSIAddUdToFavourite{

    private final String K_SAVEPOINTNAME = "INIZIO_WSAddUdToFavourite";
   
    static Logger aLogger = Logger.getLogger(WSAddUdToFavourite.class.getName());    
    
    public WSAddUdToFavourite() {
	super();
    ...
    try {
    	 aLogger.info("Inizio WSAddUdToFavourite");
		 ...
		 **************************************************************************/
	     ...
	     aLogger.info("Fine WSAddUdToFavourite");
	    
	     return risposta;
    }
  
    catch (Exception excptn) {
        aLogger.error("WSAddUdToFavourite: " + excptn.getMessage(), excptn);
	...
	finally
	{
	    ...
	    aLogger.info("Fine WSAddUdToFavourite serviceImplementation");
	}
    }

    private String callWS(AurigaLoginBean loginBean, String xmlIn) throws Exception {    	    
    	aLogger.debug("Eseguo il WS DMPK_WS->AddUdToFavourite.");    	
