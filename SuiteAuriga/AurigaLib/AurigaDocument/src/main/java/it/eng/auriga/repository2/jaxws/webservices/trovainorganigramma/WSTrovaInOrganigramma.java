...
package it.eng.auriga.repository2.jaxws.webservices.trovainorganigramma;

import it.eng.auriga.database.store.dmpk_ws.bean.DmpkWsTrovainorganigrammaBean;
import it.eng.auriga.database.store.dmpk_ws.store.Trovainorganigramma;
...
@WebService(targetNamespace = "http://trovainorganigramma.webservices.repository2.auriga.eng.it",  endpointInterface="it.eng.auriga.repository2.jaxws.webservices.trovainorganigramma.WSITrovaInOrganigramma", name = "WSTrovaInOrganigramma")
@MTOM(enabled = true, threshold = 0)

public class WSTrovaInOrganigramma extends JAXWSAbstractAurigaService implements WSITrovaInOrganigramma{

    private final String K_SAVEPOINTNAME = "INIZIO_WSTrovaInOrganigramma";
   
    static Logger aLogger = Logger.getLogger(WSTrovaInOrganigramma.class.getName());    
    
    public WSTrovaInOrganigramma() {
	super();
    ...
    try {
    	 aLogger.info("Inizio WSTrovaInOrganigramma");
		 ...
		 **************************************************************************/
	     ...
	     aLogger.info("Fine WSTrovaInOrganigramma");
	    
	     return risposta;
    }
  
    catch (Exception excptn) {
        aLogger.error("WSTrovaInOrganigramma: " + excptn.getMessage(), excptn);
	...
	finally
	{
	    ...
	    aLogger.info("Fine WSTrovaInOrganigramma serviceImplementation");
	}
    }

        
    private String callWS(AurigaLoginBean loginBean, String xmlIn) throws Exception {    	    
    	aLogger.debug("Eseguo il WS DMPK_WS->TrovaInOrganigramma.");    	
    	
    	String result = null;
    		
    	// Inizializzo l'INPUT    		
    	DmpkWsTrovainorganigrammaBean input = new DmpkWsTrovainorganigrammaBean();
    	input.setCodidconnectiontokenin(loginBean.getToken());
    	input.setXmlin(xmlIn);
	  
    	// Eseguo il servizio
    	Trovainorganigramma service = new Trovainorganigramma();
    	StoreResultBean<DmpkWsTrovainorganigrammaBean> output = service.execute(loginBean, input);
