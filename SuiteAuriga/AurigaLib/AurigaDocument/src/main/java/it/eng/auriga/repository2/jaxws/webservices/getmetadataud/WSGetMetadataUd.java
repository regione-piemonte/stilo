...
import it.eng.auriga.database.store.dmpk_ws.bean.DmpkWsGetdatiudBean;
import it.eng.auriga.database.store.dmpk_ws.store.Getdatiud;
...
@WebService(targetNamespace = "http://getmetadataud.webservices.repository2.auriga.eng.it",  endpointInterface="it.eng.auriga.repository2.jaxws.webservices.getmetadataud.WSIGetMetadataUd", name = "WSGetMetadataUd")
@MTOM(enabled = true, threshold = 0)

public class WSGetMetadataUd extends JAXWSAbstractAurigaService implements WSIGetMetadataUd{

    private final String K_SAVEPOINTNAME = "INIZIO_WSGetMetadataUd";
   
    static Logger aLogger = Logger.getLogger(WSGetMetadataUd.class.getName());    
    
    public WSGetMetadataUd() {
	super();
    ...
    try {
        ...
        aLogger.error("WSGetMetadataUd: " + excptn.getMessage(), excptn);
	...
	finally
	{
	    ...
	    aLogger.info("Fine WSGetMetadataUd serviceImplementation");
	}
    }
