...
@WebService(targetNamespace = "http://addfoldertofavourite.webservices.repository2.auriga.eng.it",  endpointInterface="it.eng.auriga.repository2.jaxws.webservices.addfoldertofavourite.WSIAddFolderToFavourite", name = "WSAddFolderToFavourite")
@MTOM(enabled = true, threshold = 0)

public class WSAddFolderToFavourite extends JAXWSAbstractAurigaService implements WSIAddFolderToFavourite{

    private final String K_SAVEPOINTNAME = "INIZIO_WSAddFolderToFavourite";
   
    static Logger aLogger = Logger.getLogger(WSAddFolderToFavourite.class.getName());    
    
    public WSAddFolderToFavourite() {
	super();
    ...
    try {
    	 aLogger.info("Inizio WSAddFolderToFavourite");
		 ...
		 **************************************************************************/
	     ...
	     aLogger.info("Fine WSAddFolderToFavourite");
	    
	     return risposta;
    }
  
    catch (Exception excptn) {
        aLogger.error("WSAddFolderToFavourite: " + excptn.getMessage(), excptn);
	...
	finally
	{
	    ...
	    aLogger.info("Fine WSAddFolderToFavourite serviceImplementation");
	}
    }

    private String callWS(AurigaLoginBean loginBean, String xmlIn) throws Exception { 
    	aLogger.debug("Eseguo il WS DMPK_WS->AddFolderToFavourite."); 
