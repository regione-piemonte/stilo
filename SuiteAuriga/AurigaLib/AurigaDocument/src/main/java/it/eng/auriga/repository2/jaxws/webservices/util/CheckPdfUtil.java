...
//			reader.unethicalreading = true;
	...
	public static File staticizzaPdf(String filePath) throws Exception {
...
//			reader.unethicalreading = true;
	...
	public static File staticizzaPdfConXfaForm(String filePath) throws Exception {
	...
	public static File rewriteDocument(String fileUrl, List<Integer> listPagesToConvert, boolean fromScannedExc) throws Exception {
		...
		PDDocument lPDDocument = PDDocument.load(new File(fileUrl));
		// Scorro le pagine da copiare
		for (int i = 1; i <= reader.getNumberOfPages(); i++) {
	...
	private static void rewritePage(String fileUrl, Document document, PdfWriter writer, PdfContentByte cb,
			PdfReader reader, int pageNumber, boolean forceA4, boolean convertPageToImage, PDDocument lPDDocument) throws Exception {
