...
package it.eng.auriga.repository2.jaxws.webservices.trovagruppi;

import it.eng.auriga.database.store.dmpk_ws.bean.DmpkWsTrovagruppiBean;
import it.eng.auriga.database.store.dmpk_ws.store.Trovagruppi;
...
@WebService(targetNamespace = "http://trovagruppi.webservices.repository2.auriga.eng.it",  endpointInterface="it.eng.auriga.repository2.jaxws.webservices.trovagruppi.WSITrovaGruppi", name = "WSTrovaGruppi")
@MTOM(enabled = true, threshold = 0)

public class WSTrovaGruppi extends JAXWSAbstractAurigaService implements WSITrovaGruppi{

    private final String K_SAVEPOINTNAME = "INIZIO_WSTrovaGruppi";
   
    static Logger aLogger = Logger.getLogger(WSTrovaGruppi.class.getName());    
    
    public WSTrovaGruppi() {
	super();
    ...
    try {
    	 aLogger.info("Inizio WSTrovaGruppi");
		 ...
		 **************************************************************************/
	     ...
	     aLogger.info("Fine WSTrovaGruppi");
	    
	     return risposta;
    }
  
    catch (Exception excptn) {
        aLogger.error("WSTrovaGruppi: " + excptn.getMessage(), excptn);
	...
	finally
	{
	    ...
	    aLogger.info("Fine WSTrovaGruppi serviceImplementation");
	}
    }

        
    private String callWS(AurigaLoginBean loginBean, String xmlIn) throws Exception {
    	aLogger.debug("Eseguo il WS DMPK_WS->Trovagruppi.");    	
    	
    	String result = null;
    	
    	// Inizializzo l'INPUT    		
    	DmpkWsTrovagruppiBean input = new DmpkWsTrovagruppiBean();
    	input.setCodidconnectiontokenin(loginBean.getToken());
    	input.setXmlin(xmlIn);
	  
    	// Eseguo il servizio
    	Trovagruppi service = new Trovagruppi();
    	StoreResultBean<DmpkWsTrovagruppiBean> output = service.execute(loginBean, input);
