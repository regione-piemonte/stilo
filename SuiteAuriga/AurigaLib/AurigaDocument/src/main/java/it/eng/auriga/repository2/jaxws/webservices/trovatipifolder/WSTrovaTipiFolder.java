...
package it.eng.auriga.repository2.jaxws.webservices.trovatipifolder;

import it.eng.auriga.database.store.dmpk_ws.bean.DmpkWsTrovatipifolderBean;
import it.eng.auriga.database.store.dmpk_ws.store.Trovatipifolder;
...
@WebService(targetNamespace = "http://trovatipifolder.webservices.repository2.auriga.eng.it",  endpointInterface="it.eng.auriga.repository2.jaxws.webservices.trovatipifolder.WSITrovaTipiFolder", name = "WSTrovaTipiFolder")
@MTOM(enabled = true, threshold = 0)

public class WSTrovaTipiFolder extends JAXWSAbstractAurigaService implements WSITrovaTipiFolder{

    private final String K_SAVEPOINTNAME = "INIZIOWSTROVATIPIDFOLDER";
   
    static Logger aLogger = Logger.getLogger(WSTrovaTipiFolder.class.getName());    
    
    public WSTrovaTipiFolder() {
	super();
    ...
    try {
    	 aLogger.info("Inizio WSTrovaTipiFolder");
		 ...
		 **************************************************************************/
	     ...
	     aLogger.info("Fine WSTrovaTipiFolder");
	    
	     return risposta;
    }
  
    catch (Exception excptn) {
        aLogger.error("WSTrovaTipiFolder: " + excptn.getMessage(), excptn);
	...
	finally
	{
	    ...
	    aLogger.info("Fine WSTrovaTipiFolder serviceImplementation");
	}
    ...
    private String callWS(AurigaLoginBean loginBean, String xmlIn) throws Exception {
    	aLogger.debug("Eseguo il WS DMPK_WS->TrovaTipiFolder.");
    	
    	String result = null;  
    	
    	// Inizializzo l'INPUT    		
    	DmpkWsTrovatipifolderBean input = new DmpkWsTrovatipifolderBean();
    	input.setCodidconnectiontokenin(loginBean.getToken());
    	input.setXmlin(xmlIn);
	  
    	// Eseguo il servizio
    	Trovatipifolder service = new Trovatipifolder();
    	StoreResultBean<DmpkWsTrovatipifolderBean> output = service.execute(loginBean, input);
