...
package it.eng.auriga.repository2.jaxws.webservices.deleteistanzaapplesterna;

import it.eng.auriga.database.store.dmpk_ws.bean.DmpkWsDelapplicazioneesternaBean;
import it.eng.auriga.database.store.dmpk_ws.store.Delapplicazioneesterna;
...
@WebService(targetNamespace = "http://deleteistanzaapplesterna.webservices.repository2.auriga.eng.it",  endpointInterface="it.eng.auriga.repository2.jaxws.webservices.deleteistanzaapplesterna.WSIDeleteIstanzaApplEsterna", name = "WSDeleteIstanzaApplEsterna")
@MTOM(enabled = true, threshold = 0)

public class WSDeleteIstanzaApplEsterna extends JAXWSAbstractAurigaService implements WSIDeleteIstanzaApplEsterna{

    private final String K_SAVEPOINTNAME = "INIZIO_WSDeleteIstanzaApplEst";
   
    static Logger aLogger = Logger.getLogger(WSDeleteIstanzaApplEsterna.class.getName());    
    
    public WSDeleteIstanzaApplEsterna() {
	super();
    ...
    try {
    	 aLogger.info("Inizio WSDeleteIstanzaApplEsterna");
		 ...
		 **************************************************************************/
	     ...
	     aLogger.info("Fine WSDeleteIstanzaApplEsterna");
	    
	     return risposta;
    }
  
    catch (Exception excptn) {
        aLogger.error("WSDeleteIstanzaApplEsterna: " + excptn.getMessage(), excptn);
	...
	finally
	{
	    ...
	    aLogger.info("Fine WSDeleteIstanzaApplEsterna serviceImplementation");
	}
    }

    private String callWS(AurigaLoginBean loginBean, String xmlIn) throws Exception {    	    
    	aLogger.debug("Eseguo il WS DMPK_WS->DeleteIstanzaApplEsterna.");    	
    	
    	String ret = null;    	
    	    		
    	// Inizializzo l'INPUT    		
    	DmpkWsDelapplicazioneesternaBean input = new DmpkWsDelapplicazioneesternaBean();
    	input.setCodidconnectiontokenin(loginBean.getToken());
    	input.setXmlin(xmlIn);
	  
    	// Eseguo il servizio
    	Delapplicazioneesterna service = new Delapplicazioneesterna();
    	StoreResultBean<DmpkWsDelapplicazioneesternaBean> output = service.execute(loginBean, input);
    	...
    	// Leggo il FLG TIPO DEL 
    	if(output.getResultBean().getFlgtipodelout()!=null)
    		ret = output.getResultBean().getFlgtipodelout();
    	else
    		throw new Exception("La store procedure DmpkWSDeleteIstanzaApplEsterna ha ritornato Flgtipodelout nullo");
    ...
    private String generaXMLRispostaWS(String xmlIn)  throws Exception {    
        ...
        try {
            ...
            xml.append("<TipoDel>" + xmlInEsc + "</TipoDel>\n");
