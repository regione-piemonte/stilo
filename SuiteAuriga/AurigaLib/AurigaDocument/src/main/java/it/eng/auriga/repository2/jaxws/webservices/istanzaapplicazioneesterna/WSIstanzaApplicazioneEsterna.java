...
package it.eng.auriga.repository2.jaxws.webservices.istanzaapplicazioneesterna;

import it.eng.auriga.database.store.dmpk_ws.bean.DmpkWsAddupdapplicazioneesternaBean;
import it.eng.auriga.database.store.dmpk_ws.store.Addupdapplicazioneesterna;
...
@WebService(targetNamespace = "http://istanzaapplicazioneesterna.webservices.repository2.auriga.eng.it",  endpointInterface="it.eng.auriga.repository2.jaxws.webservices.istanzaapplicazioneesterna.WSIIstanzaApplicazioneEsterna", name = "WSIstanzaApplicazioneEsterna")
@MTOM(enabled = true, threshold = 0)

public class WSIstanzaApplicazioneEsterna extends JAXWSAbstractAurigaService implements WSIIstanzaApplicazioneEsterna{

    private final String K_SAVEPOINTNAME = "INIZIO_WSIstanzaApplEst";
   
    static Logger aLogger = Logger.getLogger(WSIstanzaApplicazioneEsterna.class.getName());    
    
    public WSIstanzaApplicazioneEsterna() {
	super();
    ...
    try {
    	 aLogger.info("Inizio WSIstanzaApplicazioneEsterna");
		 ...
		 **************************************************************************/
	     ...
	     aLogger.info("Fine WSIstanzaApplicazioneEsterna");
	    
	     return risposta;
    }
  
    catch (Exception excptn) {
        aLogger.error("WSIstanzaApplicazioneEsterna: " + excptn.getMessage(), excptn);
	...
	finally
	{
	    ...
	    aLogger.info("Fine WSIstanzaApplicazioneEsterna serviceImplementation");
	}
    }

        
    private String callWS(AurigaLoginBean loginBean, String xmlIn) throws Exception {    
    	aLogger.debug("Eseguo il WS DMPK_WS->IstanzaApplicazioneEsterna.");
    	
    	String ret = null;
    	
    	// Inizializzo l'INPUT    		
    	DmpkWsAddupdapplicazioneesternaBean input = new DmpkWsAddupdapplicazioneesternaBean();
    	input.setCodidconnectiontokenin(loginBean.getToken());
    	input.setXmlin(xmlIn);
	  
    	// Eseguo il servizio
    	Addupdapplicazioneesterna service = new Addupdapplicazioneesterna();
    	StoreResultBean<DmpkWsAddupdapplicazioneesternaBean> output = service.execute(loginBean, input);
