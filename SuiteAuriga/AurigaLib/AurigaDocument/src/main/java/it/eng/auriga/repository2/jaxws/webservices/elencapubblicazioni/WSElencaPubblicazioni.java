...
package it.eng.auriga.repository2.jaxws.webservices.elencapubblicazioni;

import it.eng.auriga.database.store.dmpk_ws.bean.DmpkWsElencapubblicazioniBean;
import it.eng.auriga.database.store.dmpk_ws.store.Elencapubblicazioni;
...
@WebService(targetNamespace = "http://elencapubblicazioni.webservices.repository2.auriga.eng.it",  endpointInterface="it.eng.auriga.repository2.jaxws.webservices.elencapubblicazioni.WSIElencaPubblicazioni", name = "WSElencaPubblicazioni")
@MTOM(enabled = true, threshold = 0)

public class WSElencaPubblicazioni extends JAXWSAbstractAurigaService implements WSIElencaPubblicazioni{

    private final String K_SAVEPOINTNAME = "INIZIO_WSElencaPubblicazioni";
   
    static Logger aLogger = Logger.getLogger(WSElencaPubblicazioni.class.getName());    
    
    public WSElencaPubblicazioni() {
	super();
    ...
    try {
    	 aLogger.info("Inizio WSElencaPubblicazioni");
		 ...
		 **************************************************************************/
	     ...
	     aLogger.info("Fine WSElencaPubblicazioni");
	    
	     return risposta;
    }
  
    catch (Exception excptn) {
        aLogger.error("WSElencaPubblicazioni: " + excptn.getMessage(), excptn);
	...
	finally
	{
	    ...
	    aLogger.info("Fine WSElencaPubblicazioni serviceImplementation");
	}
    }

        
    private String callWS(AurigaLoginBean loginBean, String xmlIn) throws Exception {    	    
    	aLogger.debug("Eseguo il WS DMPK_WS->ElencaPubblicazioni.");
    	
    	String result = null;
    	
    	// Inizializzo l'INPUT    		
    	DmpkWsElencapubblicazioniBean input = new DmpkWsElencapubblicazioniBean();
    	input.setCodidconnectiontokenin(loginBean.getToken());
    	input.setXmlin(xmlIn);
	  
    	// Eseguo il servizio
    	Elencapubblicazioni service = new Elencapubblicazioni();
	  	StoreResultBean<DmpkWsElencapubblicazioniBean> output = service.execute(loginBean, input);
