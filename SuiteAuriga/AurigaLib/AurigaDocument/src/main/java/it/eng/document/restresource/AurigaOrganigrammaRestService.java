...
import it.eng.document.configuration.EditorOrganigrammaConfigWSBean;
...
@Singleton
@Api
@Path("/editorOrganigramma")
public class AurigaOrganigrammaRestService {
	...
	private static final Logger logger = Logger.getLogger(AurigaOrganigrammaRestService.class);
	...
	public Response getTree(@QueryParam("idUoRoot") String idUoRoot,
							@QueryParam("livelloRevisione") String livelloRevisione) throws Exception {
		...
		try {
			...
			String idRootNode = calculateIdNodeFromIdUO(idUoRoot, loginBean);
			...
			/*TODO: quando valentina aggionerà la store bisogna passare il livello di revisione*/

			DmpkDefSecurityNavigastrutturaorgtree navigarepositorytree = new DmpkDefSecurityNavigastrutturaorgtree();
			StoreResultBean<DmpkDefSecurityNavigastrutturaorgtreeBean> output = navigarepositorytree.execute(null,
					loginBean, input);

			String idRootNodeOutput = output.getResultBean().getIdrootnodeio();
			BigDecimal idOrganigramma = output.getResultBean().getIdorganigrammaio();
			...
			if (output.getResultBean().getTreexmlout() != null) {
				...
				if (lista != null) {
					for (int i = 0; i < lista.getRiga().size(); i++) {
						...
						BigDecimal nroLivelloProgressivo = v.get(0) != null ? new BigDecimal(v.get(0)) : null; // colonna
						// 1
						// dell'xml
						...
						EditorOrganigrammaTreeNodeBean node = buildBeanFromValoriRiga(v, nroProgr);
						data.add(node);
					}
				}
			}
			...
			organigrammaTree = getResponseData(dataMap, idRootNodeOutput);
			fixIdForOrganigrammaClient(idRootNodeOutput, organigrammaTree);
	...
	public Response versionaFileOrganigramma(@FormParam("idDoc") String idDoc,
											 @FormParam("filePdf") String filePdf, 
											 @FormParam("nomePdf") String nomePdf) throws Exception {
	...
	@Produces({ MediaType.APPLICATION_JSON })
	@Consumes({ MediaType.APPLICATION_FORM_URLENCODED })
	public Response saveOrganigramma(@FormParam("idDoc") String idDoc,
									 @FormParam("flgUpdate") String flgUpdate,
									 @FormParam("organigrammaXml") String organigrammaXml) throws Exception {
		...
		try {
			...
			nroVersione = versionaOrganigramma(organigrammaXml, idDoc, flgUpdate, session);
	...
	@Produces({ MediaType.APPLICATION_JSON })
	public Response annullaModifiche(@QueryParam("idDoc") String idDoc,
									 @QueryParam("nroVersLavoro") String nroVersLavoro) throws Exception {
		...
		try {
			...
			if (nroVersLavoro != null
				...
				versionaOrganigramma(organigrammaXml, idDoc, "false", session);
			}
	...
	@Path("/getVersion")
	@Produces({ MediaType.APPLICATION_JSON })
	public Response getVersion(@QueryParam("idDoc") String idDoc, 
							   @QueryParam ("nroVers") String nroVers) throws Exception {
	...
	@Produces({ MediaType.APPLICATION_JSON })
	@Consumes({ MediaType.APPLICATION_FORM_URLENCODED })
	public Response funzionigramma(@FormParam("image") String image,
								   @FormParam("organigrammaXml") String organigrammaXml,
			@FormParam("azione") String azione) throws Exception {
	...
	@Path("/getTipiUO")
	@Produces({ MediaType.APPLICATION_JSON })
	public Response getTipiUO(@QueryParam ("livelloRevisione") String livelloRevisione) throws Exception {
		...
		GenericEntity<List<TipoUOXmlBean>> list = new GenericEntity<List<TipoUOXmlBean>>(listaTipiUO) {
		};
		return Response.ok(list).header("Access-Control-Allow-Origin", "*").build();
	}
	...
	@Path("/getRuoli")
	@Produces({ MediaType.APPLICATION_JSON })
	public Response getRuoli() throws Exception {
	...
	@Path("/fetchUsers")
	@Produces({ MediaType.APPLICATION_JSON })
	public Response fetchUsers(@QueryParam("filter") String filter) throws Exception {
	...
	@Path("/explodeNode")
	@Produces({ MediaType.APPLICATION_JSON })
	public Response explodeNode(
			@QueryParam("idNodeToExplode") String idNodeToExplode,
			@QueryParam("nroProgrRootNode") BigDecimal nroProgrRootNode,
			@QueryParam("idRootNode") String idRootNode) throws Exception {
	...
	private AurigaLoginBean getAurigaLoginBean(HttpServletRequest request) {
		...
		EditorOrganigrammaConfigWSBean editorOrganigrammaConfigWSBean = (EditorOrganigrammaConfigWSBean) SpringAppContext.getContext().getBean("EditorOrganigrammaConfigWSBean");

		//Controllo nell header se ho token e schema per l autenticazione (CHIAMATA DA AURIGA)
		if(StringUtils.isNotBlank(request.getHeader(TOKEN_AURIGALOGIN)) && StringUtils.isNotBlank(request.getHeader(SCHEMA_AURIGALOGIN))) {
			...
			//Se non ho le credenziali nell header della chiamata (CHIAMATA DA FUORI) li prendo da file di configurazione
		}else {
			aurigaLoginBean.setToken(editorOrganigrammaConfigWSBean.getToken());
			aurigaLoginBean.setSchema(editorOrganigrammaConfigWSBean.getSchema());
			aurigaLoginBean.setIdUserLavoro(null);
		}	
	...
	private String versionaOrganigramma(String organigrammaXml, String idDoc, String flgUpdate, Session session) throws Exception {
	...
	private EditorOrganigrammaTreeNodeBean buildBeanFromValoriRiga(Vector<String> v, BigDecimal nroProgr) {
		EditorOrganigrammaTreeNodeBean node = new EditorOrganigrammaTreeNodeBean();
		// node.setNroLivello(v.get(18) != null ? new BigDecimal(v.get(18)) : null); //
		// colonna 19 dell'xml
		 ...
		 * node.setCodFiscale(v.get(25)); // colonna 26 dell'xml if
		 * (node.getTipo().substring(node.getTipo().indexOf("_") + 1) != null)
		 * node.setTipoRelUtenteUo(node.getTipo().substring(node.getTipo().indexOf("_")
		 * + 1)); }
		 */
		...
		// } else {
		// String idFolder =
		// node.getIdNode().substring(node.getIdNode().lastIndexOf("/") + 1);
		...
		// node.setFlgSelXFinalita(v.get(7) != null && "1".equals(v.get(7))); // colonna
		// 8 dell'xml
		...
		node.setCompetenze("<p>competenze di prova con alberatura</p>\r\n" + "\r\n"
				+ "<p>e paragrafi</p>\r\n" + "\r\n" + "<ol>\r\n"
				+ "<li>competenza 1</li>\r\n" + "<li>competenza 2\r\n" + "<ol>\r\n" + "<li>sottocompetenza 2</li>\r\n"
				+ "</ol>\r\n" + "</li>\r\n" + "<li>competenza 3</li>\r\n" + "</ol>\r\n" + "\r\n"
				+ "<p>e competenze non ordinate</p>\r\n" + "\r\n" + "<ul>\r\n"
				+ "<li><u>competenza x</u></li>\r\n" + "<li><em>competenza </em>y\r\n" + "<ul>\r\n"
				+ "<li>sottocompetenza y\r\n" + "<ul>\r\n" + "<li><strong>sotto-</strong>sottocompetenza y</li>\r\n"
				+ "</ul>\r\n" + "</li>\r\n" + "</ul>\r\n" + "</li>\r\n" + "</ul>");
		// Map<String, String> altriAttr = new TreeMap<>();
		// altriAttr.put("competenze", "AAAA,BBBB");
		// node.setAltriAttributi(altriAttr);
		// node.setFlgPuntoProtocollo(v.get(29) != null && "1".equals(v.get(29))); //
		// colonna 30 dell'xml
		// node.setUsername(v.get(30));// USERNAME: colonna 31 dell'xml
		// node.setFlgDestinatarioNeiPreferiti(v.get(31) != null &&
		// "1".equals(v.get(31))); // col.32 : la UO/scrivania e' nei preferiti (1 = si,
		// 0 = no)
		// node.setFlgSelXAssegnazione(v.get(32)); // colonna 33 dell'xml
		// node.setFlgProtocollista(v.get(33) != null && "1".equals(v.get(33))); //
		// colonna 34 dell'xml
		return node;
	}
	...
	protected Map<String, List<EditorOrganigrammaTreeNodeBean>> buildRelations(
			Map<String, EditorOrganigrammaTreeNodeBean> pNodes, String idRootNode) {
		Map<String, List<EditorOrganigrammaTreeNodeBean>> relation = new TreeMap<>();
		...
		for (EditorOrganigrammaTreeNodeBean lEditorNode : pNodes.values()) {
			if (lEditorNode.getParentIdDB().equals("") || lEditorNode.getIdNode().equals(idRootNode)) {
				...
				relation.put("root", rootNode);
				relation.put(lEditorNode.getIdNode(), new ArrayList<EditorOrganigrammaTreeNodeBean>());
			} else {
				if (!relation.containsKey(lEditorNode.getParentIdDB())) {
					relation.put(lEditorNode.getParentIdDB(), new ArrayList<EditorOrganigrammaTreeNodeBean>());
				}
				relation.get(lEditorNode.getParentIdDB()).add(lEditorNode);
			}
		}
		return relation;
	}

	private void buildHierarchy(String parentId, Map<String, List<EditorOrganigrammaTreeNodeBean>> relationship,
			Map<String, EditorOrganigrammaTreeNodeBean> nodes) {

		// se vengo da explodeTreeNode
		if (nodes.get(parentId) == null) {
			nodes.put(parentId, new EditorOrganigrammaTreeNodeBean());
		}

		for (EditorOrganigrammaTreeNodeBean child : relationship.get(parentId)) {
			if (relationship.keySet().contains(child.getIdNode())) {
				buildHierarchy(child.getIdNode(), relationship, nodes);
			} else {
				nodes.get(parentId).getChildren().add(child);
			}
		}
	}
	...
	private void fixIdForOrganigrammaClient(String idRootNode, EditorOrganigrammaTreeNodeBean organigrammaTreeNodeBean) {
		...
		if(organigrammaTreeNodeBean != null) {
			...
			for(EditorOrganigrammaTreeNodeBean lOrganigrammaTreeNodeBean : organigrammaTreeNodeBean.getChildren()) {
				fixIdForOrganigrammaClient(idRootNode +"/", lOrganigrammaTreeNodeBean);
			}
		}	
	}
