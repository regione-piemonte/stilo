...
package it.eng.auriga.repository2.jaxws.webservices.addmodprivilegi;

import it.eng.auriga.database.store.dmpk_ws.bean.DmpkWsAddupdprivilegiBean;
import it.eng.auriga.database.store.dmpk_ws.store.Addupdprivilegi;
...
@WebService(targetNamespace = "http://addmodprivilegi.webservices.repository2.auriga.eng.it",  endpointInterface="it.eng.auriga.repository2.jaxws.webservices.addmodprivilegi.WSIAddModPrivilegi", name = "WSAddModPrivilegi")
@MTOM(enabled = true, threshold = 0)

public class WSAddModPrivilegi extends JAXWSAbstractAurigaService implements WSIAddModPrivilegi{

    private final String K_SAVEPOINTNAME = "INIZIOWSADDMODPRIV";
   
    static Logger aLogger = Logger.getLogger(WSAddModPrivilegi.class.getName());    
    
    public WSAddModPrivilegi() {
	super();
    ...
    try {
    	 aLogger.info("Inizio WSAddModPrivilegi");
		 ...
		 **************************************************************************/
	     ...
	     aLogger.info("Fine WSAddModPrivilegi");
	    
	     return risposta;
    }
  
    catch (Exception excptn) {
        aLogger.error("WSAddModPrivilegi: " + excptn.getMessage(), excptn);
	...
	finally
	{
	    ...
	    aLogger.info("Fine WSAddModPrivilegi serviceImplementation");
	}
    }

    private String callWS(AurigaLoginBean loginBean, String xmlIn) throws Exception {    	    
    	aLogger.debug("Eseguo il WS DMPK_WS->AddUpdPrivilegi.");    	
    	
    	String ret = null;    	
    	
    	// Inizializzo l'INPUT    		
    	DmpkWsAddupdprivilegiBean input = new DmpkWsAddupdprivilegiBean();
    	input.setCodidconnectiontokenin(loginBean.getToken());
    	input.setXmlin(xmlIn);
	  
    	// Eseguo il servizio
    	Addupdprivilegi service = new Addupdprivilegi();
    	StoreResultBean<DmpkWsAddupdprivilegiBean> output = service.execute(loginBean, input);
