...
package it.eng.auriga.repository2.jaxws.webservices.deluser;

import it.eng.auriga.database.store.dmpk_ws.bean.DmpkWsDeluserBean;
import it.eng.auriga.database.store.dmpk_ws.store.Deluser;
...
@WebService(targetNamespace = "http://deluser.webservices.repository2.auriga.eng.it",  endpointInterface="it.eng.auriga.repository2.jaxws.webservices.deluser.WSIDelUser", name = "WSDelUser")
@MTOM(enabled = true, threshold = 0)

public class WSDelUser extends JAXWSAbstractAurigaService implements WSIDelUser{

    private final String K_SAVEPOINTNAME = "INIZIO_WSDelUser";
   
    static Logger aLogger = Logger.getLogger(WSDelUser.class.getName());    
    ...
    try {
		 ...
		 **************************************************************************/
	     ...
	     aLogger.info("Fine WSDelUser");
	    
	     return risposta;
    }
  
    catch (Exception excptn) {
        aLogger.error("WSDelUser: " + excptn.getMessage(), excptn);
	...
	finally
	{
	    ...
	    aLogger.info("Fine WSDelUser serviceImplementation");
	}
    }

    private String callWS(AurigaLoginBean loginBean, String xmlIn) throws Exception {    	    
    	aLogger.debug("Eseguo il WS DMPK_WS->DelUser.");
    	
    	String ret = null;    	
    	    		
    	// Inizializzo l'INPUT    		
    	DmpkWsDeluserBean input = new DmpkWsDeluserBean();
    	input.setCodidconnectiontokenin(loginBean.getToken());
    	input.setXmlin(xmlIn);

    	// Eseguo il servizio
    	Deluser service = new Deluser();
    	StoreResultBean<DmpkWsDeluserBean> output = service.execute(loginBean, input);
