...
package it.eng.auriga.repository2.jaxws.webservices.removefolderfromfavourite;
...
@WebService(targetNamespace = "http://removefolderfromfavourite.webservices.repository2.auriga.eng.it",  endpointInterface="it.eng.auriga.repository2.jaxws.webservices.removefolderfromfavourite.WSIRemoveFolderFromFavourite", name = "WSRemoveFolderFromFavourite")
@MTOM(enabled = true, threshold = 0)

public class WSRemoveFolderFromFavourite extends JAXWSAbstractAurigaService implements WSIRemoveFolderFromFavourite{

    private final String K_SAVEPOINTNAME = "INIZIO_WSRemoveFolderFromFav";
   
    static Logger aLogger = Logger.getLogger(WSRemoveFolderFromFavourite.class.getName());    
    
    public WSRemoveFolderFromFavourite() {
	super();
    ...
    try {
    	 aLogger.info("Inizio WSRemoveFolderFromFavourite");
		 ...
		 **************************************************************************/
	     ...
	     aLogger.info("Fine WSRemoveFolderFromFavourite");
	    
	     return risposta;
    }
  
    catch (Exception excptn) {
        aLogger.error("WSRemoveFolderFromFavourite: " + excptn.getMessage(), excptn);
	...
	finally
	{
	    ...
	    aLogger.info("Fine WSRemoveFolderFromFavourite serviceImplementation");
	}
    }
        
    private String callWS(AurigaLoginBean loginBean, String xmlIn) throws Exception {   
    	aLogger.debug("Eseguo il WS DMPK_WS->RemoveFolderFromFavourite.");
    	...
    	input.setFlgtypeobjin("F");
	  
    	// Eseguo il servizio
    	Addremovefromfavourite service = new Addremovefromfavourite();
    	StoreResultBean<DmpkWsAddremovefromfavouriteBean> output = service.execute(loginBean, input);
