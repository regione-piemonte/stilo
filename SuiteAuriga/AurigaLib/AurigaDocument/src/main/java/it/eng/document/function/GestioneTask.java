...
import it.eng.auriga.database.store.dmpk_processes.bean.DmpkProcessesIuprocobj_ligthBean;
...
import it.eng.auriga.database.store.dmpk_processes.store.impl.Iuprocobj_ligthImpl;
...
import it.eng.auriga.database.store.dmpk_wf.store.Aggprochist_postexecattwf;
...
import it.eng.document.function.bean.OstSalvatoBean;
...
import it.eng.document.function.bean.XmlDatiProcBean;
...
public class GestioneTask {
	...
	public EseguiTaskOutBean salvaTask(AurigaLoginBean pAurigaLoginBean, EseguiTaskInBean pEseguiTaskInBean) throws JAXBException, IllegalAccessException,
		...
		try {
			...
			try {
				...
				mLogger.error("\n\n############################" + "\n## Procedimento: " + pEseguiTaskInBean.getIdProcess() + "\n## Tipo Evento: "
						+ pEseguiTaskInBean.getIdEventType() + "\n## Istanza Evento: " + pEseguiTaskInBean.getIdEvento() + "\n############################\n\n");
				...
				if (pEseguiTaskInBean.getIdEventType() != null) {
					...
					lDmpkProcessesGetprocobjxmlBean.setIdtipoeventoin(new BigDecimal(pEseguiTaskInBean.getIdEventType()));
				...
				// STEP 1: la IUEventoCustom passandole come IdEventoIO quello restituito dalla IUEventoCustom del salva e TsComplEventoIn =
				// data e ora correnti (fmt GG/MM/AAAA HH24:MI:SS)

				if (pEseguiTaskInBean.getScAttributiAdd() != null) {
					...
					lIueventocustomBean.setIdprocessin(StringUtils.isNotBlank(pEseguiTaskInBean.getIdProcess()) ? new BigDecimal(pEseguiTaskInBean
							.getIdProcess()) : null);
					lIueventocustomBean.setIdeventoio(StringUtils.isNotBlank(pEseguiTaskInBean.getIdEvento()) ? new BigDecimal(pEseguiTaskInBean.getIdEvento())
							: null);
					lIueventocustomBean.setIdtipoeventoin(StringUtils.isNotBlank(pEseguiTaskInBean.getIdEventType()) ? new BigDecimal(pEseguiTaskInBean
							.getIdEventType()) : null);
					lIueventocustomBean.setTscompleventoin(new SimpleDateFormat("dd/MM/yyyy HH:mm:ss").format(new Date()));
					...
					lMarshaller.marshal(pEseguiTaskInBean.getScAttributiAdd(), sw);

					lIueventocustomBean.setAttributiaddin(sw.toString());

					lIueventocustomBean.setFlgautocommitin(0); // blocco l'autocommit
				...
				if (StringUtils.isNotBlank(pEseguiTaskInBean.getInstanceId())) {
					...
					// STEP 2: la dmpk_wf.ExecAttFlusso (solo se colonna 3 valorizzata ovvero se task del workflow) . DesEsitoAttOut viene
					...
					lExecattflussoBean.setIdprocessin(StringUtils.isNotBlank(pEseguiTaskInBean.getIdProcess()) ? new BigDecimal(pEseguiTaskInBean
							.getIdProcess()) : null);
					lExecattflussoBean.setActivitynamein(pEseguiTaskInBean.getActivityName());
					lExecattflussoBean.setAttributiaddin(null);
					lExecattflussoBean.setFlgautocommitin(0); // blocco l'autocommit
					...
					// STEP 3: solo se colonna 1 valorizzata AND se la dmpk_wf.ExecAttFlusso restituisce FlgActToCompleteInWFOut=1: si fa
					// complete di Activiti passando esito restituito allo step 2)
					...
					if (resultExecattflusso.getResultBean().getFlgacttocompleteinwfout() != null
						...
						if (StringUtils.isNotBlank(resultExecattflusso.getResultBean().getVariabiliesitiout())) {
							List<VariabileEsitoBean> listaVariabiliEsito = XmlListaUtility.recuperaLista(resultExecattflusso.getResultBean()
									.getVariabiliesitiout(), VariabileEsitoBean.class);
							if (listaVariabiliEsito != null) {
								for (int i = 0; i < listaVariabiliEsito.size(); i++) {
									...
									lEngineManager.setVariableTask(pEseguiTaskInBean.getInstanceId(), variabileEsito.getNome(), variabileEsito.getEsito());
								}
							}
						}
						...
						//
						// lEngineManager.setVariableTask(pEseguiTaskInBean.getInstanceId(), resultExecattflusso.getResultBean()
						// .getVariabileesitoout(), resultExecattflusso.getResultBean().getEsitoout());
						//
						// }
			...
			if (completeTask) {
				...
				lAggprochist_postexecattwfBean.setIdprocessin(StringUtils.isNotBlank(pEseguiTaskInBean.getIdProcess()) ? new BigDecimal(pEseguiTaskInBean
						.getIdProcess()) : null);
				lAggprochist_postexecattwfBean.setActivitynamein(pEseguiTaskInBean.getActivityName());

				final Aggprochist_postexecattwf aggprochist_postexecattwf = new Aggprochist_postexecattwf();

				StoreResultBean<DmpkWfAggprochist_postexecattwfBean> resultAggprochist_postexecattwf = aggprochist_postexecattwf.execute(pAurigaLoginBean,
						lAggprochist_postexecattwfBean);

				if (StringUtils.isNotBlank(resultAggprochist_postexecattwf.getDefaultMessage())) {
					lEseguiTaskOutBean.setWarningMessage(resultAggprochist_postexecattwf.getDefaultMessage());
				}
	...
	private boolean isRelProperty(String propertyName) {
		...
		return propertyName.startsWith("GPA_ID_OBJ_CAR_REL_PROC_OBJ") || propertyName.startsWith("GPA_NATURA_REL_PROC_OBJ")
				|| propertyName.startsWith("GPA_PROV_CI_OBJ_REL_PROC_OBJ") || propertyName.startsWith("GPA_DES_DETT_REL_PROC_OBJ")
				|| propertyName.startsWith("GPA_ID_PROC_OBJ_TY_REL_OBJ_TY") || propertyName.startsWith("GPA_ID_PROC_OBJ_TY_REL_OBJ")
				|| propertyName.startsWith("GPA_PROV_CI_OBJ_TY_REL_OBJ_TY") || propertyName.startsWith("GPA_PROV_CI_OBJ_TY_REL_OBJ")
				|| propertyName.startsWith("GPA_DES_OBJ_REL") || propertyName.startsWith("GPA_NATURA_REL_PROC_OBJ_TY");
	}

	private boolean checkRelProperty(Variabile lVariabile, WsFontiDatiRef fonteRef, Map<Integer, RelazioniOst> relazioniMap, String currentIdOst,
			Integer currentNatura, Integer currentCategoria, String currentIdProcObj) {
		boolean retVal = false;

		String propertyName = lVariabile.getNome().toUpperCase();
		String propertyValue = lVariabile.getValoreSemplice().getValue();
		...
		if (propertyName.startsWith("GPA_DES_DETT_REL_PROC_OBJ_")) {
			...
			/**
			 * SPLITTARE SU "#!#" [0] TIPOLOGIA RELAZIONE --> "ATTIVA" / "PASSIVA" [1] NATURA RELAZIONE (Stringa descrittiva uguale a GPA_NATURA_REL_PROC_OBJ
			 * [2] CODICE DELLA TIPOLOGIA DI RELAZIONE (Codice TipologiaRost) [3] CATEGORIA DELLA TIPOLOGIA OST RELAZIONATA [4] NATURA DELLA TIPOLOGIA OST
			 * RELAZIONATA
			 */
	...
	public static PropertyDescriptor getPropertyDescriptor(Object object, String property) throws Exception {
		if (StringUtils.isNotBlank(property)) {
			...
			for (int i = 0; i < props.length; i++) {
				...
				PropertyDescriptor propDescr = BeanUtilsBean2.getInstance().getPropertyUtils().getPropertyDescriptor(obj, propName);
				if (propDescr == null) {
	...
	public static void setProperty(Object object, String property, String strValue, String vocabolario) throws Exception {
						...
						if (StringUtils.isNotBlank(vocabolario)) {
							...
							if (output != null) {
								...
								if (idProperty.equals("")) {
									mLogger.error("Errore Vocabolario '" + className + "': idPropertySpecific non specificato");
								} else {
									...
									Class currentClass = Class.forName(className); currentClass.getSuperclass() != null; currentClass = currentClass
											.getSuperclass()) {
										try {
											// mLogger.info("\t\tvoc:" + className + ", currentClass --> " +
											// currentClass.getCanonicalName());
											privateSpecificField = Class.forName(currentClass.getCanonicalName()).getDeclaredField(idProperty);
											break;
										} catch (NoSuchFieldException ex) {
											// mLogger.info("\t\tvoc:" + className + ", yet not found: " + idProperty);
										}
									}

									privateSpecificField.setAccessible(true);
									String privateSpecificFieldClass = privateSpecificField.getType().getCanonicalName();

									if (privateSpecificFieldClass != null && output.getVociVocabolario().size() > 0) {
										mLogger.debug("\n\n\t" + propName + ": setting vocabulary indexed by '" + idProperty + "' as type '"
												+ privateSpecificFieldClass + "' with '" + strValue + "'");
										...
										if (privateSpecificFieldClass.equals("java.lang.Integer")) {
											...
											BeanUtilsBean2.getInstance().getPropertyUtils().setProperty(obj, propName, voc);
											...
											mLogger.error("Error " + propName + ": Voc '" + className + "' is using another class for private field '"
													+ privateSpecificField.getName() + "', used class is '" + privateSpecificFieldClass + "'");
										}
									} else {
							...
							if (StringUtils.isBlank(strValue))
								throw new Exception("Errore durante il salvataggio dell'OST: il comune è un campo obbligatorio");
							...
							lComuniItaliaId.setIstatReg(codiciComProvReg.get("R")); // es.: 20
							lComuniItaliaId.setIstatProv(codiciComProvReg.get("P")); // es.: 091
							lComuniItaliaId.setIstatCom(codiciComProvReg.get("C")); // es.: 051
							lComuniItalia.setComuniItaliaId(lComuniItaliaId);
							BeanUtilsBean2.getInstance().getPropertyUtils().setProperty(obj, "comuneItalia", lComuniItalia);
							...
							BeanUtilsBean2.getInstance().getPropertyUtils().setProperty(obj, propName, Integer.parseInt(strValue));
						} else if (className.equals("java.lang.Float")) {
							BeanUtilsBean2.getInstance().getPropertyUtils().setProperty(obj, propName, Float.parseFloat(strValue));
		...
		try {
			...
			if (publicMethods != null) {
				for (Method method : publicMethods) {
					...
					if (method.getName().equals("setCodice") && types.length == 1 && types[0].toString().contains("java.lang.Integer")) {
						return true;
					}
				}
			}
