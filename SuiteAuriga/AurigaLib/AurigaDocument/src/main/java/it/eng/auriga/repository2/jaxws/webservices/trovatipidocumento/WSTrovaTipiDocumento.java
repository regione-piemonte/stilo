...
package it.eng.auriga.repository2.jaxws.webservices.trovatipidocumento;

import it.eng.auriga.database.store.dmpk_ws.bean.DmpkWsTrovatipidocumentoBean;
import it.eng.auriga.database.store.dmpk_ws.store.Trovatipidocumento;
...
@WebService(targetNamespace = "http://trovatipidocumento.webservices.repository2.auriga.eng.it",  endpointInterface="it.eng.auriga.repository2.jaxws.webservices.trovatipidocumento.WSITrovaTipiDocumento", name = "WSTrovaTipiDocumento")
@MTOM(enabled = true, threshold = 0)

public class WSTrovaTipiDocumento extends JAXWSAbstractAurigaService implements WSITrovaTipiDocumento{

    private final String K_SAVEPOINTNAME = "INIZIOWSTROVATIPIDOCUMENTO";
   
    static Logger aLogger = Logger.getLogger(WSTrovaTipiDocumento.class.getName());    
    
    public WSTrovaTipiDocumento() {
	super();
    ...
    try {
    	 aLogger.info("Inizio WSTrovaTipiDocumento");
		 ...
		 **************************************************************************/
	     ...
	     aLogger.info("Fine WSTrovaTipiDocumento");
	    
	     return risposta;
    }
  
    catch (Exception excptn) {
        aLogger.error("WSTrovaTipiDocumento: " + excptn.getMessage(), excptn);
	...
	finally
	{
	    ...
	    aLogger.info("Fine WSTrovaTipiDocumento serviceImplementation");
	}
    ...
    private String callWS(AurigaLoginBean loginBean, String xmlIn) throws Exception {
    	aLogger.debug("Eseguo il WS DMPK_WS->TrovaTipiDocumento.");
    	
    	String result = null;
    	
    	// Inizializzo l'INPUT    		
    	DmpkWsTrovatipidocumentoBean input = new DmpkWsTrovatipidocumentoBean();
    	input.setCodidconnectiontokenin(loginBean.getToken());
    	input.setXmlin(xmlIn);
	  
    	// Eseguo il servizio
    	Trovatipidocumento service = new Trovatipidocumento();
    	StoreResultBean<DmpkWsTrovatipidocumentoBean> output = service.execute(loginBean, input);
