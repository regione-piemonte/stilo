...
package it.eng.auriga.repository2.jaxws.webservices.trovaclassiftitolario;


import it.eng.auriga.database.store.dmpk_ws.bean.DmpkWsTrovaclassiftitolarioBean;
import it.eng.auriga.database.store.dmpk_ws.store.Trovaclassiftitolario;
...
@WebService(targetNamespace = "http://trovaclassiftitolario.webservices.repository2.auriga.eng.it",  endpointInterface="it.eng.auriga.repository2.jaxws.webservices.trovaclassiftitolario.WSITrovaClassifTitolario", name = "WSTrovaClassifTitolario")
@MTOM(enabled = true, threshold = 0)

public class WSTrovaClassifTitolario extends JAXWSAbstractAurigaService implements WSITrovaClassifTitolario{

    private final String K_SAVEPOINTNAME = "INIZIO_WSTrovaClassifTitolario";
   
    static Logger aLogger = Logger.getLogger(WSTrovaClassifTitolario.class.getName());    
    
    public WSTrovaClassifTitolario() {
	super();
    ...
    try {
    	 aLogger.info("Inizio WSTrovaClassifTitolario");
		 ...
		 **************************************************************************/
	     ...
	     aLogger.info("Fine WSTrovaClassifTitolario");
	    
	     return risposta;
    }
  
    catch (Exception excptn) {
        aLogger.error("WSTrovaClassifTitolario: " + excptn.getMessage(), excptn);
	...
	finally
	{
	    ...
	    aLogger.info("Fine WSTrovaClassifTitolario serviceImplementation");
	}
    }

    private String callWS(AurigaLoginBean loginBean, String xmlIn) throws Exception {
    	aLogger.debug("Eseguo il WS DMPK_WS->TrovaClassifTitolario.");    	
    	
    	String result = null;
    	
    	// Inizializzo l'INPUT    		
    	DmpkWsTrovaclassiftitolarioBean input = new DmpkWsTrovaclassiftitolarioBean();
    	input.setCodidconnectiontokenin(loginBean.getToken());
    	input.setXmlin(xmlIn);
	  
    	// Eseguo il servizio
    	Trovaclassiftitolario service = new Trovaclassiftitolario();
    	StoreResultBean<DmpkWsTrovaclassiftitolarioBean> output = service.execute(loginBean, input);
