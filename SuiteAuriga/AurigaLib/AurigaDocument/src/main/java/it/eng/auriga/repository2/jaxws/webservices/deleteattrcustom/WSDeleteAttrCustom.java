...
package it.eng.auriga.repository2.jaxws.webservices.deleteattrcustom;
...
@WebService(targetNamespace = "http://deleteattrcustom.webservices.repository2.auriga.eng.it",  endpointInterface="it.eng.auriga.repository2.jaxws.webservices.deleteattrcustom.WSIDeleteAttrCustom", name = "WSDeleteAttrCustom")
@MTOM(enabled = true, threshold = 0)

public class WSDeleteAttrCustom extends JAXWSAbstractAurigaService implements WSIDeleteAttrCustom{

    private final String K_SAVEPOINTNAME = "INIZIOWSDELETEATTRCUSTOM";
   
    static Logger aLogger = Logger.getLogger(WSDeleteAttrCustom.class.getName());    
    
    public WSDeleteAttrCustom() {
	super();
    ...
    try {
    	 aLogger.info("Inizio WSDeleteAttrCustom");
		 ...
		 **************************************************************************/
	     ...
	     aLogger.info("Fine WSDeleteAttrCustom");
	    
	     return risposta;
    }
  
    catch (Exception excptn) {
        aLogger.error("WSDeleteAttrCustom: " + excptn.getMessage(), excptn);
	...
	finally
	{
	    ...
	    aLogger.info("Fine WSDeleteAttrCustom serviceImplementation");
	}

    }
        
    private String callWS(AurigaLoginBean loginBean, String xmlIn) throws Exception {
    	aLogger.debug("Eseguo il WS DMPK_WS->DeleteAttrCustom.");
    	
    	String result = null;    
    	
    	// Inizializzo l'INPUT    		
    	DmpkWsDelattrcustomBean input = new DmpkWsDelattrcustomBean();
    	input.setCodidconnectiontokenin(loginBean.getToken());
    	input.setXmlin(xmlIn);
	      		  
    	// Eseguo il servizio
    	Delattrcustom service = new Delattrcustom();
    	StoreResultBean<DmpkWsDelattrcustomBean> output = service.execute(loginBean, input);
    	...
    	if (output.getResultBean().getAttrnameout()!=null)
		  result = output.getResultBean().getAttrnameout().toString();
	  
    	if (result== null || result.equalsIgnoreCase(""))
		  throw new Exception("La store procedure ha ritornato AttrNameOut nullo");
    ...
    private String generaXMLRispostaWS(String xmlIn)  throws Exception {
        ...
        try {
            ...
            xml.append("<DeleteAttrCustom>" + xmlInEsc + "</DeleteAttrCustom>\n");
