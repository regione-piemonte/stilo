...
package it.eng.auriga.repository2.jaxws.webservices.trovautenti;

import it.eng.auriga.database.store.dmpk_ws.bean.DmpkWsTrovautentiBean;
import it.eng.auriga.database.store.dmpk_ws.store.Trovautenti;
...
@WebService(targetNamespace = "http://trovautenti.webservices.repository2.auriga.eng.it",  endpointInterface="it.eng.auriga.repository2.jaxws.webservices.trovautenti.WSITrovaUtenti", name = "WSTrovaUtenti")
@MTOM(enabled = true, threshold = 0)

public class WSTrovaUtenti extends JAXWSAbstractAurigaService implements WSITrovaUtenti{

    private final String K_SAVEPOINTNAME = "INIZIO_WSTrovaUtenti";
   
    static Logger aLogger = Logger.getLogger(WSTrovaUtenti.class.getName());    
    
    public WSTrovaUtenti() {
	super();
    ...
    try {
    	 aLogger.info("Inizio WSTrovaUtenti");
		 ...
		 **************************************************************************/
	     ...
	     aLogger.info("Fine WSTrovaUtenti");
	    
	     return risposta;
    }
  
    catch (Exception excptn) {
        aLogger.error("WSTrovaUtenti: " + excptn.getMessage(), excptn);
	...
	finally
	{
	    ...
	    aLogger.info("Fine WSTrovaUtenti serviceImplementation");
	}
    }
    ...
    private String callWS(AurigaLoginBean loginBean, String xmlIn) throws Exception {  
    	...
    	// Inizializzo l'INPUT    		
    	DmpkWsTrovautentiBean input = new DmpkWsTrovautentiBean();
    	input.setCodidconnectiontokenin(loginBean.getToken());
    	input.setXmlin(xmlIn);
	  
    	// Eseguo il servizio
    	Trovautenti service = new Trovautenti();
    	StoreResultBean<DmpkWsTrovautentiBean> output = service.execute(loginBean, input);
