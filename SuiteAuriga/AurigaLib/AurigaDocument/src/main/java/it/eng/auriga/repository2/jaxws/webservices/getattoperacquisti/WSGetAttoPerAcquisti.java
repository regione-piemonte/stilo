...
package it.eng.auriga.repository2.jaxws.webservices.getattoperacquisti;

import it.eng.auriga.database.store.dmpk_ws.bean.DmpkWsGetattoperacquistiBean;
import it.eng.auriga.database.store.dmpk_ws.store.Getattoperacquisti;
...
@WebService(targetNamespace = "http://getattoperacquisti.webservices.repository2.auriga.eng.it",  endpointInterface="it.eng.auriga.repository2.jaxws.webservices.getattoperacquisti.WSIGetAttoPerAcquisti", name = "WSGetAttoPerAcquisti")
@MTOM(enabled = true, threshold = 0)

public class WSGetAttoPerAcquisti extends JAXWSAbstractAurigaService implements WSIGetAttoPerAcquisti{

    private final String K_SAVEPOINTNAME = "INIZIO_WSGetAttoPerAcq";
   
    static Logger aLogger = Logger.getLogger(WSGetAttoPerAcquisti.class.getName());    
    
    public WSGetAttoPerAcquisti() {
	super();
    ...
    try {
    	 aLogger.info("Inizio WSGetAttoPerAcquisti");
		 ...
		 **************************************************************************/
	     ...
	     aLogger.info("Fine WSGetAttoPerAcquisti");
	    
	     return risposta;
    }
  
    catch (Exception excptn) {
        aLogger.error("WSGetAttoPerAcquisti: " + excptn.getMessage(), excptn);
	...
	finally
	{
	    ...
	    aLogger.info("Fine WSGetAttoPerAcquisti serviceImplementation");
	}
    }

        
    private String callWS(AurigaLoginBean loginBean, String xmlIn) throws Exception {    	    
    	aLogger.debug("Eseguo il WS DMPK_WS->GetAttoPerAcquisti.");
    	
    	String result = null;    	
    	    		
    	// Inizializzo l'INPUT    		
    	DmpkWsGetattoperacquistiBean input = new DmpkWsGetattoperacquistiBean();
    	input.setCodidconnectiontokenin(loginBean.getToken());
    	input.setXmlin(xmlIn);
	  
    	// Eseguo il servizio
    	Getattoperacquisti service = new Getattoperacquisti();
    	StoreResultBean<DmpkWsGetattoperacquistiBean> output = service.execute(loginBean, input);
