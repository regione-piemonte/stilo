...
package it.eng.auriga.repository2.jaxws.webservices.estremiistanzaapplesterna;

import it.eng.auriga.database.store.dmpk_ws.bean.DmpkWsGetdatiapplicazioneesternaBean;
import it.eng.auriga.database.store.dmpk_ws.store.Getdatiapplicazioneesterna;
...
@WebService(targetNamespace = "http://estremiistanzaapplesterna.webservices.repository2.auriga.eng.it",  endpointInterface="it.eng.auriga.repository2.jaxws.webservices.estremiistanzaapplesterna.WSIEstremiIstanzaApplEsterna", name = "WSEstremiIstanzaApplEsterna")
@MTOM(enabled = true, threshold = 0)

public class WSEstremiIstanzaApplEsterna extends JAXWSAbstractAurigaService implements WSIEstremiIstanzaApplEsterna{

    private final String K_SAVEPOINTNAME = "INIZIO_WSEstremiIstanzaApplEst";
   
    static Logger aLogger = Logger.getLogger(WSEstremiIstanzaApplEsterna.class.getName());    
    
    public WSEstremiIstanzaApplEsterna() {
	super();
    ...
    try {
    	 aLogger.info("Inizio WSEstremiIstanzaApplEsterna");
		 ...
		 **************************************************************************/
	     ...
	     aLogger.info("Fine WSEstremiIstanzaApplEsterna");
	    
	     return risposta;
    }
  
    catch (Exception excptn) {
        aLogger.error("WSEstremiIstanzaApplEsterna: " + excptn.getMessage(), excptn);
	...
	finally
	{
	    ...
	    aLogger.info("Fine WSEstremiIstanzaApplEsterna serviceImplementation");
	}
    }

    private String callWS(AurigaLoginBean loginBean, String xmlIn) throws Exception {    	    
    	aLogger.debug("Eseguo il WS DMPK_WS->Getdatiapplicazioneesterna.");
    	
    	String result = null;    	
        		
    	// Inizializzo l'INPUT    		
    	DmpkWsGetdatiapplicazioneesternaBean   input = new DmpkWsGetdatiapplicazioneesternaBean();
    	input.setCodidconnectiontokenin(loginBean.getToken());
    	input.setXmlin(xmlIn);
	  
    	// Eseguo il servizio
    	Getdatiapplicazioneesterna service = new Getdatiapplicazioneesterna();
    	StoreResultBean<DmpkWsGetdatiapplicazioneesternaBean> output = service.execute(loginBean, input);
