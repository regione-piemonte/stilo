...
package it.eng.auriga.repository2.jaxws.webservices.savagetping;
...
@WebService(targetNamespace = "http://savagetping.webservices.repository2.auriga.eng.it",  endpointInterface="it.eng.auriga.repository2.jaxws.webservices.savagetping.WSISAVAGetPing", name = "WSSAVAGetPing")
@MTOM(enabled = true, threshold = 0)

public class WSSAVAGetPing extends JAXWSAbstractAurigaService implements WSISAVAGetPing{

    private final String K_SAVEPOINTNAME = "INIZIO_WSSAVAGetPing";
   
    static Logger aLogger = Logger.getLogger(WSSAVAGetPing.class.getName());    
    
    public WSSAVAGetPing() {
	super();
    ...
    try {
    	 aLogger.info("Inizio WSSAVAGetPing");
		 ...
		 **************************************************************************/
	     ...
	     aLogger.info("Fine WSSAVAGetPing");
	    
	     return risposta;
    }
  
    catch (Exception excptn) {
        aLogger.error("WSSAVAGetPing: " + excptn.getMessage(), excptn);
	...
	finally
	{
	    ...
	    aLogger.info("Fine WSSAVAGetPing serviceImplementation");
	}
    }

        
    private String callWS(AurigaLoginBean loginBean, String xmlIn) throws Exception {    	    
    	aLogger.debug("Eseguo il WS DMPK_WS->SAVAGetPing.");    	
    	...
    	String result = null;
    		...
    		/*
    		  // Inizializzo l'INPUT    		
    		  DmpkWsSAVAGetPingBean input = new DmpkWsSAVAGetPingBean();
    		  input.setCodidconnectiontokenin(loginBean.getToken());
    		  input.setXmlin(xmlIn);
    		  
    		  // Eseguo il servizio
    		  SAVAGetPing service = new SAVAGetPing();
    		  StoreResultBean<DmpkWsSAVAGetPingBean> output = service.execute(loginBean, input);
