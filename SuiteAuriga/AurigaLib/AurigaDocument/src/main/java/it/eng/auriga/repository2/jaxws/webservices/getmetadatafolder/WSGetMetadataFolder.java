...
package it.eng.auriga.repository2.jaxws.webservices.getmetadatafolder;
...
@WebService(targetNamespace = "http://getmetadatafolder.webservices.repository2.auriga.eng.it",  endpointInterface="it.eng.auriga.repository2.jaxws.webservices.getmetadatafolder.WSIGetMetadataFolder", name = "WSGetMetadataFolder")
@MTOM(enabled = true, threshold = 0)

public class WSGetMetadataFolder extends JAXWSAbstractAurigaService implements WSIGetMetadataFolder{

    private final String K_SAVEPOINTNAME = "INIZIO_WSGetMetadataFolder";
   
    static Logger aLogger = Logger.getLogger(WSGetMetadataFolder.class.getName());    
    
    public WSGetMetadataFolder() {
	super();
    ...
    try {
    	 aLogger.info("Inizio WSGetMetadataFolder");
		 ...
		 **************************************************************************/
	     ...
	     aLogger.info("Fine WSGetMetadataFolder");
	    
	     return risposta;
    }
  
    catch (Exception excptn) {
        aLogger.error("WSGetMetadataFolder: " + excptn.getMessage(), excptn);
	...
	finally
	{
	    ...
	    aLogger.info("Fine WSGetMetadataFolder serviceImplementation");
	}
    }
        
    private String callWS(AurigaLoginBean loginBean, String xmlIn) throws Exception {
    	aLogger.debug("Eseguo il WS DMPK_WS->GetMetadataFolder.");
    	...
    	input.setFlgtipowsin("G");     // G = Get metadati
