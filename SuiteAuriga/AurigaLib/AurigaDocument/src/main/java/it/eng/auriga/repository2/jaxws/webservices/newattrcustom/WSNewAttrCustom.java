...
package it.eng.auriga.repository2.jaxws.webservices.newattrcustom;
...
@WebService(targetNamespace = "http://newattrcustom.webservices.repository2.auriga.eng.it", endpointInterface="it.eng.auriga.repository2.jaxws.webservices.newattrcustom.WSINewAttrCustom", name = "WSNewAttrCustom")
@MTOM(enabled = true, threshold = 0)

public class WSNewAttrCustom extends JAXWSAbstractAurigaService implements WSINewAttrCustom{

    private final String K_SAVEPOINTNAME = "INIZIOWSNEWATTRCUSTOM";
   
    static Logger aLogger = Logger.getLogger(WSNewAttrCustom.class.getName());    
    
    public WSNewAttrCustom() {
	super();
    ...
    try {
    	 aLogger.info("Inizio WSNewAttrCustom");
		 ...
		 **************************************************************************/
	     ...
	     aLogger.info("Fine WSNewAttrCustom");
	    
	     return risposta;
    }
  
    catch (Exception excptn) {
        aLogger.error("WSNewAttrCustom: " + excptn.getMessage(), excptn);
	...
	finally
	{
	    ...
	    aLogger.info("Fine WSNewAttrCustom serviceImplementation");
	}
    ...
    private String callWS(AurigaLoginBean loginBean, String xmlIn) throws Exception {
    	aLogger.debug("Eseguo il WS DMPK_WS->Addupdattrcustom.");
    	...
    	// viene eseguita la DMPK_WS.AddUpdAttrCustom(ADD) per aggiungere  un nuovo attributo
    	input.setFlgoperazionein("ADD");
    ...
    private String generaXMLRispostaWS(String xmlIn)  throws Exception {
        ...
        try {
            ...
            xml.append("<NewAttrCustom>" + xmlInEsc + "</NewAttrCustom>\n");
