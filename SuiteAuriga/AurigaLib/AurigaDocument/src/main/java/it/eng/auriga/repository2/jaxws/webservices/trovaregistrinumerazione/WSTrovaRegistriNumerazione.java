...
package it.eng.auriga.repository2.jaxws.webservices.trovaregistrinumerazione;

import it.eng.auriga.database.store.dmpk_ws.bean.DmpkWsTrovaregistrinumerazioneBean;
import it.eng.auriga.database.store.dmpk_ws.store.Trovaregistrinumerazione;
...
@WebService(targetNamespace = "http://trovaregistrinumerazione.webservices.repository2.auriga.eng.it",  endpointInterface="it.eng.auriga.repository2.jaxws.webservices.trovaregistrinumerazione.WSITrovaRegistriNumerazione", name = "WSTrovaRegistriNumerazione")
@MTOM(enabled = true, threshold = 0)

public class WSTrovaRegistriNumerazione extends JAXWSAbstractAurigaService implements WSITrovaRegistriNumerazione{

    private final String K_SAVEPOINTNAME = "INIZIO_WSTrovaRegistriNumerazione";
   
    static Logger aLogger = Logger.getLogger(WSTrovaRegistriNumerazione.class.getName());    
    
    public WSTrovaRegistriNumerazione() {
	super();
    ...
    try {
    	 aLogger.info("Inizio WSTrovaRegistriNumerazione");
		 ...
		 **************************************************************************/
	     ...
	     aLogger.info("Fine WSTrovaRegistriNumerazione");
	    
	     return risposta;
    }
  
    catch (Exception excptn) {
        aLogger.error("WSTrovaRegistriNumerazione: " + excptn.getMessage(), excptn);
	...
	finally
	{
	    ...
	    aLogger.info("Fine WSTrovaRegistriNumerazione serviceImplementation");
	}
    }

        
    private String callWS(AurigaLoginBean loginBean, String xmlIn) throws Exception {
    	aLogger.debug("Eseguo il WS DMPK_WS->TrovaRegistriNumerazione.");    	
    	
    	String result = null;
    	
	    // Inizializzo l'INPUT    		
	    DmpkWsTrovaregistrinumerazioneBean input = new DmpkWsTrovaregistrinumerazioneBean();
	    input.setCodidconnectiontokenin(loginBean.getToken());
	    input.setXmlin(xmlIn);
	  
	    // Eseguo il servizio
	    Trovaregistrinumerazione service = new Trovaregistrinumerazione();
	    StoreResultBean<DmpkWsTrovaregistrinumerazioneBean> output = service.execute(loginBean, input);
