...
package it.eng.auriga.repository2.jaxws.webservices.trovasoggettirubrica;

import it.eng.auriga.database.store.dmpk_ws.bean.DmpkWsTrovasoggettirubricaBean;
import it.eng.auriga.database.store.dmpk_ws.store.Trovasoggettirubrica;
...
@WebService(targetNamespace = "http://trovasoggettirubrica.webservices.repository2.auriga.eng.it",  endpointInterface="it.eng.auriga.repository2.jaxws.webservices.trovasoggettirubrica.WSITrovaSoggettiRubrica", name = "WSTrovaSoggettiRubrica")
@MTOM(enabled = true, threshold = 0)

public class WSTrovaSoggettiRubrica extends JAXWSAbstractAurigaService implements WSITrovaSoggettiRubrica{

    private final String K_SAVEPOINTNAME = "INIZIO_WSTrovaSoggettiRubrica";
   
    static Logger aLogger = Logger.getLogger(WSTrovaSoggettiRubrica.class.getName());    
    
    public WSTrovaSoggettiRubrica() {
	super();
    ...
    try {
    	 aLogger.info("Inizio WSTrovaSoggettiRubrica");
		 ...
		 **************************************************************************/
	     ...
	     aLogger.info("Fine WSTrovaSoggettiRubrica");
	    
	     return risposta;
    }
  
    catch (Exception excptn) {
        aLogger.error("WSTrovaSoggettiRubrica: " + excptn.getMessage(), excptn);
	...
	finally
	{
	    ...
	    aLogger.info("Fine WSTrovaSoggettiRubrica serviceImplementation");
	}
    }

        
    private String callWS(AurigaLoginBean loginBean, String xmlIn) throws Exception {
    	aLogger.debug("Eseguo il WS DMPK_WS->TrovaSoggettiRubrica.");    	
    	
    	String result = null;
    	
	    // Inizializzo l'INPUT    		
	    DmpkWsTrovasoggettirubricaBean input = new DmpkWsTrovasoggettirubricaBean();
	    input.setCodidconnectiontokenin(loginBean.getToken());
	    input.setXmlin(xmlIn);
	  
	    // Eseguo il servizio
	    Trovasoggettirubrica service = new Trovasoggettirubrica();
	    StoreResultBean<DmpkWsTrovasoggettirubricaBean> output = service.execute(loginBean, input);
