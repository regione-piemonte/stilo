...
package it.eng.auriga.repository2.jaxws.webservices.moduser;

import it.eng.auriga.database.store.dmpk_ws.bean.DmpkWsModuserBean;
import it.eng.auriga.database.store.dmpk_ws.store.Moduser;
...
@WebService(targetNamespace = "http://moduser.webservices.repository2.auriga.eng.it",  endpointInterface="it.eng.auriga.repository2.jaxws.webservices.moduser.WSIModUser", name = "WSModUser")
@MTOM(enabled = true, threshold = 0)

public class WSModUser extends JAXWSAbstractAurigaService implements WSIModUser{

    private final String K_SAVEPOINTNAME = "INIZIO_WSModUser";
   
    static Logger aLogger = Logger.getLogger(WSModUser.class.getName());    
    ...
    try {
		 ...
		 **************************************************************************/
	     ...
	     aLogger.info("Fine WSModUser");
	    
	     return risposta;
    }
  
    catch (Exception excptn) {
        aLogger.error("WSModUser: " + excptn.getMessage(), excptn);
	...
	finally
	{
	    ...
	    aLogger.info("Fine WSModUser serviceImplementation");
	}
    }

        
    private String callWS(AurigaLoginBean loginBean, String xmlIn) throws Exception {    
    	aLogger.debug("Eseguo il WS DMPK_WS->ModUser.");    	
    	
    	String ret = null;    	
    	    		
    	// Inizializzo l'INPUT    		
    	DmpkWsModuserBean input = new DmpkWsModuserBean();
    	input.setCodidconnectiontokenin(loginBean.getToken());
    	input.setXmlin(xmlIn);

    	// Eseguo il servizio
    	Moduser service = new Moduser();
    	StoreResultBean<DmpkWsModuserBean> output = service.execute(loginBean, input);
