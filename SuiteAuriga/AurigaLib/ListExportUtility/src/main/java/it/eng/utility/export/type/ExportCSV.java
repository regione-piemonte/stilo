...
package it.eng.utility.export.type;
...
public class ExportCSV implements IExport {
	...
	public void export(File file, ExportBean bean)throws Exception{
		...
		//Inizializzo ci CSV Writer
		CsvWriter csvw = new CsvWriter(output, bean.getCsvSeparator(),Charset.forName("ISO-8859-1"));
		...
		if(bean.showHeaders()) {
			...
			for (int c = 0; c < bean.getHeaders().length; c++) {
				headers.add(StringEscapeUtils.unescapeHtml(bean.getHeaders()[c]));	
			}
			csvw.writeRecord((String[])headers.toArray(new String[0]));
		...
		for (int r = 0; r < bean.getRecords().length; r++) {
			...
			//Inserisco in un array di stringhe i valori da inserire su csv
			for (int c = 0; c < bean.getFields().length; c++) {
				values.add(StringEscapeUtils.unescapeHtml((String) record.get(bean.getFields()[c])));	
			}
			//Aggiungo il record
			csvw.writeRecord((String[])values.toArray(new String[0]));
		}
	...
	private class CsvWriter {
		 ...
		 *            writing the data.
		 */
		public CsvWriter(String fileName, String delimiter/* stefa char delimiter */, Charset charset) {
			if (fileName == null) {
		...
		public CsvWriter(String fileName) {
			this(fileName, "," /*stefa Letters.COMMA */, Charset.forName("ISO-8859-1"));
		}
		 ...
		 *            The character to use as the column delimiter.
		 */
		public CsvWriter(Writer outputStream, String delimiter) { //stefa char delimiter) {
		 ...
		 *            writing the data.
		 */
		public CsvWriter(OutputStream outputStream, String delimiter/*stefa char delimiter*/, Charset charset) {
		 ...
		 */
		public String /*char*/ getDelimiter() {//stefano
		 ...
		 *            The character to use as the column delimiter.
		 */
		public void setDelimiter(String delimiter) {//stefano(char delimiter) {
			userSettings.Delimiter = delimiter;
		}
		...
		private class UserSettings {
			...
			public UserSettings() {
				...
				Delimiter = ",";//stefano Letters.COMMA;
