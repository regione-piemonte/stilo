...
public class RegistroPubblicazioniRequestBuilder extends DocumentRequestBuilder<DocumentoRegistroPropertiesType> {
    ...
    protected void doBuild(Map<String, String> dynamicAttributesMap, IdStatoDiEfficaciaType idStatoDiEfficaciaNonFirmatoType) {
        ...
        if( dynamicAttributesMap.containsKey("registroFirmato")  && dynamicAttributesMap.get("registroFirmato")!=null ) {
        	if( dynamicAttributesMap.get("registroFirmato").equalsIgnoreCase("true"))
    ...
    private String createObjectName() {
        final StringBuilder objectBuilder = new StringBuilder();
        objectBuilder.append("REGPUBB");
        objectBuilder.append(LocalDate.now().getYear());
        objectBuilder.append(LocalDate.now().getMonth().getValue());
