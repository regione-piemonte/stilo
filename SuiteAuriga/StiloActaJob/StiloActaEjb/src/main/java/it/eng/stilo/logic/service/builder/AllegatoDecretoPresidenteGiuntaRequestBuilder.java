...
package it.eng.stilo.logic.service.builder;
...
public class AllegatoDecretoPresidenteGiuntaRequestBuilder extends DocumentRequestBuilder<DocumentoSemplicePropertiesType> {
    ...
    protected void doBuild(Map<String, String> dynamicAttributesMap, IdStatoDiEfficaciaType idStatoDiEfficaciaNonFirmatoType) {
    	...
    	if( dynamicAttributesMap.containsKey("oggetto") && dynamicAttributesMap.get("oggetto")!=null)
    		...
    		if( ente!=null && ente.equalsIgnoreCase("RP")){
    			documentoPropertiesType.setOggetto(EIntegrationResourceRP.ALLEGATO_DECRETO_PRESIDENTE_GIUNTA.name().toLowerCase());
    		}
    		if( ente!=null && ente.equalsIgnoreCase("CMTO")){
    			documentoPropertiesType.setOggetto(EIntegrationResourceCMTO.ALLEGATO_DECRETO_PRESIDENTE_GIUNTA_CMTO.name().toLowerCase());
    		}
    		if( ente!=null && ente.equalsIgnoreCase("COTO")){
    			documentoPropertiesType.setOggetto(EIntegrationResourceCOTO.ALLEGATO_DECRETO_PRESIDENTE_GIUNTA_COTO.name().toLowerCase());
    		}
    	}
