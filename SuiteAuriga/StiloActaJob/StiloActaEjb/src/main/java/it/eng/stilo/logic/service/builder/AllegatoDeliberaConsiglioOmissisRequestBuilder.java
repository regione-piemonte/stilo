...
public class AllegatoDeliberaConsiglioOmissisRequestBuilder extends AllegatoDeterminaRequestBuilder {
    ...
    protected void doBuild(Map<String, String> dynamicAttributesMap, IdStatoDiEfficaciaType idStatoDiEfficaciaNonFirmatoType) {
        ...
        if( dynamicAttributesMap.containsKey("oggetto") && dynamicAttributesMap.get("oggetto")!=null)
    		...
    		if( ente!=null && ente.equalsIgnoreCase("RP")){
    			//documentoPropertiesType.setOggetto(EIntegrationResourceRP.ALLEGATO_DETERMINA_OMISSIS.name().toLowerCase());
    		}
    		if( ente!=null && ente.equalsIgnoreCase("CMTO")){
    			documentoPropertiesType.setOggetto(EIntegrationResourceCMTO.ALLEGATO_DELIBERA_CONSIGLIO_OMISSIS_CMTO.name().toLowerCase());
    		}
    		if( ente!=null && ente.equalsIgnoreCase("COTO")){
    			//documentoPropertiesType.setOggetto(EIntegrationResourceCOTO.ALLEGATO_DETERMINA_OMISSIS_COTO.name().toLowerCase());
    		}
    	}
