...
@Named
public class ActaDocumentItemProcessor implements ItemProcessor {
    ...
    public Object processItem(Object item) {
        ...
        logger.info(String.format("[Document=%d][#Attachments=%d]", queueDocument.getId(),
                queueDocument.getAttachments().size()));

        try {
            final OperationResult operationResult = actaIntegrationEJB.createDocument(queueDocument);
            ...
            logger.error(String.format("[ErrorProcessing-Document=%d]", queueDocument.getId()), e);
        }
