...
import it.eng.auriga.database.store.dmpk_core.bean.DmpkCoreSetregnumudBean;
...
import it.eng.bean.DocumentoRecuperatoBean;
import it.eng.bean.FieldChangeInterceptor;
import it.eng.bean.InfoFileBean;
import it.eng.bean.InfoFileUtilityJob;
...
import it.eng.client.DmpkCoreSetregnumud;
...
public class DocumentUtil {
	...
	public static InfoFileBean getFileInfo(File fileDoc) throws BeansException, DocumentUtilException {
		InfoFileBean lInfoFileBean = new InfoFileBean();
		DocumentConfiguration lDocumentConfiguration = (DocumentConfiguration) SpringHelper.getMainApplicationContext()
	...
	public static AllegatoDocumentoInBean preparaAllegatoDocumentoInBean(String nomeFileDoc, File fileDoc) throws DocumentUtilException {
		final InfoFileBean fileInfo = DocumentUtil.getFileInfo(fileDoc);
	...
	/**
	 * Aggiunge un allegato ad un'istanza di {@link AllegatiBean} passando la tipologia
	 * 
	 * @param allegatiBean
	 *            {@link AllegatiBean}, se null lo crea
	 * @param allegatoBean
	 ...
	 * @throws DocumentUtilException
	 */
	public static void aggiungiAllegato(AllegatiBean allegatiBean, AllegatoBean allegatoBean, TipoFile tipo) throws DocumentUtilException {
		if (allegatiBean == null)
			allegatiBean = creaAllegatiBean();
		if (allegatoBean != null) {
...
//	}
	...
	//Metodo utilizzato per aggiornare la RegNum
	public static boolean aggiornaRegNumUd(AurigaLoginUtil aurigaLoginUtil, BigDecimal idUd, BigDecimal idDoc, CreaModFatturaInBean inBean) throws DocumentUtilException {
		
		boolean updated = true;
		StoreResultBean<DmpkCoreSetregnumudBean> lOutputUpdRegNum = null;
		DmpkCoreSetregnumud lservizioUpdRegNum = null;
		DmpkCoreSetregnumudBean inputUpdRegNum = null;
		
		try {
			inputUpdRegNum = new DmpkCoreSetregnumudBean();
			inputUpdRegNum.setCodidconnectiontokenin(aurigaLoginUtil.getAurigaLoginBean().getToken());
			inputUpdRegNum.setIduserlavoroin(null);
			if (idUd != null) {
				inputUpdRegNum.setIdudin(idUd);
			} else {
				throw new DocumentUtilException("Parametro mancante [idUd / idDoc]");
			}
			XmlUtilitySerializer lXmlUtilitySerializer = new XmlUtilitySerializer();	
			inputUpdRegNum.setRegnumdatexmlin(lXmlUtilitySerializer.bindXmlList(inBean.getRegistroEmergenza()));
			lservizioUpdRegNum = new DmpkCoreSetregnumud();	
		} catch (Exception e) {
			updated = false;
			throw new DocumentUtilException("Errore nella serializzazione degli attributi", e);
		}
		...
		while (numRetry < businessCallRetryNum) {
			try {
				numRetry++;
				lOutputUpdRegNum = lservizioUpdRegNum.execute(aurigaLoginUtil.getLocale(), aurigaLoginUtil.getAurigaLoginBean(), inputUpdRegNum);
				break;
			} catch (Exception e) {
				updated = false;
				if (numRetry < businessCallRetryNum) {
					log.warn("Tentativo n." + numRetry + " di chiamata a DmpkCoreSetregnumud.execute fallito.", e);
					log.warn("Nuovo tentativo fra " + (businessCallRetryInterval / 1000) + " secondi", e);
					try {
						Thread.sleep(businessCallRetryInterval);
					} catch (InterruptedException intex) {
					...
					log.error("Errore nella chiamata al servizio di aggiornamento RegNum", e);
					updated = false;
					throw new DocumentUtilException(e);
				}
			}
		...
		if (lOutputUpdRegNum.isInError()) {
			log.error("Il servizio di aggiornamento documento ha restituito l'errore: " + lOutputUpdRegNum.getDefaultMessage());
			throw new DocumentUtilException(lOutputUpdRegNum.getDefaultMessage());
		}
	...
	public static DownloadTicketBean getDownloadTicket(AurigaLoginUtil aurigaLoginUtil, String uriFileDoc, String nomeFileDoc, BigDecimal idUd)
			throws DocumentUtilException {
		try {
			String nrTicket =randomId(); //StringUtils.defaultString(String.valueOf(HibernateUtil.getNextVal(SEQ_DOWNLOAD_TICKET)));
        ...
        public static String randomId() throws DocumentUtilException{
		
		log.debug("Effettuo la query per recueperare il random ID ");
		...
		try {
			session = HibernateUtil.begin("randomId");
			
			StringBuilder sbSql = new StringBuilder("select dbms_random.string('A', 40) str from dual ");
			...
			log.error("Errore in randomId ", e);
			throw new DocumentUtilException(e);
		} finally{
	...
	public static boolean equalsMimetype(File fileTemplateCorpo, String mimetype) throws DocumentUtilException {
		...
		String result = null;
		try {
			session = HibernateUtil.begin("StampaRegistroProtocollazioniJob");
