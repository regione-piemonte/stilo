		...
		if (result.isSuccessful()) {
			...
			for (JobBean currentJob : results.getData()) {
				...
				if (currentJob.getIdJob()!= null) {
					...
					filterBean4.setParameterSubtype("CONN_TOKEN");
					...
					filterBean5.setParameterSubtype("ID_USER_LAVORO");
					...
					filterBean6.setParameterSubtype("FIRMA_REMOTA");
					...
					try {
						...
						if (parameter4 != null && parameter4.getParameterValue() != null) {
							// parametro da DB
							aurigaLoginBean.setToken(parameter4.getParameterValue());
						}
						else {
							// parametro da DB null allora da configurzione
						...
						if (parameter5 != null && parameter5.getParameterValue() != null) {
							// parametro da DB
							aurigaLoginBean.setIdUserLavoro(parameter5.getParameterValue());
						}
						else {
							// parametro da DB null allora da configurzione
						...
						String firmaremota = null;
						if (parameter6 != null && parameter6.getParameterValue() != null) {
							// parametro da DB
							firmaremota = parameter6.getParameterValue();
						}
						else {
							// parametro da DB null allora da configurzione
