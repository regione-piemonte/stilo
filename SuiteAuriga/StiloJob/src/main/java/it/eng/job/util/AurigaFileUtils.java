...
/*				} else if (algoritmo.equalsIgnoreCase("SHA-1")) {
	...
	public InfoFileBean  getInfoFirmaFromFile(File fileToCheck, String displayFileName, Date dataRif)
			throws Exception {
		InfoFileUtilityJob infoFileUtility = new InfoFileUtilityJob();
		return infoFileUtility.getInfoFirmaFromFile(fileToCheck, true, dataRif, true);
	}
	...
	public static String getExtensionFromMimeType(String mimeType) {
		...
		try {
			...
			logger.error("Eccezione durante il recupero dell'estensione del file ", e);
		}
