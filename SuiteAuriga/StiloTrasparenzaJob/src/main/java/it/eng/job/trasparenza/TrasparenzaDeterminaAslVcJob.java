...
@Job(type = "TrasparenzaDeterminaAslVcJob")
@Named
public class TrasparenzaDeterminaAslVcJob extends AbstractJob<String> {

	private static Logger log = Logger.getLogger(TrasparenzaDeterminaAslVcJob.class);
	...
	private static final String JOB_CLASS_NAME = TrasparenzaDeterminaAslVcJob.class.getName();
	...
	private void elabora() throws Exception {
		...
		try {
			log.info("******************************************");
			log.info("0. Inizio elaborazione Trasparenza Determina Asl VC");
			...
			List<TCodaXExportTrasp> codaTraspList = trasparenzaDAOImpl.getCodaExportTrasparenza(FruitoreEnum.ASLVC_DD.getDescription(), numTryMax, sessionJob);
					...
					try {
						...
						if (item.getOggDaEsportare() != null && !item.getOggDaEsportare().equals("")) {
							...
							if (provExp != null) {
								...
								provvedimentoTrasparenzaBean = trasparenza.aggiungiDeterminaTrasparenzaAslVc(provvedimentoTrasparenzaBean);
			...
			log.info("8. Fine job Trasparenza Determina ASL VC: elaborati " + numRecordElaborati + " record");
			log.info("******************************************");
		}
	}
