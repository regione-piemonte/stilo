...
import net.sourceforge.vietocr.CustomImageIOHelper;
 ...
 */
public class CustomTesseract {

	private static Logger aLogger = Logger.getLogger(CustomTesseract.class.getName());

	private static CustomTesseract instance;
	 ...
	 * @return instance
	 */
	public static synchronized CustomTesseract getInstance() {
		if (instance == null) {
			instance = new CustomTesseract();
		}
			...
			if( files!=null ){
				for (File tiff : files) {
					...
					try {
						...
						String command = "C:/Program Files/Tesseract-OCR/tesseract \"" + tiff.getCanonicalPath() + "\" \"" + tiff.getAbsolutePath() + "\"";
						...
						if (txt.exists()) {
							...
							throw new Exception("Il file di elaborazione di tesseract non � stato creato!");
						}

				} catch (Throwable e) {
					throw e;
				} finally {
					boolean esitocancellazione = FileUtil.deleteFile(tiff);
					aLogger.debug("esitocancellazione tiff "+ esitocancellazione);
					esitocancellazione = FileUtil.deleteFile(txt);
					aLogger.debug("esitocancellazione txt "+ esitocancellazione);
	...
	public String executableTesseract4Unix(File imageFile) throws TesseractException {
		...
		try {
			...
			if( files!=null){
				...
				aLogger.error("Nessun files dad elaborare. OCR non effettuata");
			}
	...
	private String doOCR(File imageFile, Rectangle rect) throws TesseractException {
		try {
			List<IIOImage> imageList = CustomImageIOHelper.getIIOImageList(imageFile);
	...
	private String doOCR(List<IIOImage> imageList, Rectangle rect) throws TesseractException {
		...
		aLogger.debug("doOCR imageList " + imageList);
		for (IIOImage oimage : imageList) {
			try {
				ByteBuffer buf = CustomImageIOHelper.getImageByteBuffer(oimage);
