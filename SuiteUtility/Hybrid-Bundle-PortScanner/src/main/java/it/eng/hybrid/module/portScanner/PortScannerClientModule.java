...
public class PortScannerClientModule implements IClientHttpModule, IClientWebSocketModule {

	public final static Logger logger = Logger.getLogger(PortScannerClientModule.class);
	...
	public static void activateBuiltin(IClientModuleManager moduleManager) throws Exception {
		Resources.start("PortScannerClientModule");
		moduleManager.registerModule(new PortScannerClientModule("PortScannerClientModule", "1.0.0"));
	}
	...
	public PortScannerClientModule(String name, String version) {
		this.name = name;
	...
	public Map<String, Object> processHttpRequest(Map<String, Object> request) throws Exception {
		...
		if (uri.equals(getModuleUri() + "/stub")) {
			...
			try {
				...
				if ("portScanner".equals(method)) {
					logger.debug("portScanner equals method");
					// TODO Togliere?
	...
	private void showAppl(JSONArray parameters) {
		...
		final PortScannerApplication appl = new PortScannerApplication(this, parameters);
		...
		// Il modulo termina
		logger.debug("PortScannerHybridModule terminato");
	}
