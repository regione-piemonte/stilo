...
import it.eng.hybrid.module.jpedal.function.FunctionFactory;
import it.eng.hybrid.module.jpedal.function.PDFFunction;
...
import it.eng.hybrid.module.jpedal.objects.raw.FunctionObject;
...
public class ImageCommands {
    ...
    static{
        ...
        if(operlapValue!=null)
            ImageCommands.rejectSuperimposedImages=(operlapValue!=null && operlapValue.toLowerCase().contains("true"));

        //hidden value to turn on function
        String imgSetting=System.getProperty("org.jpedal.trackImages");
        if(imgSetting!=null)
            trackImages=(imgSetting!=null && imgSetting.toLowerCase().contains("true"));
    ...
    static BufferedImage addMaskObject(GenericColorSpace decodeColorData,
                                       int d, boolean isDCT, boolean isJPX, BufferedImage image,
                                       int colorspaceID, byte[] index, PdfObject newMask, int optionsApplied, PdfObjectReader currentPdfFile) {
        {
            ...
            if(maskArray!=null){
                ...
                if(image.getType()==BufferedImage.TYPE_BYTE_GRAY && maskArray.length==6 && maskArray[0]==maskArray[1] && maskArray[2]==maskArray[3] && maskArray[4]==maskArray[5]){
                ...
                if(maskDecodeArray!=null && (colorspaceID== ColorSpaces.DeviceRGB || colorspaceID==ColorSpaces.Separation))
                    applyDecodeArray(objectData, maskDecodeArray.length/2, maskDecodeArray,colorspaceID, newMask);
    ...
    public static BufferedImage applySmask(BufferedImage image, BufferedImage smask,
                                           PdfObject newSMask, boolean isForm, boolean isRGB, PdfObject ColorSpace, PdfObject XObject, GraphicsState gs) {
                ...
                if(!noMatch){
                    ...
                    if(colorComponents==1){
                        ...
                        if(values[0]==pix[0]){//pix[0]>32 && pix[1]>32 && pix[2]>32 && values[0]<32)
                            if(pix[0]==255 && (imageType==1 || imageType==5)  && isDeviceGray) { //white is different in diff Buffered images
    ...
    static BufferedImage simulateOverprint(GenericColorSpace decodeColorData,
...
//                    }else
            ...
            }else if(isDCT && newSMask==null &&  newMask==null &&
                    (gs.nonstrokeColorSpace.getColor().getRGB()==-16777216 || gs.nonstrokeColorSpace.getColor().getRGB()==-1)
                    && decodeColorData.isImageYCCK() && decodeColorData.getIntent()!=null && decodeColorData.getIntent().equals("RelativeColorimetric") ){
