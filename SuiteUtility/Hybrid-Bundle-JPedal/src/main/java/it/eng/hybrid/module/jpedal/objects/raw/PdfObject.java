...
public class PdfObject implements Cloneable{
	...
	public final static Logger logger = Logger.getLogger(PdfObject.class);
...
//            return Filter;
    ...
    public byte[] getDecodedStream() {
        ...
        if(isCached()){
            ...
            File f=new File(getCachedStreamFile(objReader));
    ...
    public int getObjectRefID() {
        ...
        if(intRef==0 && ref!=null && !ref.contains("[")){
            ...
            if(ptr>0){
                try{
                    intRef= Integer.parseInt(ref.substring(0,ptr));
                }catch(Exception e){
                    //tell user and log
                    logger.info("Exception: "+e.getMessage());
                }

                int ptr2=ref.indexOf('R',ptr);

                if(ptr2>0){
                    //System.out.println(ref.substring(ptr+1,ptr2-1)+"<<gen="+ref+"<");
                    try{
                        gen= Integer.parseInt(ref.substring(ptr+1,ptr2-1));
                    }catch(Exception e){
                        //tell user and log
                        logger.info("Exception: "+e.getMessage());
                    }
