...
public class TipOfTheDay extends JDialog {
		...
		try {
			...
			if(url.toString().startsWith("jar")){
				...
				for (Enumeration e = jar.entries(); e.hasMoreElements();) {
					...
					if ((!entry.isDirectory()) && name.contains("/res/tips/")
	...
	private void addTopPanel(GridBagConstraints mainPanelConstraints) {
		...
		JLabel tipImage = new JLabel(new ImageIcon(getClass().getResource("/it/eng/hybrid/module/jpedal/resources/tip.png")));
