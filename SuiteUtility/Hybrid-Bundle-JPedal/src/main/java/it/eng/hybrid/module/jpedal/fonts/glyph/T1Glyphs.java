...
* ---------------
*/
package it.eng.hybrid.module.jpedal.fonts.glyph;

import it.eng.hybrid.module.jpedal.fonts.FontData;
import it.eng.hybrid.module.jpedal.fonts.StandardFonts;
import it.eng.hybrid.module.jpedal.fonts.Type1;
...
public class T1Glyphs extends PdfJavaGlyphs {
	...
	public final static Logger logger = Logger.getLogger( T1Glyphs.class );
    ...
    public static String[] readFontNames(FontData fontData,int mode) {
        ...
        if(br!=null){
            try{
            	...
            	logger.info("Exception " + e + " closing stream");
            }
        }
				...
				if (nextVal >31 || nextVal==28) {  //if its a number get it and update pointer p
                    ...
                    if (lastNumberStart == 0) {
                        if (nominalWidthX.length == 1) {
                            ...
                            int glyphNo = (Integer)glyphNumbers.get(""+rawInt)-1;
                    ...
                    if (commandCount == -1 &&                                   //Only on first command
                            baseFontName != null &&                             //Check right call
                            dynamicVectorRenderer != null &&                    //Check right call
                            (dynamicVectorRenderer.getType()== org.jpedal.render.output.html.HTMLDisplay.CREATE_HTML ||
                                    dynamicVectorRenderer.getType()== org.jpedal.render.output.html.HTMLDisplay.CREATE_JAVAFX ||
                                    dynamicVectorRenderer.getType()== org.jpedal.render.output.html.HTMLDisplay.CREATE_SVG)) {       //Just to be safe
