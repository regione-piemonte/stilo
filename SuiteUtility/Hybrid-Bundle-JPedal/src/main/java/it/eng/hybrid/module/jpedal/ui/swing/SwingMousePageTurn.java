...
public class SwingMousePageTurn {
	...
	public void mousePressed(MouseEvent event) {
		...
		if (previewTurnover && currentCommands.getPages().getBoolean(Display.BoolValue.TURNOVER_ON) && decode_pdf.getDisplayView()==Display.FACING &&
				event.getButton()==MouseEvent.BUTTON1) {
...
//				mover.testFall(corner,event.getPoint(),dragLeft);
	...
	public void mouseDragged(MouseEvent event) {
		if(SwingUtilities.isLeftMouseButton(event)){
			...
			if (currentCommands.getPages().getBoolean(Display.BoolValue.TURNOVER_ON) && (drawingTurnover || previewTurnover) && decode_pdf.getDisplayView()==Display.FACING) {
				currentGUI.setCursor(SwingGUI.GRABBING_CURSOR);

				//update coords
				if (currentGUI.getDragLeft()) {
					if (currentGUI.getDragTop())
						decode_pdf.setUserOffsets(event.getX(), event.getY(), OffsetOptions.INTERNAL_DRAG_CURSOR_TOP_LEFT);
					else
						decode_pdf.setUserOffsets(event.getX(), event.getY(), OffsetOptions.INTERNAL_DRAG_CURSOR_BOTTOM_LEFT);
				} else {
					if (currentGUI.getDragTop())
						decode_pdf.setUserOffsets(event.getX(), event.getY(), OffsetOptions.INTERNAL_DRAG_CURSOR_TOP_RIGHT);
					else
						decode_pdf.setUserOffsets(event.getX(), event.getY(), OffsetOptions.INTERNAL_DRAG_CURSOR_BOTTOM_RIGHT);
				}
			}
	...
	public void mouseMoved(MouseEvent event) {
		...
		if (decode_pdf.getDisplayView()==Display.PAGEFLOW) {
			...
			if (x > 0 && x <= decode_pdf.getPdfPageData().getPageCount())
                ...
                currentCommands.getPages().getBoolean(Display.BoolValue.TURNOVER_ON) &&
			...
			if (commonValues.getCurrentPage()+1 < commonValues.getPageCount()) {
				...
				if (cursor.x > corner.x-30 && cursor.x <= corner.x &&
					...
					if (cursor.y >= corner.y-pageH && cursor.y <corner.y-pageH+30) {
						...
						decode_pdf.setUserOffsets((int)cursor.getX(), (int)cursor.getY(), OffsetOptions.INTERNAL_DRAG_CURSOR_TOP_RIGHT);
					} else
			...
			if (commonValues.getCurrentPage() != 1) {
				...
				if (cursor.x < corner.x+30 && cursor.x >= corner.x &&
					...
					if (cursor.y >= corner.y-pageH && cursor.y < corner.y-pageH+30) {
						corner.y = (int) (corner.y - pageH);
						decode_pdf.setUserOffsets((int)cursor.getX(), (int)cursor.getY(), OffsetOptions.INTERNAL_DRAG_CURSOR_TOP_LEFT);
					} else
		...
		if (currentGUI.useNewLayout) {
			...
			if (decode_pdf.getDisplayView() == Display.SINGLE_PAGE || SwingMouseSelector.activateMultipageHighlight) {
				//get raw w and h
				int rawW,rawH;
				if (currentGUI.getRotation()%180==90) {
					rawW = decode_pdf.getPdfPageData().getCropBoxHeight(1);
				...
				Thread animation = new Thread() {
					public void run() {
						...
						if (!fallBack) {
							...
							if (currentCommands.getPages().getBoolean(Display.BoolValue.SEPARATE_COVER) && forwardPage%2==1)
								forwardPage++;
                            else if (!currentCommands.getPages().getBoolean(Display.BoolValue.SEPARATE_COVER) && forwardPage%2==0)
                                forwardPage++;
