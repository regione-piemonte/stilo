...
public class ThumbnailDecoder {
	...
	public final static Logger logger = Logger.getLogger(ThumbnailDecoder.class);
	...
	/**
        ...
        BufferedImage newImg =null;
			//stopDecoding=false;
			try {
				BufferedImage pageImage = decode_pdf.getPageAsImage(pageNumber);
        ...
        /**
			...
			DynamicVectorRenderer imageDisplay=null;// ObjectStore.getCachedPage(new Integer(pageNumber));
			...
			if(imageDisplay!=null){
				...
				try {
					...
					if (pageNumber > decode_pdf.getPageCount()) {
						...
						logger.info("Page " + pageNumber + " out of bounds");
						...
						if ((currentPageOffset != null)) {
							...
							if (pdfObject != null) {
                                ...
                                imageDecoder = new PdfStreamDecoder(decode_pdf.useHiResImageForDisplay, null, decode_pdf.globalResources);
