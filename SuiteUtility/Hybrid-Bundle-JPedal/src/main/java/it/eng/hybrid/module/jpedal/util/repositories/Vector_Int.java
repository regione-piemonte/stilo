...
* ---------------
*/
package it.eng.hybrid.module.jpedal.util.repositories;
...
public class Vector_Int implements Serializable
{
	...
	public String toString(){
		...
		//copy all items back except item to delete
        for (int item : items) returnString = returnString + ' ' + item;
	...
	final public void addElement( int value )
	{
        ...
        if(debug)
            System.out.println(current_item+"="+value);
	}
...
//			}
    ...
    public void resetToCheckpoint() {
        ...
        if(debug)
        System.out.println("checkpoint="+checkPoint+" current="+current_item);
    ...
    public void setCheckpoint() {
        ...
        if(debug)
            System.out.println("1set checkpoint="+current_item+ ' ' +checkPoint);

        if(checkPoint==-1)
            checkPoint=current_item;

        if(debug)
                System.out.println("2set checkpoint="+current_item+ ' ' +checkPoint);
