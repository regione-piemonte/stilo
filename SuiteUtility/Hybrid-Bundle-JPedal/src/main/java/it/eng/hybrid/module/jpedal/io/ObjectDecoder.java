...
import it.eng.hybrid.module.jpedal.exception.PdfSecurityException;
import it.eng.hybrid.module.jpedal.objects.raw.ColorSpaceObject;
import it.eng.hybrid.module.jpedal.objects.raw.FormObject;
import it.eng.hybrid.module.jpedal.objects.raw.NamesObject;
import it.eng.hybrid.module.jpedal.objects.raw.ObjectFactory;
...
public class ObjectDecoder implements Serializable {
	...
	public final static Logger logger = Logger.getLogger(ObjectDecoder.class);
    ...
    private int readObjectDataValues(PdfObject pdfObject, int i, byte[] raw, int length) {
        ...
        while(true){
            ...
            if(raw[i]==60 && raw[i+1]==60){
                ...
                ArrayDecoder objDecoder=new ArrayDecoder(objectReader,i, raw.length, PdfDictionary.VALUE_IS_MIXED_ARRAY,null, PdfDictionary.Names);
    ...
    private void getKeyType(PdfObject pdfObject, int i, byte[] raw, int length, int keyLength, int keyStart, int type) {
        ...
        /**if((pdfKeyType==-1 && id== PdfDictionary.ClassMap) ||
                ((pdfKeyType==-1 || (keyLength<3 &&
                        (id==PdfDictionary.N || id==PdfDictionary.D || id==PdfDictionary.R))) &&
            ...
            if(debugFastCode)
                System.out.println("Returns "+pdfKeyType+" i="+i);
        }/**/
    ...
    private int setValue(PdfObject pdfObject,int i, byte[] raw, int length, boolean map) {
        ...
        switch(pdfKeyType){
            ...
            case PdfDictionary.VALUE_IS_TEXTSTREAM:{
                ...
                ArrayDecoder objDecoder=new ArrayDecoder(objectReader,i, endPt, PdfDictionary.VALUE_IS_STRING_ARRAY);
                ...
                ArrayDecoder objDecoder=new ArrayDecoder(objectReader, i, endPt, PdfDictionary.VALUE_IS_BOOLEAN_ARRAY);
                i=objDecoder.readArray(false, raw, pdfObject, PDFkeyInt);
                break;

                //read Object Refs in [] (may be indirect ref)
            }case PdfDictionary.VALUE_IS_KEY_ARRAY:{
                ArrayDecoder objDecoder=new ArrayDecoder(objectReader, i, endPt, PdfDictionary.VALUE_IS_KEY_ARRAY);
                i=objDecoder.readArray(ignoreRecursion, raw, pdfObject, PDFkeyInt);
                break;

                //read numbers in [] (may be indirect ref)
            }case PdfDictionary.VALUE_IS_MIXED_ARRAY:{
                ArrayDecoder objDecoder=new ArrayDecoder(objectReader, i, endPt, PdfDictionary.VALUE_IS_MIXED_ARRAY);
                i=objDecoder.readArray(ignoreRecursion, raw, pdfObject, PDFkeyInt);
                break;
                ...
                ArrayDecoder objDecoder=new ArrayDecoder(objectReader, i, endPt, PdfDictionary.VALUE_IS_OBJECT_ARRAY);
                i=objDecoder.readArray(false, raw, pdfObject, PDFkeyInt);
                break;

                //read numbers in [] (may be indirect ref)
            }case PdfDictionary.VALUE_IS_DOUBLE_ARRAY:{
                ArrayDecoder objDecoder=new ArrayDecoder(objectReader, i, endPt, PdfDictionary.VALUE_IS_DOUBLE_ARRAY);
                i=objDecoder.readArray(false, raw, pdfObject,PDFkeyInt);
                break;

                //read numbers in [] (may be indirect ref)
            }case PdfDictionary.VALUE_IS_INT_ARRAY:{
                ArrayDecoder objDecoder=new ArrayDecoder(objectReader, i, endPt, PdfDictionary.VALUE_IS_INT_ARRAY);
                i=objDecoder.readArray(false, raw, pdfObject, PDFkeyInt);
                break;

                //read numbers in [] (may be indirect ref)
            }case PdfDictionary.VALUE_IS_FLOAT_ARRAY:{
                ArrayDecoder objDecoder=new ArrayDecoder(objectReader, i, endPt, PdfDictionary.VALUE_IS_FLOAT_ARRAY);
                i=objDecoder.readArray(false, raw, pdfObject, PDFkeyInt);
                break;
                ...
                i = setBooleanValue(pdfObject, i, raw, PDFkeyInt);
                break;
                ...
                i = setNumberValue(pdfObject, i, raw, PDFkeyInt);
                break;
                ...
                if(raw.length-5>0 && raw[i+1]=='n' && raw[i+2]=='u' && raw[i+3]=='l' && raw[i+4]=='l'){ //ignore null value and skip (ie /N null)
                    ...
                    i = setVariousValue(pdfObject, i, raw, length, PDFkeyInt, map, ignoreRecursion);
                }
                break;
        ...
        if(raw[i]=='/'){
            ...
            while(true){
                ...
                if(data[j]=='R' && !isString){
                    ...
                    byte[] newData = objectReader.readObjectAsByteArray(pdfObject, objectReader.isCompressed(ref, generation), ref, generation);
                    ...
                    if(pdfObject.getID()== PdfDictionary.AA && newData[0]=='<' && newData[1]=='<'){
                    ...
                    }else if((pdfObject.getID()== -1) && newData[0]=='<' && newData[1]=='<'){
                        isNumber=false;
                        typeFound=0;
                        ...
                        ArrayDecoder objDecoder=new ArrayDecoder(objectReader, jj2, endPt, PdfDictionary.VALUE_IS_MIXED_ARRAY);
                        ...
                        if(data==null){
                            ...
                            logger.info("[Linearized] " + objRef + " not yet available (4)");
            ...
            if(typeFound==4){//direct ref done above
                ...
                ArrayDecoder objDecoder=new ArrayDecoder(objectReader, jj, endPt, PdfDictionary.VALUE_IS_STRING_ARRAY);
        ...
        if(isRef){
            ...
            if(!ignoreRecursion){
                ...
                if(data==null){
                    ...
                    logger.info("[Linearized] " + pdfObject.getObjectRefAsString() + " not yet available (1)");
    ...
    private int setDictionaryValue(PdfObject pdfObject,int i, byte[] raw, boolean ignoreRecursion) {
        ...
        readDictionary(pdfObject,i, raw, PDFkeyInt, ignoreRecursion);
    ...
    private int setFloatValue(PdfObject pdfObject, int i, byte[] raw, int length) {
        ...
        if(raw[jj]>= 48 && raw[jj]<=57){ //if next char is number 0-9 its a ref
            ...
            if(data==null){
                ...
                logger.info("[Linearized] " + pdfObject.getObjectRefAsString() + " not yet available (3)");
        ...
        if(isRef){
            ...
            if(!ignoreRecursion){
                ...
                if(data==null){
                    ...
                    logger.info("[Linearized] " + pdfObject.getObjectRefAsString() + " not yet available (2)");
            ...
            if(isRef){
                ...
                if(!ignoreRecursion){
                    ...
                    if(data==null){
                        ...
                        logger.info("[Linearized] "+pdfObject.getObjectRefAsString()+" not yet available (6)");
                ...
                if(isRef){
                    ...
                    if(!ignoreRecursion){
                        ...
                        if(data==null){
                            ...
                            logger.info("[Linearized] "+pdfObject.getObjectRefAsString()+" not yet available (7)");
                    ...
                    if(data[start]=='<'){
                        ...
                        while(true){
                            ...
                            if(topHex>='A' && topHex<='F'){
                                ...
                                logger.info("Unexpected number "+(char)data[start]);
    ...
    int setNumberValue(PdfObject pdfObject, int i, byte[] raw, int PDFkeyInt) {
        ...
        if(isRef){
            ...
            if(data==null){
                ...
                logger.info("[Linearized] "+pdfObject.getObjectRefAsString()+" not yet available (8)");
            ...
            }else if(len<3){ //fix for short indirect value /K 30 0 R where 30 0R is -1 ie (Customers-Dec2011/Real Estate Tax Bill 2011.pdf)
            }else{
                j=3;
    ...
    int readDictionaryFromRefOrDirect(int id, PdfObject pdfObject, String objectRef, int i, byte[] raw, int PDFkeyInt) {
        ...
        while (true) {
            ...
            if (PDFkeyInt == PdfDictionary.ColorSpace || id == PdfDictionary.ColorSpace || pdfObject.getPDFkeyInt() == PdfDictionary.ColorSpace){
                ColorObjectDecoder colDecoder=new ColorObjectDecoder(objectReader);
                ...
                return processArray(pdfObject, raw, PDFkeyInt, possibleArrayStart);
            ...
            if (raw[i] == 60) { //[<<data inside brackets>>]
                ...
                while (true) {
                    ...
                    if (data == null) {
                        ...
                        logger.info("[Linearized] " + pdfObject.getObjectRefAsString() + " not yet available (11)");
    ...
    private int processArray(PdfObject pdfObject, byte[] raw, int PDFkeyInt, int possibleArrayStart) {
        ...
        ArrayDecoder objDecoder=new ArrayDecoder(objectReader, possibleArrayStart, endPoint, type);
            ...
            if(isDirect){
...

                {
                    ...
                    if(!isCountOnly){
                        ...
                        if(isNull){
                            objs[pairs-1]=null;
                        }else if(PdfDictionary.getKeyType(id, pdfObject.getObjectType())==PdfDictionary.VALUE_IS_UNREAD_DICTIONARY){
                            ...
                            if(rawDictData==null){
                                ...
                                logger.info("[Linearized] "+pdfObject.getObjectRefAsString()+" not yet available (12)");
    ...
    private int setNameStringValue(PdfObject pdfObject, int i, byte[] raw, boolean isMap) {
        ...
        if(isIndirect){ //its in another object so we need to fetch
            ...
            if(arrayData==null){
                ...
                logger.info("[Linearized] "+pdfObject.getObjectRefAsString()+" not yet available (13)");
