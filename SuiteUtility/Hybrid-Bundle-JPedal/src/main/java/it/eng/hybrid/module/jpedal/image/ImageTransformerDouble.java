...
* ---------------
*/
package it.eng.hybrid.module.jpedal.image;
...
public class ImageTransformerDouble {
	...
	public final static Logger logger = Logger.getLogger(ImageTransformerDouble.class);
	...
	*/
	final public void doubleScaleTransformShear(boolean alreadyTurned){
		...
		if(!isSwapped && clip!=null){
			...
			clipImage(final_clip,alreadyTurned);
...
//ShowGUIMessage.showGUIMessage("",current_image,"xx="+i_x+" "+i_y+" "+i_w+" "+i_h+" h="+current_image.getHeight());
	...
	final public int getImageX() {
		return i_x;
			 ...
			 * clip the image
			 */
			final private void clipImage(Area final_clip, boolean alreadyTurned) {
				...
				if(removeClippedBorder){
					...
					if((x==0)&&(y==0)&&(w==current_image.getWidth())&&(h==current_image.getHeight())){
						...
						try {
							...
							logger.info("Exception " + e + " extracting clipped image with values x="+x+" y="+y+" w="+w+" h="+h+" from image "+current_image);
