...
public class CMAP extends Table {
	...
	public final static Logger logger = Logger.getLogger(CMAP.class);
...
//
	...
	public CMAP(FontFile2 currentFontFile,int startPointer, Glyf currentGlyf){
		...
		//read 'cmap' table
		if(startPointer==0){
			logger.info("No CMAP table found");
		}else{
...
//
	...
	public int convertIndexToCharacterCode(String glyph,int index,boolean remapFont,boolean isSubsetted, String[] diffTable){
		...
		if((fontMapping==1 || (!remapFont &&fontMapping==4))&&(glyph!=null)&&(!"notdef".equals(glyph))){
	...
	public void setEncodingToUse(boolean hasEncoding,int fontEncoding,boolean isSubstituted,boolean isCID) {
        ...
        if(formatToUse==-1 && CMAPformats[0]!=12 && ((macScore>0 && winScore>0)|| CMAPformats.length==1) && !isCID && (!isSubstituted ||(CMAPformats.length==1 && CMAPformats[0]==0))){
