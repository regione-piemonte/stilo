...
public class SignWizardModel implements WizardPanelModel {	
	...
	public final static Logger logger = Logger.getLogger(SignWizardModel.class);
	...
	private SignWizardModel.OptionPanel.CardPresentThread cardPresentThread;
    ...
    private class OptionPanel extends GenericPanel {		
		...
		public void collectData()	{
			...
			logger.info("pin=" + new String(jPin.getPassword()) );
...
//			logger.info("Imposto la proprietà " + ConfigConstants.FIRMA_PROPERTY_ACRO6LAYER + "=" + signData.isAcro6Layers() );
			...
			logger.info("Render mode vale " + renderMode);
			try {
				renderMode = preferenceManager.getConfiguration().getString( ConfigConstants.FIRMA_PROPERTY_RENDERMODE );
				logger.info("Letta dal preference manager: Render mode vale " + renderMode);
			} catch (Exception e) {
