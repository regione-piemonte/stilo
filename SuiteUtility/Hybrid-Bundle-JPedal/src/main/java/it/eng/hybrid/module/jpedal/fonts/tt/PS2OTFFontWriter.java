...
public class PS2OTFFontWriter extends FontWriter{
	...
	public final static Logger logger = Logger.getLogger(PS2OTFFontWriter.class);
...
//        }
    ...
    void readTables() {
        ...
        for (int i=0; i<glyphCount && i<256; i++) {
            PdfGlyph glyph = glyphs.getEmbeddedGlyph(new T1GlyphFactory(),
...
//                    }
                ...
                case CMAP :
                	if(subType==TTF){
               		 fontData=orginTTTables.getTableBytes(CMAP);
    	           	}else{
                ...
                case OS2 :
    				if(subType==TTF){
    				    fontData=orginTTTables.getTableBytes(OS2);
    				}else{
    				    tableWriter=new OS2Writer(glyphs, xAvgCharWidth, minCharCode,  maxCharCode, fontBBox, emSquareSize);
    				}
                    break;
                ...
                case POST:
                	if(subType==TTF){
                		fontData = orginTTTables.getTableBytes(POST);
                	}
                	else{
