...
public class LinearizedHintTable {
	...
	public final static Logger logger = Logger.getLogger(LinearizedHintTable.class);
    ...
    private void parseHintTable(int N,int O, long Ooffset,byte[] hintTableData){
        ...
        //item 1
        int leastNumberOfObjects=((hintTableData[startByte]& 255)<<24)+((hintTableData[startByte+1]& 255)<<16)+((hintTableData[startByte+2]& 255)<<8)+(hintTableData[startByte+3]& 255);
        if(showHeader)
...
//            System.out.println("2="+word+" ( "+hintTableData[startByte]+ ' ' +hintTableData[startByte+1]+ ' ' +hintTableData[startByte+2]+ ' ' +hintTableData[startByte+3]+" )");
        ...
        //item 4
        int smallestPageSize=((hintTableData[startByte]& 255)<<24)+((hintTableData[startByte+1]& 255)<<16)+((hintTableData[startByte+2]& 255)<<8)+(hintTableData[startByte+3]& 255);
        if(showHeader)
...
//            System.out.println("13="+demominator+" ( "+hintTableData[startByte]+ ' ' +hintTableData[startByte+1]+" )");
        ...
        for(int pageReached=0;pageReached<N;pageReached++){
            ...
            objectCount=objectCount+pageObjectCount[pageReached+1];
        ...
        if(showHeader)
            System.out.println("shared Object count");
...
//    }
    ...
    public synchronized byte[] getObjData(int objID) {
        ...
        int start= (Integer) startRefs.get(key);
        int end= (Integer) endRefs.get(key);
