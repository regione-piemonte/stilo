...
import it.eng.hybrid.module.jpedal.objects.raw.CompressedObject;
import it.eng.hybrid.module.jpedal.objects.raw.EncryptionObject;
...
import it.eng.hybrid.module.jpedal.objects.raw.StreamObject;
...
import it.eng.hybrid.module.jpedal.util.repositories.Vector_boolean;
...
public class PdfFileReader
{
	...
	public final static Logger logger = Logger.getLogger(PdfFileReader.class);
    ...
    public static int alwaysCacheInMemory=-1;//custom 16384;
            ...
            if(isCompressed){
                ...
                if(objectRef.charAt(0)=='<'){
                    ...
                    if(ObjLengthTable==null || refTableInvalid){ //isEncryptionObject
                        ...
                        logger.info(objectRef+ " cannot have offset 0");
    ...
    private byte[] readCompressedObject(PdfObject pdfObject, String objectRef) {
        ...
        while(startID==null){
            ...
            if(Extends!=null){
                ...
                raw = readObjectData(ObjLengthTable[compressedID],null);
...
private PdfObject readCompressedStream(int pointer) throws PdfException {
        ...
        ObjLengthTable=calculateObjectLength((int) eof);
        ...
        if(data==null){
            ...
            if(stream!=null){ /**decode and save stream*/
                ...
                try{
                    ...
                    logger.info("Exception "+e+" with "+pdfObject.getObjectRefAsString());
                }
            }
            ...
            if(stream!=null || isCachedOnDisk){
                ...
                if (filters != null && firstValue!=PdfDictionary.Unknown && firstValue!=PdfFilteredReader.JPXDecode &&
                    ...
                    try{
                        ...
                        logger.info("[PDF] Problem "+e+" decompressing stream ");
    ...
    private PdfObject readLegacyReferenceTable(int pointer, int eof) throws PdfException {
        ...
        while (true) {
            ...
            try {
                ...
                try {
                	...
                	logger.info("Exception "+e+" closing file");
                }
        ...
        if(!refTableInvalid )
            ObjLengthTable=calculateObjectLength(eof);
    ...
    /**
       ...
       private int readFirstStartRef() throws PdfException {
           ...
           try {
        	   ...
        	   logger.info("Exception " + e + " reading last 1024 bytes");
           ...
           if(i==-1){
               try {
            	   ...
            	   logger.info("Exception "+e1+" closing file");
               }
           ...
           if (pointer == -1){
        	   logger.info("No Startref found in last 1024 bytes ");
               try {
                   closeFile();
               } catch (IOException e1) {
            	   logger.info("Exception "+e1+" closing file");
    ...
    final public PdfObject readReferenceTable(PdfObject linearObj) throws PdfException {
        ...
        if (pointer >= eof) {
        	...
        	logger.info("Pointer not if file - trying to manually find startref");
...
//                       blocksRead++;
               ...
               logger.info("Exception " + e + " reading object");
           }
...
private boolean isCompressedStream(int pointer, int eof) throws PdfException {
        ...
        if(type==UNSET){
            try {
            	...
            	logger.info("Exception "+1+" closing file");
            }
            ...
            switch(nextValue[0]){
                ...
                case 1: //non-compressed
                    ...
                    if(refIsvalid || !hasCase0)
                        storeObjectOffset(current, id, gen, false,false);
    ...
    {
        movePointer( offset.elementAt(currentID ) );
    }
    ...
    private String findOffsets() throws PdfSecurityException {
    	...
    	logger.info("Corrupt xref table - trying to find objects manually");
        ...
        while (true) {
            ...
            try {
            	...
            	logger.info("Exception " + e + " reading line");
            }
            if (line == null)
                break;
            ...
            if (line.contains(" obj")) {
                ...
                if (pointer > -1)
                    storeObjectOffset(Integer.parseInt(line.substring(0,pointer)), i, 1,false,true);
    ...
    final public String getType()
    {
        ...
        try{
        	...
        	logger.info( "Exception " + e + " in reading type" );
        }
        return pdf_type;
    }
    ...
    private void movePointer(long pointer)
    {
        try
        {
            ...
            if( pointer > pdf_datafile.length() ){
            	...
            	logger.info( "Attempting to access ref outside file" );
                //throw new PdfException("Exception moving file pointer - ref outside file");
            }else{
                pdf_datafile.seek( pointer );
            }
        }catch( Exception e ){
        	logger.info( "Exception " + e + " moving pointer to  "+pointer+" in file.");
        }
    }
    ...
    private long getPointer()
    {
        ...
        try{
        	...
        	logger.info( "Exception " + e + " getting pointer in file" );
        }
        return old_pointer;
    }
    //////////////////////////////////////////////////////////////////////////
    ...
    private void storeObjectOffset(int current_number, int current_offset, int current_generation, boolean isEntryCompressed, boolean isBumData)
    {
        ...
        if( existing_generation < current_generation  || offsetNumber== 0 || isBumData && (current_offset>offset.elementAt(current_number)))
        {
            ...
            //logger.info("Object "+current_number + ", generation "+
    ...
    private int getCompressedStreamObject(String value)
    {
        ...
        if( value.endsWith( "R" )){
            ...
            logger.info( "Error with reference ..value=" + value+ '<');
    ...
    private int getOffset(String value)
    {
        ...
        if(value.endsWith("R")){
            ...
            logger.info( "2. Error with reference .." + value+"<<" );
     ...
     *
     ...
     logger.info( "3. Error with reference .." + value+"<" );
     ...
     *
     ...
     logger.info( "4. Error with reference .." + value+"<" );
    ...
    private boolean isCompressed(String value)
    {
        ...
        if(value.endsWith("R"))
        {
            ...
            logger.info( "5.Error with reference .." + value+ '<');
    ...
    byte[] readObjectData(PdfObject pdfObject){
        ...
        if(pdfObject.isDataExternal()){
            ...
            if(data==null){
                ...
                logger.info("[Linearized] "+pdfObject.getObjectRefAsString()+" not yet available (15)");
