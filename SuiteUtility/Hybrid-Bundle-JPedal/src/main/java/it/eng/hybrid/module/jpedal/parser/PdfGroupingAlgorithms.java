...
import it.eng.hybrid.module.jpedal.util.repositories.Vector_Object;
import it.eng.hybrid.module.jpedal.util.repositories.Vector_Rectangle;
import it.eng.hybrid.module.jpedal.util.repositories.Vector_String;
...
public class PdfGroupingAlgorithms {
	...
	public final static Logger logger = Logger.getLogger(PdfGroupingAlgorithms.class);
...
//		StringBuilder justText=Strip.stripXML(current_text);
	...
	public final Map extractTextAsTable(
		...
		logger.info("extracting Text As Table");
...
//						+ "). ";
				...
				x1 = x2;
				x2 = temp;
				logger.info("x1 > x2, coordinates were swapped to validate");
			}
...
//						+ "). ";
				...
				y1 = y2;
				y2 = temp;
				logger.info("y1 < y2, coordinates were swapped to validate");
			}
//			throw new PdfException(errorMessage);
	 ...
	 * @throws PdfException If the co-ordinates are not valid
	 */
	final public List extractTextAsWordlist(
		int x1,
		int y1,
		int x2,
		int y2,
		 ...
		 */
		if(count==0){
			logger.info("Less than 1 text item on page");
...
//					System.out.println(">>>>>"+content[i]);
		...
		logger.info("Text extraction as wordlist completed");
        ...
        for (int j = 1; j < count; j++) {
            ...
            if (!isUsed[c]) {
                ...
                if(writingMode[c]==PdfData.HORIZONTAL_LEFT_TO_RIGHT || writingMode[c]==PdfData.HORIZONTAL_RIGHT_TO_LEFT){
                    ...
                    j=count;
                    logger.info("Text of multiple orientations found. Only horizontal text used.");
                }
            }
        }
		...
		for (int i = count - 2; i > -1; i--) {
			...
			separator = "";
				...
				if(debug){
                    ...
                    StringBuilder child_textX = Strip.stripXML(content[child],isXMLExtraction);
					String master_textX =Strip.stripXML(content[master],isXMLExtraction).toString();
