...
import it.eng.hybrid.module.jpedal.external.ImageHandler;
import it.eng.hybrid.module.jpedal.image.ImageOps;
import it.eng.hybrid.module.jpedal.image.ImageTransformer;
import it.eng.hybrid.module.jpedal.image.ImageTransformerDouble;
import it.eng.hybrid.module.jpedal.image.SamplingFactory;
import it.eng.hybrid.module.jpedal.io.ColorSpaceConvertor;
import it.eng.hybrid.module.jpedal.io.IDObjectDecoder;
import it.eng.hybrid.module.jpedal.io.JAIHelper;
import it.eng.hybrid.module.jpedal.io.ObjectStore;
import it.eng.hybrid.module.jpedal.io.ObjectUtils;
import it.eng.hybrid.module.jpedal.io.PdfFilteredReader;
...
import it.eng.hybrid.module.jpedal.objects.raw.MaskObject;
...
import it.eng.hybrid.module.jpedal.objects.raw.XObject;
import it.eng.hybrid.module.jpedal.pdf.PdfDecoder;
import it.eng.hybrid.module.jpedal.render.RenderUtils;
...
public class ImageDecoder extends BaseDecoder{
	...
	public final static Logger logger = Logger.getLogger(ImageDecoder.class);
    ...
    public BufferedImage processImageXObject(PdfObject XObject, String image_name, byte[] objectData, boolean saveRawData, String details) throws PdfException {
        ...
        //tell user and log
        logger.info("Processing XObject: "+ image_name+ ' ' +XObject.getObjectRefAsString()+ " width="+ width+ " Height="+ height+
                ...
                if (image != null || alreadyCached || previousUse > 0) {
                    ...
                    if (renderDirectly || useHiResImageForDisplay || previousUse > 0) {
                        ...
                        if(finalImagesExtracted || rawImagesExtracted){
                            ...
                            pdfImages.setImageInfo(currentImage, pageNum, gs.x, gs.x, w, h);
                        }
...
//                            }
                        ...
                        if (clippedImagesExtracted  || current.isScalingControlledByUser()) {
                            ...
                            try {
                                ...
                                logger.info("Exception " + e + " on transforming image in file");
                            }
                        }
                    }
...
//            System.out.println("<<"+i_data.length+" end="+endPtr);
    ...
    public void generateTransformedImage(BufferedImage image,String image_name) {
        ...
        if (image != null) {
            ...
            ImageTransformerDouble image_transformation =new ImageTransformerDouble(gs,image,createScaledVersion,true);
            ...
            if(current.isScalingControlledByUser()){
                ...
                if (image != null) {
                    ...
                    if(renderImages || finalImagesExtracted || clippedImagesExtracted || rawImagesExtracted){
                        pdfImages.setImageInfo(currentImage, pageNum, x, y, w, h);
                    }
    ...
    BufferedImage processImage(GenericColorSpace decodeColorData,
        ...
        if(renderPage && streamType!= ValueTypes.PATTERN && !current.avoidDownSamplingImage()){
            ...
            if(isPrinting && SamplingFactory.isPrintDownsampleEnabled && w<4000){
                pX= pageData.getCropBoxWidth(this.pageNum)*4;
                pY= pageData.getCropBoxHeight(this.pageNum)*4;
        ...
        if((newMask!=null || newSMask!=null) )
            logger.info("newMask= "+ newMask + " newSMask="+ newSMask);
                    ...
                    for(int y=0;y<newH;y++){
                        for(int x=0;x<newW;x++){
                            ...
                            if(count>0){
                                ...
                                }else if(index!=null && decodeColorData.getID()==ColorSpaces.Separation && 1==2){
        ...
        if (imageMask) {/** create an image from the raw data*/
            ...
            //save out image
            logger.info("Image "+ name+ ' ' + w+ "W * "+ h+ "H with No Compression at BPC "+ d);

            image =makeImage(decodeColorData,w,h,d,data,compCount,XObject);

        } else if (isDCT) { //handle JPEGS

            logger.info("JPeg Image "+ name+ ' ' + w+ "W * "+ h+ 'H'+" arrayInverted="+arrayInverted);
            ...
            if(colorspaceID== ColorSpaces.DeviceCMYK && extractRawCMYK){
                ...
                logger.info("Raw CMYK image " + name + " saved.");
            ...
            /**
             ...
             logger.info("Unable to save jpeg " + name);
            ...
            logger.info("JPeg 2000 Image "+ name+ ' ' + w+ "W * "+ h+ 'H');
            ...
            logger.info(name+ ' ' + w+ "W * "+ h+ "H BPC="+d+ ' '+decodeColorData);
        ...
        if (image != null) {
            ...
            if(!renderDirectly && (finalImagesExtracted || rawImagesExtracted) && !cache.testIfImageAlreadySaved(XObject))
...
//        }
        ...
        if (index != null) { //indexed images
            ...
            logger.info("Converting Separation/DeviceN colorspace to sRGB ");
            ...
            logger.info("Converting ICC/CMYK colorspace to sRGB ");
            ...
            logger.info("Converting 3 comp colorspace to sRGB index="+index);
            ...
            if (w * h == data.length) {
                ...
                logger.info("Converting data to sRGB ");
            ...
            logger.info("Image "+ cs.getType()+ " not currently supported with components "+ comp);
                ...
                if(!ignoreMask){
                    ...
                    if((isWhiteAndDownscaled  && (isDCT || isJPX || isIndexed))){
                        ...
                        image = processImageXObject(newSMask,name, objectData,true,null);
                            ...
                            if(pageNum>0 && pageData.getRotation(pageNum)==0  && rawOptions==PDFImageProcessing.IMAGE_ROTATED){
