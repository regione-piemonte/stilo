...
import it.eng.hybrid.module.jpedal.ui.popup.firma.PasswordHandler;
...
public class ItextFunctions {
	...
	public final static Logger logger = Logger.getLogger(ItextFunctions.class);
	...
	public FileOperationResponse verificaFirme(SignData signData) throws Exception/**/ { 
		...
		try {
			...
			logger.info( "Proprietà "+ ConfigConstants.FIRMA_PROPERTY_ENABLECACHECK  + ": " + caCheck );
			crlCheck = preferenceManager.getConfiguration().getBoolean( ConfigConstants.FIRMA_PROPERTY_ENABLECRLCHECK );
			logger.info( "Proprietà "+ ConfigConstants.FIRMA_PROPERTY_ENABLECRLCHECK  + ": " + crlCheck );
		} catch (Exception e) {
			logger.info("Errore nel recupero delle proprietà " + ConfigConstants.FIRMA_PROPERTY_ENABLECACHECK 
		...
		try {
			...
			logger.info("Errore nella chiamata al metodo di verifica firma, la verifica non può essere effettuata ", e );
		}
	...
	public boolean verificaVersionePdf(SignData signData) throws Exception/**/ { 
		...
		logger.info("hashAlgorithm " + hashAlgorithm);

		logger.info("reader.getPdfVersion() " + reader.getPdfVersion());
		logger.info("hashAlgorithm.getPdfVersion() " + hashAlgorithm.getPdfVersion());
		if (reader.getPdfVersion() < hashAlgorithm.getPdfVersion() && signData.isAppendMode()) {
			...
			logger.info( MessageConstants.MSG_FIRMAMARCA_INVALIDPDFVERSION );
	...
	public boolean sign(SignData signData) throws Exception/**/ { 
		logger.info("Inizio metodo Firma (ed eventualmente marca) ");
		...
		try {
			...
			while(enumeration.hasMoreElements()){
				...
				try {
					...
					logger.info("Proprietà " + ConfigConstants.PROPERTY_EMULATION + "=" + emulationString );
				} catch (Exception e) {
					logger.info("Errore", e);
				}
				...
				try{
					...
					logger.info("Errore nella verifica del certificato. La verifica non è stata effettuata", e);
				}
				if( result!=null ){
					if( !result.isValid() ){
						logger.info("Controlli non superati " + result.getErrorsString());
						throw new Exception(  Messages.getMessage( MessageConstants.MSG_FIRMAMARCA_INVALIDCERTIFICATE ) + result.getErrorsString() );
					} else {
						logger.info("Controlli superati ");
					}
				}
			} 
			...
			if (ArrayUtils.isEmpty(chain)) {
				logger.info("Nessun certificato trovato");
		...
		try {
			...
			logger.info("Path del file firmato " + outFile);
		...
		final HashAlgorithm hashAlgorithm = signData.getHashAlgorithm();
		logger.info("Algoritmo di firma=" + hashAlgorithm);

		char tmpPdfVersion = '\0'; // default version - the same as input
		logger.info("Versione del pdf reader=" + reader.getPdfVersion());
		logger.info("Versione minima richiesta dall'algoritmo=" + hashAlgorithm.getPdfVersion());
		if (reader.getPdfVersion() < hashAlgorithm.getPdfVersion() && signData.isAppendMode()) {
			signData.setAppend(false);
			tmpPdfVersion = hashAlgorithm.getPdfVersion();
		}
		logger.info("Versione pdf che verrà utilizzata in fase di firma=" + tmpPdfVersion);

		PdfStamper stamper;
		try {
			logger.info("Modalita' append? " + signData.isAppendMode() );
		...
		//				logger.info("console.setEncryption");
		...
		//					if (encCert == null) {
		//						logger.info("console.pdfEncError.wrongCertificateFile" +
		//								StringUtils.defaultString(options.getPdfEncryptionCertFile()));
		//						return;
		//					}
		//					if (!KeyStoreUtils.isEncryptionSupported(encCert)) {
		//						logger.info("console.pdfEncError.cantUseCertificate" + encCert.getSubjectDN().getName());
		//						return;
		//					}
		...
		//					break;
		//				default:
		//					logger.info("console.unsupportedEncryptionType");
		//					return;
		//				}
		//			}
		...
		if (StringUtils.isNotEmpty(signData.getReason())) {
			logger.info("Imposto la proprietà Reason:" + signData.getReason());
			appearance.setReason(signData.getReason());
		}
		if (StringUtils.isNotEmpty(signData.getLocation())) {
			logger.info("Imposto la proprietà:" + signData.getLocation());
			appearance.setLocation(signData.getLocation());
		}
		if (StringUtils.isNotEmpty(signData.getContact())) {
			logger.info("Imposto la proprietà contact:" +  signData.getContact());
			appearance.setContact(signData.getContact());
		}
		logger.info("Imposto il livello di certificazione a " + signData.getCertifyMode().getLevel());
		...
		try {
			if (signData.isVisibleSignature()) {
				...
				logger.info("Viene effettuata la firma visibile");
				...
				if( signData.getBgImgScale()!=0 ){
					logger.info("Imposto il fattore di scala dell'immagine di sfondo " + signData.getBgImgScale()/100);
					appearance.setImageScale(signData.getBgImgScale()/100);
				}
				if (signData.getL2Text() != null) {
					logger.info("Imposto il testo della firma");
				...
				if (renderMode == RenderMode.GRAPHIC_AND_DESCRIPTION && appearance.getSignatureGraphic() == null) {
					logger.info("Render mode of visible signature is set to GRAPHIC_AND_DESCRIPTION, but no image is loaded. Fallback to DESCRIPTION_ONLY.");
					renderMode = RenderMode.DESCRIPTION_ONLY;
				}
				logger.info("Imposto la modalita' di renderizzazione: " + renderMode.getRender() );
...
//		}
		...
		logger.info("Aggiungo la marca?" + signData.isTimestamp());
			...
			tsc.setTSAInfo(
				new TSAInfoBouncyCastle() {
					public void inspectTimeStampTokenInfo(TimeStampTokenInfo info) {
						logger.info("Marca generata " + info.getGenTime());
						logger.info("Policy della marca " + info.getPolicy());
					}
				}
			);
	...
	public boolean mark(SignData signData) throws Exception { 
		...
		logger.info("Inizio metodo Marca");
		...
		for (int k = 0; k < names.size(); ++k) {
			...
			logger.info("contentLenght " + contentLenght);
			...
			logger.info("lenght "+ paddedSig.length);
