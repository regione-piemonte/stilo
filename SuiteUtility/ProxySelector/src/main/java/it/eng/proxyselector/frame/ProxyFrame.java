...
public class ProxyFrame extends JFrame {
	...
	private String jSessionIdFrame;
	private String domainFrame;
	...
	public ProxyFrame(String pStrAddress){
		...
		lJButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					mLogger.debug("ProxySelector.testDiConnessione(mStrAddress): " + mStrAddress);
	...
	public ProxyFrame(String pStrAddress, String jSessionId, String domain, String path){
		super("Impostazioni di rete");
		mLogger.debug("Tolto lo useSystemProxyes");
		System.clearProperty("java.net.useSystemProxies");
		mStrAddress = pStrAddress;
		jSessionIdFrame = jSessionId;
		domainFrame = domain;
		pathFrame = path;
		final ProxyFrame instance = this;
		setSize(600, 600);
		...
		lJButton.addActionListener(new ActionListener() {
			...
			@Override
			public void actionPerformed(ActionEvent e) {
				ProxyDefaultHttpClient.getClientToUse(jSessionIdFrame, domainFrame, pathFrame);
				int porta = lProxyPanel.getPort();
				if (!StringUtils.isEmpty(lProxyPanel.getProxy())){
					if (lProxyPanel.getUseScript()){
						PacProxySelector pacProxySelector = new PacProxySelector(
						...
						if (lProxyPanel.getUseNLTM()){
							ProxyUtils.setProxyScriptNTLM(pacProxySelector, mStrAddress, lProxyPanel.getUsername(), lProxyPanel.getPassword(), 
									lProxyPanel.getDominio(), null, jSessionIdFrame, domainFrame, pathFrame);
						} else {
							ProxyUtils.setProxyScript(pacProxySelector, mStrAddress, lProxyPanel.getUsername(), lProxyPanel.getPassword(), jSessionIdFrame, domainFrame, pathFrame);
						}
					} else {
						if (lProxyPanel.getUseNLTM()){
							ProxyUtils.setProxyNTLM(lProxyPanel.getProxy(), 
									porta, lProxyPanel.getUsername(), 
									lProxyPanel.getPassword(), lProxyPanel.getDominio(), jSessionIdFrame, domainFrame, pathFrame);
						} else {
							ProxyUtils.setProxy(lProxyPanel.getProxy(), 
									porta, lProxyPanel.getUsername(), 
									lProxyPanel.getPassword(), jSessionIdFrame, domainFrame, pathFrame);
						}
					}
				} else {
					ProxyUtils.resetProxy(jSessionIdFrame, domainFrame, pathFrame);
				}
				try {
					List<java.net.Proxy> listProxy = java.net.ProxySelector.getDefault().select(new URI(mStrAddress));
					for (Proxy lProxy : listProxy){
