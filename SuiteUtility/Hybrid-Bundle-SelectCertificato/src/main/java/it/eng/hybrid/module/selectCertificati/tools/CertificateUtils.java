...
public class CertificateUtils {
	...
	public final static Logger logger = Logger.getLogger(CertificateUtils.class);
	...
	public static ValidationInfos checkCertificate(X509Certificate certificate ) throws Exception{
		logger.info("Inizio verifica sul certificato " + certificate );
		if( certificate!=null ){
			...
			if (cognomeNomeCertificato!=null) {
				...
				if (x500name != null) {
					...
					if (cns != null && cns.length > 0) {
						RDN cn = cns[0];
						if (cn != null && cn.getFirst() != null && cn.getFirst().getValue() != null && !IETFUtils.valueToString(cn.getFirst().getValue()).equalsIgnoreCase(cognomeNomeCertificato)) {
