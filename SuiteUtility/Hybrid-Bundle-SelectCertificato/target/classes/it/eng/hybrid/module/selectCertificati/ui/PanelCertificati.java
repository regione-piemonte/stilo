...
import it.eng.hybrid.module.selectCertificati.thread.CardPresentThread;
import it.eng.hybrid.module.selectCertificati.thread.CertificatiThread;
import it.eng.hybrid.module.selectCertificati.thread.ConfigurationThread;
...
public class PanelCertificati extends JPanel implements ActionListener {
	...
	public final static Logger logger = Logger.getLogger(PanelCertificati.class);

	private SelectCertificatiApplication appl;
	...
	/*
	 * Parametro che serve per identificare se c'� solo la firma o anche marca, etc.
	 * In questo modo si capisce se procedere direttamente con l'azione successiva o meno
	 * 
	 * Il secondo flag invece � per indicare il fatto che ci sia uno o pi� certificati
	 */
	 ...
	 */
	public PanelCertificati(SelectCertificatiApplication appl) {
	...
	private void initialize() {
		...
		ConfigurationThread thread = new ConfigurationThread(getJProgressBar(), (SelectCertificatiApplication) appl, getJPin(), getJButtonCert(), this);
		thread.start();

		((SelectCertificatiApplication) appl).getJTabbedPane().setEnabled(false);
	...
	private JPanel createComboSelezioneCertificatoPanel() {
		...
		// Rendo invisibile la combo (verr� abilitata solo in alcuni casi)
...
//				showMessageDialog("Certificato selezionato " + jComboCertificati.getSelectedItem().toString(), "", JOptionPane.INFORMATION_MESSAGE);
				...
				//searchStopped(true, pin, firmatario, alias); // Ritorno indicando che si � terminata l'elaborazione
	...
	/**
	 * Metodo che viene richiamato dal Thread che � stato avviato e che rappresenta la fine delle operazioni
	 * 
	 * @param esito
	 *            true se l'operazione � andata a buon fine, false altrimenti
	 */
	public void searchStopped(boolean esito, String pin, String firmatario, String alias, String metodo) {
		...
		if (esito && pin != null && firmatario != null && alias != null && metodo != null) {
			// Chiamata del metodo che esegue la callback di ritorno
			appl.closeFrame(pin, firmatario, alias, metodo);

			// Visualizzo il messaggio indicando che l'acquisizione del certificato � avvenuto correttamente
	...
	public SelectCertificatiApplication getAppl() {
		return appl;
	}
	...
	public void showSignButton(String pin, String firmatario, String alias){
		...
		Boolean counterSignEnabled = PreferenceManager.getBoolean(PreferenceKeys.PROPERTY_COUNTERSIGN_ENABLED);
		if (counterSignEnabled) {
			onlySignature = false; //Non c'� solo la firma
		...
		logger.info("moltepliciFirmatariIndividuati: " + moltepliciFirmatariIndividuati);
		logger.info("onlySignature: " + onlySignature);
		/*
		 * Se c'� la possibilit� di eseguire solamente la firma e non marca o altro allora procedo direttamente
	...
	public void addComboCertificati(String pin, ArrayList<Firmatario> firmatari) {
		...
		// Visualizzo il messaggio indicando che ci sono piu' certificati e che bisogna selezionarne uno
		...
		// Gestisco la visibilit� dei vari componenti
	...
	private void manageComponentVisibility(String situation) {
		if ("START_SEARCH".equals(situation)) {
			...
			// Nel caso sia stata generata un'eccezione, come ad esempio quando � stato inserito un pin non corretto
			if (appl.getJTabbedPane() != null) {
