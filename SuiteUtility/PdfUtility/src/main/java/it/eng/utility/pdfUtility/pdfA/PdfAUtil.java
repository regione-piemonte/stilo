package it.eng.utility.pdfUtility.pdfA;
...
public class PdfAUtil {
	...
	public static final Logger logger = LogManager.getLogger(PdfAUtil.class);
	
	public static PdfBean isPdfA(File  file) throws Exception {
		PdfReader reader = null;
		boolean isPdfA = false;
		PdfBean pdfBean = new PdfBean();
		pdfBean.setPdfA( isPdfA );
		try {
			reader = new PdfReader( file.getAbsolutePath() );
			isPdfA = reader.getMetadata() != null && (new String(reader.getMetadata())).contains( PdfConstant.pdfaMetadata );
			pdfBean.setPdfA( isPdfA );
		} catch (Exception e) {
	...
	public static File convertToPdfA3U(File in, File temporaryDirectory) throws DocumentException, IOException, URISyntaxException {
		...
		InputStream iccFile = PdfAUtil.class.getResourceAsStream("/srgb_color_space_profile.icm");
