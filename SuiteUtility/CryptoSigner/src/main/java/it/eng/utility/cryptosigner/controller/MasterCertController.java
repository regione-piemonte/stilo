 ...
 * L'analisi viene innescata dalla chiamata al metodo {@link MasterCertController#executeControll(InputSignerBean)} 
 * e iterata su tutti i controller definiti nell'attributo controllers
 *
 */
public class MasterCertController {
	 ...
	 */
	public OutputCertBean executeControll(InputCertBean input, boolean childValidation)throws ExceptionController{
		OutputCertBean output = new OutputCertBean();
	 ...
	 *
	 */
	private void execute(InputCertBean input,OutputCertBean output, boolean childValidation)throws ExceptionController{
		boolean result;
		...
		for (ISignerController controller:controllers) {
			if (controller.canExecute(input)) {
				try {
					...
					log.debug("Controllo: " + controller.getClass().getSimpleName() + " eseguito result="+result+" in " + elapsedTimeMillis + "ms");
