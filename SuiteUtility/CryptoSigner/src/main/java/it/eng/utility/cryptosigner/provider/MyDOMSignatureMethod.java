 ...
 * @author Sean Mullan
 */
public abstract class MyDOMSignatureMethod extends DOMStructure implements
		SignatureMethod {
...
//			.getLogger("org.jcp.xml.dsig.internal.dom");
	 ...
	 *             if the parameters are not appropriate for this signature
	 *             method
	 */
	MyDOMSignatureMethod(AlgorithmParameterSpec params)
			throws InvalidAlgorithmParameterException {
	 ...
	 *             if <code>key</code>, <code>si</code> or
	 *             <code>signature</code> are <code>null</code>
	 * @throws InvalidKeyException
	 *             if the key is improperly encoded, of the wrong type, or
	 *             parameters are missing, etc
	 * @throws SignatureException
	 *             if an unexpected error occurs, such as the passed in
	 *             signature is improperly encoded
...
//		}
	...
	/**
	 * Signs the bytes with the specified key, using the underlying signature or
	 * MAC algorithm.
	 * 
	 * @param key
	 *            the signing key
...
//		}
	...
	/**
	 * Marshals the algorithm-specific parameters to an Element and appends it
	 * to the specified parent element. By default, this method throws an
	 * exception since most SignatureMethod algorithms do not have parameters.
	 * Subclasses should override it if they have parameters.
	 * 
	 * @param parent
	...
	public boolean equals(Object o) {
		...
		return (getAlgorithm().equals(osm.getAlgorithm()) && paramsEqual(osm
				.getParameterSpec()));
	}
	...
	private static byte[] convertASN1toXMLDSIG(byte asn1Bytes[])
		...
		for (j = sLength; (j > 0)
				&& (asn1Bytes[(6 + rLength + sLength) - j] == 0); j--)
			;
