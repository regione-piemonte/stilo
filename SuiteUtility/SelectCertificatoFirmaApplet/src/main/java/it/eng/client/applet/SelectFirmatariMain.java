...
public class SelectFirmatariMain {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					...
					SelectFirmatariMain window = new SelectFirmatariMain();
	...
	private void initialize() {
		...
		System.out.println("SmartCardMain - mainPanelWidth=" + mainPanelWidth + " mainPanelHeight=" + mainPanelHeight );
		frame.setSize(mainPanelWidth,mainPanelHeight);
		frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);

		frame.setResizable(true);
		
		SelectFirmatari sc = new SelectFirmatari(frame);
		sc.init();
