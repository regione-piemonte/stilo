	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		...
		if (job != null) {
			...
			try {
				...
				/*
				 * Rilascio una delle risorse che sono state occupate e che stanno bloccando il chiamante
				 * 
				 * Le devo rilasciare sia in caso di errore sia che sia tutto ok.
				 */
				GestioneTerminazioneProgramma.endThreadExecution();
	...
	protected AbstractJob<T> createJob(JobExecutionContext context) {
		...
		try {
			...
			HashMap<String, Object> attributes = (HashMap<String, Object>) context.getJobDetail().getJobDataMap()
					.get("JOB_ATTRIBUTES");
			String[] args = (String[]) context.getJobDetail().getJobDataMap().get("JOB_ARGS");
			...
			log.error("Durante la creazione del job si e verificata la seguente eccezione", e);
