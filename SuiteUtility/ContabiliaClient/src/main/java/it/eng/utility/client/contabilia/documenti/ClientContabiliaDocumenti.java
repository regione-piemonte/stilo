...
public class ClientContabiliaDocumenti {
	...
	public OutputElaboraAttiAmministrativi aggiornaProposta(ElaboraAttiAmministrativiRequest input) {
		...
		if (!response.isResponseElaborazione()) {
			logger.info("aggiornaProposta - prima chiamata con numeroProposta in errore");
			if (input.getNumeroAttoDefinitivo() != null && input.getAnnoAttoDefinitivo() != null && input.getTipoAttoDefinitivo() != null) {
				logger.info("aggiornaProposta - numeroDefinitivo valorizzato esecuzione seconda chiamata");
				
				// composizione bean per generare stringa contenutoDocumento
				ContenutoDocumentoParam param2 = new ContenutoDocumentoParam();
				param2.setTipoVariazione("U");
				param2.setEnte(ente);
				param2.setAnnoAttoChiave(Integer.toString(input.getAnnoAttoDefinitivo()));
				param2.setNumeroAttoChiave(input.getNumeroAttoDefinitivo());
				param2.setTipoAttoChiave(input.getTipoAttoDefinitivo());
				param2.setDirezioneChiave(input.getCentroResponsabilita());
				param2.setCentroCostoChiave(input.getCentroCosto());
				param2.setAnnoAtto(Integer.toString(input.getAnnoAttoDefinitivo()));
				param2.setNumeroAtto(input.getNumeroAttoDefinitivo());
				param2.setTipoAtto(input.getTipoAttoDefinitivo());
				param2.setDirezione(input.getCentroResponsabilita());
				param2.setCentroCosto(input.getCentroCosto());
				param2.setDataCreazione(input.getDataCreazione());
				param2.setDataProposta(input.getDataProposta());
				param2.setDataApprovazione(input.getDataApprovazione());
				param2.setDataEsecutivita(input.getDataEsecutivita());
				param2.setStatoOperativo("D");
				param2.setOggetto(input.getOggetto());
				param2.setNote(input.getNote());
				param2.setIdentificativo(input.getIdentificativo());
				param2.setDirigenteResponsabile(input.getDirigenteResponsabile());
				param2.setTrasparenza(input.getTrasparenza());
				param2.setCodiceBlocco("0");
				
				// generazione stringa da passare come parametro al WS elaboraAttiAmministrativi
				String contenutoDocumento2 = generazioneContenutoDocumento(param2);
				
				// chiamata elaboraAttiAmministrativi - update per aggiornaProposta
				it.eng.utility.client.contabilia.documenti.data.ElaboraAttiAmministrativiResponse response2 = elaboraAttiAmministrativi(contenutoDocumento2, input.getIdSpAoo(), "aggiornaProposta");
				
				logger.info("aggiornaProposta - seconda chiamata eseguita - response " + response2.isResponseElaborazione());
				
				result.setEntry(response2);
				return result;
			}
			else {
				logger.info("aggiornaProposta - numeroDefinitivo non valorizzato: return response chiamata con numeroProposta");
			...
			logger.info("aggiornaProposta - chiamata con numeroProposta andata a buon fine: " + response.isResponseElaborazione());
	...
	public OutputElaboraAttiAmministrativi bloccoDatiProposta(ElaboraAttiAmministrativiRequest input) {
		...
		if (!response.isResponseElaborazione()) {
			logger.info("bloccoDatiProposta - prima chiamata con numeroProposta in errore");
			if (input.getNumeroAttoDefinitivo() != null && input.getAnnoAttoDefinitivo() != null && input.getTipoAttoDefinitivo() != null) {
				logger.info("bloccoDatiProposta - numeroDefinitivo valorizzato esecuzione seconda chiamata");
				...
				param2.setDirigenteResponsabile(input.getDirigenteResponsabile());
				param2.setTrasparenza(input.getTrasparenza());
				param2.setCodiceBlocco("1");
				
				// generazione stringa da passare come parametro al WS elaboraAttiAmministrativi
				String contenutoDocumento2 = generazioneContenutoDocumento(param2);
				
				// chiamata elaboraAttiAmministrativi - update per bloccoDatiProposta
				it.eng.utility.client.contabilia.documenti.data.ElaboraAttiAmministrativiResponse response2 = elaboraAttiAmministrativi(contenutoDocumento2, input.getIdSpAoo(), "bloccoDatiProposta");
				
				logger.info("bloccoDatiProposta - seconda chiamata eseguita - response " + response2.isResponseElaborazione());
				
				result.setEntry(response2);
				return result;
			}
			else {
				logger.info("bloccoDatiProposta - numeroDefinitivo non valorizzato: return response chiamata con numeroProposta");
			...
			logger.info("bloccoDatiProposta - chiamata con numeroProposta andata a buon fine: " + response.isResponseElaborazione());
	...
	public OutputElaboraAttiAmministrativi invioAttoDef(ElaboraAttiAmministrativiRequest input) {
		...
		if (!response.isResponseElaborazione()) {
			logger.info("invioAttoDef - prima chiamata con numeroDefinitivo in errore");
			
			// composizione bean per generare stringa contenutoDocumento
			ContenutoDocumentoParam param2 = new ContenutoDocumentoParam();
			param2.setTipoVariazione("U");
			param2.setEnte(ente);
			param2.setAnnoAttoChiave(Integer.toString(input.getAnnoAttoProposta()));
			param2.setNumeroAttoChiave(input.getNumeroAttoProposta());
			param2.setTipoAttoChiave(input.getTipoAttoProposta());
			param2.setDirezioneChiave(input.getCentroResponsabilita());
			param2.setCentroCostoChiave(input.getCentroCosto());
			param2.setAnnoAtto(Integer.toString(input.getAnnoAttoDefinitivo()));
			param2.setNumeroAtto(input.getNumeroAttoDefinitivo());
			...
			logger.info("invioAttoDef - esecuzione seconda chiamata con numeroProposta");
			
			// chiamata elaboraAttiAmministrativi - update per invioAttoDef
			it.eng.utility.client.contabilia.documenti.data.ElaboraAttiAmministrativiResponse response2 = elaboraAttiAmministrativi(contenutoDocumento2, input.getIdSpAoo(), "invioAttoDef");
			
			logger.info("invioAttoDef - seconda chiamata eseguita con response: " + response2.isResponseElaborazione());
			
			result.setEntry(response2);
			return result;
		}
		else {
			logger.info("invioAttoDef - chiamata con numeroDefinitivo andata a buon fine: " + response.isResponseElaborazione());
	...
	public OutputElaboraAttiAmministrativi invioAttoDefEsec(ElaboraAttiAmministrativiRequest input) {
		...
		if (!response.isResponseElaborazione()) {
			logger.info("invioAttoDefEsec - prima chiamata con numeroDefinitivo in errore");
			...
			logger.info("invioAttoDefEsec - esecuzione seconda chiamata con numeroProposta");
			
			// chiamata elaboraAttiAmministrativi - update per invioAttoDefEsec
			it.eng.utility.client.contabilia.documenti.data.ElaboraAttiAmministrativiResponse response2 = elaboraAttiAmministrativi(contenutoDocumento2, input.getIdSpAoo(), "invioAttoDefEsec");
			
			logger.info("invioAttoDefEsec - seconda chiamata eseguita con response: " + response2.isResponseElaborazione());
			
			result.setEntry(response2);
			return result;
		}
		else {
			logger.info("invioAttoDefEsec - chiamata con numeroDefinitivo andata a buon fine: " + response.isResponseElaborazione());
	...
	public OutputElaboraAttiAmministrativi sbloccoDatiProposta(ElaboraAttiAmministrativiRequest input) {
		...
		if (!response.isResponseElaborazione()) {
			logger.info("sbloccoDatiProposta - prima chiamata con numeroProposta in errore");
			if (input.getNumeroAttoDefinitivo() != null && input.getAnnoAttoDefinitivo() != null && input.getTipoAttoDefinitivo() != null) {
				logger.info("sbloccoDatiProposta - numeroDefinitivo valorizzato esecuzione seconda chiamata");
				...
				// chiamata elaboraAttiAmministrativi - update per sbloccoDatiProposta
				it.eng.utility.client.contabilia.documenti.data.ElaboraAttiAmministrativiResponse response2 = elaboraAttiAmministrativi(contenutoDocumento2, input.getIdSpAoo(), "sbloccoDatiProposta");
				
				logger.info("sbloccoDatiProposta - seconda chiamata eseguita - response " + response2.isResponseElaborazione());
				
				result.setEntry(response2);
				return result;
			}
			else {
				logger.info("sbloccoDatiProposta - numeroDefinitivo non valorizzato: return response chiamata con numeroProposta");
			...
			logger.info("sbloccoDatiProposta - chiamata con numeroProposta andata a buon fine: " + response.isResponseElaborazione());
	...
	public OutputElaboraAttiAmministrativi annullamentoProposta(ElaboraAttiAmministrativiRequest input) {
		...
		if (!response.isResponseElaborazione()) {
			logger.info("annullamentoProposta - prima chiamata con numeroProposta in errore");
			if (input.getNumeroAttoDefinitivo() != null && input.getAnnoAttoDefinitivo() != null && input.getTipoAttoDefinitivo() != null) {
				logger.info("annullamentoProposta - numeroDefinitivo valorizzato esecuzione seconda chiamata");
				...
				// chiamata elaboraAttiAmministrativi - update per annullamentoProposta
				it.eng.utility.client.contabilia.documenti.data.ElaboraAttiAmministrativiResponse response2 = elaboraAttiAmministrativi(contenutoDocumento2, input.getIdSpAoo(), "annullamentoProposta");
				
				logger.info("annullamentoProposta - seconda chiamata eseguita - response " + response2.isResponseElaborazione());
				
				result.setEntry(response2);
				return result;
			}
			else {
				logger.info("annullamentoProposta - numeroDefinitivo non valorizzato: return response chiamata con numeroProposta");
			...
			logger.info("annullamentoProposta - chiamata con numeroProposta andata a buon fine: " + response.isResponseElaborazione());
	...
	private it.eng.utility.client.contabilia.documenti.data.ElaboraAttiAmministrativiResponse elaboraAttiAmministrativi(String contenutoDocumento, String idSpAoo, String metodo) {
		...
		try {
			...
			String esito = responseWs != null && responseWs.getEsito() != null ? responseWs.getEsito().value() : null;
			if (esito != null && "SUCCESSO".equalsIgnoreCase(esito) && responseWs.getErrori().size() == 0) {
				response.setResponseElaborazione(true);
				if (responseWs.getMessaggi().size() > 0) {
			...
			response.setEsitoElaborazione(esito);
			
			logger.info("Chiamata WS " + metodo + 
						" - response: " + response.isResponseElaborazione() + 
						" - esito: " + response.getEsitoElaborazione() + 
			...
			logger.error("Errore chiamata WS " + metodo + " - " + e.getMessage());
		}
