...
public class TwainSource extends TwainIdentity implements TwainConstants,ScannerDevice{
  ...
  private int     iff        = TWFF_TIFF;        // image file format
...
//    case TWRC_DATANOTAVAILABLE: throw new TwainResultException.DataNotAvailable();
  ...
  public boolean isADF(){                          // [1] 9-369 CAP_AUTOFEED
    try{                                                    // TW_ONEVALUE/TW_BOOL
      return getCapability(CAP_FEEDERENABLED).booleanValue();
    }catch(Exception e){                                    // if some error assume source is not endorser
      jtwain.signalException(getClass().getName()+".isADF:\n\t"+e);return false;                                          
    }
  }
