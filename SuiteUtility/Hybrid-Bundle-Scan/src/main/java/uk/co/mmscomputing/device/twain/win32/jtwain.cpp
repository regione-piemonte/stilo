...
JNIEXPORT void JNICALL Java_uk_co_mmscomputing_device_twain_jtwain_ntrigger(JNIEnv* env, jclass clazz,
  ...
  PostMessage(hWnd,WM_COMMAND,(WPARAM)cmd,(LPARAM)caller);
}
...
LRESULT FAR PASCAL wndProc(HWND hWnd,UINT iMessage,WPARAM wParam,LPARAM lParam){
  ...
  case WM_COMMAND:{
    ...
    env->CallStaticVoidMethod(jtwain,jexecute,(jobject)lParam,wParam);
...
bool loadTwainLib(void){                  
  ...
  if(DSM_ENTRY==NULL){return false;}                  
...
bool openDataSourceManager(void){
  ...
  lstrcpy (appID.Version.Info, "2006-04-01");
  ...
  return(rc==TWRC_SUCCESS);                           // twain state 2 -> 3
}
...
JNIEXPORT jint JNICALL Java_uk_co_mmscomputing_device_twain_jtwain_ninitLib(JNIEnv* env, jclass clazz){
  ...
  if((jexecute!=NULL) && createWindow() && openDataSourceManager()){
    return (int)hWnd;
  }
  return NULL;  
}
