    ...
    static{
        ...
        if(mode!=null){
            if(mode.equals("EMBED_ALL"))
                defaultMode= FontMapper.EMBED_ALL;
            else if(mode.equals("EMBED_ALL_EXCEPT_BASE_FAMILIES"))
                defaultMode= FontMapper.EMBED_ALL_EXCEPT_BASE_FAMILIES;
            else
...
//		writeCustom(TOFILE, "\t\tprimaryStage.resizableProperty().set(false);");
		...
		writeCustom(TOFILE, "\t\tscene = new Scene(root, " + pageWidth  +", "+ (pageHeight+75)+");");
		...
		writeCustom(TOFILE, "\t\t");
		writeCustom(TOFILE, "\t\tprimaryStage.setWidth("+pageWidth+");");//set the width of the new page
		writeCustom(TOFILE, "\t\tprimaryStage.setHeight("+(pageHeight+75)+");");//set the height of the new page including the nav bar
		...
		if (!onePageFile) {
			...
			writeCustom(TOFILE,"\t\tImage logo= new Image(page" + pageNumberAsString +".class.getResourceAsStream(\""+ '/' + packageName + "/icons/logo.gif\"));");    
			writeCustom(TOFILE, "");
			...
			writeCustom(TOFILE, "\t\tfinal int pageNumber= "+pageNumber+",pageCount= "+endPage+ ';');
			writeCustom(TOFILE, "");

			writeCustom(TOFILE, "\t\tButton button = new Button();");
			writeCustom(TOFILE, "\t\tbutton.setTooltip(new Tooltip(toolTip));");
			writeCustom(TOFILE, "\t\t");
			writeCustom(TOFILE,"\t\tImage image= new Image(page" + pageNumberAsString +".class.getResourceAsStream(\""+ '/' + packageName + "/icons/\"+iconName));"); 
	...
	protected void writeoutTextAsDiv(float fontScaling) {
        ...
        //adjustment doesn't get used by either HTML or SVG.
//        int adjustment=8;
//        if(encloseWithinDiv){
//            adjustment=fontSizeAsString;
//        }
        ...
        if(!setFontInCSS){
            ...
            if(textMode ==INVISIBLE_TEXT_ON_IMAGE){ //make text invisible
...
//		 return -1;
		...
		if (flag == -2) { // returned by Super to show we use
			...
			case 90:
				writeCustom(SCRIPT, "\n");
				writeCustom(SCRIPT,"\tImage "+name+" = new Image(page" + pageNumberAsString +".class.getResourceAsStream(\""+ '/' + packageName +'/'+ imageName +"\"));");
				...
				writeCustom(SCRIPT, '\t' +name+"ImageView.setX("+(int)(cropBox.getHeight()-coords[1]-ih)+");");
...
//			break;
	...
	protected void drawPatternedShape(Shape currentShape, GraphicsState gs) {
		...
		if (currentPatternedShape[0] != -1) {
			...
			// imagePrefix = rootDir.substring(0, rootDir.length()-1);
			writeCustom(SCRIPT, "\n");
			writeCustom(SCRIPT,"\tImage "+currentPatternedShapeID+" = new Image(page" + pageNumberAsString +".class.getResourceAsStream(\""+ '/' + packageName +'/'+ currentPatternedShapeName +"\"));");		
			writeCustom(SCRIPT,"\tImageView "+currentPatternedShapeID+"ImageView = new ImageView("+currentPatternedShapeID+");");
			writeCustom(SCRIPT, '\t' + currentPatternedShapeID + "ImageView.setImage(" + currentPatternedShapeID + ");");
			writeCustom(SCRIPT, '\t' +currentPatternedShapeID+"ImageView.setFitWidth("+currentPatternedShape[2]+");");
			writeCustom(SCRIPT, '\t' +currentPatternedShapeID+"ImageView.setFitHeight("+currentPatternedShape[3]+");");
			writeCustom(SCRIPT, '\t' + currentPatternedShapeID + "ImageView.setX(" + currentPatternedShape[0] + ");");
			writeCustom(SCRIPT, '\t' + currentPatternedShapeID + "ImageView.setY(" + currentPatternedShape[1] + ");");
			writeCustom(SCRIPT, "\taddToGroup.add(" + currentPatternedShapeID + "ImageView);");
			writeCustom(SCRIPT, "**CHOP**");
		}
	...
	protected void drawNonPatternedShape(Shape currentShape, GraphicsState gs, int cmd,  String name,Rectangle2D cropBox,Point2D midPoint) {
		...
		ShapeFactory shape = new org.jpedal.render.output.javafx.JavaFXShape(cmd,offset,shapeCount, scaling, currentShape, gs, new AffineTransform(), midPoint, cropBox.getBounds(), currentColor, dpCount, pageRotation, pageData, pageNumber, includeClip);
