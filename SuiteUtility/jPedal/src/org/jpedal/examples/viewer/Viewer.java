 ...
 * Very simple example at http://www.jpedal.org/gplSrc/org/jpedal/examples/jpaneldemo/JPanelDemo.java.html
...
//	public final static String PREFERENCES_CONTENTEXTRACTOR = "jar:/org/jpedal/examples/viewer/res/preferences/ContentExtractor.xml";
	...
	//custom
	public final static String PREFERENCES_ENGINEERING_BASE = "jar:/org/jpedal/examples/viewer/res/preferences/EngineeringProps.xml";
	public final static String PREFERENCES_ENGINEERING_EXTENDED = "jar:/org/jpedal/examples/viewer/res/preferences/EngineeringPropsExtended.xml";
	...
	public void openDefaultFile() {
     ...
     String defaultFile=this.defaultFile;
     if(defaultFile==null){
            ...
            //<end-wrap>
				...
				if(page!=null && !isURL){
					...
					try {
						...
						LogWriter.writeLog("Errore nell'apertura del file " + e.getMessage());
					} catch (Exception e) {
						LogWriter.writeLog("Errore nell'apertura del file " + e.getMessage());
					}
			    //<start-wrap>
...
//	            			// to find your fonts.
//	            			// LogWriter.writeLog("Found a match!");
...
//	        }
	...
	public void setRootContainer(Container rootContainer){
		...
		if(customWindowSize!=null){
			...
			LogWriter.writeLog("values.countTokens()" + values.countTokens());
...
//		decode_pdf.setDisplayView(Display.SINGLE_PAGE,Display.DISPLAY_CENTERED);
		...
		//see also <link><a href="http://www.jpedal.org/gplSrc/org/jpedal/examples/viewer/gui/GUI.java.html#handleAnnotations">org.jpedal.examples.viewer.gui.GUI.handleAnnotations()</a>
...
//			decode_pdf.setDefaultDisplayFont("SansSerif");
			...
			LogWriter.writeLog(Messages.getMessage("PdfViewerFontsFound.message"));
			LogWriter.writeLog("=====================\n");
			int count = fontList.length;
			for (int i = 0; i < count; i++) {
				Font f=new Font(fontList[i],1,10);
				LogWriter.writeLog(fontList[i]+" ("+Messages.getMessage("PdfViewerFontsPostscript.message")+ '=' +f.getPSName()+ ')');
...
//		}
	...
	private static void openFile(Viewer current){
        ...
        if( fileInputProviderClass==null ){
        	LogWriter.writeLog("Errore - Non è stato configurato il fileInputProvider da utilizzare");
			return;
        }
        ...
        try {
			...
			LogWriter.writeLog("Errore nel recupero del fileInputProvider: " + e.getMessage());
			return;
        ...
        if( fileOutputProviderClass==null ){
	        ...
	        try {
				...
				fileOutputProvider.saveOutputParameter(null, preferenceManager);
