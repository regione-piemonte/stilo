...
import org.jpedal.examples.viewer.gui.popups.firma.VirtualCard;
...
{	
	...
	private org.jpedal.examples.viewer.gui.popups.SignMarkWizardModel.OptionPanel.CardPresentThread cardPresentThread;
	...
	private class OptionPanel extends GenericPanel {		
		...
		private void init() {
			...
			try {
				...
				LogWriter.writeLog("Errore nel recupero della proprietà " + ConfigConstants.PROPERTY_VERIFYCERT_WSENDPOINT );
			}
		...
		public void collectData()	{
			...
			LogWriter.writeLog("pin=" + jPin.getPassword() );
			...
			if( currentOutputFilePath.getText()!=null && !currentOutputFilePath.getText().equalsIgnoreCase("") ){
				...
				try {
					...
					LogWriter.writeLog("Errore nel recupero della proprietà " + ConfigConstants.FIRMA_PROPERTY_OUTPUTFILENAME );
				}
				...
				LogWriter.writeLog("OutputFilePath =" + outputFilePath, false );
			...
			try {
				...
				LogWriter.writeLog("Errore nel recupero della proprietà " + ConfigConstants.FIRMA_PROPERTY_HASHALGORITHM );
			}
			signData.setHashAlgorithm( HashAlgorithm.fromValue( algoritmoHash ) );
			LogWriter.writeLog("Proprietà "+ ConfigConstants.FIRMA_PROPERTY_HASHALGORITHM +"=" + algoritmoHash );
			...
			try {
				...
				LogWriter.writeLog("Errore nel recupero della proprietà " + ConfigConstants.FIRMA_PROPERTY_CERTIFYMODE );
			}
			signData.setCertifyMode( CertificationLevel.findCertificationCode( certifyMode ));
			LogWriter.writeLog("Proprietà "+ ConfigConstants.FIRMA_PROPERTY_CERTIFYMODE +"=" + certifyMode );
			
			LogWriter.writeLog("Abilito la marcatura");
			...
			try {
				...
				LogWriter.writeLog("Errore nel recupero della proprietà " + ConfigConstants.FIRMA_PROPERTY_TSAURL );
			}
			signData.setTsaUrl( tsaUrl );
			LogWriter.writeLog("Proprietà "+ ConfigConstants.FIRMA_PROPERTY_TSAURL +"=" + tsaUrl );
			
			boolean tsaAuth = false;
			try {
				tsaAuth = preferenceManager.getConfiguration().getBoolean( ConfigConstants.FIRMA_PROPERTY_TSAAUTH );
			} catch (Exception e1) {
				LogWriter.writeLog("Errore nel recupero della proprietà " + ConfigConstants.FIRMA_PROPERTY_TSAAUTH );
			}
			...
			try {
				...
				LogWriter.writeLog("Errore nel recupero della proprietà " + ConfigConstants.FIRMA_PROPERTY_TSAUSER );
			}
			signData.setTsaUser( tsaUser );
			LogWriter.writeLog("Proprietà "+ ConfigConstants.FIRMA_PROPERTY_TSAUSER +"=" + tsaUser );
			
			String tsaPassword="";
			try {
				tsaPassword = preferenceManager.getConfiguration().getString( ConfigConstants.FIRMA_PROPERTY_TSAPASSWORD );
			} catch (Exception e) {
				LogWriter.writeLog("Errore nel recupero della proprietà " + ConfigConstants.FIRMA_PROPERTY_TSAPASSWORD );
			}
			signData.setTsaPasswd(tsaPassword);
			LogWriter.writeLog("Proprietà "+ ConfigConstants.FIRMA_PROPERTY_TSAPASSWORD +"=" + tsaPassword );
		...
		private class CardPresentThread extends Thread {
			...
			public CardPresentThread(OptionPanel panelsign,CardTerminals cardterminals){
				...
				try {
					...
					LogWriter.writeLog("emulation " + emulationString );
					this.emulation = Boolean.valueOf( emulationString );
					LogWriter.writeLog("emulation " + emulation );
					if( this.emulation ){
...
//				} catch (Exception e) {
//					LogWriter.writeLog("Errore nel recupero della proprietà " + ConfigConstants.FIRMA_PROPERTY_APPENDMODE );
//				}
//				signData.setAppend(appendMode);
				signData.setAppend(true);
			}
			LogWriter.writeLog("Imposto la modalità append " + signData.isAppendMode() );
...
//			} catch (Exception e) {
//				LogWriter.writeLog("Errore nel recupero della proprietà " + ConfigConstants.FIRMA_PROPERTY_ACRO6LAYER );
//			}
//			signData.setAcro6Layers( acro6Layers );
//			LogWriter.writeLog("Imposto la proprietà " + ConfigConstants.FIRMA_PROPERTY_ACRO6LAYER + "=" + signData.isAcro6Layers() );
			...
			try {
				...
				LogWriter.writeLog("Errore nel recupero della proprietà " + ConfigConstants.FIRMA_PROPERTY_RENDERMODE );
			}
			signData.setRenderMode( RenderMode.fromValue( renderMode ) );
			LogWriter.writeLog("Imposto la proprietà " + ConfigConstants.FIRMA_PROPERTY_RENDERMODE + "=" + signData.getRenderMode() );
			...
			if(visibleCheck.isSelected()) {
				...
				try {
					...
					LogWriter.writeLog("Errore nel recupero della proprietà " + ConfigConstants.FIRMA_PROPERTY_L2TEXTFONTSIZE );
				}
				signData.setL2TextFontSize(l2TextFontSize);
				LogWriter.writeLog("Imposto la proprietà " + ConfigConstants.FIRMA_PROPERTY_L2TEXTFONTSIZE + "=" + signData.getL2TextFontSize() );
				...
				try {
					...
					LogWriter.writeLog("Errore nel recupero della proprietà " + ConfigConstants.FIRMA_PROPERTY_L2TEXT );
				}
				signData.setL2Text(l2Text);
				LogWriter.writeLog("Imposto la proprietà " + ConfigConstants.FIRMA_PROPERTY_L2TEXT + "=" + signData.getL2Text() );
				
				String l4Text="";
				try {
					l4Text = preferenceManager.getConfiguration().getString( ConfigConstants.FIRMA_PROPERTY_L4TEXT );
				} catch (Exception e) {
					LogWriter.writeLog("Errore nel recupero della proprietà " + ConfigConstants.FIRMA_PROPERTY_L4TEXT );
				}
				signData.setL4Text( l4Text );
				LogWriter.writeLog("Imposto la proprietà " + ConfigConstants.FIRMA_PROPERTY_L4TEXT + "=" + signData.getL4Text() );
				...
				if( signData.getRenderMode()==RenderMode.GRAPHIC_AND_DESCRIPTION || signData.getRenderMode()==RenderMode.GRAPHIC ){
					...
					try {
						...
						LogWriter.writeLog("Errore nel recupero della proprietà " + ConfigConstants.FIRMA_PROPERTY_IMGPATH );
					}
					signData.setImgPath(imgPath);
					LogWriter.writeLog("Imposto la proprietà " + ConfigConstants.FIRMA_PROPERTY_IMGPATH + "=" + signData.getImgPath() );
					
					String bgImgPath="";
					try {
						bgImgPath = preferenceManager.getConfiguration().getString( ConfigConstants.FIRMA_PROPERTY_BGIMGPATH );
					} catch (Exception e) {
						LogWriter.writeLog("Errore nel recupero della proprietà " + ConfigConstants.FIRMA_PROPERTY_BGIMGPATH );
					}
					signData.setBgImgPath(bgImgPath);
					LogWriter.writeLog("Imposto la proprietà " + ConfigConstants.FIRMA_PROPERTY_BGIMGPATH + "=" + signData.getBgImgPath() );
					...
					try {
						...
						LogWriter.writeLog("Errore nel recupero della proprietà " + ConfigConstants.FIRMA_PROPERTY_BGIMGSCALE );
					}
					signData.setBgImgScale(bgImgScale);
					LogWriter.writeLog("Imposto la proprietà " + ConfigConstants.FIRMA_PROPERTY_BGIMGSCALE + "=" + signData.getBgImgScale() );
				}
				...
				LogWriter.writeLog("Imposto le coordinate di firma a x1=" + x1 + " x2=" + x2 + " y1=" + y1 + " y2=" + y2 );
