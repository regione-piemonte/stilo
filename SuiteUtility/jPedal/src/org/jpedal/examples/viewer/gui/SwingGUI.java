...
import org.jpedal.examples.viewer.gui.popups.InvioLogMailPanel;
import org.jpedal.examples.viewer.gui.popups.PreferenceFirmaMarcaPanel;
import org.jpedal.examples.viewer.gui.popups.PreferenceInvioLogPanel;
import org.jpedal.examples.viewer.gui.popups.PreferenceTimbroPanel;
...
import org.jpedal.examples.viewer.gui.swing.TimbroPanel;
...
import org.jpedal.utils.PropertyConstants;
...
//				}
				...
				if( selectedFile!=null && isFileFirmato) {
					...
					try {
						...
						LogWriter.writeLog( "caCheck: " + caCheck );
						crlCheck = commonValues.getPreferenceManager().getConfiguration().getBoolean( ConfigConstants.FIRMA_PROPERTY_ENABLECRLCHECK );
						LogWriter.writeLog( "crlCheck: " + crlCheck );
					} catch (Exception e) {
						LogWriter.writeLog("Errore nel recupero delle proprietà " + ConfigConstants.FIRMA_PROPERTY_ENABLECACHECK + " e " +
						...
						if( responseVerificaFirme!=null && 	responseVerificaFirme.getFileoperationResponse()!=null 
							...
							for(AbstractResponseOperationType fileOpResult: fileOpResults){
								if( fileOpResult instanceof ResponseSigVerify){
									LogWriter.writeLog("Costruisco la gerarchia di firme");
									WSClientUtils.processHierarchy( root, ((ResponseSigVerify)fileOpResult).getSigVerifyResult() );
								}
							}
						}
					} catch (Exception e) {
						LogWriter.writeLog("Non posso costruire la rappresentazione grafica della firma", e );
					}
...
//							LogWriter.writeLog("Signature covers whole document: " + af.signatureCoversWholeDocument(name));
//							LogWriter.writeLog("Document revision: " + af.getRevision(name) + " of " + af.getTotalRevisions());
//
...
//							//root.add(sigName);
//
//							LogWriter.writeLog("Digest algorithm: " + pkcs7.getHashAlgorithm());
...
//							LogWriter.writeLog("Signed on: " + date_format.format(calendardate.getTime()));
...
//							LogWriter.writeLog("Contact info: " + contact);
...
//									LogWriter.writeLog("Timestamp date: " + strdate);
//								} 
...
//						LogWriter.writeLog("Uploadok vale " + commands.uploadOk);
//						if (commands.uploadOk){
//							LogWriter.writeLog("Chiudo la console");
			...
			if(fileMenu!=null){ //all of these will be null in 'Wrapper' mode so ignore
				...
				if( commonValues.getSelectedFile()!=null) {
					if( commonValues.getSaveEnabled()!=null ){
						...
						LogWriter.writeLog("Abilitazione funzionalità di salvataggio file dalle preferences " + propValue );
					} else {
						propValue = properties.getValue("Save");
						LogWriter.writeLog("Abilitazione funzionalità di salvataggio file dalla configurazione " + propValue );
					}
					set = propValue.length()>0 && propValue.toLowerCase().equals("true");
					save.setEnabled(set);
					save.setVisible(set);
					...
					if( commonValues.getSelectedFile()!=null ){
						if( commonValues.getTimbroEnabled()!=null ){
							...
							LogWriter.writeLog("Abilitazione funzionalità di timbratura dalle preferences " + propValue );
						} else {
							propValue = properties.getValue( PropertyConstants.TIMBRO );
							LogWriter.writeLog("Abilitazione funzionalità di timbratura dalla configurazione " + propValue );
						}
						set = propValue.length()>0 && propValue.toLowerCase().equals("true");
				...
				if(enableShowSign && signPDF!=null && commonValues.getSelectedFile()!=null){
					if( commonValues.getSignEnabled()!=null ){
						...
						LogWriter.writeLog("Abilitazione funzionalità di firma dalle preferences " + propValue );
					} else {
						propValue = properties.getValue( PropertyConstants.SIGN );
						LogWriter.writeLog("Abilitazione funzionalità di firma dalla configurazione " + propValue );
					}
					set = propValue.length()>0 && propValue.toLowerCase().equals("true");
					...
					if( commonValues.getMarkEnabled()!=null ){
						...
						LogWriter.writeLog("Abilitazione funzionalità di marcatura dalle preferences " + propValue );
					} else {
						propValue = properties.getValue( PropertyConstants.MARK );
						LogWriter.writeLog("Abilitazione funzionalità di marcatura dalla configurazione " + propValue );
					}
					//set = propValue.length()>0 && propValue.toLowerCase().equals("true");
					...
					if( commonValues.getMarkEnabled()!=null && commonValues.getMarkEnabled()!=null){
						...
						LogWriter.writeLog("Abilitazione funzionalità di firma e marcatura dalle preferences " + propValue );
					} else {
						propValue = properties.getValue( PropertyConstants.SIGN ) + properties.getValue( PropertyConstants.MARK );
						LogWriter.writeLog("Abilitazione funzionalità di firma e marcatura dalla configurazione " + propValue );
					}
				...
				if( commonValues.getSelectedFile()!=null){
					if( commonValues.getPrintEnabled()!=null ){
						...
						LogWriter.writeLog("Abilitazione funzionalità di stampa dalle preferences " + propValue );
					} else {
						propValue = properties.getValue( PropertyConstants.PRINT );
						LogWriter.writeLog("Abilitazione funzionalità di stampa dalla configurazione " + propValue );
					}
					set = propValue.length()>0 && propValue.toLowerCase().equals("true");
