...
{	
	...
	private org.jpedal.examples.viewer.gui.popups.SignWizardModel.OptionPanel.CardPresentThread cardPresentThread;
	 ...
	 */
	public SignWizardModel(SignData signData, String pdfFile, String rootDir, PreferenceManager preferenceManager) 
	{
    ...
    private class OptionPanel extends GenericPanel {		
		...
		private void init() {
			...
			//aggiungiRigaSeparazione( pane, c, 0, y++, 20, 10);
		...
		public void collectData()	{
			...
			LogWriter.writeLog("pin=" + new String(jPin.getPassword()) );
    ...
    private class SignaturePanel extends GenericPanel {
		...
		public SignaturePanel() {        
			...
			try {
	            ...
	            LogWriter.writeLog( "Exception: "+e.getMessage(), e );
			}
...
//			LogWriter.writeLog("Imposto la proprietà " + ConfigConstants.FIRMA_PROPERTY_ACRO6LAYER + "=" + signData.isAcro6Layers() );
			...
			LogWriter.writeLog("Render mode vale " + renderMode);
			try {
				renderMode = preferenceManager.getConfiguration().getString( ConfigConstants.FIRMA_PROPERTY_RENDERMODE );
				LogWriter.writeLog("Letta dal preference manager: Render mode vale " + renderMode);
			} catch (Exception e) {
		...
		private JPanel previewPanel() {
			...
			sigPreviewComp.setToolTipText( Messages.getMessage( MessageConstants.PANEL_FIRMAMARCA_PREVIEW_TOOLTIP ) );
