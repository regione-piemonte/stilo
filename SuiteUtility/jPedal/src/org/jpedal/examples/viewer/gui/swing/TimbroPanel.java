...
import org.jpedal.examples.viewer.gui.popups.GenericPanel;
import org.jpedal.examples.viewer.utils.DigestUtility;
import org.jpedal.examples.viewer.utils.ItextFunctions;
import org.jpedal.examples.viewer.utils.TimbroUtils;
...
//		}
	...
	public boolean timbra( SwingGUI gui) throws Exception {
		...
		LogWriter.writeLog("File da timbrare " + selectedFile);
		...
		try {
			...
			LogWriter.writeLog("Errore nel recupero della proprietà " + ConfigConstants.TIMBRO_PROPERTY_OUTPUTFILENAME );
		}
		String outputFileName = fileDaTimbrare.getName().replace(".pdf", outputFileNameString+".pdf");
		LogWriter.writeLog("outputFileName " + outputFileName, false);
		...
		try {
			...
			LogWriter.writeLog("Errore nel recupero della proprietà " + ConfigConstants.TIMBRO_PROPERTY_MAX_LENGHT_TIMBRO );
		}
		LogWriter.writeLog( ConfigConstants.TIMBRO_PROPERTY_MAX_LENGHT_TIMBRO + "=" + maxLenghtTimbro);
		...
		try {
			...
			LogWriter.writeLog("Errore nel recupero della proprietà " + ConfigConstants.TIMBRO_PROPERTY_MARGINE_SUP);
		}
		LogWriter.writeLog( ConfigConstants.TIMBRO_PROPERTY_MARGINE_SUP + "=" + margineSup);
		...
		try {
			...
			LogWriter.writeLog("Errore nel recupero della proprietà " + ConfigConstants.TIMBRO_PROPERTY_MARGINE_INF );
		}
		LogWriter.writeLog( ConfigConstants.TIMBRO_PROPERTY_MARGINE_INF + "=" + margineInf);
		...
		try {
			...
			LogWriter.writeLog("Errore nel recupero della proprietà " + ConfigConstants.TIMBRO_PROPERTY_MARGINE_DX );
		}
		LogWriter.writeLog( ConfigConstants.TIMBRO_PROPERTY_MARGINE_DX + "=" + margineDx);
		...
		try {
			...
			LogWriter.writeLog("Errore nel recupero della proprietà " + ConfigConstants.TIMBRO_PROPERTY_MARGINE_SN );
		}
		LogWriter.writeLog( ConfigConstants.TIMBRO_PROPERTY_MARGINE_SN + "=" + margineSn);
		...
		try {
			...
			LogWriter.writeLog("Errore nel recupero della proprietà " + ConfigConstants.TIMBRO_PROPERTY_MARGINE_TIMBRO_TESTO );
		}
		LogWriter.writeLog( ConfigConstants.TIMBRO_PROPERTY_MARGINE_TIMBRO_TESTO + "=" + margineTimbroTesto);

		LogWriter.writeLog( "moreLines? " + moreLines );
		...
		LogWriter.writeLog( ConfigConstants.TIMBRO_PROPERTY_FONT_SIZE + "=" + fontSizeTimbro);
		LogWriter.writeLog( ConfigConstants.TIMBRO_PROPERTY_FONT_NAME + "=" + fontNameTimbro);
		...
		LogWriter.writeLog( ConfigConstants.TIMBRO_PROPERTY_CODIFICA + "=" + this.codificaTimbro);
		CodificaTimbro codificaTimbro = CodificaTimbro.fromValue(this.codificaTimbro);
		LogWriter.writeLog( ConfigConstants.TIMBRO_PROPERTY_CODIFICA + "=" + codificaTimbro);
		TipoRotazione rotazioneTimbro = TipoRotazione.fromValue((String)listaRotazioniTimbro.getSelectedItem());
		LogWriter.writeLog( ConfigConstants.TIMBRO_PROPERTY_ROTAZIONE + "=" + rotazioneTimbro);
		...
		LogWriter.writeLog( ConfigConstants.TIMBRO_PROPERTY_POSIZIONE + "=" + posizioneTimbroNellaPagina);
		String testo = testoTimbro;
		LogWriter.writeLog( ConfigConstants.TIMBRO_PROPERTY_TESTO + "=" + testo);
		...
		if( this.addData ){
			try {
				...
				LogWriter.writeLog("Errore nell'aggiunta della data" );
			}
		}
		if( this.addDn ){
			//verifico se il file da timbrare è firmato
			boolean isFileFirmato = ItextFunctions.isFileFirmato( selectedFile );
			LogWriter.writeLog("Il file da firmare è firmato? " + isFileFirmato );
			if( isFileFirmato ){
				PdfReader reader;
				try {
					...
					for (int k = 0; k < names.size(); ++k) {
						...
						LogWriter.writeLog("Signature name: " + name);
						PdfPKCS7 pkcs7 = af.verifySignature( name );
						X509Certificate cert = (X509Certificate) pkcs7.getSigningCertificate();
						String dn = cert.getSubjectDN().getName();
						LogWriter.writeLog("Signature dn: " + dn);
						testo += " " + dn;
					}
				} catch (IOException e1) {
					LogWriter.writeLog("Errore " + e1.getMessage(), e1 );
				}
		...
		try {
			...
			LogWriter.writeLog( ConfigConstants.TIMBRO_PROPERTY_PARAMETER_LIST + "=" + listaParametriTimbro);
		} catch (Exception e1) {
			LogWriter.writeLog("Errore nel recupero della proprietà " + ConfigConstants.TIMBRO_PROPERTY_PARAMETER_LIST );
		}
		if( listaParametriTimbro!=null ){
			...
			while(tokenizer.hasMoreTokens()){
				...
				if( tokenizerParam.hasMoreTokens() ) {
					...
					LogWriter.writeLog("chiave " + chiave);
					p.setChiave( chiave );
				}
				if( tokenizerParam.hasMoreTokens() ) {
					String valore  = tokenizerParam.nextToken();
					LogWriter.writeLog("valore " + valore);
					p.setValore( valore );
				}
		...
		LogWriter.writeLog( ConfigConstants.TIMBRO_PROPERTY_TESTO_INTESTAZIONE + "=" + testoIntestazione);
		...
		if( !this.posizioneIntestazione.equalsIgnoreCase("--")) { 
			...
			LogWriter.writeLog( ConfigConstants.TIMBRO_PROPERTY_POSIZIONE_INTESTAZIONE + "=" + posizioneIntestazione);
		}
		...
		if( !this.posizioneTestoInChiaro.equalsIgnoreCase("--")) {
			...
			LogWriter.writeLog( ConfigConstants.TIMBRO_PROPERTY_POSIZIONE_TESTO_IN_CHIARO + "=" + posizioneTestoInChiaro);
		}
		LogWriter.writeLog( ConfigConstants.TIMBRO_PROPERTY_TIMBROSINGOLO + "=" + timbroSingolo);
		...
		if( timbroPaginaCorrenteField.isSelected() ) {
			...
			LogWriter.writeLog("Timbro la pagina corrente, le altre opzioni sulla pagina da timbrare verranno ignorate" );
			...
			try{
				...
				LogWriter.writeLog("Timbro le pagine da " + paginaDa + " a " + paginaA + ", le altre opzioni sulla pagina da timbrare verranno ignorate" );
				paginaTimbro.setPagine(pagine);
			} catch (Exception e1) {
				JOptionPane.showMessageDialog( this,
			...
			if( tipoPaginaSelezionato!=null && !tipoPaginaSelezionato.equalsIgnoreCase("--") ) {
				...
				LogWriter.writeLog("Timbro le pagine in base al tipo " + tipoPagina + ", le altre opzioni sulla pagina da timbrare verranno ignorate" );
