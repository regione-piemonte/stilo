...
public class SmartCard implements ISmartCard {
	...
	public void init() {
		...
		this.mainComponent.setContentPane( getJContentPane() );
			...
			try {
				...
				LogWriter.writeLog("Verra'� utilizzato il fileOutputProvider " + fileOutputProviderClass);
			} catch (Exception e) {
				LogWriter.writeLog("Errore - Non e' stato possibile recuperare il fileOutputProvider da utilizzare", e );
			}
			if( fileOutputProviderClass==null ){
				LogWriter.writeLog("Errore - Non e' stato configurato il fileOutputProvider da utilizzare");
				//return;
			} else {
				try {
	...
	private JScrollPane getJContentPane() {
		...
		//LogWriter.writeLog("Esco da getJContentPane ");
		return jScrollPane;
	}
				...
				if (configurazioniFirmaMarcaEnabled) {
					...
					firmaMarca.addActionListener(new ActionListener() {
						...
						public void actionPerformed(ActionEvent e) {
							...
							new PreferenceFirmaMarcaPanel(preferenceFirmaMarcaPanelWidth, preferenceFirmaMarcaPanelHeight).show(mainComponent.getContentPane(),
									getPanelsign());
						}
					});
	...
	public void closeApplet() {
		...
		LogWriter.writeLog("mainComponent: " + mainComponent.getClass().getName() );
		...
		if (fileOutputProvider.getAutoClosePostSign()) {
			if (mainComponent instanceof JApplet)
			{
				...
				// Sia che l'applet invochi un JS di callback che no, essa deve terminare il suo lavoro
				lApplet.stop();
				lApplet.destroy();
				System.exit( 1 );
