...
public class Factory {
		...
		if(dllFile.isFile()){	
			...
			if( osname.equals("windows") || 
				...
				try {
					...
					//e.printStackTrace();
					LogWriter.writeLog("Errore nel recupero dello slot ", e);
				}
				...
				try {
					...
					if( provider!=null ){
						...
						//LogWriter.writeLog("pin " + new String(pin));
						provider.login(new Subject(), new PasswordHandler(pin));
						
						LogWriter.writeLog("Login al provider effettuato");
	...
	/**
	 * verifica se il file passato e' firmato, ritorna il tipo di busta riconoscito se rileva la firma
	 * null altrimenti
	 * @param file
	 * @return
