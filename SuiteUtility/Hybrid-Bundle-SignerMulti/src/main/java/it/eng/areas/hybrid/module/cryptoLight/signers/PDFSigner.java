...
import it.eng.areas.hybrid.module.cryptoLight.sign.CertificationLevel;
...
import it.eng.areas.hybrid.module.cryptoLight.sign.RenderMode;
import it.eng.areas.hybrid.module.cryptoLight.sign.SignData;
...
import it.eng.areas.hybrid.module.cryptoLight.util.ProxyUtil;
...
public class PDFSigner implements ISigner {
	...
	public final static Logger logger = Logger.getLogger( PDFSigner.class );
	...
	private RenderMode selectRenderMode(){
		...
		try {
			...
			logger.info("Proprietà " + PreferenceKeys.PROPERTY_SIGN_PDF_RENDER_MODE + "=" + renderMode);
 		} catch (Exception e) {
			logger.info("Errore nel recupero della proprietà " + PreferenceKeys.PROPERTY_SIGN_PDF_RENDER_MODE );
		}
	...
	public Image getImg(String image){
		...
		logger.info("Proprieta' " + image + "=" + imgPath );
	...
	public boolean firma(FileBean bean, PrivateKeyAndCert pvc, AuthProvider provider,
		...
		logger.info("Inizio metodo firma pdf - timemark = " + timemark + ", detached = " + detached +
		...
		try {
			...
			logger.info("Errore nel recupero della proprietà " + PreferenceKeys.PROPERTY_SIGN_PDF_CERTIFICATIONLEVEL );
		}
		...
		if(signData.isVisibleSignature()){
			...
			logger.info("Pagina da firmare " + paginaDaFirmare );
			if( paginaDaFirmare>pdfReader.getNumberOfPages() ){
				logger.info("La pagina da firmare non esiste, firmo la prima pagina");
				paginaDaFirmare = 1;
			}
			...
			try {
				...
				logger.info("proprietà " + PreferenceKeys.PROPERTY_SIGN_PDF_IMAGE_BG_SCALE + "=" + scale);
			} catch (Exception e) {
				logger.info("Errore nel recupero della proprietà " + PreferenceKeys.PROPERTY_SIGN_PDF_IMAGE_BG_SCALE );
			}
		...
		return sign( signData, FileUtils.openInputStream(bean.getFile()), pvc, outputStream );	
	}
