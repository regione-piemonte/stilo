...
public class SVGGraphicContext implements SVGConstants, ErrorConstants
{
    ...
    private void computeGroupAndGraphicElementContext() {
        ...
        for (int i = 0; i < SVGGraphicContext.leafOnlyAttributes.length; ++i) {
            final Object value = this.groupContext.get(SVGGraphicContext.leafOnlyAttributes[i]);
            if (value != null) {
                if (!value.equals(SVGGraphicContext.defaultValues[i])) {
                    this.graphicElementContext.put(SVGGraphicContext.leafOnlyAttributes[i], value);
                }
