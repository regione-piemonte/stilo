...
{
    public FilterAlphaRable(final Filter filter) {
        super(filter, null);
    }
    ...
    public RenderedImage createRendering(final RenderContext renderContext) {
        ...
        if (wrap.getProperty("org.apache.batik.gvt.filter.Colorspace") == ColorSpaceHintKey.VALUE_COLORSPACE_ALPHA) {
            return wrap;
        }
        return new FilterAlphaRed(wrap);
    }
}
