...
public class GVTLineMetrics
{
    ...
    public GVTLineMetrics(final LineMetrics lineMetrics, final float n) {
        this.ascent = lineMetrics.getAscent() * n;
        this.baselineIndex = lineMetrics.getBaselineIndex();
        this.baselineOffsets = lineMetrics.getBaselineOffsets();
        for (int i = 0; i < this.baselineOffsets.length; ++i) {
            final float[] baselineOffsets = this.baselineOffsets;
            final int n2 = i;
            baselineOffsets[n2] *= n;
        }
        this.descent = lineMetrics.getDescent() * n;
        this.height = lineMetrics.getHeight() * n;
        this.leading = lineMetrics.getLeading();
        this.numChars = lineMetrics.getNumChars();
        this.strikethroughOffset = lineMetrics.getStrikethroughOffset() * n;
        this.strikethroughThickness = lineMetrics.getStrikethroughThickness() * n;
        this.underlineOffset = lineMetrics.getUnderlineOffset() * n;
        this.underlineThickness = lineMetrics.getUnderlineThickness() * n;
        this.overlineOffset = -this.ascent;
        this.overlineThickness = this.underlineThickness;
    }
