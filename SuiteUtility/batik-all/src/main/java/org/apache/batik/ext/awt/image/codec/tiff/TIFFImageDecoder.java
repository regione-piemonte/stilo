...
public class TIFFImageDecoder extends ImageDecoderImpl
{
    ...
    public TIFFImageDecoder(final SeekableStream seekableStream, final ImageDecodeParam imageDecodeParam) {
    ...
    public RenderedImage decodeAsRenderedImage(final int n) throws IOException {
        if (n < 0 || n >= this.getNumPages()) {
            throw new IOException("TIFFImageDecoder0");
        }
        return new TIFFImage(this.input, (TIFFDecodeParam)this.param, n);
    }
}
