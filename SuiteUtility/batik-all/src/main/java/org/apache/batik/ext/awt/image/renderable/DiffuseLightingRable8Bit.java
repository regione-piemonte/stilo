...
public class DiffuseLightingRable8Bit extends AbstractColorInterpolationRable implements DiffuseLightingRable
{
    ...
    public DiffuseLightingRable8Bit(final Filter filter, final Rectangle2D litRegion, final Light light, final double kd, final double surfaceScale, final double[] kernelUnitLength) {
        super(filter, null);
    ...
    public RenderedImage createRendering(RenderContext renderContext) {
        ...
        if (this.kernelUnitLength != null) {
            if (this.kernelUnitLength[0] > 0.0f && sqrt > 1.0f / this.kernelUnitLength[0]) {
                sqrt = 1.0f / this.kernelUnitLength[0];
            }
            if (this.kernelUnitLength[1] > 0.0f && sqrt2 > 1.0f / this.kernelUnitLength[1]) {
        ...
        AbstractRed abstractRed = new DiffuseLightingRed(this.kd, this.light, new BumpMap(GraphicsUtil.wrap(this.getSource().createRendering(renderContext)), this.surfaceScale, sqrt, sqrt2), bounds2, 1.0 / sqrt, 1.0 / sqrt2, this.isColorSpaceLinear());
