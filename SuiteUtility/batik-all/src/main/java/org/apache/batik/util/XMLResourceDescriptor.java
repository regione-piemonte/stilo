...
public class XMLResourceDescriptor
{
    ...
    protected static synchronized Properties getParserProps() {
        ...
        try {
            XMLResourceDescriptor.parserProps.load(XMLResourceDescriptor.class.getResourceAsStream("resources/XMLResourceDescriptor.properties"));
        }
        catch (IOException ex) {
            throw new MissingResourceException(ex.getMessage(), "resources/XMLResourceDescriptor.properties", null);
        }
        return XMLResourceDescriptor.parserProps;
    }
