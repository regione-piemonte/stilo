...
public class DaoTTipologiaProcedimento {

	public TPagingList<TTipologiaProcedimento> search(TFilterFetch<TTipologiaProcedimento> filter) throws Exception {
		...
		try {
			...
			TPagingList<TTipologiaProcedimento> paginglist = new TPagingList<TTipologiaProcedimento>();
			...
			for (Object obj : HibernateUtil.pagingByCriteria(criteria, filter.getStartRow(), filter.getEndRow())) {
				paginglist.addData((TTipologiaProcedimento) obj);
			}
			return paginglist;
		} catch (Exception e) {
			throw e;
	...
	public Long count(TFilterFetch<TTipologiaProcedimento> filter) throws Exception{
		Session session = null;
		try {
			session = HibernateUtil.begin();
			// Creo i criteri di ricerca
	...
	private Criteria buildHibernateCriteriaByFilter(Session session, TFilterFetch<TTipologiaProcedimento> filter) {
		Criteria lCriteria = session.createCriteria(TTipologiaProcedimento.class);
