...
import it.eng.dm.sira.entity.VVocTipoAda;
...
public class DaoTipoADA {

	public TPagingList<VVocTipoAda> search(TFilterFetch<VVocTipoAda> filter) throws Exception {
		...
		try {
			...
			TPagingList<VVocTipoAda> paginglist = new TPagingList<VVocTipoAda>();
			...
			for (Object obj : HibernateUtil.pagingByCriteria(criteria, filter.getStartRow(), filter.getEndRow())) {
				paginglist.addData((VVocTipoAda) obj);
			}
			return paginglist;
		} catch (Exception e) {
			throw e;
	...
	private Criteria buildHibernateCriteriaByFilter(Session session, TFilterFetch<VVocTipoAda> filter) {
		Criteria lCriteria = session.createCriteria(VVocTipoAda.class);
