...
public class DaoRiferimentiNormativi {

	public TPagingList<VRiferimentiNormativi> search(TFilterFetch<VRiferimentiNormativi> filter) throws Exception {
		...
		try {
			...
			TPagingList<VRiferimentiNormativi> paginglist = new TPagingList<VRiferimentiNormativi>();
			...
			for (Object obj : HibernateUtil.pagingByCriteria(criteria, filter.getStartRow(), filter.getEndRow())) {
				paginglist.addData((VRiferimentiNormativi) obj);
			}
			return paginglist;
		} catch (Exception e) {
			throw e;
	...
	private Criteria buildHibernateCriteriaByFilter(Session session, TFilterFetch<VRiferimentiNormativi> filter) {
		Criteria lCriteria = session.createCriteria(VRiferimentiNormativi.class);
