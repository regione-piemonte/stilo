...
import it.eng.dm.sira.entity.VOggettiAda;
...
public class DaoOggettiADA {

	public TPagingList<VOggettiAda> search(TFilterFetch<VOggettiAda> filter) throws Exception {
		...
		try {
			...
			TPagingList<VOggettiAda> paginglist = new TPagingList<VOggettiAda>();
			...
			for (Object obj : HibernateUtil.pagingByCriteria(criteria, filter.getStartRow(), filter.getEndRow())) {
				paginglist.addData((VOggettiAda) obj);
			}
			return paginglist;
		} catch (Exception e) {
			throw e;
	...
	private Criteria buildHibernateCriteriaByFilter(Session session, TFilterFetch<VOggettiAda> filter) {
		Criteria lCriteria = session.createCriteria(VOggettiAda.class);
