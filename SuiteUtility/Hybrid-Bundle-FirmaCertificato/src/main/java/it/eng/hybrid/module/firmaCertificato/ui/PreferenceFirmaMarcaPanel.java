...
//	private JTextField outputFileName;
	...
	private void createPreferenceWindow(){
        ...
        //if( !panelSign.getSignApplication().getHashfilebean().isEmpty() ){
        ...
        List<HashFileBean> hashBeanList = panelSign.getFirmaCertificatoApplication().getHashfilebean();
        boolean useHash = false;
        if( hashBeanList!=null && hashBeanList.size()>0){
        	useHash = true;
        } 
		if( markEnabled && !useHash ){
	...
	private void salvaConfigurazione(){
		try {
			...
			if( tsaUrl!=null && tsaUrl.getSelectedItem()!=null )
			...
			if( tsaUser!=null && tsaUser.getText()!=null )
				PreferenceManager.saveProp( PreferenceKeys.PROPERTY_SIGN_TSAUSER, tsaUser.getText() );
			if( tsaPassword!=null && tsaPassword.getText()!=null )
	...
	private JPanel addPannelloOpzioniFirma(){
		...
		if( !panelSign.getFirmaCertificatoApplication().getHashfilebean().isEmpty() ){	
