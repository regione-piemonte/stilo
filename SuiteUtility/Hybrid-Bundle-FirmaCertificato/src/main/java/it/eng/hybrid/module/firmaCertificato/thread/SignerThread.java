		...
		for (String path : pathProviders) {
			...
			try {
				...
				if (sunProvider != null) {
					...
					/*
					 * Controllo se il certificato � null o meno. Il certificato pu� essere null ad esempio se il provider che si sta utilizzando non lo riconosce
					 */
					if (certificate != null) {
						// controllo MACRO
						boolean codeCheck = PreferenceManager.getBoolean(PreferenceKeys.PROPERTY_SIGN_CODECHECKENABLED);
						logger.info("Proprietà " + PreferenceKeys.PROPERTY_SIGN_CODECHECKENABLED + "=" + codeCheck);
						if (codeCheck) {
							...
							if (fileBeanList != null) {
								for (FileBean bean : fileBeanList) {
									try {
										...
										logger.error("Errore nella verifica macro, il controllo non e' stato eseguito", e);
									}
								...
								if (fileBeanList != null) {
									...
									for (FileBean bean : fileBeanList) {
										...
										if (!bean.getIsPdf() && tipoFirma.equals(SignerType.PDF)) {
											...
											logger.info("cambio il tipo busta a cades bes poiche' il file non è un PDF non posso fare una busta PADES");
