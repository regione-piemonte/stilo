...
class MagicMimeEntry {
	...
	public static final Logger log = LogManager.getLogger(MagicMimeEntry.class);
	...
	MagicMimeEntry getMatch(InputStream in) throws IOException {
		...
		try {
			...
			if( in instanceof PushbackInputStream){
				...
				//((PushbackInputStream)in).reset();
				//byte[] bom = new byte[3];
				//((PushbackInputStream)in).unread(bom);
			} else {
				in.reset();
			}
		}
		...
		try {
			switch (getType()) {
				...
				case MagicMimeEntry.BYTE_TYPE: {
					...
					//log.debug("BUF " + new String(buf.array()));
					break;
				}
	...
	private boolean matchString(ByteBuffer bbuf) throws IOException {
		...
		if (operation.equals(MagicMimeEntryOperation.EQUALS)) {
			...
			for (int j = 0; j < read; j++) {
				//System.out.println(getContent().charAt(j) + " " + bbuf.get(j));
	...
	private boolean matchByte(ByteBuffer bbuf) throws IOException {
		//System.out.println("contentNumber " + contentNumber + " " + bbuf.get(0) + " " + bbuf.array().length );
		short found = (short) ((bbuf.get(0) & 0xff) & (short)getMask(typeStr));
		
		/*log.debug(mimeType.getMediaType() + " " + mimeType.getSubType());
		if( mimeType.getMediaType()!=null && "video".equalsIgnoreCase(mimeType.getMediaType()) &&
				"x-unknown".equalsIgnoreCase(mimeType.getSubType()))
